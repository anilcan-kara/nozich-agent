# OpenAI-Compatible API Server for Nozich Agent
...
SSE streaming. By exposing this endpoint, nozich-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  nozich-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.nozich/.env`
2. Run `nozich gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with nozich-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns nozich-agent as a model) |
...
Authorization: Bearer nozich-api-key-here
...
But nozich-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "nozich-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured nozich-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of nozich-agent's core prompt
...
Once implemented, nozich-agent works as a drop-in backend for:
