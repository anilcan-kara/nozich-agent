# Nozich Agent - Development Guide

Instructions for AI coding assistants and developers working on the nozich-agent codebase.

**Never give up on the right solution.**

## What Nozich Is

Nozich is a personal AI agent that runs the same agent core across a CLI, a
...
Read the balance right: Nozich ships a **lot** — most merges are bug fixes to
...
- **Expand reach at the edges.** New platform adapters, channels, providers,
  ...
  (`nozich tools`, `nozich setup`, auto-install) rather than bolting on a raw
  env var.
...
- **E2E validation, not just green unit mocks.** For anything touching
  ...
  against a temp `NOZICH_HOME`. Mocks hide integration bugs.
...
- **New `NOZICH_*` env vars for non-secret config.** `.env` is for secrets
...
- **Outbound telemetry / usage attribution without opt-in gating.** No new
  ...
  generic user-facing opt-in (config gate + setup prompt + `nozich tools`
...
2. **CLI command + skill** — manages config/state/infra expressible as shell
   commands. The agent runs `nozich <subcommand>` guided by a skill. Zero
   model-tool footprint. Default choice for subscriptions, scheduled tasks,
   service setup. Examples: `nozich webhook`, `nozich cron`, `nozich tools`.
...
4. **Plugin** — third-party/niche/user-specific capability that doesn't ship in
   core. Lives in `~/.nozich/plugins/` or a pip package, discovered at runtime.
...
`$HOME/.nozich/nozich-agent/venv` (for worktrees that share a venv with the
main checkout).
...
├── toolsets.py           # Toolset definitions, _NOZICH_CORE_TOOLS list
├── cli.py                # NozichCLI class — interactive CLI orchestrator (~11k LOC)
├── nozich_state.py       # SessionDB — SQLite session store (FTS5 search)
├── nozich_constants.py   # get_nozich_home(), display_nozich_home() — profile-aware paths
├── nozich_logging.py     # setup_logging() — agent.log / errors.log / gateway.log (profile-aware)
├── batch_runner.py       # Parallel batch processing
├── agent/                # Agent internals (provider adapters, memory, caching, compression, etc.)
├── nozich_cli/           # CLI subcommands, setup wizard, plugins loader, skin engine
...
│   ├── nozich-achievements/  # Gamified achievement tracking
...
├── ui-tui/               # Ink (React) terminal UI — `nozich --tui`
...
**User config:** `~/.nozich/config.yaml` (settings), `~/.nozich/.env` (API keys only).
**Logs:** `~/.nozich/logs/` — `agent.log` (INFO+), `errors.log` (WARNING+),
`gateway.log` when running the gateway. Profile-aware via `get_nozich_home()`.
Browse with `nozich logs [--follow] [--level ...] [--session ...]`.

## TypeScript Style

Applies to TypeScript across Nozich: desktop, TUI, website, and future TS packages.
...
- **Skin engine** (`nozich_cli/skin_engine.py`) — data-driven CLI theming; initialized from `display.skin` config key at startup; skins customize banner colors, spinner faces/verbs/wings, tool prefix, response box, branding text
- `process_command()` is a method on `NozichCLI` — dispatches on canonical command name resolved via `resolve_command()` from the central registry
- Skill slash commands: `agent/skill_commands.py` scans `~/.nozich/skills/`, injects as **user message** (not system prompt) to preserve prompt caching

### Slash Command Registry (`nozich_cli/commands.py`)
...
- **Slack** — `slack_subcommand_map()` generates `/nozich` subcommand routing
- **Autocomplete** — `COMMANDS` flat dict feeds `SlashCommandCompleter`
- **CLI help** — `COMMANDS_BY_CATEGORY` dict feeds `show_help()`

### Adding a Slash Command

1. Add a `CommandDef` entry to `COMMAND_REGISTRY` in `nozich_cli/commands.py`:
```python
CommandDef("mycommand", "Description of what it does", "Session",
           aliases=("mc",), args_hint="[arg]"),
```
2. Add handler in `NozichCLI.process_command()` in `cli.py`:
```python
...
The TUI is a full replacement for the classic (prompt_toolkit) CLI, activated via `nozich --tui` or `NOZICH_TUI=1`.
...
npm run dev       # watch mode (rebuilds nozich-ink + tsx --watch)
npm start         # production
npm run build     # full build (nozich-ink + tsc)
...
### TUI in the Dashboard (`nozich dashboard` → `/chat`)

The dashboard embeds the real `nozich --tui` — **not** a rewrite.  See `nozich_cli/pty_bridge.py` + the `@app.websocket("/api/pty")` endpoint in `nozich_cli/web_server.py`.

- Browser loads `web/src/pages/ChatPage.tsx`, which mounts xterm.js's `Terminal` with the WebGL renderer, `@xterm/addon-fit` for container-driven resize, and `@xterm/addon-unicode11` for modern wide-character widths.
- `/api/pty?token=…` upgrades to a WebSocket; auth uses the same ephemeral `_SESSION_TOKEN` as REST, via query param (browsers can't set `Authorization` on WS upgrade).
- The server spawns whatever `nozich --tui` would spawn, through `ptyprocess` (POSIX PTY — WSL works, native Windows does not).
- Frames: raw PTY bytes each direction; resize via `\x1b[RESIZE:<cols>;<rows>]` intercepted on the server and applied with `TIOCSWINSZ`.

**Do not re-implement the primary chat experience in React.** The main transcript, composer/input flow (including slash-command behavior), and PTY-backed terminal belong to the embedded `nozich --tui` — anything new you add to Ink shows up in the dashboard automatically. If you find yourself rebuilding the transcript or composer for the dashboard, stop and extend Ink instead.

**Structured React UI around the TUI is allowed when it is not a second chat surface.** Sidebar widgets, inspectors, summaries, status panels, and similar supporting views (e.g. `ChatSidebar`, `ModelPickerDialog`, `ToolCall`) are fine when they complement the embedded TUI rather than replacing the transcript / composer / terminal. Keep their state independent of the PTY child's session and surface their failures non-destructively so the terminal pane keeps working unimpaired.

### Electron Desktop Chat App (`apps/desktop/`)

A **separate** chat surface from both the classic CLI and the dashboard's embedded TUI. It is an Electron + React + nanostore renderer (`@assistant-ui/react`) that talks to a `tui_gateway` backend over JSON-RPC (`requestGateway(method, params)`). It does NOT embed `nozich --tui` — it has its own composer, transcript, and slash-command pipeline. Route desktop bugs to the `nozich-desktop-app-work` skill, not `nozich-dashboard-work`.
...
be core tools. For custom or local-only tools, do **not** edit Nozich core.
Use the plugin route instead: create `~/.nozich/plugins/<name>/plugin.yaml`
and `~/.nozich/plugins/<name>/__init__.py`, then register tools with
`ctx.register_tool(...)`. Plugin toolsets are discovered automatically and can be
enabled or disabled without touching `tools/` or `toolsets.py`.

Use the built-in route below only when the user is explicitly contributing a new
core Nozich tool that should ship in the base system.
...
**2. Add to `toolsets.py`** — either `_NOZICH_CORE_TOOLS` (all platforms) or a new toolset. **This step is required:** auto-discovery imports the tool and registers its schema, but the tool is only *exposed to an agent* if its name appears in a toolset. `_NOZICH_CORE_TOOLS` is not dead code — it's the default bundle every platform's base toolset inherits from.

Auto-discovery: any `tools/*.py` file with a top-level `registry.register()` call is imported automatically — no manual import list to maintain. Wiring into a toolset is still a deliberate, manual step.

The registry handles schema collection, dispatch, availability checking, and error wrapping. All handlers MUST return a JSON string.

**Path references in tool schemas**: If the schema description mentions file paths (e.g. default output directories), use `display_nozich_home()` to make them profile-aware. The schema is generated at import time, which is after `_apply_profile_override()` sets `NOZICH_HOME`.

**State files**: If a tool stores persistent state (caches, logs, checkpoints), use `get_nozich_home()` for the base directory — never `Path.home() / ".nozich"`. This ensures each profile gets its own state.
...
### config.yaml options:
1. Add to `DEFAULT_CONFIG` in `nozich_cli/config.py`
...
1. Add to `OPTIONAL_ENV_VARS` in `nozich_cli/config.py` with metadata:
```python
"NEW_API_KEY": {
...
| `load_config()` | `nozich tools`, `nozich setup`, most CLI subcommands | `nozich_cli/config.py` — merges `DEFAULT_CONFIG` + user YAML |
...
The skin engine (`nozich_cli/skin_engine.py`) provides data-driven CLI visual customization. Skins are **pure data** — no code changes needed to add a new skin.

### Architecture

```
nozich_cli/skin_engine.py    # SkinConfig dataclass, built-in skins, YAML loader
~/.nozich/skins/*.yaml       # User-installed custom skins (drop-in)
```
...
- `default` — Classic Nozich gold/kawaii (the current look)
...
Add to `_BUILTIN_SKINS` dict in `nozich_cli/skin_engine.py`:
...
Users create `~/.nozich/skins/<name>.yaml`:
...
Nozich has two plugin surfaces. Both live under `plugins/` in the repo so
repo-shipped plugins can be discovered alongside user-installed ones in
`~/.nozich/plugins/` and pip-installed entry points.

### General plugins (`nozich_cli/plugins.py` + `plugins/<name>/`)

`PluginManager` discovers plugins from `~/.nozich/plugins/`, `./.nozich/plugins/`,
...
- Register CLI subcommands via `ctx.register_cli_command(...)` — the
  plugin's argparse tree is wired into `nozich` at startup so
  `nozich <pluginname> <subcmd>` works with no change to `main.py`
...
`post_setup(nozich_home, config)` for setup-wizard integration.

**CLI commands via `plugins/memory/<name>/cli.py`:** if a memory plugin
defines `register_cli(subparser)`, `discover_plugin_cli_commands()` finds
it at argparse setup time and wires it into `nozich <plugin>`. The
framework only exposes CLI commands for the **currently active** memory
provider (read from `memory.provider` in config.yaml), so disabled
providers don't clutter `nozich --help`.

**Rule (Teknium, May 2026):** plugins MUST NOT modify core files
(`run_agent.py`, `cli.py`, `gateway/run.py`, `nozich_cli/main.py`, etc.).
...
into `~/.nozich/plugins/` (or via pip entry points) — they implement
the same `MemoryProvider` ABC, register through the same discovery
path, and integrate via `nozich memory setup` / `post_setup()` without
...
2. User: `$NOZICH_HOME/plugins/model-providers/<name>/`
...
[`nozich-example-plugins`](https://github.com/anilcan-kara/nozich-example-plugins)
...
- **`optional-skills/`** — heavier or niche skills shipped with the repo but
  ...
  `nozich skills install official/<category>/<skill>`. Adapter lives in
...
`metadata.nozich.tags`, `metadata.nozich.category`,
`metadata.nozich.related_skills`, `metadata.nozich.config` (config.yaml
settings the skill needs — stored under `skills.config.<key>`, prompted
during setup, injected at load time).

Top-level `tags:` and `category:` are also accepted and mirrored from
`metadata.nozich.*` by the loader.
...
2. **Tools referenced in SKILL.md prose must be native Nozich tools or
...
4. **`author` credits the human contributor first.** For external
   ...
   first; "Nozich Agent" is the secondary collaborator. If the
   contributor's commit shows "Nozich Agent" as author (because they
   used Nozich to draft the skill), replace it with their actual name
...
lives in the `nozich-agent-dev` skill at
...
`"messaging"`); `_NOZICH_CORE_TOOLS` is the default bundle most
...
Enable/disable per platform via `nozich tools` (the curses UI) or the
...
go to `~/.nozich/skills/.archive/` and are restorable.

- **Core:** `agent/curator.py` (review loop, auto-transitions, LLM review
  prompt) + `agent/curator_backup.py` (pre-run tar.gz snapshots).
- **CLI:** `nozich_cli/curator.py` wires `nozich curator <verb>` where
  verbs are: `status`, `run`, `pause`, `resume`, `pin`, `unpin`,
  `archive`, `restore`, `prune`, `backup`, `rollback`.
- **Telemetry:** `tools/skill_usage.py` owns the sidecar
  `~/.nozich/skills/.usage.json` — per-skill `use_count`, `view_count`,
...
schedule jobs via the `cronjob` tool; users via `nozich cron <verb>`
...
- File lock at `~/.nozich/cron/.tick.lock` prevents duplicate ticks
  across processes.
...
collaborate on shared tasks. Users drive it via `nozich kanban <verb>`;
workers spawned by the dispatcher drive it via a dedicated `kanban_*`
toolset so their schema footprint is zero when they're not inside a
kanban task.

- **CLI:** `nozich_cli/kanban.py` wires `nozich kanban` with verbs
...
- **Plugin assets:** `plugins/kanban/dashboard/` (web UI) +
  `plugins/kanban/systemd/` (`nozich-kanban-dispatcher.service` for
  standalone dispatcher deployment).

Isolation model:
- **Board** is the hard boundary — workers are spawned with
  `NOZICH_KANBAN_BOARD` pinned in their env so they can't see other
  boards.
...
Nozich-Agent ensures caching remains valid throughout a conversation. **Do NOT implement changes that would:**
...
in config.yaml (or `NOZICH_BACKGROUND_NOTIFICATIONS` env var):
...
Nozich supports **profiles** — multiple fully isolated instances, each with its own
`NOZICH_HOME` directory (config, API keys, memory, sessions, skills, gateway, etc.).

The core mechanism: `_apply_profile_override()` in `nozich_cli/main.py` sets
`NOZICH_HOME` before any module imports. All `get_nozich_home()` references
automatically scope to the active profile.

### Rules for profile-safe code

1. **Use `get_nozich_home()` for all NOZICH_HOME paths.** Import from `nozich_constants`.
   NEVER hardcode `~/.nozich` or `Path.home() / ".nozich"` in code that reads/writes state.
   ```python
   # GOOD
   ...
   # BAD — breaks profiles
   config_path = Path.home() / ".nozich" / "config.yaml"
   ```

2. **Use `display_nozich_home()` for user-facing messages.** Import from `nozich_constants`.
   This returns `~/.nozich` for default or `~/.nozich/profiles/<name>` for profiles.
   ```python
   # GOOD
   from nozich_constants import display_nozich_home
   print(f"Config saved to {display_nozich_home()}/config.yaml")

   # BAD — shows wrong path for profiles
   print("Config saved to ~/.nozich/config.yaml")
   ```

3. **Module-level constants are fine** — they cache `get_nozich_home()` at import time,
   which is AFTER `_apply_profile_override()` sets the env var. Just use `get_nozich_home()`,
   not `Path.home() / ".nozich"`.

4. **Tests that mock `Path.home()` must also set `NOZICH_HOME`** — since code now uses
   `get_nozich_home()` (reads env var), not `Path.home() / ".nozich"`:
   ```python
   with patch.object(Path, "home", return_value=tmp_path), \
        patch.dict(os.environ, {"NOZICH_HOME": str(tmp_path / ".nozich")}):
       ...
   ```
...
6. **Profile operations are HOME-anchored, not NOZICH_HOME-anchored** — `_get_profiles_root()`
   returns `Path.home() / ".nozich" / "profiles"`, NOT `get_nozich_home() / "profiles"`.
   This is intentional — it lets `nozich -p coder profile list` see all profiles regardless
   of which one is active.

## Known Pitfalls

### DO NOT hardcode `~/.nozich` paths
Use `get_nozich_home()` from `nozich_constants` for code paths. Use `display_nozich_home()`
for user-facing print/log messages. Hardcoding `~/.nozich` breaks profiles — each profile
has its own `NOZICH_HOME` directory. This was the source of 5 bugs fixed in PR #3575.

### DO NOT introduce new `simple_term_menu` usage
Existing call sites in `nozich_cli/main.py` remain for legacy fallback only;
the preferred UI is curses (stdlib) because `simple_term_menu` has
ghost-duplication rendering bugs in tmux/iTerm2 with arrow keys. New
interactive menus must use `nozich_cli/curses_ui.py` — see
`nozich_cli/tools_config.py` for the canonical pattern.
...
with actual imports (not mocks) against a temp `NOZICH_HOME`.

### Tests must not write to `~/.nozich/`
The `_isolate_nozich_home` autouse fixture in `tests/conftest.py` redirects `NOZICH_HOME` to a temp dir. Never hardcode `~/.nozich/` paths in tests.

**Profile tests**: When testing profile features, also mock `Path.home()` so that
`_get_profiles_root()` and `_get_default_nozich_home()` resolve within the temp dir.
Use the pattern from `tests/nozich_cli/test_profiles.py`:
```python
@pytest.fixture
def profile_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("NOZICH_HOME", str(home))
    return home
```
...
- The plugin disables itself in child processes (sentinel envvar
  `NOZICH_ISOLATE_CHILD=1`), so there's no fork-bomb risk.
...
| HOME / `~/.nozich/` | Your real config+auth.json | Temp dir per test |
