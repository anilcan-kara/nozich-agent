# Nozich Agent Security Policy

This document describes Nozich Agent's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/anilcan-kara/nozich-agent/security/advisories/new)
or **security@anilcan-kara.com**. Do not open public issues for
security vulnerabilities. **Nozich Agent does not operate a bug
bounty program.**
...
- Environment details (`nozich version`, commit SHA, OS, Python
  version).
...
Nozich Agent is a single-tenant personal agent. Its posture is
layered, and the layers are not equally load-bearing. Reporters and
operators should reason about them in the same terms.

### 2.1 Definitions

- **Agent process.** The Python interpreter running Nozich Agent,
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Nozich Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Nozich Agent's documentation
...
Nozich Agent supports two OS-level isolation postures. They address
...
Nozich Agent supports this in two ways:

- **Nozich Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Nozich Agent's in-process heuristics
...
Nozich Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Nozich Agent
itself. Bugs in Nozich Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Nozich Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Nozich Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  Nozich Agent has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  nozich-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Nozich Agent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Nozich Agent. Bugs in Nozich
...
- Nozich Agent includes supply-chain guards for MCP server
