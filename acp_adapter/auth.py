"""ACP auth helpers — detect and advertise Nozich authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "nozich-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Nozich runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Nozich can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Nozich.
    ...
    not have Nozich provider credentials configured yet, so Nozich always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Nozich using the currently configured "
