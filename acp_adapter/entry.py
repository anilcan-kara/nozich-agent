"""CLI entry point for the nozich-agent ACP adapter.

Loads environment variables from ``~/.nozich/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from NOZICH_HOME (default ``~/.nozich``)."""
    from nozich_cli.env_loader import load_nozich_dotenv

    nozich_home = get_nozich_home()
    loaded = load_nozich_dotenv(nozich_home=nozich_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", nozich_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="nozich-acp",
        description="Run Nozich Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Nozich version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Nozich provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.nozich/node/ "
...
def _print_version() -> None:
    ...
    print(nozich_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import NozichACPAgent  # noqa: F401

    print("Nozich ACP check OK")


def _run_setup() -> None:
    from nozich_cli.main import main as nozich_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "nozich", "model"]
        nozich_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``nozich postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting nozich-agent ACP adapter")
    ...
    import acp
    from .server import NozichACPAgent
