...
``auth.json``.  They intentionally have no dependency on ``nozich_cli.auth`` so
...
# Sources Nozich owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Nozich-owned OAuth/device-code state)
