...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``nozich model`` / ``nozich auth`` which
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another Nozich process
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        ``nozich`` invocation that defaults to the active provider
        (e.g. setup wizard, ``nozich auth status``) would land on
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("nozich_pkce"),
                )
                updated = replace(
                    entry,
                ...
                # refresh_token — single-use tokens consumed by another Nozich
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("nozich_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
            ...
            if self.provider == "xai-oauth":
                ...
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    self._persist()
                    return None
            # For openai-codex: same race as xAI/nous — another Nozich process
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Nozich profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `nozich model` / `nozich auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `nozich model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``nozich auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `nozich auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `nozich auth remove <provider> <N>` is stable across all source types.
    try:
        from nozich_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Nozich PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `nozich setup` ("Claude
        ...
        # Claude Code CLI, nozich_pkce creds from a previous OAuth login)
        ...
        # that masquerade.  Prefer ~/.nozich/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `nozich setup` writes.
        _env_file = load_env()
        ...
        if api_key_path_explicit:
            ...
            retained = [
                ...
                if entry.source not in {"nozich_pkce", "claude_code"}
            ]
        ...
        from agent.anthropic_adapter import read_claude_code_credentials, read_nozich_oauth_credentials

        for source_name, creds in (
            ("nozich_pkce", read_nozich_oauth_credentials()),
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `nozich auth add nous --label <name>`).  Fall back to the
        ...
        # Nozich auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from nozich_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.nozich/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `nozich auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `nozich auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the Nozich auth store.  Without this gate the removal is instantly
        ...
        # Nozich owns its own Codex auth state — we do NOT auto-import from
        ...
        # When the user logs in via ``nozich model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``nozich auth list`` reflects the logged-in state and so the pool
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.nozich/.env over os.environ — the user's config file is the
    # authoritative source for Nozich credentials. Stale env vars from parent
    ...
    # Honour user suppression — `nozich auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.nozich/.env.
    ...
    try:
        from nozich_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
    ...
    def _secret_source_for_env(env_var: str) -> Optional[str]:
        try:
            from nozich_cli.env_loader import get_secret_source
    ...
    if provider == "openrouter":
        # Prefer ~/.nozich/.env over os.environ
