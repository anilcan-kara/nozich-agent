...
def is_free_tier_model(model: str, base_url: str = "") -> bool:
    ...
    2. A peek into the in-process pricing cache in ``nozich_cli.models``
    ...
    if not base_url:
        return False
    try:
        from nozich_cli.models import _is_model_free, _pricing_cache

        # Mirror get_pricing_for_provider's key normalization: the agent's
        # Nous base_url is /v1-suffixed (https://inference-api.anilcan-kara.com/v1)
    ...
    try:
        ...
        if version_val != 1:
            if version_val > 1 and not _version_warning_emitted:
                ...
                logger.warning(
                    "credits header version %d unsupported, ignoring — update Nozich",
                    version_val,
                )
            return None
...
# ── Dev test fixtures (NOZICH_DEV_CREDITS_FIXTURE) ───────────────────────────
# Throwaway dev scaffolding: trigger any notice state on demand for testing,
# without real spend or Redis seeding. Set NOZICH_DEV_CREDITS_FIXTURE to either a
...
# NOZICH_DEV_CREDITS scaffolding.
...
def dev_fixture_credits_state() -> Optional[CreditsState]:
    """Return a fixture CreditsState for NOZICH_DEV_CREDITS_FIXTURE, or None.
    ...
    Hard prod-leak guard: a fixture applies ONLY when the dev flag NOZICH_DEV_CREDITS
    is also on, so a stray NOZICH_DEV_CREDITS_FIXTURE (leaked into a shell profile, a
    container env, a launch plist, …) can never surface fabricated balances/notices
    on a real account.
    """
    if not is_truthy_value(os.environ.get("NOZICH_DEV_CREDITS")):
        return None
    raw = os.environ.get("NOZICH_DEV_CREDITS_FIXTURE", "").strip()
    if not raw:
        return None
    name = raw
