...
def _state_file() -> Path:
    return get_nozich_home() / "skills" / ".curator_state"
...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.nozich/config.yaml. Tolerates missing file."""
    try:
...
def get_prune_builtins() -> bool:
    ...
    suppression list keeping them archived across `nozich update` re-seeds.
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``nozich update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``nozich curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `nozich curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    "You are running as Nozich' background skill CURATOR. This is an "
    ...
    "into ~/.nozich/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: nozich-config-*, nozich-dashboard-*, gateway-*, codex-*, "
    ...
    "~/.nozich/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.nozich/logs/curator/``)
    ...
    authored skill data in ``~/.nozich/skills/``.

    ``ensure_nozich_home()`` pre-creates this dir on every CLI launch and
    ...
    that bypasses both.
    """
    root = get_nozich_home() / "logs" / "curator"
    try:
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every nozich install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: nozich curator status
        keep an umbrella stable: nozich curator pin document-tools
    ...
    lines.append("full report: nozich curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: nozich curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.nozich/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.nozich/skills/.archive/` for "
            "safety and can be restored via `nozich curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.nozich/skills/.archive/`. "
            "Restore any via `nozich curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `nozich curator restore <name>`")
    lines.append("- All archives live under `~/.nozich/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    # real pass out. We still record a summary so `nozich curator status`
    ...
    def _llm_pass():
            ...
            if "No agent-created skills" in candidate_list:
                ...
                if get_prune_builtins():
                    builtins_note = (
                        ...
                        "delete). It will be restored on `nozich update` only if "
        ...
        # recorded in state so `nozich curator status` can point at it.
        try:
            after_report = skill_usage.agent_created_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``nozich model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `nozich model` → auxiliary
