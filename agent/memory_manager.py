...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_NOZICH_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, Nozich expands the turn into
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``nozich_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_nozich_home()`` themselves.
        """
        if "nozich_home" not in kwargs:
            from nozich_constants import get_nozich_home
            kwargs["nozich_home"] = str(get_nozich_home())
        for provider in self._providers:
            try:
