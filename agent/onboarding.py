...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Nozich starts and finds ``~/.openclaw/``.

    Points users at ``nozich claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``nozich claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Nozich, run "
        "`nozich claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `nozich claw cleanup` (renames it to ~/.openclaw.pre-migration — "
