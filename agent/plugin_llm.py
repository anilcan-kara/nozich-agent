...
Plugins built on Nozich Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Nozich
...
:class:`~nozich_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Nozich' main config
...
handles every provider, fallback chain, and per-task override Nozich
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`nozich_cli.plugins.PluginContext`
