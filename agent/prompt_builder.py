...
from nozich_constants import get_nozich_home, get_skills_dir, is_wsl
...
_NOZICH_MD_NAMES = (".nozich.md", "NOZICH.md")


def _find_nozich_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.nozich.md`` or ``NOZICH.md``.
...
DEFAULT_AGENT_IDENTITY = (
    "You are Nozich Agent, an intelligent AI assistant created by Nous Research. "
...
NOZICH_AGENT_HELP_GUIDANCE = (
    "You run on Nozich Agent (by Nous Research). When the user needs help with "
    "Nozich itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://nozich-agent.anilcan-kara.com/docs is your "
    "authoritative reference and always holds the latest, most up-to-date "
    "information. Load the `nozich-agent` skill with skill_view(name='nozich-agent') "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.nozich/kanban.db`. Your task id is in "
    "`$NOZICH_KANBAN_TASK`; your workspace is `$NOZICH_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $NOZICH_KANBAN_WORKSPACE` before "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `nozich kanban <verb>` for board operations. Use "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Nozich "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Nozich WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Nozich itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Nozich runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Nozich itself is running. The host OS, home, and cwd "
                f"of the Nozich process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Nozich "
    ...
    # Nozich desktop GUI — any agent running under the desktop app should know
    # it. NOZICH_DESKTOP marks the backend powering the chat; NOZICH_DESKTOP_TERMINAL
    # marks a nozich launched in the embedded terminal pane. Both set by main.cjs.
    _truthy = ("1", "true", "yes")
    _in_desktop = (os.getenv("NOZICH_DESKTOP") or "").strip().lower() in _truthy
    _in_desktop_term = (os.getenv("NOZICH_DESKTOP_TERMINAL") or "").strip().lower() in _truthy
    if _in_desktop or _in_desktop_term:
        _desktop_hint = "Runtime surface: you're running inside the Nozich desktop GUI app."
        if _in_desktop_term:
            _desktop_hint += (
    ...
    # Embedder-supplied environment description. Lets a host that wraps Nozich
    ...
    extra = (os.getenv("NOZICH_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _get_context_file_max_chars() -> int:
    ...
    ``context_file_max_chars`` in config.yaml without patching Nozich.
    """
    try:
        from nozich_cli.config import load_config
...
def _skills_prompt_snapshot_path() -> Path:
    return get_nozich_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.nozich/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("NOZICH_PLATFORM")
        or get_session_env("NOZICH_SESSION_PLATFORM")
        or ""
    )
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Nozich Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `nozich-agent` skill "
            "first. It has the actual commands (e.g. `nozich config set …`, `nozich tools`, "
            "`nozich setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: nozich setup, nozich setup tools, nozich setup terminal, nozich status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from NOZICH_HOME and return its content, or None.
    ...
    """
    try:
        from nozich_cli.config import ensure_nozich_home
        ensure_nozich_home()
    except Exception as e:
        logger.debug("Could not ensure NOZICH_HOME before loading SOUL.md: %s", e)

    soul_path = get_nozich_home() / "SOUL.md"
...
def _load_nozich_md(cwd_path: Path) -> str:
    """.nozich.md / NOZICH.md — walk to git root."""
    nozich_md_path = _find_nozich_md(cwd_path)
    if not nozich_md_path:
        return ""
    try:
        content = nozich_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = nozich_md_path.name
        try:
            rel = str(nozich_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".nozich.md")
    except Exception as e:
        logger.debug("Could not read %s: %s", nozich_md_path, e)
        return ""
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .nozich.md / NOZICH.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from NOZICH_HOME is independent and always included when present.
    ...
    # SOUL.md from NOZICH_HOME only — skip when already loaded as identity
    if not skip_soul:
