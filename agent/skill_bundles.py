...
Storage
-------
Bundles live in ``~/.nozich/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``NOZICH_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``nozich bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under NOZICH_HOME.

    Honors ``NOZICH_BUNDLES_DIR`` for tests; falls back to
    ``<NOZICH_HOME>/skill-bundles``.
    """
    override = os.environ.get("NOZICH_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_nozich_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `nozich bundles` CLI subcommand.
# ---------------------------------------------------------------------------
