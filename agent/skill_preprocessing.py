...
# Matches ${NOZICH_SKILL_DIR} / ${NOZICH_SESSION_ID} tokens in SKILL.md.
# Tokens that don't resolve (e.g. ${NOZICH_SESSION_ID} with no session) are
# left as-is so the user can debug them.
_SKILL_TEMPLATE_RE = re.compile(r"\$\{(NOZICH_SKILL_DIR|NOZICH_SESSION_ID)\}")
...
def substitute_template_vars(
    ...
    """Replace ${NOZICH_SKILL_DIR} / ${NOZICH_SESSION_ID} in skill content.
    ...
    def _replace(match: re.Match) -> str:
        token = match.group(1)
        if token == "NOZICH_SKILL_DIR" and skill_dir_str:
            return skill_dir_str
        if token == "NOZICH_SESSION_ID" and session_id:
