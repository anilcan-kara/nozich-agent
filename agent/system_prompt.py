...
upstream prefix cache warm.  See ``nozich-agent-dev``'s
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Nozich never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want NOZICH_HOME persona while keeping
    ...
    # Pointer to the nozich-agent skill + docs for user questions about Nozich itself.
    stable_parts.append(NOZICH_AGENT_HELP_GUIDANCE)
    ...
    # NOZICH_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base Nozich, any interactive coding surface in a code
    ...
    # Active-profile hint — names the Nozich profile the agent is running
    # under so it doesn't conflate ~/.nozich/skills/ (default profile) with
    # ~/.nozich/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Nozich profile: default. Other profiles (if any) live "
            "under ~/.nozich/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Nozich profile: {active_profile}. This session reads "
            f"and writes ~/.nozich/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.nozich/skills/, ~/.nozich/plugins/, "
            f"~/.nozich/cron/, ~/.nozich/memories/ — those belong to a "
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Nozich never rebuilds or reinjects parts of it
