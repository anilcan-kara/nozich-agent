...
Owns one Codex thread per Nozich session. Drives `turn/start`, consumes
...
# Nozich' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_NOZICH_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Nozich/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Nozich session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _NOZICH_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("NOZICH_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="nozich",
            client_title="Nozich Agent",
            client_version=_get_nozich_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Nozich' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Nozich' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own nozich-tools callback we
            # auto-accept — the user already approved Nozich' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Nozich approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Nozich returns 'once', 'session', 'always', or 'deny'.
...
def _get_nozich_version() -> str:
    """Best-effort Nozich version string for codex's userAgent line."""
    try:
