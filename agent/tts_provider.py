...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Nozich with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.nozich/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Nozich
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``nozich tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``nozich setup`` for
