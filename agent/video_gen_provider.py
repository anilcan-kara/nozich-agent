...
as ``kind: backend``) or ``~/.nozich/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``nozich tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$NOZICH_HOME/cache/videos/``, creating parents as needed."""
    from nozich_constants import get_nozich_home

    path = get_nozich_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$NOZICH_HOME/cache/videos/``.
