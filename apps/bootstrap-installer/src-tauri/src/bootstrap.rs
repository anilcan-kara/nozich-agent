...
//! NOZICH_HOME/logs/bootstrap-<timestamp>.log.
//!
//! Lifecycle:
...
pub struct StartBootstrapArgs {
    ...
    /// Optional override for NOZICH_HOME. Tests use this; production
    /// almost always falls back to the OS default.
    pub nozich_home: Option<String>,
}
...
/// Spawn the locally-built Nozich desktop binary, then close the installer
...
#[tauri::command]
pub async fn launch_nozich_desktop(
    app: AppHandle,
    install_root: String,
) -> Result<(), String> {
    let install_root = PathBuf::from(install_root);
    let exe_path = resolve_nozich_desktop_exe(&install_root).ok_or_else(|| {
        format!(
            "Couldn't find a built Nozich desktop at {}. The desktop build step \
             may have been skipped or failed. Run `nozich desktop` from a \
             terminal to build and launch it.",
            install_root.join("apps").join("desktop").join("release").display()
        )
    })?;

    tracing::info!(?exe_path, "launching Nozich desktop");

    // Detach from us — the installer is about to exit. On macOS launch the
    // bundle through LaunchServices instead of exec'ing Contents/MacOS/Nozich
...
/// <os>-unpacked/<exe>).
pub(crate) fn resolve_nozich_desktop_exe(install_root: &std::path::Path) -> Option<PathBuf> {
    let release_dir = install_root.join("apps").join("desktop").join("release");
    let candidates: &[(&str, &str)] = if cfg!(target_os = "windows") {
        &[
            ("win-unpacked", "Nozich.exe"),
            ("win-arm64-unpacked", "Nozich.exe"),
        ]
    } else if cfg!(target_os = "macos") {
        &[
            ("mac/Nozich.app/Contents/MacOS", "Nozich"),
            ("mac-arm64/Nozich.app/Contents/MacOS", "Nozich"),
        ]
    } else {
        &[("linux-unpacked", "nozich")]
    };
...
pub(crate) fn resolve_nozich_desktop_app(install_root: &std::path::Path) -> Option<PathBuf> {
    let exe = resolve_nozich_desktop_exe(install_root)?;
    #[cfg(target_os = "macos")]
    {
        // .../Nozich.app/Contents/MacOS/Nozich -> .../Nozich.app
...
/// path so a bare re-open just opens Nozich instead of re-running setup.
pub(crate) fn nozich_is_installed(install_root: &std::path::Path) -> bool {
    install_root.join(".nozich-bootstrap-complete").exists()
        && resolve_nozich_desktop_exe(install_root).is_some()
}
...
pub(crate) fn spawn_installed_desktop(install_root: &std::path::Path) -> std::io::Result<()> {
    let exe = resolve_nozich_desktop_exe(install_root).ok_or_else(|| {
        std::io::Error::new(std::io::ErrorKind::NotFound, "no built Nozich desktop app")
    })?;
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        // installer exits, mirroring launch_nozich_desktop. Kept correct here
...
pub(crate) fn open_macos_app_detached(app_bundle: &std::path::Path) -> std::io::Result<()> {
    ...
    cmd.arg(app_bundle);
    cmd.current_dir(crate::paths::nozich_home());
    cmd.spawn().map(|_child| ())
}
...
async fn run_bootstrap(
    ...
    // explicitly; we infer it from $NozichHome which Stage-Repository clones
    // the repo INTO at $NozichHome\nozich-agent. Mirrors nozich_constants.
    let nozich_home = args
        .nozich_home
        .clone()
        .unwrap_or_else(|| crate::paths::nozich_home().to_string_lossy().into_owned());
    let install_root = PathBuf::from(&nozich_home).join("nozich-agent");

    // Copy ourselves to NOZICH_HOME/nozich-setup.exe so the desktop app can
    ...
    if let Err(err) = crate::paths::copy_self_to_nozich_home() {
        tracing::warn!(?err, "failed to copy installer into NOZICH_HOME (non-fatal)");
        emit_log(&format!(
...
async fn run_install_script(
    ...
    args: &[String],
    nozich_home_override: Option<&str>,
    ...
    powershell::run_script(script_path, args, sink, nozich_home_override, cancel_rx)
        .await
        .map_err(|e| {
...
mod tests {
    ...
    fn make_release_tree(install_root: &Path) -> PathBuf {
        ...
        if cfg!(target_os = "macos") {
            ...
            std::fs::write(macos_dir.join("Nozich"), b"#!/bin/sh\n").unwrap();
            macos_dir.parent().unwrap().parent().unwrap().to_path_buf() // .../Nozich.app
        } else if cfg!(target_os = "windows") {
            let dir = release.join("win-unpacked");
            std::fs::create_dir_all(&dir).unwrap();
            let exe = dir.join("Nozich.exe");
            std::fs::write(&exe, b"stub").unwrap();
            exe
        } else {
            let dir = release.join("linux-unpacked");
            std::fs::create_dir_all(&dir).unwrap();
            let exe = dir.join("nozich");
            std::fs::write(&exe, b"stub").unwrap();
            exe
        }
    }
    ...
    // what the updater ditto's over /Applications/Nozich.app). A regression in
    // this derivation breaks the post-update auto-relaunch, so guard it.
    #[test]
    fn resolve_nozich_desktop_app_finds_built_bundle() {
        let root = unique_tmp_dir("app-ok");
        let expected = make_release_tree(&root);

        let resolved = resolve_nozich_desktop_app(&root)
    ...
    #[test]
    fn resolve_nozich_desktop_app_is_none_without_a_build() {
        let root = unique_tmp_dir("app-none");
        // No release tree created.
        assert!(
            resolve_nozich_desktop_app(&root).is_none(),
