...
//!
//! Resolution order:
//!   1. Dev shortcut: a sibling repo checkout via $NOZICH_SETUP_DEV_REPO_ROOT
...
//! Electron app's APP_ROOT/../.. trick, because Nozich-Setup.exe is meant
...
///
/// `pin` is the commit-or-branch from either Nozich-Setup's build-time
...
pub async fn resolve(
    ...
    // 1. Dev shortcut.
    if let Ok(repo_root) = std::env::var("NOZICH_SETUP_DEV_REPO_ROOT") {
...
async fn download(kind: ScriptKind, commit_or_ref: &str, dest_path: &Path) -> Result<()> {
    let url = format!(
        "https://raw.githubusercontent.com/anilcan-kara/nozich-agent/{}/scripts/{}",
        commit_or_ref,
        kind.filename()
    );
    ...
    let response = reqwest::Client::new()
        .get(&url)
        .header("User-Agent", "nozich-setup/0.0.1")
        .send()
        .await
