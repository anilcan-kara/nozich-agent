...
/// desktop app handing off via `Nozich-Setup.exe --update`).
///
...
pub fn run() {
    // Tracing → bootstrap-installer.log under NOZICH_HOME/logs/ so install
    ...
    // Nozich is already installed, so users can re-run setup to repair a broken
    ...
    tracing::info!(?mode, force_setup, "Nozich installer starting");
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // Nozich is already installed should NOT show the installer or
            // rebuild — it should just open the app, so the /Applications
            // "Nozich" doubles as a normal launcher (first run installs, every
            ...
            // Menu/Desktop "Nozich" shortcuts that install.ps1 creates, and a
            // reliable detached relaunch there needs the DETACHED_PROCESS +
            // startup-grace handling used by launch_nozich_desktop — out of
            ...
            if cfg!(target_os = "macos") && mode == AppMode::Install && !force_setup {
                let install_root = paths::nozich_home().join("nozich-agent");
                if bootstrap::nozich_is_installed(&install_root) {
                    match bootstrap::spawn_installed_desktop(&install_root) {
                        Ok(()) => {
                            // Brief grace so the spawned app is registered
                            // before we exit (mirrors launch_nozich_desktop).
                            std::thread::sleep(std::time::Duration::from_millis(200));
                            tracing::info!(
                                "nozich already installed — relaunched desktop; exiting installer"
                            );
                            app.handle().exit(0);
                            return Ok(());
                        }
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            update::start_update,
            // Hand-off
            bootstrap::launch_nozich_desktop,
            // Diagnostics
            paths::get_log_path,
            paths::get_nozich_home,
            paths::open_log_dir,
        ])
        .run(tauri::generate_context!())
        .expect("error while running Nozich Setup");
}
