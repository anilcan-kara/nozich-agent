...
//!
//! Mirrors `nozich_constants.get_nozich_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\nozich
//!   macOS:   ~/.nozich
//!   Linux:   ~/.nozich  (override via $NOZICH_HOME)
//!
//! NOTE (macOS): Python's get_nozich_home(), scripts/install.sh, and the
//! Electron desktop's resolveNozichHome() ALL use ~/.nozich on macOS — there
...
/// Returns the canonical Nozich home directory, respecting $NOZICH_HOME if set.
pub fn nozich_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("NOZICH_HOME") {
    ...
    {
        // %LOCALAPPDATA%\nozich — matches scripts/install.ps1's $NozichHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("nozich");
        }
    }

    // macOS + Linux + fallback: ~/.nozich (matches Python get_nozich_home(),
    // install.sh, and the Electron desktop's resolveNozichHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".nozich");
    }
...
pub fn bootstrap_cache_dir() -> PathBuf {
    nozich_home().join("bootstrap-cache")
}
...
/// NOZICH_HOME so it survives repo checkout deletion (unlike anything under
/// nozich-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\nozich\nozich-setup.exe`; on other
...
pub fn copy_self_to_nozich_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to NOZICH_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_NOZICH_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".nozich-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under NOZICH_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[nozich-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("NOZICH_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_nozich_home() -> String {
    nozich_home().to_string_lossy().into_owned()
}
