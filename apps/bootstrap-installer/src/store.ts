...
export const $nozichHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  try {
    const [logPath, nozichHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_nozich_home'),
      invoke<AppMode>('get_mode')
    ])
    $logPath.set(logPath)
    $nozichHome.set(nozichHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Nozich" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  // Update is driven by the desktop handing off (Nozich-Setup.exe --update);
...
export async function launchNozichDesktop(): Promise<void> {
  const installRoot = $bootstrap.get().installRoot
  if (!installRoot) throw new Error('no install root')
  await invoke('launch_nozich_desktop', { installRoot })
}
