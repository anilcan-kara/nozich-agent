 ...
 *
 * Drives apps/desktop's first-launch install of Nozich Agent by spawning
...
function bootstrapCacheDir(nozichHome) {
  return path.join(nozichHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.nozich/nozich-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(nozichHome) {
  if (!nozichHome) return null
  const candidate = path.join(nozichHome, 'nozich-agent', 'scripts', installScriptName())
  try {
    fs.accessSync(candidate, fs.constants.R_OK)
    return candidate
  } catch {
...
function cachedScriptPath(nozichHome, commit) {
  return path.join(bootstrapCacheDir(nozichHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(commit, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/anilcan-kara/nozich-agent/${commit}/scripts/${scriptName}`
...
async function resolveInstallScript({ installStamp, sourceRepoRoot, nozichHome, emit, _download = downloadInstallScript }) {
  ...
  const cached = cachedScriptPath(nozichHome, installStamp.commit)
  try {
  ...
  try {
    ...
    const installed = installedAgentInstallScript(nozichHome)
    if (installed) {
      emit({
        type: 'log',
        line:
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, nozichHome } = {}) {
  return new Promise((resolve, reject) => {
    ...
    const child = spawn(ps, fullArgs, hiddenWindowsChildOptions({
      ...
      env: {
        ...process.env,
        // Pass NOZICH_HOME through so install.ps1 respects the caller's
        // choice rather than re-computing the default.
        NOZICH_HOME: nozichHome || process.env.NOZICH_HOME || ''
      }
    }))
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, nozichHome } = {}) {
  return new Promise((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, nozichHome }) {
  const args = ['--dir', activeRoot, '--nozich-home', nozichHome]
  if (installStamp && installStamp.branch) {
    args.push('--branch', installStamp.branch)
  }
  if (installStamp && installStamp.commit) {
...
async function fetchManifest({ scriptPath, installerKind, emit, nozichHome, activeRoot, installStamp }) {
  const isPosix = installerKind === 'posix'
  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, nozichHome })]
...
async function runStage({ scriptPath, installerKind, stage, emit, nozichHome, activeRoot, abortSignal, installStamp }) {
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, nozichHome })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(nozichHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, nozichHome, emit })
