...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'nozich-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/nozich-runner-test',
    sourceRepoRoot: null,
    nozichHome: '/tmp/nozich-runner-test',
    logRoot: '/tmp/nozich-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'nozich-agent', 'scripts')
