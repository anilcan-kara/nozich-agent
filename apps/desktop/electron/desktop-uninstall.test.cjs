...
test('uninstallArgsForMode maps each mode to the module-runner argv', () => {
  assert.deepEqual(uninstallArgsForMode('gui'), ['-m', 'nozich_cli.uninstall', '--mode', 'gui'])
  assert.deepEqual(uninstallArgsForMode('lite'), ['-m', 'nozich_cli.uninstall', '--mode', 'lite'])
  assert.deepEqual(uninstallArgsForMode('full'), ['-m', 'nozich_cli.uninstall', '--mode', 'full'])
})
...
test('resolveRemovableAppPath finds the .app bundle on macOS', () => {
  assert.equal(
    resolveRemovableAppPath('/Applications/Nozich.app/Contents/MacOS/Nozich', 'darwin'),
    '/Applications/Nozich.app'
  )
  assert.equal(
    resolveRemovableAppPath('/Users/x/Applications/Nozich.app/Contents/MacOS/Nozich', 'darwin'),
    '/Users/x/Applications/Nozich.app'
  )
})
...
test('resolveRemovableAppPath finds the install dir on Windows', () => {
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\Programs\\Nozich\\Nozich.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\Programs\\Nozich'
  )
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\nozich-desktop\\Nozich.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\nozich-desktop'
  )
})

test('resolveRemovableAppPath returns null for an unrecognized Windows dir', () => {
  assert.equal(resolveRemovableAppPath('C:\\Temp\\foo\\Nozich.exe', 'win32'), null)
})

test('resolveRemovableAppPath uses APPIMAGE on Linux when set', () => {
  assert.equal(
    resolveRemovableAppPath('/tmp/.mount_NozichXXXX/nozich', 'linux', { APPIMAGE: '/home/x/Apps/Nozich.AppImage' }),
    '/home/x/Apps/Nozich.AppImage'
  )
})

test('resolveRemovableAppPath finds the unpacked dir on Linux', () => {
  assert.equal(
    resolveRemovableAppPath('/opt/nozich/linux-unpacked/nozich', 'linux', {}),
    '/opt/nozich/linux-unpacked'
  )
  // A system-package install (/usr/bin) → null, left to apt/dnf.
  assert.equal(resolveRemovableAppPath('/usr/bin/nozich', 'linux', {}), null)
})
...
test('shouldRemoveAppBundle requires packaged AND a resolved path', () => {
  assert.equal(shouldRemoveAppBundle(true, '/Applications/Nozich.app'), true)
  assert.equal(shouldRemoveAppBundle(false, '/Applications/Nozich.app'), false)
...
test('buildPosixCleanupScript waits for the PID, runs the uninstall module, removes bundle', () => {
  const script = buildPosixCleanupScript({
    desktopPid: 4321,
    pythonExe: '/home/x/.nozich/nozich-agent/venv/bin/python',
    pythonPath: null,
    agentRoot: '/home/x/.nozich/nozich-agent',
    uninstallArgs: ['-m', 'nozich_cli.uninstall', '--mode', 'gui'],
    appPath: '/opt/nozich/linux-unpacked',
    nozichHome: '/home/x/.nozich'
  })
  ...
  assert.match(script, /'-m' 'nozich_cli\.uninstall' '--mode' 'gui'/)
  assert.match(script, /rm -rf '\/opt\/nozich\/linux-unpacked'/)
  assert.match(script, /export NOZICH_HOME='\/home\/x\/\.nozich'/)
})
...
test('buildPosixCleanupScript exports PYTHONPATH when pythonPath is set (lite/full)', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: '/home/x/.nozich/nozich-agent',
    agentRoot: '/home/x/.nozich/nozich-agent',
    uninstallArgs: ['-m', 'nozich_cli.uninstall', '--mode', 'full'],
    appPath: null,
    nozichHome: '/home/x/.nozich'
  })
  // System python + source on PYTHONPATH so import nozich_cli works while the
  // venv is torn down.
  assert.match(script, /export PYTHONPATH='\/home\/x\/\.nozich\/nozich-agent'/)
  assert.match(script, /'\/usr\/bin\/python3' '-m' 'nozich_cli\.uninstall' '--mode' 'full'/)
})
...
test('buildPosixCleanupScript omits the bundle rm when appPath is null', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: null,
    agentRoot: '/a',
    uninstallArgs: ['-m', 'nozich_cli.uninstall', '--mode', 'lite'],
    appPath: null,
    nozichHome: '/h'
  })
  assert.doesNotMatch(script, /rm -rf '\//)
  // Still runs the uninstall.
  assert.match(script, /'-m' 'nozich_cli\.uninstall' '--mode' 'lite'/)
})
...
test('buildWindowsCleanupScript waits (bounded) for PID, runs uninstall, rmdir bundle', () => {
  const script = buildWindowsCleanupScript({
    ...
    agentRoot: 'C:\\nozich',
    uninstallArgs: ['-m', 'nozich_cli.uninstall', '--mode', 'full'],
    appPath: 'C:\\Users\\x\\AppData\\Local\\Programs\\Nozich',
    nozichHome: 'C:\\Users\\x\\AppData\\Local\\nozich'
  })
  assert.match(script, /@echo off/)
  assert.match(script, /set "PID=9988"/)
  // PYTHONPATH set so a system python can import nozich_cli from source.
  assert.match(script, /set "PYTHONPATH=C:\\nozich;%PYTHONPATH%"/)
  assert.match(script, /"C:\\Python313\\python.exe" "-m" "nozich_cli\.uninstall" "--mode" "full"/)
  ...
  assert.match(script, /rmdir \/s \/q "C:\\Users\\x\\AppData\\Local\\Programs\\Nozich" >nul 2>&1/)
