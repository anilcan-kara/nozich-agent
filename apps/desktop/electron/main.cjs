...
const { buildDesktopBackendEnv, normalizeNozichHomeRoot } = require('./backend-env.cjs')
...
const USER_DATA_OVERRIDE = process.env.NOZICH_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.NOZICH_DESKTOP_DEV_SERVER
...
// switches only apply pre-launch. Override with NOZICH_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[nozich] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[nozich] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
          continue
        }
...
if (INSTALL_STAMP) {
  console.log(
    `[nozich] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[nozich] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// NOZICH_HOME — the user-facing root for everything Nozich-related. Mirrors
// scripts/install.ps1's $NozichHome and scripts/install.sh's $NOZICH_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\nozich (matches install.ps1)
//   macOS / Linux: ~/.nozich (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.nozich directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\nozich yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// NOZICH_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// NOZICH_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.nozich / %LOCALAPPDATA%\nozich.
function resolveNozichHome() {
  if (process.env.NOZICH_HOME) return normalizeNozichHomeRoot(process.env.NOZICH_HOME)
  if (USER_DATA_OVERRIDE) return path.join(path.resolve(USER_DATA_OVERRIDE), 'nozich-home')
  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a NOZICH_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\nozich and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('NOZICH_HOME')
    if (fromRegistry) return normalizeNozichHomeRoot(fromRegistry)
  }
  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'nozich')
    const legacy = path.join(app.getPath('home'), '.nozich')
    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.nozich setup (no LOCALAPPDATA install yet) so users don't lose state.
    if (!directoryExists(localappdata) && directoryExists(legacy)) return legacy
    return localappdata
  }
  return path.join(app.getPath('home'), '.nozich')
}

const NOZICH_HOME = resolveNozichHome()
// ACTIVE_NOZICH_ROOT — the canonical mutable Nozich install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_NOZICH_ROOT = path.join(NOZICH_HOME, 'nozich-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_NOZICH_ROOT, 'venv')
...
// boots, going straight to `resolveNozichBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_NOZICH_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_NOZICH_ROOT, '.nozich-bootstrap-complete')
const BOOTSTRAP_MARKER_SCHEMA_VERSION = 1

const DESKTOP_CONNECTION_CONFIG_PATH = path.join(app.getPath('userData'), 'connection.json')
const DESKTOP_UPDATE_CONFIG_PATH = path.join(app.getPath('userData'), 'updates.json')
// active-profile.json records which Nozich profile the desktop launches its
// local backend as. When set, startNozich() passes `nozich --profile <name>
// dashboard …`, which deterministically pins NOZICH_HOME (see
// _apply_profile_override in nozich_cli/main.py) and bypasses the sticky
// ~/.nozich/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors nozich_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// nozichDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under NOZICH_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by nozich_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(NOZICH_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (nozich_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.NOZICH_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.NOZICH_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
  if (!Number.isFinite(raw) || raw <= 0) return 650
  return Math.max(120, raw)
})()
const APP_NAME = 'Nozich'
...
// 'nozich:native-theme' ('dark' | 'light' | 'system'); we pin
...
// Seed the native About panel with the live Nozich version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `nozich update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveNozichVersion(),
...
const MEDIA_PROTOCOL = 'nozich-media'
// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(nozich-media://…)`.
...
let mainWindow = null
let nozichProcess = null
let connectionPromise = null
// Additional per-profile backends, keyed by profile name. The PRIMARY backend
// (the desktop's launch profile) stays managed by nozichProcess +
// connectionPromise + startNozich(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.NOZICH_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.NOZICH_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// onto the error so subsequent startNozich() calls (e.g. the renderer's
...
let bootProgressState = {
  ...
  message: 'Waiting to start Nozich backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
function rememberLog(chunk) {
  ...
  if (!text) return
  const lines = text.split(/\r?\n/).map(line => `[nozich] ${line}`)
  nozichLog.push(...lines)
  if (nozichLog.length > 300) {
    nozichLog.splice(0, nozichLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-nozich-wsl-windows-fonts.conf')
    let existing = ''
    try {
...
function broadcastBootProgress() {
  ...
  webContents.send('nozich:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the nozich:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('nozich:bootstrap:event', ev)
}
...
function isNozichSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'nozich_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.NOZICH_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Nozich deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. Nozich' terminal tool requires
...
function findGitBash() {
  ...
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'nozich', 'git', 'bin', 'bash.exe'))
    candidates.push(path.join(localAppData, 'nozich', 'git', 'usr', 'bin', 'bash.exe'))
  }
...
function resolveGitBinary() {
  ...
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'nozich', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'nozich', 'git', 'bin', 'git.exe'))
  }
...
function recentNozichLog() {
  return nozichLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's nozich root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_NOZICH_ROOT.
// NOZICH_DESKTOP_NOZICH_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.NOZICH_DESKTOP_NOZICH_ROOT && path.resolve(process.env.NOZICH_DESKTOP_NOZICH_ROOT),
    !IS_PACKAGED && isNozichSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isNozichSourceRoot(ACTIVE_NOZICH_ROOT) ? ACTIVE_NOZICH_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_NOZICH_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('nozich:updates:progress', merged)
  }
}
...
// NOZICH_HOME/nozich-setup.exe on a successful install (see
// apps/bootstrap-installer paths::copy_self_to_nozich_home). That binary owns
// ALL repo mutation — running `nozich update` + rebuilding the desktop — so
...
function resolveUpdaterBinary() {
  const name = IS_WINDOWS ? 'nozich-setup.exe' : 'nozich-setup'
  const candidate = path.join(NOZICH_HOME, name)
...
// Path to the venv shim whose lock decides whether `nozich update` can write
// fresh entry points. On Windows this is the file the running backend
// `nozich.exe` holds open; on POSIX it's never mandatory-locked.
function venvNozichShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'nozich.exe')
    : path.join(updateRoot, 'venv', 'bin', 'nozich')
}
...
// only signals the direct child, so on Windows a backend `nozich.exe` that
// spawned its own grandchildren (a `nozich` REPL, a pty terminal session, the
...
// `nozichProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `nozich.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a nozich
// REPL, a pty terminal, the gateway) keep `nozich.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const pids = []
  if (nozichProcess && Number.isInteger(nozichProcess.pid)) pids.push(nozichProcess.pid)
  ...
  if (nozichProcess && !nozichProcess.killed) {
    try {
      nozichProcess.kill('SIGTERM')
    } catch {
      void 0
    }
  }
  ...
  const shim = venvNozichShimPath(updateRoot)
...
// `nozich update`). Instead we spawn the staged Nozich-Setup binary with
// --update and quit, so it can run `nozich update` (which refuses while we
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux drag-install: no staged Tauri nozich-setup. Unlike Windows
      ...
      // whole update itself: `nozich update` (backend) + `nozich desktop
    ...
    if (!updater) {
      ...
      // `nozich desktop`, never the Tauri installer that self-copies
      // nozich-setup.exe into NOZICH_HOME). They DO have a working `nozich`
      ...
      // checkout they're on — bare `nozich update` defaults to main and would
      ...
      const updateRoot = resolveUpdateRoot()
      let command = 'nozich update'
      try {
        const head = await runGit(['rev-parse', '--abbrev-ref', 'HEAD'], { cwd: updateRoot })
        const current = (head.stdout || '').trim()
        if (head.code === 0 && current && current !== 'HEAD') {
          const branch = await resolveHealedBranch(updateRoot, current)
          if (branch !== 'main') command = `nozich update --branch ${branch}`
        }
      } catch {
        // Best-effort: fall back to bare `nozich update` if branch detection fails.
      }
      rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
      emitUpdateProgress({ stage: 'manual', message: command, percent: null })
      return { ok: true, manual: true, command, nozichRoot: updateRoot }
    }

    emitUpdateProgress({ stage: 'restart', message: 'Handing off to the Nozich updater…', percent: 100 })
    ...
    // nozich.exe (held by the backend child / its grandchildren) and the update
    // bricks. See releaseBackendLockForUpdate for the full failure analysis.
    await releaseBackendLockForUpdate(updateRoot)

    // Detached so the updater outlives this process — it needs us GONE before
    // `nozich update` will run (the venv shim is locked while we live).
    const child = spawn(updater, updaterArgs, {
      cwd: NOZICH_HOME,
      env: {
        ...process.env,
        NOZICH_HOME,
        PATH: [path.join(NOZICH_HOME, 'node', 'bin'), venvBin, process.env.PATH].filter(Boolean).join(path.delimiter)
      },
      detached: true,
      stdio: 'ignore',
      windowsHide: false
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvNozich = path.join(venvBin, IS_WINDOWS ? 'nozich.exe' : 'nozich')
  const updaterArgs = fileExists(venvNozich) ? ['--update', '--branch', branch] : ['--repair', '--branch', branch]
...
// Resolve the nozich CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `nozich` on PATH.
function resolveNozichCliBinary(updateRoot) {
  const venvNozich = path.join(updateRoot, 'venv', 'bin', 'nozich')
  if (fileExists(venvNozich)) return venvNozich
  return findOnPath('nozich') || null
}
...
// macOS/Linux in-app update: backend (`nozich update`) + OS-aware GUI rebuild
// (`nozich desktop --build-only`), then atomically swap the running .app bundle
// with the freshly built one and relaunch. Degrades to "backend updated,
// restart to load the new GUI" if the swap can't be performed.
async function applyUpdatesPosixInApp() {
  const updateRoot = resolveUpdateRoot()
  const nozich = resolveNozichCliBinary(updateRoot)
  if (!nozich) {
    emitUpdateProgress({ stage: 'manual', message: 'nozich update', percent: null })
    return { ok: true, manual: true, command: 'nozich update', nozichRoot: updateRoot }
  }

  // Put the Nozich-managed Node and the venv on PATH so `nozich desktop`'s
  // npm build can find them on a machine with no system Node.
  const extraPath = [path.join(NOZICH_HOME, 'node', 'bin'), path.join(updateRoot, 'venv', 'bin')]
    .filter(Boolean)
    .join(path.delimiter)
  const env = {
    NOZICH_HOME,
  ...
  // `nozich update` reaps stale `nozich dashboard` backends (a code update
  ...
  // _kill_stale_dashboard_processes reads NOZICH_DESKTOP_CHILD_PID and excludes
  ...
  if (nozichProcess && Number.isInteger(nozichProcess.pid)) {
    desktopChildPids.push(nozichProcess.pid)
  }
  for (const entry of backendPool.values()) {
  ...
  if (desktopChildPids.length) {
    env.NOZICH_DESKTOP_CHILD_PID = desktopChildPids.join(',')
  }
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating Nozich (git + dependencies)…', percent: 10 })
  const updated = await runStreamedUpdate(nozich, ['update', '--yes', ...branchArgs], {
    cwd: updateRoot,
    env,
    stage: 'update'
  })
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'nozich update failed.', error: updated.error || 'update-failed' })
    return { ok: false, error: 'nozich update failed' }
  }

  emitUpdateProgress({ stage: 'rebuild', message: 'Rebuilding the desktop app…', percent: 60 })
  const rebuilt = await runStreamedUpdate(nozich, ['desktop', '--build-only'], {
    cwd: updateRoot,
    env,
    stage: 'rebuild'
  })
  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart Nozich to retry.',
  ...
  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'Nozich.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'Nozich.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart Nozich to load the new version.',
      percent: 100
    })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.nozich-update-new"; then
    rm -rf "$DST.nozich-update-old" 2>/dev/null || true
    mv "$DST" "$DST.nozich-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.nozich-update-new" "$DST"
    rm -rf "$DST.nozich-update-old" 2>/dev/null || true
  fi
fi
...
/usr/bin/open "$DST"
`
  const scriptPath = path.join(app.getPath('temp'), `nozich-desktop-update-${Date.now()}.sh`)
  try {
    fs.writeFileSync(scriptPath, swapScript, { mode: 0o755 })
  } catch (err) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart Nozich to load the new version.',
      percent: 100
    })
...
function isBootstrapComplete() {
  ...
  // pinned commit -- users update via the in-app update path or `nozich
  ...
  return isNozichSourceRoot(ACTIVE_NOZICH_ROOT) && fileExists(getVenvPython(VENV_ROOT))
}
...
function resolveWebDist() {
  const override = process.env.NOZICH_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set NOZICH_DESKTOP_WEB_DIST.`
    )
  }
  return fallback
}
...
function resolveRendererIndex() {
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: nozich desktop --force-build`
  )
  return candidates[0]
}
...
function resolveNozichCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Nozich.app/Contents/...`
  ...
  const candidates = [
    ...
    process.env.NOZICH_DESKTOP_CWD,
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveNozichCwd(), sanitized: Boolean(trimmed) }
  }
...
function createPythonBackend(root, label, dashboardArgs, options = {}) {
  ...
  return {
    ...
    label,
    command: python,
    args: ['-m', 'nozich_cli.main', ...dashboardArgs],
    env: buildDesktopBackendEnv({
      nozichHome: NOZICH_HOME,
...
// createActiveBackend — build a backend pointing at ACTIVE_NOZICH_ROOT, the
...
function createActiveBackend(dashboardArgs) {
  ...
  return {
    kind: 'python',
    label: `Nozich at ${ACTIVE_NOZICH_ROOT}`,
    command: fileExists(venvPython) ? venvPython : findSystemPython(),
    args: ['-m', 'nozich_cli.main', ...dashboardArgs],
    env: buildDesktopBackendEnv({
      nozichHome: NOZICH_HOME,
      pythonPathEntries: [ACTIVE_NOZICH_ROOT],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_NOZICH_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveNozichBackend(dashboardArgs) {
  // 1. Explicit override -- NOZICH_DESKTOP_NOZICH_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.NOZICH_DESKTOP_NOZICH_ROOT && path.resolve(process.env.NOZICH_DESKTOP_NOZICH_ROOT)
  if (overrideRoot && isNozichSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Nozich source at ${overrideRoot}`, dashboardArgs)
  ...
  //    installed `nozich` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isNozichSourceRoot.)
  if (!IS_PACKAGED && isNozichSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Nozich source at ${SOURCE_REPO_ROOT}`, dashboardArgs)
    if (backend) return backend
  }

  // 3. Bootstrap-complete ACTIVE_NOZICH_ROOT -- the canonical install at
  //    %LOCALAPPDATA%\nozich\nozich-agent (Windows) or ~/.nozich/nozich-agent.
  ...
  //    to spawning nozich. Updates flow through the in-app update path
  //    (applyUpdates -> git pull) or `nozich update` from the CLI.
  if (isBootstrapComplete()) {
    return createActiveBackend(dashboardArgs)
  }

  // 4. Existing `nozich` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    NOZICH_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.NOZICH_DESKTOP_IGNORE_EXISTING !== '1') {
    let nozichCommand = null
    const nozichOverride = process.env.NOZICH_DESKTOP_NOZICH

    if (nozichOverride) {
      const resolvedOverride = findOnPath(nozichOverride)
      if (resolvedOverride) {
        nozichCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(nozichOverride, { isWsl: IS_WSL })) {
        nozichCommand = nozichOverride
      } else {
        rememberLog(`Ignoring Windows Nozich override under WSL: ${nozichOverride}`)
      }
    } else {
      nozichCommand = findOnPath('nozich')
    }

    if (nozichCommand) {
      if (looksLikeDesktopAppBinary(nozichCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Nozich CLI: ${nozichCommand}`)
        nozichCommand = null
      }
    }

    if (nozichCommand) {
      // Smoke-test the candidate before trusting it. A `nozich` shim
      ...
      const shellForProbe = isCommandScript(nozichCommand)
      if (verifyNozichCli(nozichCommand, { shell: shellForProbe })) {
        return {
          label: `existing Nozich CLI at ${nozichCommand}`,
          command: nozichCommand,
          args: dashboardArgs,
          bootstrap: false,
          env: {},
          kind: 'command',
      ...
      rememberLog(
        `Ignoring existing Nozich CLI at ${nozichCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed nozich_cli module via system Python.
  ...
  if (python) {
    ...
    // having nozich_cli installed -- common on dev boxes that have
    ...
    if (canImportNozichCli(python)) {
      return {
        kind: 'python',
        label: `installed nozich_cli module via ${python}`,
        command: python,
        args: ['-m', 'nozich_cli.main', ...dashboardArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: nozich_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveNozichBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Nozich Agent not installed yet; bootstrap required',
    command: null,
    args: dashboardArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveNozichBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Nozich install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError = new Error('Nozich recovery was handed off to Nozich Setup. The desktop will restart when recovery completes.')
    ...
    const bootstrapResult = await runBootstrap({
      ...
      nozichHome: NOZICH_HOME,
      logRoot: path.join(NOZICH_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Nozich install was cancelled.')
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Nozich bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(NOZICH_HOME, 'logs', 'desktop.log')} for the full transcript.`
      )
      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startNozich() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // nozich:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveNozichBackend(backend.args))
  }
  ...
  if (!isNozichSourceRoot(ACTIVE_NOZICH_ROOT)) {
    throw new Error(
      `Nozich install at ${ACTIVE_NOZICH_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Nozich' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\nozich\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `nozich` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Nozich on Windows (provides Git Bash, ' +
        "which the agent's terminal tool uses). Install it from " +
        'https://git-scm.com/download/win or run `winget install -e --id Git.Git`, ' +
        'then relaunch Nozich.'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    // isNozichSourceRoot() and the bootstrap writes the marker only after
    // install.ps1 succeeds. If we hit this, the user (or a deleted venv)
    // broke the invariant; tell them to re-run the install.
    throw new Error(
      `Nozich venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = venvPython
  backend.label = `Nozich at ${ACTIVE_NOZICH_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Nozich runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function fetchJson(url, token, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported Nozich backend URL protocol: ${parsed.protocol}`))
      return
    }
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the Nozich backend.'
              )
            )
            return
          }
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to Nozich backend after ${timeoutMs}ms`))
    })
...
function fetchPublicJson(url, options = {}) {
  ...
  // NO ``X-Nozich-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('nozich:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveNozichCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('nozich:preview-file-changed', payload)
}
...
async function waitForNozich(baseUrl, token) {
  ...
  throw new Error(`Nozich backend did not become ready: ${lastError?.message || 'timeout'}`)
}
...
function sendBackendExit(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('nozich:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('nozich:close-preview-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Nozich...".
function sendPowerResume() {
  if (!mainWindow || mainWindow.isDestroyed()) return
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('nozich:power-resume')
}
...
function sendOpenUpdatesRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('nozich:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen) {
  ...
  webContents.send('nozich:window-state-changed', state)
}
...
const ZOOM_STORAGE_KEY = 'nozich:desktop:zoomLevel'
...
//
// Hosted Nozich gateways gate the dashboard behind an OAuth provider (e.g.
// Nous Research) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``nozich_session_at``),
...
//     alongside the ~15-min access token (Portal NAS #293 / nozich #37247).
//     Both are set as HttpOnly cookies (``nozich_session_at`` ~15 min,
//     ``nozich_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
const OAUTH_SESSION_PARTITION = 'persist:nozich-remote-oauth'
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to Nozich gateway',
...
function fetchJsonViaOauthSession(url, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Nozich backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// a valid stored value (pins the root NOZICH_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.NOZICH_DESKTOP_REMOTE_URL)

  const remoteToken = decryptDesktopSecret(block.token)
  const authMode = normAuthMode(block.authMode)
  const remoteUrl = envOverride ? String(process.env.NOZICH_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    // scope is never overridden by NOZICH_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    // Portal issues a 24h rotating refresh token (nozich #37247), and the
    ...
    if (!(await hasLiveOauthSession(baseUrl))) {
      const err = new Error(
        'Remote Nozich gateway uses OAuth, but you are not signed in. ' +
  ...
  if (!token) {
    throw new Error(
      'Remote Nozich gateway is selected, but no session token is saved. ' +
...
//   2. env override (NOZICH_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.NOZICH_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.NOZICH_DESKTOP_REMOTE_TOKEN
  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        'NOZICH_DESKTOP_REMOTE_URL is set but NOZICH_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Nozich backend.'
      )
    }
...
function globalRemoteActive() {
  if (process.env.NOZICH_DESKTOP_REMOTE_URL) {
    return true
  }
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Nozich
...
async function testDesktopConnectionConfig(input = {}) {
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startNozich())
  ...
  // connect to Nozich gateway". Mirror the renderer's connect here so the test
...
function resetNozichConnection() {
...
// startNozich() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait() {
  // Capture the reference before resetNozichConnection() nulls nozichProcess.
  const dying = nozichProcess && !nozichProcess.killed ? nozichProcess : null
  resetNozichConnection()
...
// profile to startNozich() (the window backend: boot UI, bootstrap, remote
...
// local-spawn portion of startNozich() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForNozich(remote.baseUrl, remote.token)
    return {
      ...remote,
      profile,
      logs: nozichLog.slice(-80),
      ...getWindowState()
    }
  }

  const token = crypto.randomBytes(32).toString('base64url')
  // --profile wins over the inherited NOZICH_HOME env (see _apply_profile_override
  // step 3 in nozich_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const dashboardArgs = ['--profile', profile, 'dashboard', '--no-open', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveNozichBackend(dashboardArgs))
  const nozichCwd = resolveNozichCwd()
  const webDist = resolveWebDist()

  rememberLog(`Starting Nozich backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: nozichCwd,
        NOZICH_DASHBOARD_SESSION_TOKEN: token,
        // Marks this dashboard backend as desktop-spawned so it runs the cron
        // scheduler tick loop (the gateway isn't running under the app).
        NOZICH_DESKTOP: '1',
        NOZICH_WEB_DIST: webDist
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`Nozich backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Nozich backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)
    if (!ready) {
      rejectStart?.(
        new Error(`Nozich backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
  ...
  await Promise.race([waitForNozich(baseUrl, token), startFailed])
  ready = true
  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Nozich backend for profile "${profile}"`,
    rememberLog
  })
...
async function startNozich() {
  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startNozich() call re-throws the same error
  ...
  connectionPromise = (async () => {
    await advanceBootProgress('backend.resolve', 'Resolving Nozich backend', 8)
    // Resolve for the desktop's primary profile so a per-profile remote
    // override on the active profile is honored (falls back to env / global).
    const remote = await resolveRemoteBackend(primaryProfileKey())
    if (remote) {
      await advanceBootProgress('backend.remote', `Connecting to remote Nozich backend at ${remote.baseUrl}`, 24)
      await waitForNozich(remote.baseUrl, remote.token)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Nozich backend is ready',
        progress: 94,
        running: true,
        error: null
      })
    ...
    // deterministic (it wins over the sticky ~/.nozich/active_profile file) and
    // resolves NOZICH_HOME the same way `nozich -p <name>` does on the CLI. An
    ...
    await advanceBootProgress('backend.runtime', 'Resolving Nozich runtime', 28)
    const backend = await ensureRuntime(resolveNozichBackend(dashboardArgs))
    const nozichCwd = resolveNozichCwd()
    const webDist = resolveWebDist()

    await advanceBootProgress('backend.spawn', `Starting Nozich backend via ${backend.label}`, 84)
    rememberLog(`Starting Nozich backend via ${backend.label}`)
    ...
    nozichProcess = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Explicitly pin NOZICH_HOME for the child so Python's get_nozich_home()
          // resolves to the SAME location our resolveNozichHome() picked. Without
          // this pin, Python falls back to ~/.nozich on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\nozich, which differs from C:\Users\<u>\.nozich.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets NOZICH_HOME via setx; the desktop
    ...
    nozichProcess.stdout.on('data', rememberLog)
    nozichProcess.stderr.on('data', rememberLog)
    let backendReady = false
    let rejectBackendStart = null
    const backendStartFailed = new Promise((_resolve, reject) => {
      rejectBackendStart = reject
    })
    nozichProcess.once('error', error => {
      rememberLog(`Nozich backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Nozich backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    nozichProcess.once('exit', (code, signal) => {
      rememberLog(`Nozich backend exited (${signal || code})`)
      nozichProcess = null
      connectionPromise = null
      sendBackendExit({ code, signal })
      if (!backendReady) {
        const message = `Nozich backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Nozich backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentNozichLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Nozich backend to launch', 86)
    // Discover the ephemeral port the child bound to
    const port = await Promise.race([waitForDashboardPort(nozichProcess), backendStartFailed])

    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Nozich backend to become ready', 90)
    await Promise.race([waitForNozich(baseUrl, token), backendStartFailed])
    backendReady = true
    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      // The exit/error handlers null nozichProcess when the child dies.
      childAlive: () => nozichProcess !== null && nozichProcess.exitCode === null && !nozichProcess.killed,
      rememberLog
    })
    updateBootProgress({
      phase: 'backend.ready',
      message: 'Nozich backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
function createWindow() {
  ...
  mainWindow.webContents.once('did-finish-load', () => {
    ...
    startNozich().catch(error => rememberLog(error.stack || error.message))
  })
}

ipcMain.handle('nozich:connection', async (_event, profile) => ensureBackend(profile))
...
// "Starting Nozich…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('nozich:connection:revalidate', async () => {
  ...
  try {
    ...
    // tick rebuilds a fresh, reachable descriptor. resetNozichConnection only
    // nulls connectionPromise for a remote (no child to SIGTERM).
    rememberLog('Cached remote Nozich backend failed liveness probe; dropping stale connection.')
    resetNozichConnection()
    return { ok: true, rebuilt: true }
  }
})
ipcMain.handle('nozich:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)
  return { ok: true }
})
ipcMain.handle('nozich:gateway:ws-url', async (_event, profile) => freshGatewayWsUrl(profile))
ipcMain.handle('nozich:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('nozich:window:openNewSession', async () => {
  createNewSessionWindow()

  return { ok: true }
})
ipcMain.handle('nozich:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startNozich() call restarts the
...
ipcMain.handle('nozich:bootstrap:repair', async () => {
  // Forceful repair: drop the bootstrap-complete marker so the next
  // startNozich() re-runs the full installer (refreshing a broken/partial
...
ipcMain.handle('nozich:bootstrap:cancel', async () => {
...
ipcMain.handle('nozich:boot-progress:get', async () => bootProgressState)
ipcMain.handle('nozich:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('nozich:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('nozich:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
ipcMain.handle('nozich:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('nozich:connection-config:oauth-login', async (_event, rawUrl) => {
...
ipcMain.handle('nozich:connection-config:oauth-logout', async (_event, rawUrl) => {
...
ipcMain.handle('nozich:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('nozich:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
...
ipcMain.handle('nozich:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('nozich:profile:set', async (_event, name) => {
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new NOZICH_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('nozich:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('nozich:requestMicrophoneAccess', async () => {
...
ipcMain.handle('nozich:api', async (_event, request) => {
...
ipcMain.handle('nozich:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('nozich:focus-session', payload.sessionId)
    }
  })
  notification.on('action', (_actionEvent, index) => {
    ...
    if (action?.id) {
      mainWindow.webContents.send('nozich:notification-action', { sessionId: payload?.sessionId, actionId: action.id })
    }
  })
  notification.show()
  return true
...
ipcMain.handle('nozich:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('nozich:readFileText', async (_event, filePath) => {
...
ipcMain.handle('nozich:selectPaths', async (_event, options = {}) => {
...
ipcMain.handle('nozich:writeClipboard', (_event, text) => {
  clipboard.writeText(String(text || ''))
  return true
})

ipcMain.handle('nozich:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))

ipcMain.handle('nozich:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('nozich:saveClipboardImage', async () => {
...
ipcMain.handle('nozich:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('nozich:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('nozich:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))

ipcMain.on('nozich:titlebar-theme', (_event, payload) => {
...
ipcMain.on('nozich:native-theme', (_event, mode) => {
...
ipcMain.on('nozich:translucency', (_event, payload) => {
...
ipcMain.handle('nozich:openExternal', (_event, url) => {
...
// it via writeDefaultProjectDir so resolveNozichCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('nozich:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveNozichCwd()
}))

ipcMain.handle('nozich:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('nozich:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('nozich:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('nozich:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('nozich:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('nozich:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: nozichLog.slice(-200) }))
...
function terminalShellCommand() {
  // NOZICH_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
  ...
  const override = (process.env.NOZICH_DESKTOP_SHELL || (IS_WINDOWS ? '' : process.env.SHELL) || '').trim()
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip Nozich' TUI into
  ...
  env.TERM_PROGRAM = 'Nozich'
  env.TERM_PROGRAM_VERSION = app.getVersion()

  // Let a nozich/--tui launched in this pane know it's embedded in the desktop
  // GUI (build_environment_hints surfaces this). Distinct from NOZICH_DESKTOP,
  // which marks the agent *backend* and gates cron/gateway behavior.
  env.NOZICH_DESKTOP_TERMINAL = '1'

  return env
}

function terminalChannel(id, suffix) {
  return `nozich:terminal:${id}:${suffix}`
}
...
ipcMain.handle('nozich:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

ipcMain.handle('nozich:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

ipcMain.handle('nozich:fs:worktrees', async (_event, cwds) => worktreesForIpc(cwds))

ipcMain.handle('nozich:terminal:start', async (event, payload = {}) => {
  if (!nodePty) {
    throw new Error('PTY support is unavailable. Reinstall desktop dependencies and restart Nozich.')
  }
...
ipcMain.handle('nozich:terminal:write', (_event, id, data) => {
...
ipcMain.handle('nozich:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('nozich:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

ipcMain.handle('nozich:updates:check', async () =>
...
ipcMain.handle('nozich:updates:apply', async (_event, payload) =>
...
ipcMain.handle('nozich:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('nozich:updates:branch:set', async (_event, name) => {
  const branch = typeof name === 'string' && name.trim() ? name.trim() : DEFAULT_UPDATE_BRANCH
  writeDesktopUpdateConfig({ branch })
  return { branch }
})

// Resolve the canonical Nozich version (the one `release.py` bumps in
// nozich_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Nozich version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveNozichVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'nozich_cli', '__init__.py')
...
// Re-resolve the live Nozich version and push it into the native About panel
// just before showing it, so an in-place `nozich update` is reflected without
...
ipcMain.handle('nozich:version', async () => ({
  appVersion: resolveNozichVersion(),
  electronVersion: process.versions.electron,
  nodeVersion: process.versions.node,
  platform: process.platform,
  nozichRoot: resolveUpdateRoot()
}))
...
// the actual removal via `nozich uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (nozich_cli/uninstall.py + nozich_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_NOZICH_ROOT
  ...
  const fallback = () => ({
    nozich_home: NOZICH_HOME,
    agent_installed: isNozichSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'nozich_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, NOZICH_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
      child.stdout.on('data', chunk => {
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Nozich agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import nozich_cli` resolves from
  ...
  try {
    await releaseBackendLock(ACTIVE_NOZICH_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `nozich-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `nozich-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('nozich:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('nozich:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('nozich:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('nozich:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// nozich:// deep links (e.g. nozich://blueprint/morning-brief?time=08:00).
// A docs/dashboard "Send to App" button opens this URL; we route it into the
// running app's chat composer. Three delivery paths: macOS 'open-url',
// Win/Linux running-app 'second-instance' (argv), Win/Linux cold-start argv.
// ---------------------------------------------------------------------------
const NOZICH_PROTOCOL = 'nozich'
let _pendingDeepLink = null
let _rendererReadyForDeepLink = false

function _extractDeepLink(argv) {
  if (!Array.isArray(argv)) return null
  return argv.find(a => typeof a === 'string' && a.startsWith(`${NOZICH_PROTOCOL}://`)) || null
}
...
function handleDeepLink(url) {
  ...
  // nozich://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('nozich:deep-link', payload)
...
ipcMain.handle('nozich:deep-link-ready', () => {
  _rendererReadyForDeepLink = true
  if (_pendingDeepLink) {
    const queued = _pendingDeepLink
    _pendingDeepLink = null
    handleDeepLink(
      `${NOZICH_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(NOZICH_PROTOCOL, process.execPath, [path.resolve(process.argv[1])])
    } else {
      app.setAsDefaultProtocolClient(NOZICH_PROTOCOL)
    }
  } catch (err) {
...
// second-instance argv. Without the lock a second `nozich://` launch spawns a
...
app.whenReady().then(() => {
  ...
  // Win/Linux cold start: the launching nozich:// URL is in our own argv.
