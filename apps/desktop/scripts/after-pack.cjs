 ...
 *
 * Stamps the Nozich icon + identity onto the packed Windows Nozich.exe via
 * rcedit (delegated to set-exe-identity.cjs). This runs for EVERY packed build
 * — first install, `nozich desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Nozich')
 */
...
exports.default = async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Nozich'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Nozich.exe keeps the stock Electron icon`)
  }
}
