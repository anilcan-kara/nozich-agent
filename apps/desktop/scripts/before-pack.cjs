 ...
 * product name (`Nozich`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Nozich'
 *
 * This is a hard failure with no obvious cause for the user — `nozich desktop`
 ...
 * (the mac-unpacked Nozich.app bundle) and Windows (win-unpacked), so we
