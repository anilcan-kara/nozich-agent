...
// no longer bundles the Nozich Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Nozich.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Nozich'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Nozich.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'nozich'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default NOZICH_HOME for non-sandboxed runs -- matches main.cjs's
// resolveNozichHome(). On Windows it's %LOCALAPPDATA%\nozich; elsewhere
// it's ~/.nozich. The fresh-install sandbox launchFresh() sets its own
// NOZICH_HOME and never touches this.
const DEFAULT_NOZICH_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'nozich')
  }
  return path.join(os.homedir(), '.nozich')
})()
const VENV_ROOT = path.join(DEFAULT_NOZICH_HOME, 'nozich-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'nozich-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.NOZICH_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Nozich-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Nozich-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Nozich-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Nozich-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.NOZICH_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.NOZICH_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const nozichHome = path.join(sandbox, 'nozich-home')
  const cwd = path.join(sandbox, 'workspace')

  fs.mkdirSync(userDataDir, { recursive: true })
  fs.mkdirSync(nozichHome, { recursive: true })
  ...
  env.NOZICH_DESKTOP_CWD = cwd
  env.NOZICH_DESKTOP_IGNORE_EXISTING = '1'
  env.NOZICH_DESKTOP_TEST_MODE = 'fresh-install'
  env.NOZICH_DESKTOP_USER_DATA_DIR = userDataDir
  env.NOZICH_HOME = nozichHome
  delete env.NOZICH_DESKTOP_NOZICH
  delete env.NOZICH_DESKTOP_NOZICH_ROOT
  ...
  console.log(`  NOZICH_HOME: ${nozichHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(nozichHome, 'nozich-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Nozich Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of nozich_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'nozich-agent', 'nozich_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Nozich
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + NOZICH_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  NOZICH_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
