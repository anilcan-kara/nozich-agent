...
import { getSessionMessages, listAllProfileSessions } from '@/nozich'
...
import type { SessionInfo, SessionMessage } from '@/types/nozich'
  ...
  const openArtifact = useCallback(async (href: string) => {
    try {
      if (window.nozichDesktop?.openExternal) {
        await window.nozichDesktop.openExternal(href)
      } else {
