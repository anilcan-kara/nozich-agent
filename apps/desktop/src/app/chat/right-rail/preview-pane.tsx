        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.nozichDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Nozich to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.nozichDesktop?.watchPreviewFile ||
      !window.nozichDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.nozichDesktop.onPreviewFileChanged(payload => {
    ...
    void window.nozichDesktop
      .watchPreviewFile(target.url)
      .then(watch => {
        if (!active) {
          void window.nozichDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.nozichDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:nozich-preview')
