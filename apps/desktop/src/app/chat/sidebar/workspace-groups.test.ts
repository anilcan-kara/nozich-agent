...
import type { NozichWorktreeInfo } from '@/global'
import type { SessionInfo } from '@/types/nozich'
...
describe('workspaceGroupsFor', () => {
  it('groups by full cwd, not by basename — same-named folders are separate groups', () => {
    const groups = workspaceGroupsFor(
      [makeSession('/a/nozich-agent/apps/desktop'), makeSession('/a/nozich-agent-wt-rtl/apps/desktop')],
      'No workspace'
    )
  ...
  it('disambiguates colliding basenames by walking up the path', () => {
    expect(
      labels([makeSession('/a/nozich-agent/apps/desktop'), makeSession('/a/nozich-agent-wt-rtl/apps/desktop')])
    ).toEqual(['nozich-agent/apps/desktop', 'nozich-agent-wt-rtl/apps/desktop'])
  })

  it('leaves a unique basename as its short label', () => {
    expect(labels([makeSession('/a/nozich-agent/apps/desktop'), makeSession('/b/heval-py')])).toEqual([
      'desktop',
      'heval-py'
    ])
  })
...
const info = (over: Partial<NozichWorktreeInfo> & Pick<NozichWorktreeInfo, 'repoRoot' | 'worktreeRoot'>): NozichWorktreeInfo => ({
  branch: null,
...
describe('workspaceTreeFor', () => {
  it('heuristic nests `<repo>-wt-<branch>` under its sibling repo', () => {
    const tree = workspaceTreeFor(
      [makeSession('/www/nozich-agent'), makeSession('/www/nozich-agent-wt-rtl')],
      'No workspace'
    )

    expect(tree).toHaveLength(1)
    expect(tree[0].label).toBe('nozich-agent')
    expect(tree[0].groups.map(g => g.label).sort()).toEqual(['nozich-agent', 'rtl'])
  })
  ...
  it('git metadata is authoritative — worktrees group by repoRoot regardless of directory naming', () => {
    const resolver: WorktreeResolver = cwd => {
      if (cwd === '/www/nozich-agent') {
        return info({ repoRoot: '/www/nozich-agent', worktreeRoot: '/www/nozich-agent', isMainWorktree: true, branch: 'main' })
      }

      if (cwd === '/elsewhere/ha-rtl') {
        return info({ repoRoot: '/www/nozich-agent', worktreeRoot: '/elsewhere/ha-rtl', branch: 'rtl' })
      }
    ...
    const tree = workspaceTreeFor(
      [makeSession('/www/nozich-agent'), makeSession('/elsewhere/ha-rtl')],
      'No workspace',
      resolver
    )
    ...
    expect(tree[0].groups.map(g => g.label)).toEqual(['nozich-agent', 'rtl'])
  })
