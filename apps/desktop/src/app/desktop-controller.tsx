...
import { getCronJobs, getSessionMessages, listAllProfileSessions, type SessionInfo, triggerCronJob } from '../nozich'
...
import { useNozichConfig } from './session/hooks/use-nozich-config'
...
export function DesktopController() {
  ...
  useEffect(() => {
    window.nozichDesktop?.setPreviewShortcutActive?.(Boolean(chatOpen && (filePreviewTarget || previewTarget)))
  }, [chatOpen, filePreviewTarget, previewTarget])

  useEffect(() => {
    startUpdatePoller()
    const unsubscribe = window.nozichDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  useEffect(() => {
    const unsubscribe = window.nozichDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.nozichDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // nozich:// deep links (e.g. a docs "Send to App" button for an automation blueprint).
  ...
  useEffect(() => {
    const unsubscribe = window.nozichDesktop?.onDeepLink?.(payload => {
    ...
    void window.nozichDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.nozichDesktop?.onClosePreviewRequested?.(closeActiveRightRailTab)
  ...
  const { refreshNozichConfig, sttEnabled, voiceMaxRecordingSeconds } = useNozichConfig({
    activeSessionIdRef,
  ...
  useEffect(() => {
    if (gatewayState === 'open' && !activeSessionId && freshDraftReady) {
      void refreshCurrentModel()
      void refreshNozichConfig()
    }
  }, [activeSessionId, freshDraftReady, gatewayState, refreshCurrentModel, refreshNozichConfig])
