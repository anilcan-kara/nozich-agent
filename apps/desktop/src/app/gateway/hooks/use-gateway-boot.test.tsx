...
// bug that drives the REAL useGatewayBoot hook + REAL NozichGateway through a
...
function Harness() {
  useGatewayBoot({
    ...
    refreshNozichConfig: async () => undefined,
...
beforeEach(() => {
  ...
  ;(window as { nozichDesktop?: unknown }).nozichDesktop = fakeDesktop()
...
afterEach(() => {
  ...
  delete (window as { nozichDesktop?: unknown }).nozichDesktop
})
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForNozich) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startNozich()'s remote branch awaits waitForNozich() for 45s before it
    ...
    ;(window as { nozichDesktop?: unknown }).nozichDesktop = desktop
    ...
    // After ~45s waitForNozich gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('Nozich backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
