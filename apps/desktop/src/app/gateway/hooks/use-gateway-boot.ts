...
import { NozichGateway } from '@/nozich'
...
import type { RpcEvent } from '@/types/nozich'

interface GatewayBootOptions {
  handleGatewayEvent: (event: RpcEvent) => void
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.nozichDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: NozichGateway | null) => void
  refreshNozichConfig: () => Promise<void>
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: NozichConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Nozich...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Nozich…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to Nozich gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshNozichConfig().catch(() => undefined)
    ...
    const gateway = new NozichGateway()
    ...
    async function boot() {
      try {
        ...
        await callbacksRef.current.refreshNozichConfig()
