...
import type { NozichGateway } from '@/nozich'
...
export function useGatewayRequest() {
  ...
  const gatewayRef = useRef<NozichGateway | null>(null)

  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.nozichDesktop>['getConnection']>> | null>(
    null
  )

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<NozichGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as NozichGateway | null
      }),
    []
  )
  ...
  const requestGateway = useCallback(
    async <T>(method: string, params: Record<string, unknown> = {}) => {
      ...
      if (!gateway) {
        throw new Error('Nozich gateway unavailable')
      }
