...
export type ProjectTreeEntry = NozichReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: NozichReadDirEntry) {
...
async function filterIgnored(entries: NozichReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<NozichReadDirResult> {
  if (!window.nozichDesktop) {
