...
import type { NozichReadDirResult } from '@/global'
...
beforeEach(() => {
  ...
  readDir.mockReset()
  ;(window as unknown as { nozichDesktop: { readDir: typeof readDir } }).nozichDesktop = { readDir }
})
...
function ok(entries: { name: string; path: string; isDirectory: boolean }[]): NozichReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { nozichDesktop: unknown }).nozichDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: NozichReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<NozichReadDirResult>(resolve => {
  ...
  it('falls back to the sanitized workspace dir when the session cwd is gone', async () => {
    ...
    ;(window as unknown as { nozichDesktop: unknown }).nozichDesktop = { readDir, sanitizeWorkspaceCwd }
  ...
  it('returns no-bridge gracefully when window.nozichDesktop is missing', async () => {
    delete (window as unknown as { nozichDesktop?: unknown }).nozichDesktop
