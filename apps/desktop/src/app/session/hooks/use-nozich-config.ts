...
import { getNozichConfig, getNozichConfigDefaults } from '@/nozich'
...
interface NozichConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
  refreshProjectBranch: (cwd: string) => Promise<void>
}

export function useNozichConfig({ activeSessionIdRef, refreshProjectBranch }: NozichConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)

  const refreshNozichConfig = useCallback(async () => {
    try {
      const [config, defaults] = await Promise.all([getNozichConfig(), getNozichConfigDefaults().catch(() => ({}))])
  ...
  return { refreshNozichConfig, sttEnabled, voiceMaxRecordingSeconds }
}
