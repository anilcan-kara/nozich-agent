...
import { getProfiles, transcribeAudio } from '@/nozich'
...
async function readFileDataUrlForAttach(filePath: string): Promise<string | null> {
  const reader = window.nozichDesktop?.readFileDataUrl
...
export function usePromptActions({
  ...
  // separate `nozich gateway` process performs the actual transfer, so we
