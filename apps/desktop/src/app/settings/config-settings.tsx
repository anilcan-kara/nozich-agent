...
import {
  getElevenLabsVoices,
  getNozichConfigDefaults,
  getNozichConfigRecord,
  getNozichConfigSchema,
  saveNozichConfig
} from '@/nozich'
import { useI18n } from '@/i18n'
import { cn } from '@/lib/utils'
import { notify, notifyError } from '@/store/notifications'
import type { ConfigFieldSchema, NozichConfigRecord } from '@/types/nozich'
...
export function ConfigSettings({
  ...
  const [_defaults, setDefaults] = useState<NozichConfigRecord | null>(null)
  ...
  useEffect(() => {
    let cancelled = false
    Promise.all([getNozichConfigRecord(), getNozichConfigDefaults(), getNozichConfigSchema()])
      .then(([c, d, s]) => {
        if (cancelled) {
          return
        }
  ...
  const updateConfig = (next: NozichConfigRecord) => {
