...
describe('settings helpers', () => {
  ...
  it('reads and writes nested config paths', () => {
    const config: NozichConfigRecord = { display: { theme: 'mono' } }
  ...
  it('rejects prototype-polluting config paths', () => {
    const config: NozichConfigRecord = {}
  ...
  describe('providerGroup', () => {
    ...
    it('prefers the longest matching prefix so CN/regional buckets win', () => {
      ...
      // NOZICH_QWEN_ and NOZICH_GEMINI_ both share the NOZICH_ stem.
      expect(providerGroup('NOZICH_QWEN_BASE_URL')).toBe('DashScope (Qwen)')
      expect(providerGroup('NOZICH_GEMINI_CLIENT_ID')).toBe('Gemini')
    })
