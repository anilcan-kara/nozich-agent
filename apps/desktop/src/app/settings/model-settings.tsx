...
import {
  ...
  getNozichConfigRecord,
  getRecommendedDefaultModel,
  saveNozichConfig,
  setEnvVar,
  setModelAssignment
} from '@/nozich'
import type { AuxiliaryModelsResponse, ModelOptionProvider, StaleAuxAssignment } from '@/nozich'
import { useI18n } from '@/i18n'
import { AlertTriangle, Cpu, Loader2 } from '@/lib/icons'
import { cn } from '@/lib/utils'
import { notifyError } from '@/store/notifications'
import { startManualLocalEndpoint, startManualProviderOAuth } from '@/store/onboarding'
import type { NozichConfigRecord } from '@/types/nozich'

import { CONTROL_TEXT } from './constants'
import { getNested, setNested } from './helpers'
import { ListRow, LoadingState, Pill, SectionHeading } from './primitives'

// Nozich' reasoning levels (VALID_REASONING_EFFORTS); `none` = thinking off.
// Empty config = Nozich default (medium), shown as Medium.
...
// backend now surfaces the full `nozich model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in nozich_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged }: ModelSettingsProps) {
  ...
  const [config, setConfig] = useState<NozichConfigRecord | null>(null)
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `nozich model` curation). Best-effort — fall through to the refreshed
        ...
        {needsSetup && !setupIsApiKey && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — Nozich runs the flow for you.`}
          </p>
        )}
