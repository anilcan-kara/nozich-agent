...
import { disconnectOAuthProvider, listOAuthProviders } from '@/nozich'
import { useI18n } from '@/i18n'
import { Check, ChevronDown, ChevronRight, KeyRound, Loader2, Terminal, Trash2 } from '@/lib/icons'
import { cn } from '@/lib/utils'
import { notify, notifyError } from '@/store/notifications'
import { $desktopOnboarding, startManualProviderOAuth } from '@/store/onboarding'
import type { EnvVarInfo, OAuthProvider } from '@/types/nozich'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.nozichDesktop?.terminal)
...
function ConnectedProviderRow({
  ...
  // Nozich can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Nozich can't clear via the API, but ships a
...
export function ProvidersSettings({ onClose, onViewChange, view }: ProvidersSettingsProps) {
  ...
  // Nozich never deletes creds another tool owns behind a silent API call.
