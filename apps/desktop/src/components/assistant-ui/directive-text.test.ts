...
import { formatRefValue, nozichDirectiveFormatter } from './directive-text'
...
describe('nozichDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = nozichDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = nozichDirectiveFormatter.parse('@file:src/main.tsx the entry point')
