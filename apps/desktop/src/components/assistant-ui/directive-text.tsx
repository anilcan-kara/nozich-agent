...
const NOZICH_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
type NozichRefType = (typeof NOZICH_REF_TYPES)[number]
...
const ICON_PATHS: Record<NozichRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as NozichRefType] ?? ICON_FALLBACK
...
const NOZICH_DIRECTIVE_RE = new RegExp(
...
export const nozichDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(NOZICH_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as NozichRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: NozichRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
/**
 * Renders text containing Nozich directives (`@file:...`, `@image:...`) as
...
export function DirectiveContent({ text }: { text: string }) {
  ...
  const segments = useMemo(() => nozichDirectiveFormatter.parse(cleanedText), [cleanedText])
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.nozichDesktop && isRemoteGateway()
        ? gatewayMediaDataUrl(id)
        : window.nozichDesktop?.readFileDataUrl(id)
