...
declare global {
  interface Window {
    nozichDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<NozichConnection>
      ...
      profile: {
        ...
        // backend under the new NOZICH_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: NozichApiRequest) => Promise<T>
      notify: (payload: NozichNotification) => Promise<boolean>
      requestMicrophoneAccess: () => Promise<boolean>
      readFileDataUrl: (filePath: string) => Promise<string>
      readFileText: (filePath: string) => Promise<NozichReadFileTextResult>
      selectPaths: (options?: NozichSelectPathsOptions) => Promise<string[]>
      writeClipboard: (text: string) => Promise<boolean>
      saveImageFromUrl: (url: string) => Promise<boolean>
      saveImageBuffer: (data: ArrayBuffer | Uint8Array, ext: string) => Promise<string>
      saveClipboardImage: () => Promise<string>
      getPathForFile: (file: File) => string
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<NozichPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<NozichPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setTitleBarTheme?: (payload: NozichTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<NozichReadDirResult>
      ...
      worktrees?: (cwds: string[]) => Promise<Record<string, NozichWorktreeInfo | null>>
      terminal: {
        dispose: (id: string) => Promise<boolean>
        onData: (id: string, callback: (payload: string) => void) => () => void
        onExit: (id: string, callback: (payload: NozichTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<NozichTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: NozichWindowState) => void) => () => void
      onFocusSession?: (callback: (sessionId: string) => void) => () => void
      onNotificationAction?: (callback: (payload: { actionId: string; sessionId?: string }) => void) => () => void
      onPreviewFileChanged: (callback: (payload: NozichPreviewFileChanged) => void) => () => void
...
export interface NozichTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface NozichTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `nozich update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  nozichRoot?: string
}
...
export interface NozichConnection {
  baseUrl: string
...
export interface NozichTitleBarTheme {
  background: string
  foreground: string
}

export interface NozichWindowState {
  isFullscreen: boolean
  nativeOverlayWidth: number
  windowButtonPosition: { x: number; y: number } | null
}
...
export interface NozichApiRequest {
  path: string
  method?: string
  body?: unknown
  timeoutMs?: number
...
export interface NozichNotification {
  title?: string
  body?: string
  silent?: boolean
  kind?: string
...
export interface NozichPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface NozichReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface NozichPreviewWatch {
  id: string
  path: string
}

export interface NozichWorktreeInfo {
...
export interface NozichReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface NozichReadDirResult {
  entries: NozichReadDirEntry[]
  error?: string
}

export interface NozichPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface NozichSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
