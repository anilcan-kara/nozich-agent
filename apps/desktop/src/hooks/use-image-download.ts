...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'nozich:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.nozichDesktop?.saveImageFromUrl) {
        if (await window.nozichDesktop.saveImageFromUrl(src)) {
