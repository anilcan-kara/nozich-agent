...
import { getNozichConfigRecord, type NozichConfigRecord, saveNozichConfig } from '@/nozich'
...
export interface I18nConfigClient {
  getConfig: () => Promise<NozichConfigRecord>
  saveConfig: (config: NozichConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.nozichDesktop?.api) {
      return Promise.resolve({})
    }

    return getNozichConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.nozichDesktop?.api) {
    ...
    return saveNozichConfig(config)
  }
}
...
export function getConfigDisplayLanguage(config: NozichConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: NozichConfigRecord, locale: Locale): NozichConfigRecord {
