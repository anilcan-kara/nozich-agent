import type {
  NozichConnection,
  NozichReadDirResult,
  NozichReadFileTextResult,
  NozichSelectPathsOptions,
  NozichWorktreeInfo
} from '@/global'
...
function connectionCacheKey(connection: NozichConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
function bridge() {
  const desktop = window.nozichDesktop
  if (!desktop) {
    throw new Error('Nozich Desktop bridge is unavailable')
  }
  return desktop
}

export async function readDesktopDir(path: string): Promise<NozichReadDirResult> {
  const desktop = bridge()
  if (!isDesktopFsRemoteMode()) {
    return desktop.readDir(path)
  }
  return desktop.api<NozichReadDirResult>({ path: fsPath('list', path) })
}

export async function readDesktopFileText(path: string): Promise<NozichReadFileTextResult> {
  const desktop = bridge()
  if (!isDesktopFsRemoteMode()) {
    return desktop.readFileText(path)
  }
  return desktop.api<NozichReadFileTextResult>({ path: fsPath('read-text', path) })
}
...
export async function desktopWorktrees(cwds: string[]): Promise<Record<string, NozichWorktreeInfo | null>> {
  if (isDesktopFsRemoteMode()) {
    return {}
  }
...
export async function selectDesktopPaths(options?: NozichSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
  if (!isDesktopFsRemoteMode()) {
