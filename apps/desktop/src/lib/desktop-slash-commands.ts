...
/** Why a known Nozich command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Nozich profile', surface: action('profile') },
  ...
  { name: '/version', description: 'Show Nozich Agent version', surface: exec() },
...
function isKnownNozichSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Nozich' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownNozichSlashCommand(normalized)
}
