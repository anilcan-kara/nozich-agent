...
export function mediaStreamUrl(path: string): string {
  return `nozich-media://stream/${encodeURIComponent(filePathFromMediaPath(path))}`
}
...
// expose GET /api/media (nozich_cli/web_server.py).
export async function gatewayMediaDataUrl(path: string): Promise<string> {
  const file = filePathFromMediaPath(path)

  const result = await window.nozichDesktop!.api<{ data_url: string }>({
