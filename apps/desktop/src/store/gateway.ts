import type { ConnectionState, GatewayEvent } from '@nozich/shared'
...
const isOpen = (gateway: NozichGateway | null): boolean => gateway?.connectionState === 'open'
...
export const $gateway = atom<NozichGateway | null>(null)
...
let primaryGateway: NozichGateway | null = null
let primaryProfile = 'default'

export function setPrimaryGateway(gateway: NozichGateway | null, profile = 'default'): void {
...
export function activeGateway(): NozichGateway | null {
...
export async function ensureActiveGatewayOpen(): Promise<NozichGateway | null> {
  if (activeKey === primaryProfile) {
    return primaryGateway
  }
