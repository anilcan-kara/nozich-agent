...
const STORAGE_KEY = 'nozich:native-notifications'
...
// "Backgrounded" = the user isn't on Nozich. `document.hidden` only flips when
...
export function dispatchNativeNotification(input: NativeNotificationInput): void {
  ...
  void window.nozichDesktop?.notify({
...
export async function sendTestNativeNotification(title: string, body: string): Promise<boolean> {
  const bridge = window.nozichDesktop
