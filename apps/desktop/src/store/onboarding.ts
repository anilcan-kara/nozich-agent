...
import type { ModelOptionProvider, OAuthProvider, OAuthStartResponse } from '@/types/nozich'
...
const CONFIGURED_CACHE_KEY = 'nozich-desktop-onboarded-v1'
const SKIP_CACHE_KEY = 'nozich-onboarding-skipped-v1'
const POLL_MS = 2000
...
function notifyReady(provider: string) {
  notify({ kind: 'success', title: 'Nozich is ready', message: `${provider} connected.` })
}

// Human-friendly labels for tools auto-routed through the Nous Tool Gateway,
// mirroring nozich_cli/nous_subscription._GATEWAY_TOOL_LABELS so the GUI and
...
async function fetchProviderDefaultModel(
  ...
  // `nozich model` does (for Nous it honors the user's free/paid tier, so a
...
function providerResolutionFailure(reason: null | string) {
  ...
  return detail
    ? `Connected, but Nozich still cannot resolve a usable provider. ${detail}`
    : 'Connected, but Nozich still cannot resolve a usable provider.'
}
...
async function openSignInUrl(url: string) {
  if (window.nozichDesktop?.openExternal) {
    try {
      await window.nozichDesktop.openExternal(url)
...
export async function recheckExternalSignin(ctx: OnboardingContext) {
  ...
  await completeWithModelConfirm(ctx, provider.name, [provider.id], reason =>
    setFlow({
      ...
      message:
        reason?.trim() ||
        `Nozich still cannot reach ${provider.name}. Run \`${provider.cli_command}\` in a terminal first.`
    })
  )
}
...
export async function saveOnboardingLocalEndpoint(baseUrl: string, apiKey: string, ctx: OnboardingContext) {
  ...
  try {
    ...
    if (!runtime.ready) {
      ...
      return { ok: false, message: detail || `Saved, but Nozich still cannot reach ${url}.` }
    }
