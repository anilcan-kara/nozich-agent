...
const remoteConn = (over: Partial<NozichConnection> = {}): NozichConnection =>
  ({ baseUrl: 'https://nozich-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as NozichConnection

const localConn = (over: Partial<NozichConnection> = {}): NozichConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as NozichConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<NozichConnection>>()
...
beforeEach(() => {
  ...
  vi.stubGlobal('window', { nozichDesktop: { getConnection } })
})
