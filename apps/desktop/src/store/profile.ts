...
import { getProfiles, setApiRequestProfile } from '@/nozich'
...
import type { ProfileInfo } from '@/types/nozich'
...
// /api/profiles/active `current`). "default" is the root ~/.nozich. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'nozich.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'nozich.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.nozichDesktop.api<ActiveProfileResponse>({ path: '/api/profiles/active' })
...
// Persist the choice and relaunch the backend under the new NOZICH_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.nozichDesktop.profile.set(name)
}
...
async function syncConnectionToActiveProfile(profile: string): Promise<void> {
  const getConnection = window.nozichDesktop?.getConnection
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'nozich.desktop.showAllProfiles'
...
// Switch to the default (root ~/.nozich) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.nozichDesktop?.touchBackend?.(target).catch(() => undefined)
}
