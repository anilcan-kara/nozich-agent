...
import type { NozichConnection } from '@/global'
import type { ChatMessage } from '@/lib/chat-messages'
import { persistBoolean, persistString, storedBoolean, storedString } from '@/lib/storage'
import type { SessionInfo, UsageStats } from '@/types/nozich'

type Updater<T> = T | ((current: T) => T)

const WORKSPACE_CWD_KEY = 'nozich.desktop.workspace-cwd'
...
const COMPOSER_MODEL_KEY = 'nozich.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'nozich.desktop.composer.provider'
const COMPOSER_EFFORT_KEY = 'nozich.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'nozich.desktop.composer.fast'

let configuredDefaultProjectDir = ''

function workspaceCwdKey(connection: NozichConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(): Promise<string> {
  const settings = window.nozichDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(): Promise<void> {
  const sanitize = window.nozichDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<NozichConnection | null>(null)
...
export const setConnection = (next: Updater<NozichConnection | null>) => updateAtom($connection, next)
