...
const SKIN_KEY = 'nozich-desktop-theme-v2'
const MODE_KEY = 'nozich-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'nozich-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'nozich-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'nozich-desktop-active-profile-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.nozichTheme = skinName
  root.dataset.nozichMode = rendered
  ...
  window.nozichDesktop?.setTitleBarTheme?.({
  ...
  try {
    window.localStorage.setItem('nozich-boot-background', c.background)
    window.localStorage.setItem('nozich-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.nozichThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.nozichDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
/** Sync the desktop skin with the active Nozich backend theme on connect. */
