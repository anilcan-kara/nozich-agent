...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Nozich doesn't know how to remove it. */
...
export type NozichConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `nozich model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
export interface ToolProvider {
   ...
   *  CLI `nozich tools` active-provider detection). */
  is_active: boolean
}
...
/** Shape of `GET /api/nozich/update/check` — the backend's own update state.
