...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``nozich://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``nozich://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"nozich://blueprint/{quote(blueprint.key)}{qs}"
