#!/bin/sh
# shellcheck shell=sh
# /opt/nozich/bin/nozich — `docker exec` privilege-drop shim.
#
# Background
# ----------
# The s6 image runs the supervised gateway/main process as the unprivileged
# `nozich` user (UID 10000). When an operator runs `docker exec <c> nozich ...`
# the default UID is root (0), and any file the command writes under
# $NOZICH_HOME — auth.json, .env, config.yaml — ends up root-owned and
# unreadable to the supervised gateway. The most common manifestation: the
# user runs `docker exec <c> nozich login`, this writes
# /opt/data/auth.json as root:root mode 0600, and from then on the gateway
# returns "Provider authentication failed: Nozich is not logged into Nous
# Portal" on every incoming message — even though `docker exec <c> nozich
...
# Fix
# ---
# This shim sits at /opt/nozich/bin/nozich and is placed earliest on PATH.
# When invoked as root, it drops to the nozich user (via s6-setuidgid)
# before exec'ing the real venv binary, so anything that writes under
# $NOZICH_HOME is uid-aligned with the supervised processes. When invoked
# as any non-root UID — including the supervised processes themselves,
# `docker exec --user nozich`, kanban subagents, etc. — it short-circuits
...
# (/opt/nozich/.venv/bin/nozich), so the second hop cannot re-enter this
# shim regardless of PATH state. No sentinel env var needed.
#
# Opt-out: set NOZICH_DOCKER_EXEC_AS_ROOT=1 (1/true/yes, case-insensitive)
...
# state via the nozich CLI. Default is to drop.

set -e

REAL=/opt/nozich/.venv/bin/nozich

# Defensive: if the venv binary is missing (corrupted image, partial
# install), fail loudly rather than silently masking it.
if [ ! -x "$REAL" ]; then
    echo "nozich-shim: $REAL not found or not executable" >&2
    exit 127
fi

# Already non-root? Just exec the real binary. This is the hot path for
# supervised processes (uid 10000) and for `docker exec --user nozich`.
if [ "$(id -u)" != "0" ]; then
    exec "$REAL" "$@"
fi

# Root, with opt-out set? Honor it.
case "${NOZICH_DOCKER_EXEC_AS_ROOT:-}" in
    1|true|TRUE|True|yes|YES|Yes)
        exec "$REAL" "$@"
        ;;
esac

# Root, no opt-out. Drop to the nozich user.
#
...
if [ ! -x "$S6_SUID" ]; then
    ...
    echo "nozich-shim: $S6_SUID not found; refusing to silently run as root." >&2
    echo "nozich-shim: re-run with --user nozich or set NOZICH_DOCKER_EXEC_AS_ROOT=1." >&2
    exit 126
fi

# Reset HOME to the nozich user's home before dropping privileges. Without
