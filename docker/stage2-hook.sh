...
#
# Wired into the image as /etc/cont-init.d/01-nozich-setup by the
...
NOZICH_HOME="${NOZICH_HOME:-/opt/data}"
INSTALL_DIR="/opt/nozich"

# Drop to nozich via s6-setuidgid, but skip it when already non-root.
as_nozich() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid nozich "$@"; }
...
# the container starts non-root. The baked image trees (/opt/data, /opt/nozich/
# .venv, ui-tui, node_modules) stay owned by the nozich build UID (10000), so an
...
# into $NOZICH_HOME). See #34837 for the supervision-tree side of this.
#
# The supported way to match host-side ownership is to start as root (the image
# default) and pass NOZICH_UID/NOZICH_GID — or the PUID/PGID aliases — which the
...
# `id -u nozich` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# nozich UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u nozich)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-nozich UID).
...
and the baked image directories are owned by the nozich user (UID $(id -u nozich)),
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e NOZICH_UID=\$(id -u) -e NOZICH_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the nozich user to that UID/GID at boot and chowns the data
...
# --- Bootstrap NOZICH_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid nozich mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom NOZICH_HOME paths whose parents only
# root can create (e.g. `NOZICH_HOME=/home/nozich/.nozich` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for NOZICH_UID/NOZICH_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  NOZICH_UID/
# NOZICH_GID still win when both are set.  See #15290, salvages #25872.
NOZICH_UID="${NOZICH_UID:-${PUID:-}}"
NOZICH_GID="${NOZICH_GID:-${PGID:-}}"

if [ -n "${NOZICH_UID:-}" ] && validate_uid_gid "$NOZICH_UID" && [ "$NOZICH_UID" != "$(id -u nozich)" ]; then
    echo "[stage2] Changing nozich UID to $NOZICH_UID"
    usermod -u "$NOZICH_UID" nozich
fi
if [ -n "${NOZICH_GID:-}" ] && validate_uid_gid "$NOZICH_GID" && [ "$NOZICH_GID" != "$(id -g nozich)" ]; then
    echo "[stage2] Changing nozich GID to $NOZICH_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$NOZICH_GID" nozich 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised nozich user (UID 10000)
...
# nozich process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes nozich. Idempotent across container
...
#   - nozich is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G nozich 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] nozich already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" nozich 2>/dev/null; then
        echo "[stage2] Added nozich to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group nozich failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When NOZICH_UID is remapped or the top-level $NOZICH_HOME isn't owned by
# the runtime nozich UID, restore ownership to nozich — but ONLY for the
# directories nozich actually writes to. The full $NOZICH_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of nozich-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_nozich_uid=$(id -u nozich)
needs_chown=false
if [ "$(stat -c %u "$NOZICH_HOME" 2>/dev/null)" != "$actual_nozich_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $NOZICH_HOME (targeted) to nozich ($actual_nozich_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $NOZICH_HOME: chown the directory itself (not its contents)
    # so nozich can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    chown nozich:nozich "$NOZICH_HOME" 2>/dev/null || \
        echo "[stage2] Warning: chown $NOZICH_HOME failed (rootless container?) — continuing"
    # Nozich-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by nozich (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing; do
        if [ -e "$NOZICH_HOME/$sub" ]; then
            chown -R nozich:nozich "$NOZICH_HOME/$sub" 2>/dev/null || \
                echo "[stage2] Warning: chown $NOZICH_HOME/$sub failed (rootless container?) — continuing"
        fi
    done
fi

# --- Fix ownership of build trees under $INSTALL_DIR ---
# Nozich-owned trees under $INSTALL_DIR must be re-chowned whenever the
# runtime nozich UID no longer owns them — otherwise:
...
#     the source mtime is newer than dist/ or when NOZICH_TUI_FORCE_BUILD
#     is set) and writes to ui-tui/dist/. Without this chown the new
#     nozich UID can't write the build output (#28851).
...
# The set mirrors the build-time `chown -R nozich:nozich` line in the
# Dockerfile — keep them in sync if the Dockerfile chown set changes.
# These are under $INSTALL_DIR (not $NOZICH_HOME), so the bind-mount
# concern doesn't apply — recursive is fine.
#
# This MUST be gated independently of the $NOZICH_HOME ownership check
# above. `usermod -u <new> nozich` re-chowns the nozich home dir
# ($NOZICH_HOME == /opt/data) to the new UID as a side effect, so after a
# NOZICH_UID/PUID remap `stat $NOZICH_HOME` always already matches the new
# UID and `needs_chown` is false — but the build trees under /opt/nozich
# are NOT touched by usermod and remain owned by the build-time UID
# (10000). Gating them on $NOZICH_HOME ownership (as #35027 did) silently
...
# when the venv is not already owned by the runtime nozich UID. Idempotent
...
if [ -n "$venv_owner" ] && [ "$venv_owner" != "$actual_nozich_uid" ]; then
    echo "[stage2] Fixing ownership of build trees under $INSTALL_DIR to nozich ($actual_nozich_uid)"
    chown -R nozich:nozich \
        "$INSTALL_DIR/.venv" \
        "$INSTALL_DIR/ui-tui" \
        "$INSTALL_DIR/gateway" \
...
# Always reset ownership of $NOZICH_HOME/profiles to nozich on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> nozich …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as nozich and walks
# the profiles dir. Idempotent; skipped on rootless containers where
# chown would fail.
if [ -d "$NOZICH_HOME/profiles" ]; then
    chown -R nozich:nozich "$NOZICH_HOME/profiles" 2>/dev/null || true
fi

# Always reset ownership of $NOZICH_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged nozich runtime even
# after root-context maintenance commands or scheduler writes.
if [ -d "$NOZICH_HOME/cron" ]; then
    chown -R nozich:nozich "$NOZICH_HOME/cron" 2>/dev/null || true
fi

# Reset ownership of nozich-owned top-level state files on every boot.
# The targeted data-volume chown above only covers nozich-owned
# *subdirectories*; loose state files living directly under $NOZICH_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> nozich …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged nozich runtime then hits
...
# sweep so host-owned files in a bind-mounted $NOZICH_HOME are never
...
# entries of nozich_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$NOZICH_HOME/$f" ]; then
        chown nozich:nozich "$NOZICH_HOME/$f" 2>/dev/null || true
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the nozich runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$NOZICH_HOME/config.yaml" ]; then
    chown nozich:nozich "$NOZICH_HOME/config.yaml" 2>/dev/null || true
    chmod 640 "$NOZICH_HOME/config.yaml" 2>/dev/null || true
fi

# --- Seed directory structure as nozich user ---
# Run as nozich via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $NOZICH_HOME values
# containing shell metacharacters. PR #30136 review item O2.
as_nozich mkdir -p \
    "$NOZICH_HOME/cron" \
    "$NOZICH_HOME/sessions" \
    "$NOZICH_HOME/logs" \
    "$NOZICH_HOME/logs/gateways" \
    "$NOZICH_HOME/hooks" \
    "$NOZICH_HOME/memories" \
    "$NOZICH_HOME/skills" \
    "$NOZICH_HOME/skins" \
    "$NOZICH_HOME/plans" \
    "$NOZICH_HOME/workspace" \
    "$NOZICH_HOME/home" \
    "$NOZICH_HOME/pairing" \
    "$NOZICH_HOME/platforms/pairing"

# --- Install-method stamp (read by detect_install_method() in nozich status) ---
# Preserved from the tini-era entrypoint (PR #27843). Must be written as
# the nozich user so ownership matches the file's documented owner.
# tee is invoked directly via s6-setuidgid (no `sh -c` wrapper) for the
# same shell-metacharacter safety described above.
printf 'docker\n' | as_nozich tee "$NOZICH_HOME/.install_method" >/dev/null \
    || true
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$NOZICH_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        as_nozich cp "$INSTALL_DIR/$src" "$NOZICH_HOME/$dest"
    fi
}
...
if [ -f "$NOZICH_HOME/.env" ]; then
    chown nozich:nozich "$NOZICH_HOME/.env" 2>/dev/null || true
    chmod 600 "$NOZICH_HOME/.env" 2>/dev/null || true
fi

# --- Migrate persisted config schema ---
# Docker image upgrades replace the code under $INSTALL_DIR but preserve
# $NOZICH_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `nozich update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set NOZICH_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$NOZICH_HOME/config.yaml" ]; then
    s6-setuidgid nozich "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$NOZICH_HOME/auth.json" ] && [ -n "${NOZICH_AUTH_JSON_BOOTSTRAP:-}" ]; then
    printf '%s' "$NOZICH_AUTH_JSON_BOOTSTRAP" > "$NOZICH_HOME/auth.json"
    chown nozich:nozich "$NOZICH_HOME/auth.json" 2>/dev/null || true
    chmod 600 "$NOZICH_HOME/auth.json"
fi
...
# can set NOZICH_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# NOZICH_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$NOZICH_HOME/gateway_state.json" ] && \
        [ "${NOZICH_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    printf '{"gateway_state":"running"}\n' > "$NOZICH_HOME/gateway_state.json"
    chown nozich:nozich "$NOZICH_HOME/gateway_state.json" 2>/dev/null || true
    chmod 644 "$NOZICH_HOME/gateway_state.json"
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_nozich "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/nozich/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Nozich spawns for the
...
# main-wrapper.sh propagates it into the supervised ``nozich`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-nozich, dashboard, etc.).
