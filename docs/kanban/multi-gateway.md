...
Nozich supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.nozich/config.yaml`:
...
Or set the env var: `NOZICH_KANBAN_DISPATCH_IN_GATEWAY=false`
