...
Nozich middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Nozich'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Nozich calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Nozich retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Nozich guardrails,
...
- `telemetry_schema_version`: currently `nozich.observer.v1`
- `middleware_schema_version`: currently `nozich.middleware.v1`
...
Nozich stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Nozich runs
them as a nested chain in registration order. Middleware failures are fail-open:
Nozich logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Nozich applies middleware in this order:
...
For each tool call, Nozich applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Nozich pre-execution path against the effective arguments:
...
```bash
nozich plugins enable <plugin-name>
```

For isolated local testing, use one `NOZICH_HOME` for plugin enablement and the
agent run:

```bash
export NOZICH_HOME=/tmp/nozich-middleware-test
mkdir -p "$NOZICH_HOME"
nozich plugins enable <plugin-name>
nozich chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run nozich plugins enable <plugin-name>
uv run nozich chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["nozich_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/nozich-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Nozich expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Nozich treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Nozich preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Nozich does not convert downstream
