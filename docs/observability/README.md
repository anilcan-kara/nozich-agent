# Nozich Observer Hooks

Nozich observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Nozich' planner, model providers, memory, tool registry,
...
Hook callbacks are fail-open. Nozich catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Nozich only builds
