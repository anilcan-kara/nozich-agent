...
When running Nozich inside Docker, the default `network_mode: host` gives the
...
The Nozich [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   NOZICH_UID=$(id -u) NOZICH_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=nozich,nozich-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://nozich-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Nozich trust model and vulnerability reporting
