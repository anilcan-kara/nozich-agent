...
There are two ways to add a platform to the Nozich gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.nozich/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Nozich code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `nozich gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `nozich_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Nozich core.
...
And add it to the `nozich-gateway` composite:

```python
"nozich-gateway": {
    "includes": [..., "nozich-your-platform"]
}
```
...
## 12. Status Display (`nozich_cli/status.py`)
...
## 13. Gateway Setup Wizard (`nozich_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ nozich_cli/ toolsets.py \
  --include="*.py" -l | sort -u
