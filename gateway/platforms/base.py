...
# Audio file extensions Nozich recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Nozich' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Nozich-created Telegram private-chat
...
from nozich_constants import get_default_nozich_root, get_nozich_dir, get_nozich_home


GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    "Secure secret entry is not supported over messaging. "
    "Load this skill in the local CLI to be prompted, or add the key to ~/.nozich/.env manually."
)
...
# Default location: {NOZICH_HOME}/cache/images/ (legacy: image_cache/)
IMAGE_CACHE_DIR = get_nozich_dir("cache/images", "image_cache")
        ...
        for attempt in range(retries + 1):
            try:
                response = await client.get(
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; NozichAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                )
...
AUDIO_CACHE_DIR = get_nozich_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_nozich_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_nozich_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_nozich_dir("cache/screenshots", "browser_screenshots")
_NOZICH_HOME = get_nozich_home()
_NOZICH_ROOT = get_default_nozich_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "NOZICH_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "NOZICH_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "NOZICH_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "NOZICH_MEDIA_DELIVERY_STRICT"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _NOZICH_HOME / "image_cache",
    _NOZICH_HOME / "audio_cache",
    _NOZICH_HOME / "video_cache",
    _NOZICH_HOME / "document_cache",
    _NOZICH_HOME / "browser_screenshots",
    ...
    _NOZICH_HOME / "cache" / "images",
    _NOZICH_HOME / "cache" / "audio",
    _NOZICH_HOME / "cache" / "videos",
    _NOZICH_HOME / "cache" / "documents",
    _NOZICH_HOME / "cache" / "screenshots",
)
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Nozich profile and shared Nozich root both contain control
    # files and credentials. Only cache subdirectories under them are
    # explicitly allowlisted above.
    for nozich_root in (_NOZICH_HOME, _NOZICH_ROOT):
        denied.append(nozich_root / ".env")
        denied.append(nozich_root / "auth.json")
        denied.append(nozich_root / "credentials")
        denied.append(nozich_root / "config.yaml")
    return denied
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and Nozich
    secrets (``~/.nozich/.env``, ``auth.json``) are *separate, more-specific*
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    ``NOZICH_MEDIA_DELIVERY_STRICT=1``): the file MUST live under a
    Nozich-managed cache, under an operator-allowlisted root
    (``NOZICH_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # The denylist still blocks /etc, /proc, ~/.ssh, ~/.aws, ~/.nozich/.env,
    # ~/.nozich/auth.json, etc. — so the obvious prompt-injection sites
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?nozich\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+nozich[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    # Nozich commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        self._busy_text_mode: str = (
            os.environ.get("NOZICH_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower()
            or "interrupt"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "NOZICH_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "NOZICH_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    def prefers_fresh_final_streaming(
        ...
        implementation supports. Telegram is the motivating case: Nozich sends
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.nozich/media/generated/stale.png"}
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from nozich_cli.commands import should_bypass_active_session
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          NOZICH_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          NOZICH_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          NOZICH_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("NOZICH_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("NOZICH_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("NOZICH_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            # not at the top of this task.  _nozich_run_generation is set on
