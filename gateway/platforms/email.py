"""
Email platform adapter for the Nozich gateway.

Allows users to interact with Nozich by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "nozich-agent" "version" "{_nozich_version}" '
            '"vendor" "anilcan-kara" '
            '"support-email" "noreply@anilcan-kara.com")',
        )
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Nozich Agent")
        ...
        msg_id = f"<nozich-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
