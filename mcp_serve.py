"""
Nozich MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Nozich-specific extra)

Usage:
    nozich mcp serve
    nozich mcp serve --verbose
...
logger = logging.getLogger("nozich.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using NOZICH_HOME."""
    try:
        from nozich_constants import get_nozich_home
        return get_nozich_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("NOZICH_HOME", Path.home() / ".nozich")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from nozich_constants import get_nozich_home
        directory_file = get_nozich_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("NOZICH_HOME", Path.home() / ".nozich")
...
class EventBridge:
    ...
    This is the Nozich equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from nozich_constants import get_nozich_home
            db_file = get_nozich_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("NOZICH_HOME", Path.home() / ".nozich")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Nozich MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "nozich",
        instructions=(
            "Nozich Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Nozich MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
