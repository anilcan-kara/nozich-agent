...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      nozich-agent = self'.packages.default;
      nozichVenv = nozich-agent.nozichVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "nozich-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${nozichVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "nozich-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "nozich-build-package" { } ''
          echo "PASS: package built at ${nozich-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "nozich-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "nozich-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${nozich-agent}/bin/nozich || (echo "FAIL: nozich binary missing"; exit 1)
          test -x ${nozich-agent}/bin/nozich-agent || (echo "FAIL: nozich-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${nozich-agent}/bin/nozich version 2>&1 | grep -qi "nozich" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "nozich-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in nozich nozich-agent nozich-acp; do
            test -x ${nozich-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "nozich-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking nozich --help ==="
          ${nozich-agent}/bin/nozich --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${nozich-agent}/bin/nozich --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "nozich-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${nozich-agent}/share/nozich-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${nozich-agent}/share/nozich-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "NOZICH_BUNDLED_SKILLS" ${nozich-agent}/bin/nozich || \
            (echo "FAIL: NOZICH_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: NOZICH_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "nozich-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${nozich-agent}/share/nozich-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${nozich-agent}/share/nozich-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "NOZICH_BUNDLED_PLUGINS" ${nozich-agent}/bin/nozich || \
            (echo "FAIL: NOZICH_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: NOZICH_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "nozich-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${nozich-agent}/share/nozich-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          LOC_COUNT=$(find ${nozich-agent}/share/nozich-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${nozich-agent}/share/nozich-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "NOZICH_BUNDLED_LOCALES" ${nozich-agent}/bin/nozich || \
            (echo "FAIL: NOZICH_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: NOZICH_BUNDLED_LOCALES set in wrapper"

          echo "=== Rendering via the wrapper override (NOZICH_BUNDLED_LOCALES) ==="
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && NOZICH_BUNDLED_LOCALES=${nozich-agent}/share/nozich-agent/locales \
            ${nozichVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with NOZICH_BUNDLED_LOCALES set"; exit 1)
          ...
          BARE_DIR=$(cd "$HOME" && ${nozichVenv}/bin/python3 -c "from agent import i18n; print(i18n._locales_dir())")
          BARE=$(cd "$HOME" && ${nozichVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "resolved dir (no env var): $BARE_DIR"
          echo "rendered: $BARE"
          test "$BARE" != "gateway.reset.header_default" || \
            (echo "FAIL: sealed venv could not resolve locales without NOZICH_BUNDLED_LOCALES — data-files materialization regressed"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "nozich-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${nozich-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${nozich-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "NOZICH_TUI_DIR" ${nozich-agent}/bin/nozich || \
            (echo "FAIL: NOZICH_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: NOZICH_TUI_DIR set in wrapper"
        ...
        # Verify NOZICH_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        nozich-node = pkgs.runCommand "nozich-node-version" { } ''
          set -e
          echo "=== Checking NOZICH_NODE in wrapper ==="
          grep -q "NOZICH_NODE" ${nozich-agent}/bin/nozich || \
            (echo "FAIL: NOZICH_NODE not set in wrapper"; exit 1)
          echo "PASS: NOZICH_NODE present in wrapper"

          NOZICH_NODE=$(sed -n "s/^export NOZICH_NODE='\(.*\)'/\1/p" ${nozich-agent}/bin/nozich)
          test -x "$NOZICH_NODE" || (echo "FAIL: NOZICH_NODE=$NOZICH_NODE not executable"; exit 1)
          echo "PASS: NOZICH_NODE executable at $NOZICH_NODE"

          NODE_MAJOR=$("$NOZICH_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All NOZICH_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify NOZICH_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "nozich-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(NOZICH_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking NOZICH_MANAGED guards ==="
          check_blocked "config set" ${nozich-agent}/bin/nozich config set model foo
          check_blocked "config edit" ${nozich-agent}/bin/nozich config edit
        ...
        extra-python-packages = let
          ...
          nozichWithExtra = nozich-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "nozich-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${nozichWithExtra}/bin/nozich || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${nozichWithExtra}/bin/nozich || \
          ...
          if grep -q "PYTHONPATH" ${nozich-agent}/bin/nozich; then
        ...
        extra-dependency-groups = let
          nozichWithGroups = nozich-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "nozich-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${nozichWithGroups}"
          echo "venv: ${nozichWithGroups.nozichVenv}"
        ...
        messaging-variant = pkgs.runCommand "nozich-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.nozichVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "nozich-config-roundtrip" {
          ...
          merge_and_load() {
            local nozich_home="$1"
            export NOZICH_HOME="$nozich_home"
            ${configMergeScript} ${nixSettings} "$nozich_home/config.yaml"
            ${nozichVenv}/bin/python3 -c '
import json, sys
