# nix/desktop.nix — Nozich Desktop (Electron) app build + wrapper
#
# `nozichAgent` is the fully-built `.#default` package — it ships the
# `nozich` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `NOZICH_DESKTOP_NOZICH` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing Nozich CLI").
...
let
  npm = nozichNpmLib.mkNpmPassthru {
    folder = "apps/desktop";
    attr = "desktop";
    pname = "nozich-desktop";
  };
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/nozich-desktop $out/bin
    cp -r ${renderer}/* $out/share/nozich-desktop/
    ...
    substituteInPlace $out/share/nozich-desktop/electron/main.cjs \
      --replace-fail "process.resourcesPath" "'$out/share/nozich-desktop'"

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # NOZICH_DESKTOP_NOZICH to the absolute path of the nix-built `nozich`
    # binary so the desktop's resolver step 4 ("existing Nozich CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/nozich-desktop \
      --add-flags "$out/share/nozich-desktop" \
      --set NOZICH_DESKTOP_NOZICH "${lib.getExe nozichAgent}" \
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for Nozich Agent";
    homepage = "https://github.com/anilcan-kara/nozich-agent";
    license = licenses.mit;
    platforms = platforms.unix;
    mainProgram = "nozich-desktop";
  };
}
