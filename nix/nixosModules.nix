# nix/nixosModules.nix — NixOS module for nozich-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: nozich runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $NOZICH_HOME/.env
# and read by nozich at startup — no container recreation needed for env changes.
#
# Tool resolution: the nozich wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.nozich-agent;
    effectivePackage =
    ...
    nozich-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-nozich-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "nozich-config-attrs";
      description = "Nozich YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "nozich-config.yaml" configJson;
    configFile = if cfg.configFile != null then cfg.configFile else generatedConfigFile;

    configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

    # config.yaml mode: group-writable (0660) when interactive users share this
    # NOZICH_HOME via addToSystemPackages, so they can save settings through the
    ...
    documentDerivation = pkgs.runCommand "nozich-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'NOZICH_DOC_EOF'\n${value}\nNOZICH_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "nozich-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/nozich";
    ...
    # nozich user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "nozich-container-entrypoint" ''
      set -eu

      NOZICH_UID="''${NOZICH_UID:?NOZICH_UID must be set}"
      NOZICH_GID="''${NOZICH_GID:?NOZICH_GID must be set}"

      # ── Group: ensure a group with GID=$NOZICH_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$NOZICH_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="nozich"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$NOZICH_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$NOZICH_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$NOZICH_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$NOZICH_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="nozich"
        TARGET_HOME="/home/nozich"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$NOZICH_UID" -g "$NOZICH_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$NOZICH_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$NOZICH_UID:$NOZICH_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure NOZICH_HOME is owned by the target user.
      ...
      if [ -n "''${NOZICH_HOME:-}" ] && [ -d "$NOZICH_HOME" ]; then
        find "$NOZICH_HOME" \! -user "$NOZICH_UID" -exec chown "$NOZICH_UID:$NOZICH_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/nozich-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/nozich-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/nozich ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/nozich
        chmod 0440 /etc/sudoers.d/nozich
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$NOZICH_UID" --regid="$NOZICH_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $NOZICH_HOME/.env.
    ...
    # Default: /var/lib/nozich/workspace → /data/workspace.
  ...
  in {
    options.services.nozich-agent = with lib; {
      enable = mkEnableOption "Nozich Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = nozich-agent;
        description = "The nozich-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/nozich";
        description = "State directory. Contains .nozich/ subdir (NOZICH_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Nozich config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $NOZICH_HOME/.env at activation time.
          Nozich reads this file on every startup via load_nozich_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $NOZICH_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $NOZICH_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `nozich gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the nozich user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the nozich plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Nozich discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          nozich_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as nozich (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in nozich-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the nozich CLI to environment.systemPackages and export
          NOZICH_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.nozich symlink to the service
            stateDir. These users are automatically added to the nozich group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.nozich-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the nozich CLI to system PATH and export NOZICH_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.nozich/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.NOZICH_HOME = "${cfg.stateDir}/.nozich";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.nozich-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the nozich user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.nozich-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed nozich
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure nozich is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.nozich        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.nozich/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.nozich/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.nozich/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.nozich/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.nozich/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."nozich-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.nozich
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.nozich ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.nozich ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.nozich -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.nozich/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.nozich/$_subdir"
            chmod 2770 "${cfg.stateDir}/.nozich/$_subdir"
            find "${cfg.stateDir}/.nozich/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          # nozich-group users can save settings via the CLI/TUI, else 0640).
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m ${configYamlMode} -D ${configFile} ${cfg.stateDir}/.nozich/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.nozich/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.nozich/config.yaml
            chmod ${configYamlMode} ${cfg.stateDir}/.nozich/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.nozich/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.nozich/.managed
          chmod 0644 ${cfg.stateDir}/.nozich/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.nozich/.container-mode <<'NOZICH_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    nozich_bin=${containerDataDir}/current-package/bin/nozich
    NOZICH_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.nozich/.container-mode
            chmod 0644 ${cfg.stateDir}/.nozich/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.nozich/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.nozich";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.nozich" ]; then
                  rm -f "${symlinkPath}"
                  echo "nozich-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.nozich -> stateDir/.nozich for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.nozich";
                target = "${cfg.stateDir}/.nozich";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "nozich-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.nozich/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.nozich/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.nozich/auth.json
              fi
            ''}
          ''}
          ...
          # Nozich reads $NOZICH_HOME/.env at startup via load_nozich_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.nozich/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'NOZICH_NIX_ENV_EOF'
    ${envFileContent}
    NOZICH_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.nozich/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.nozich/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.nozich/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.nozich-agent = {
          description = "Nozich Agent Gateway";
          ...
          environment = {
            HOME = cfg.stateDir;
            NOZICH_HOME = "${cfg.stateDir}/.nozich";
            NOZICH_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $NOZICH_HOME/.env by the activation script. load_nozich_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/nozich"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the nozich group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.nozich-agent = {
          description = "Nozich Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              NOZICH_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              NOZICH_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env NOZICH_UID="$NOZICH_UID" \
                --env NOZICH_GID="$NOZICH_GID" \
                --env NOZICH_HOME=${containerDataDir}/.nozich \
                --env NOZICH_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/nozich gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
