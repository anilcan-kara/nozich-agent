"""Windows UTF-8 bootstrap for Nozich entry points.
...
should be imported at the very top of every Nozich entry point
(``nozich``, ``nozich-agent``, ``nozich-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import nozich_bootstrap``
# (or ``from nozich_bootstrap import apply_windows_utf8_bootstrap``) at
