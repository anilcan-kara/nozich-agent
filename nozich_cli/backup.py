"""
Backup and import commands for nozich CLI.

`nozich backup` creates a zip archive of the entire ~/.nozich/ directory
(excluding the nozich-agent repo and transient files).

`nozich import` restores from a backup zip, overlaying onto the current
NOZICH_HOME root.
"""
...
from nozich_constants import get_default_nozich_root, get_nozich_home, display_nozich_home
...
# ``nozich-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/nozich-agent/``
# are not accidentally excluded.
_EXCLUDED_DIRS = {
    "nozich-agent",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to nozich root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``nozich-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/nozich-agent/`` — must be preserved.
        if part == "nozich-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside NOZICH_HOME.
...
def run_backup(args) -> None:
    """Create a zip backup of the Nozich home directory."""
    nozich_root = get_default_nozich_root()

    if not nozich_root.is_dir():
        print(f"Error: Nozich home directory not found at {nozich_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"nozich-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"nozich-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_nozich_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(nozich_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(nozich_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``nozich-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "nozich-agent" and not is_root)
        ]
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(nozich_root)
    ...
    print(f"\nRestore with: nozich import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Nozich backup.
    ...
    # Look for telltale files that a nozich home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Nozich backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.nozich/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a nozich dir name
        if prefix in {".nozich", "nozich"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_nozich_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (nozich_root / "config.yaml").exists()
        has_env = (nozich_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Nozich configuration.")
        ...
        for member in members:
            ...
            target = nozich_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(nozich_root.resolve())
            except ValueError:
        ...
        profiles_dir = nozich_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # nozich_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: nozich profile list  (after installing nozich)")

        # Guidance
        print()
        if not (nozich_root / "nozich-agent").is_dir():
            print("Note: The nozich-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: nozich update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  nozich -p {pname} gateway install")

        print("Done. Your Nozich configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and nozich backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to NOZICH_HOME).
...
# — `nozich update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(nozich_home: Optional[Path] = None) -> Path:
    home = nozich_home or get_nozich_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None,
    nozich_home: Optional[Path] = None,
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(nozich_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside NOZICH_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``nozich update``.
    ...
    Args:
        ...
        nozich_home: Override for the Nozich home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(nozich_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for nozich backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_nozich_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, nozich_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``nozich_root`` to ``out_path``.
...
def _pre_update_backup_dir(nozich_home: Optional[Path] = None) -> Path:
    home = nozich_home or get_nozich_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of NOZICH_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<NOZICH_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``nozich update``) should continue even if the backup fails.
    """
    nozich_root = nozich_home or get_default_nozich_root()
    if not nozich_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(nozich_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, nozich_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `nozich claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of NOZICH_HOME under ``backups/`` before a
    ``nozich claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``nozich import <archive>``.  Writes to
    ``<NOZICH_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `nozich import` and the
