"""
Baked-in build metadata for Nozich Agent.

Source installs report their git revision live via ``git rev-parse`` (see
``nozich_cli/dump.py`` and ``nozich_cli/banner.py``).  That doesn't work inside
...
To make ``nozich dump`` and the startup banner identify the exact commit the
image was built from, the Docker build writes the build-time ``$NOZICH_GIT_SHA``
arg into ``<project_root>/.nozich_build_sha``.  This module is the single
...
- Returns ``None`` when the file is absent.  Source installs and dev images
  built without the ``NOZICH_GIT_SHA`` build-arg fall through to live-git
...
_BUILD_SHA_FILE = Path(__file__).parent.parent / ".nozich_build_sha"


def get_build_sha(short: int = 8) -> Optional[str]:
    """Return the baked-in build SHA, truncated to ``short`` chars, or None.

    Reads ``<project_root>/.nozich_build_sha`` if present.  The file is
    written by the Dockerfile's ``NOZICH_GIT_SHA`` build-arg and contains
