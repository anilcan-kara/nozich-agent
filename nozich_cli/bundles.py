"""Implementation of the ``nozich bundles`` CLI subcommand.

Mirrors the structure of ``nozich_cli/skills_hub.py`` but for skill
...
def _console() -> Console:
    # Bind to stderr so piping `nozich bundles list | grep …` doesn't
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  nozich bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``nozich bundles`` argparse tree.

    Called from ``nozich_cli/main.py`` where it owns the top-level
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `nozich bundles list`",
    )
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``nozich bundles <subcommand>`` to the right handler."""
