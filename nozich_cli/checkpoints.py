"""`nozich checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.nozich/checkpoints/``.  Actions:

    nozich checkpoints               # same as `status`
    nozich checkpoints status        # total size, project count, breakdown
    nozich checkpoints list          # per-project checkpoint counts + workdir
    nozich checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    nozich checkpoints clear [-f]    # nuke the entire base (asks first)
    nozich checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    nozich checkpoints
    nozich checkpoints prune --retention-days 3 --max-size-mb 200
    nozich checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: nozich checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``nozich checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `nozich checkpoints` → status
