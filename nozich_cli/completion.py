"""Shell completion script generation for nozich CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_nozich_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Nozich Agent bash completion
# Add to ~/.bashrc:
#   eval "$(nozich completion bash)"

_nozich_profiles() {{
    local profiles_dir="$HOME/.nozich/profiles"
...
_nozich_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_nozich_profiles)" -- "$cur"))
        return
    fi
...
complete -F _nozich_completion nozich
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef nozich
# Nozich Agent zsh completion
# Add to ~/.zshrc:
...
_nozich_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.nozich/profiles" ]]; then
        profiles+=($HOME/.nozich/profiles/*(N/:t))
    fi
...
_nozich() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_nozich_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'nozich command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Nozich Agent fish completion",
        "# Add to your config:",
        "#   nozich completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __nozich_profiles",
        "    echo default",
        "    if test -d $HOME/.nozich/profiles",
        "        for d in $HOME/.nozich/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c nozich -f -s p -l profile"
        " -d 'Profile name' -xa '(__nozich_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__nozich_profiles)' -d 'Profile name'"
                )
