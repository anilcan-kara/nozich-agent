"""
Configuration management for Nozich Agent.

Config files are stored in ~/.nozich/ for easy access:
- ~/.nozich/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.nozich/.env         - API keys and secrets

This module provides:
- nozich config          - Show current configuration
- nozich config edit     - Open config in editor
- nozich config set      - Set a specific value
- nozich config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``nozich config set`` (which rewrites
    ...
    ``config.yaml`` in place: nozich never silently mutates the user's config,
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.nozich/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``nozich logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  nozich config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Nozich makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Nozich itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``nozich update``, the TUI build.
...
#   on every plugin install / ``nozich update``.
...
# * ``NOZICH_HOME`` / ``NOZICH_PROFILE`` / ``NOZICH_CONFIG`` /
#   ``NOZICH_ENV`` — Nozich runtime location flags. Writing these into
...
#
# IMPORTANT: ``NOZICH_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (NOZICH_GEMINI_CLIENT_ID,
# NOZICH_LANGFUSE_PUBLIC_KEY, NOZICH_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Nozich runtime location — never via dashboard env writer.
    # NOT a NOZICH_* blanket: integration credentials (NOZICH_GEMINI_*,
    # NOZICH_LANGFUSE_*, NOZICH_SPOTIFY_*, ...) ARE allowed.
    "NOZICH_HOME", "NOZICH_PROFILE", "NOZICH_CONFIG", "NOZICH_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Nozich runtime location "
            "(NOZICH_HOME, NOZICH_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "NOZICH_TOOL_PROGRESS", "NOZICH_TOOL_PROGRESS_MODE",
    ...
    # Activation is via plugins.enabled (opt-in through `nozich plugins enable
    # observability/langfuse` or `nozich tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "NOZICH_LANGFUSE_ENV",
    "NOZICH_LANGFUSE_RELEASE",
    "NOZICH_LANGFUSE_SAMPLE_RATE",
    "NOZICH_LANGFUSE_MAX_CHARS",
    "NOZICH_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from nozich_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("NOZICH_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_nozich_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Nozich is running in package-manager-managed mode.

    Two signals: the NOZICH_MANAGED env var (set by the systemd service),
    or a .managed marker file in NOZICH_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade nozich-agent"
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Nozich was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Stamped ``~/.nozich/.install_method`` file (written by installers)
    2. NOZICH_MANAGED env / .managed marker (NixOS, Homebrew)
    ...
    container detection here:
      ...
      - the published ``anilcan-kara/nozich-agent`` image stamps ``docker``
    ...
    so ``nozich update`` bailed with "doesn't apply inside the Docker
    ...
    """
    stamp = get_nozich_home() / ".install_method"
    try:
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.nozich/.install_method."""
    stamp = get_nozich_home() / ".install_method"
    try:
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Nozich lives in a ``uv tool`` layout.

    ``uv tool install nozich-agent`` places the install at
    ``.../uv/tools/nozich-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``nozich-agent`` happens to be uv-tool-installed on the machine while
    the *active* Nozich is a regular pip/venv install, causing
    ``nozich update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/nozich-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade nozich-agent"
    if method == "docker":
        return "docker pull anilcan-kara/nozich-agent:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade nozich-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade nozich-agent"
        return "pip install --upgrade nozich-agent"
    return "nozich update"
...
# Long-form text for ``nozich update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# nozich_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Nozich, it doesn't update
...
✗ ``nozich update`` doesn't apply inside the Docker container.

Nozich Agent runs as a published image (anilcan-kara/nozich-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull anilcan-kara/nozich-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate nozich-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm anilcan-kara/nozich-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/anilcan-kara/nozich-agent/tags
  • Your config and session history live under ``$NOZICH_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``nozich update`` inside Docker.
...
def format_managed_message(action: str = "modify this Nozich installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("NOZICH_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Nozich installation is managed by NixOS "
            f"(NOZICH_MANAGED={env_hint}).\n"
            "Edit services.nozich-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Nozich installation is managed by Homebrew "
            f"(NOZICH_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade nozich-agent"
        )

    return (
        f"Cannot {action}: this Nozich installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Nozich."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from NOZICH_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, nozich_bin
    or None if container mode is not active, we're already inside the
    container, or NOZICH_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("NOZICH_DEV") == "1":
        return None
    ...
    container_mode_file = get_nozich_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "nozich-agent")
    exec_user = info.get("exec_user", "nozich")
    nozich_bin = info.get("nozich_bin", "/data/current-package/bin/nozich")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "nozich_bin": nozich_bin,
    }
...
# Re-export from nozich_constants — canonical definition lives there.
from nozich_constants import get_nozich_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_nozich_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_nozich_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_nozich_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the NOZICH_UID / NOZICH_GID env vars set by Docker deployments.

    Docker containers running Nozich commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level NOZICH_HOME once, but
    subdirectories created at runtime by ``ensure_nozich_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("NOZICH_UID", "").strip()
    gid_str = os.environ.get("NOZICH_GID", "").strip()
    try:
...
def _chown_to_nozich_uid(path) -> None:
    """Chown ``path`` to ``NOZICH_UID:NOZICH_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_nozich_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_nozich_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the nozich group can
    share state with the gateway service.

    The mode can be overridden via the NOZICH_HOME_MODE environment variable
    (e.g. NOZICH_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse NOZICH_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``NOZICH_UID``/``NOZICH_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("NOZICH_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_nozich_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Nozich runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("NOZICH_CONTAINER") or os.environ.get("NOZICH_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set NOZICH_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into NOZICH_HOME if the user doesn't have one yet."""
...
def ensure_nozich_home():
    """Ensure ~/.nozich directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_nozich_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_nozich_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"NOZICH_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Nozich-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Nozich
        ...
        # identity slot (SOUL.md). Empty by default. The NOZICH_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Nozich adds a coding operating brief
    ...
    "terminal": {
        ...
        #             NOZICH_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force NOZICH_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Nozich auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Nozich sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Nozich
        ...
        # bundled Nozich image, which drops to the `nozich` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Nozich sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Nozich should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``nozich chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.nozich/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: nozich sweeps the checkpoint base at startup
    ...
    # SOUL.md, AGENTS.md, CLAUDE.md, .nozich.md, or .cursorrules before Nozich
    ...
    # single read_file page exceeds these limits, Nozich truncates the
    ...
    "auxiliary": {
        ...
        # Invoked by ``nozich kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``nozich kanban decompose`` and the kanban
        ...
        # ``nozich profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `nozich model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `nozich` (and `nozich chat`) launches by default:
        ...
        # REPL and `--tui` (or NOZICH_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `nozich --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `nozich -c` muscle memory.  Default off so existing
        # users aren't surprised.  NOZICH_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `nozich --tui` drops a one-time hint
        ...
        # e.g. `model · 68% · ~/projects/nozich`. Per-platform overrides go under
    ...
    "dashboard": {
        ...
        # ``NOZICH_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``NOZICH_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # ``portal_url`` defaults to https://portal.anilcan-kara.com).
        "oauth": {
        ...
        # (``NOZICH_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # Public URL override (env: ``NOZICH_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/nozich``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Nozich appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.nozich/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.nozich/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # response. If not, Nozich feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Nozich auto-pauses the goal and
    ...
    # always goes to ~/.nozich/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${NOZICH_SKILL_DIR} and ${NOZICH_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `nozich curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # normally restored on every `nozich update`, so pruning them only
        ...
        # skipped), snapshot ~/.nozich/skills/ into
        # ~/.nozich/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `nozich curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for nozich-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Nozich Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (NOZICH_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.nozich/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or NOZICH_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `nozich doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``nozich_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Nozich to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # Also overridable via NOZICH_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `nozich -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `nozich` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `nozich kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     nozich-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Nozich tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.nozich/logs/.
    ...
    # update model picker lists without shipping a nozich-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://nozich-agent.anilcan-kara.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `nozich model` invocation; network failures
    ...
    "gateway": {
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.nozich/.env,
        ...
        # behavior: files must live under the Nozich cache, under
        ...
        # user. Bridged to NOZICH_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Nozich
        # cache (~/.nozich/cache/{documents,images,audio,video,screenshots})
        ...
        # to NOZICH_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # NOZICH_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to NOZICH_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.nozich/state.db.
    ...
    "sessions": {
        ...
        # default of ``nozich sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.nozich/sessions/session_{sid}.json`` on every turn
    ...
    # ``nozich update`` behaviour.
    "updates": {
        # Run a full ``nozich backup``-style zip of NOZICH_HOME before every
        # ``nozich update``.  Backups land in ``<NOZICH_HOME>/backups/`` and
        # can be restored with ``nozich import <path>``.  Off by default —
        # on large NOZICH_HOME directories the zip can add minutes to every
        ...
        "backup_keep": 5,
        # What `nozich update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<NOZICH_HOME>/lsp/bin/`` on first use.
    ...
    # `nozich tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.nozich/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.nozich/.env (or your shell) and never in config.yaml.
            ...
            # ~/.nozich/bin/ on first use.  When False you must install
            ...
            # `nozich secrets bitwarden setup`.
            "server_url": "",
        },
    },
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'nozich model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for Nous Subscribers only, used to derive vendor hosts, e.g. anilcan-kara.com -> firecrawl-gateway.anilcan-kara.com",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Nozich auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @nozich:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. NOZICH_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #nozich)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: nozich-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'nozich-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Nozich API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Nozich API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Nozich API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # NOZICH_TOOL_PROGRESS and NOZICH_TOOL_PROGRESS_MODE are deprecated —
    ...
    "NOZICH_EPHEMERAL_SYSTEM_PROMPT": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.nozich.config`` entries, then checks
    ...
    try:
        ...
        # should never break `nozich update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Nozich versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``nozich_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'nozich setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Nozich won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'nozich doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("NOZICH_HOME", "~/.nozich")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("NOZICH_TOOL_PROGRESS")
            old_mode = get_env_value("NOZICH_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from NOZICH_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from NOZICH_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("NOZICH_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from NOZICH_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$NOZICH_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_nozich_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `nozich plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `nozich curator status` has no
    ...
    #   3. Creates `~/.nozich/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_nozich_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_nozich_home() / "logs" / "curator"
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from nozich_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: nozich config set <key> <value>")
    ...
    # metadata.nozich.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _nozich_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.nozich/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.nozich/config.yaml.
    ...
    (which change ``NOZICH_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
#   openai-codex (OAuth — nozich auth) — OpenAI Codex
#   nous         (OAuth — nozich auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.nozich/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.nozich/.env.
    ...
    interactive menu render (`nozich tools`, `nozich setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `nozich tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Nozich env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.nozich/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.nozich/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.nozich/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.nozich/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Nozich — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Nozich vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.nozich/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ Nozich Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from nozich_cli.auth import get_anthropic_key
    ...
    # Warn on stale NOZICH_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("NOZICH_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale NOZICH_MAX_ITERATIONS={_env_ghost} "
                f"(run 'nozich doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  nozich config edit     # Edit config file", Colors.DIM))
    print(color("  nozich config set <key> <value>", Colors.DIM))
    print(color("  nozich setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: nozich config set <key> <value>")
            print()
            print("Examples:")
            print("  nozich config set model anthropic/claude-sonnet-4")
            print("  nozich config set terminal.backend docker")
            print("  nozich config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'nozich config migrate' to add them")
        ...
        print("  nozich config           Show current configuration")
        print("  nozich config edit      Open config in editor")
        print("  nozich config set <key> <value>   Set a config value")
        print("  nozich config check     Check for missing/outdated config")
        print("  nozich config migrate   Update config with new options")
        print("  nozich config path      Show config file path")
        print("  nozich config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``nozich config`` UI so users
