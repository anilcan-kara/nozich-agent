...
``$NOZICH_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-nozich-setup
(the stage2 hook) has chowned the volume and seeded $NOZICH_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$NOZICH_HOME``,
    not under ``profiles/``). The dispatcher in ``nozich_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``nozich gateway start`` (no ``-p``) targets. Without it,
    bare ``nozich gateway start`` inside the container would land on
    ...
    ``$NOZICH_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        nozich_home: The container's NOZICH_HOME (typically /opt/data).
            Profiles live under ``<nozich_home>/profiles/<name>/``;
            the default profile lives at ``<nozich_home>`` itself.
    ...
    # ``nozich gateway start`` (no ``-p``) has somewhere to land;
    ...
    default_prior_state = legacy_default_state or _read_desired_state(nozich_home)
    default_should_start = default_prior_state in _AUTOSTART_STATES
    if not dry_run:
        _cleanup_stale_runtime_files(nozich_home)
    ...
    profiles_root = nozich_home / "profiles"
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `nozich profile create` (config.yaml
            # is not — that comes later via `nozich setup`). Use it as the
            ...
            # via ``nozich -p default-named gateway start`` if they
    ...
    if not dry_run:
        _write_reconcile_log(nozich_home, actions)
    return actions
...
def _maybe_migrate_legacy_gateway_run_state(
    ...
    *,
    ...
    """
    state_file = nozich_home / "gateway_state.json"
    if state_file.exists():
        return None

    if os.environ.get("NOZICH_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes"):
        return None
...
def _strip_container_argv_prefix(argv: Sequence[str]) -> list[str]:
    """Strip the s6/wrapper prefix off PID 1 argv, leaving the nozich args.

    The container PID 1 argv looks like
    ``/init /opt/nozich/docker/main-wrapper.sh <subcommand> [args...]`` and
    the wrapper re-execs ``nozich <subcommand>``. Peel ``init`` →
    ``main-wrapper.sh`` → ``nozich`` so callers can match on the bare
    ...
    if args and args[0].endswith("main-wrapper.sh"):
        args = args[1:]
    if args and Path(args[0]).name == "nozich":
        args = args[1:]
    return args
...
def _is_dashboard_container(argv: Sequence[str]) -> bool:
    ...
    A dashboard-only container (``nozich dashboard ...``) never spawns or
    ...
    the gateway and dashboard containers share a bind-mounted NOZICH_HOME,
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `nozich -p <profile> gateway start`
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit nozich
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the nozich user) won't hit EACCES. See
...
def _write_reconcile_log(
    nozich_home: Path, actions: list[ReconcileAction],
) -> None:
    """Append one line per profile to $NOZICH_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    # bind-mounted NOZICH_HOME, both race to flock() the same s6-log lock
    ...
    actions = reconcile_profile_gateways(
        nozich_home=nozich_home, scandir=scandir,
    )
    for a in actions:
        print(
