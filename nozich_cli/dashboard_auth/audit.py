...
Profile-aware location: ``$NOZICH_HOME/logs/dashboard-auth.log``.
...
from ``nozich_constants`` or other nozich_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$NOZICH_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``nozich_constants.get_nozich_home`` semantics: env var wins,
    else ``~/.nozich``. A local copy avoids an import cycle with the
    middleware which lives below ``nozich_cli``.
    """
    home = os.environ.get("NOZICH_HOME") or str(Path.home() / ".nozich")
