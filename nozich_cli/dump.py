"""
Dump command for nozich CLI.

Outputs a compact, plain-text summary of the user's Nozich setup
...
from nozich_cli.config import get_nozich_home, get_env_path, get_project_root, load_config
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.nozich_build_sha`` by the Dockerfile's
    ``NOZICH_GIT_SHA`` build-arg (see ``nozich_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``nozich dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from nozich_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(nozich_home: Path) -> int:
...
def _cron_summary(nozich_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = nozich_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"nozich_home:      {display_nozich_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # A credential added via `nozich auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `nozich auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["nozich-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(nozich_home)}")
    lines.append(f"  skills:             {_count_skills(nozich_home)}")
