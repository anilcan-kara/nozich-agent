"""
nozich fallback — manage the fallback provider chain.
...
Subcommands:
  nozich fallback [list]   Show the current fallback chain (default when no subcommand)
  nozich fallback add      Pick provider + model via the same picker as `nozich model`,
                           then append the selection to the chain
  nozich fallback remove   Pick an entry to delete from the chain
  nozich fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.nozich/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from nozich_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `nozich model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  nozich fallback add")
        print()
        return
    ...
    print("  Docs: https://nozich-agent.anilcan-kara.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `nozich model`, then append the selection to the chain."""
    from nozich_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `nozich model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `nozich fallback list` to view, or `nozich fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``nozich fallback [subcommand]``."""
