...
* We write two files: a shared ``gateway.cmd`` wrapper script (cwd + env + the
  actual ``python -m nozich_cli.main gateway run --replace`` invocation) and
...
_TASK_NAME_DEFAULT = "Nozich_Gateway"
_TASK_DESCRIPTION = "Nozich Agent Gateway - Messaging Platform Integration"
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current Nozich profile."""
    from nozich_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "nozich_cli.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def _launch_elevated_install(
    ...
    *,
    ...
    old_start_now = os.environ.get("NOZICH_GATEWAY_INSTALL_START_NOW")
    old_start_on_login = os.environ.get("NOZICH_GATEWAY_INSTALL_START_ON_LOGIN")
    old_handoff = os.environ.get("NOZICH_GATEWAY_ELEVATED_HANDOFF")
    try:
        if start_now is not None:
            os.environ["NOZICH_GATEWAY_INSTALL_START_NOW"] = "1" if start_now else "0"
        if start_on_login is not None:
            os.environ["NOZICH_GATEWAY_INSTALL_START_ON_LOGIN"] = "1" if start_on_login else "0"
        os.environ["NOZICH_GATEWAY_ELEVATED_HANDOFF"] = "1"
        ...
        for key, old in (
            ("NOZICH_GATEWAY_INSTALL_START_NOW", old_start_now),
            ("NOZICH_GATEWAY_INSTALL_START_ON_LOGIN", old_start_on_login),
            ("NOZICH_GATEWAY_ELEVATED_HANDOFF", old_handoff),
        ):
            if old is None:
...
def get_task_name() -> str:
    ...
    Default profile: ``Nozich_Gateway``
    Named profile X: ``Nozich_Gateway_<X>``
    """
    _assert_windows()
    # Local import to avoid circular module initialization during nozich_cli boot.
    from nozich_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    Lives under ``%LOCALAPPDATA%\\nozich\\gateway-service\\<task_name>.cmd``
    (or ``<NOZICH_HOME>/gateway-service/<task_name>.cmd`` so per-profile
    Nozich installs stay self-contained).
    """
    _assert_windows()
    from nozich_cli.config import get_nozich_home

    script_dir = Path(get_nozich_home()) / "gateway-service"
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    ...
    Mirror the POSIX service invariant: anchor at ``NOZICH_HOME`` whenever it
    ...
    source checkout only if ``NOZICH_HOME`` cannot be resolved yet.
    """
    from nozich_cli.config import get_nozich_home
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports NOZICH_HOME, PYTHONIOENCODING, VIRTUAL_ENV
      - invokes ``pythonw -m nozich_cli.main [--profile X] gateway run``
    ...
    lines.append(f'set "NOZICH_HOME={nozich_home}"')
    lines.append('set "PYTHONIOENCODING=utf-8"')
    lines.append('set "NOZICH_GATEWAY_DETACHED=1"')
    # VIRTUAL_ENV lets the gateway's own python detection find the venv
    # if someone imports nozich_constants-based logic during startup.
    venv_dir = str(Path(python_path).resolve().parent.parent)
    lines.append(f'set "VIRTUAL_ENV={venv_dir}"')

    pythonw_path = _derive_venv_pythonw(python_path)
    prog_args = [pythonw_path, "-m", "nozich_cli.main"]
    if profile_arg:
...
def _write_task_script() -> Path:
    ...
    nozich_home = str(Path(get_nozich_home()).resolve())
    profile_arg = _profile_arg(nozich_home)

    content = _build_gateway_cmd_script(python_path, working_dir, nozich_home, profile_arg)
...
def _install_scheduled_task(task_name: str, script_path: Path) -> tuple[bool, str]:
    ...
    Always recreate instead of ``/Change``. Older Nozich builds and failed
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    argv = [python_exe, "-m", "nozich_cli.main"]
    if profile_arg:
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m nozich_cli.main gateway run``
    ...
    log_dir = Path(get_nozich_home()) / "logs"
...
def _prompt_install_choices(
    ...
    env_start_now = _install_choice_from_env("NOZICH_GATEWAY_INSTALL_START_NOW")
    env_start_on_login = _install_choice_from_env("NOZICH_GATEWAY_INSTALL_START_ON_LOGIN")
    ...
    from nozich_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    # Re-running `nozich -p <profile> gateway install` must be safe.
    ...
    from nozich_cli.gateway import find_gateway_pids, _profile_arg
    ...
    if running_pids:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"nozich {profile_arg} gateway start" if profile_arg else "nozich gateway start"
    ...
    *,
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run later with: nozich gateway start")
        return
    ...
    if not _is_running_as_admin() and not elevated_handoff:
        ...
        if prompt_yes_no("  Open the UAC prompt now?", False):
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                print("✓ Launched elevated Nozich gateway install prompt.")
                if start_now:
                    print("  Approve the Windows UAC prompt; the elevated install will start the gateway afterwards.")
                else:
                    print("  Approve the Windows UAC prompt, then run: nozich gateway status")
                return
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: nozich gateway start")
        _print_next_steps()
        return
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from nozich_cli.config import get_nozich_home
        print(f"    type {Path(get_nozich_home()).resolve()}\\logs\\gateway.log")
        print(f"    type {Path(get_nozich_home()).resolve()}\\logs\\gateway-stdio.log")


def _print_next_steps() -> None:
    from nozich_cli.config import get_nozich_home

    nozich_home = Path(get_nozich_home()).resolve()
    print()
    print("Next steps:")
    print("  nozich gateway status                      # Check status")
    print(f"  type {nozich_home}\\logs\\gateway.log       # View logs")
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_uninstall():
                    print("✓ Launched elevated Nozich gateway uninstall prompt.")
                    print("  Approve the Windows UAC prompt, then run: nozich gateway status")
                    return
...
def _print_deep_probes() -> None:
    ...
    home = Path(get_nozich_home()).resolve()
...
def start() -> None:
    ...
    if not task_installed and not startup_installed:
        ...
        if not prompt_yes_no("  Install it now so the gateway starts on login?", True):
            print("  Run: nozich gateway install")
            return
        install(force=False)
        task_installed = is_task_registered()
        startup_installed = is_startup_entry_installed()
        if not task_installed and not startup_installed:
            ...
            print("  If a UAC prompt opened, approve it, then run: nozich gateway start")
            return
...
def stop() -> None:
    ...
    """
    _assert_windows()
    from nozich_cli.gateway import kill_gateway_processes, _get_restart_drain_timeout
    from gateway.status import get_running_pid
