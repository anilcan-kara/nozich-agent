"""
Nozich Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``nozich desktop`` / ``nozich gui``)
     Built inside the agent checkout under ``$NOZICH_HOME/nozich-agent/``:
       ...
       - ``$NOZICH_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Nozich.app`` or ``~/Applications/Nozich.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Nozich`` (NSIS per-user)
...
the app name ("Nozich"), separate from ``$NOZICH_HOME``:
  - macOS:   ``~/Library/Application Support/Nozich``
  - Windows: ``%APPDATA%\\Nozich``
  - Linux:   ``$XDG_CONFIG_HOME/Nozich`` (default ``~/.config/Nozich``)
...
``nozich uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(nozich_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return nozich_home / "nozich-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Nozich"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Nozich"
...
def source_built_gui_artifacts(nozich_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``nozich desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``nozich-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(nozich_home)
    ...
    return [
        ...
        nozich_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Nozich" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Nozich.app"),
            home / "Applications" / "Nozich.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Nozich).
            local_base / "Programs" / "Nozich",
            # Older / alternate layout some builds used.
            local_base / "nozich-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Nozich")
    else:
        ...
        paths += [
            data_base / "applications" / "nozich.desktop",
            data_base / "applications" / "Nozich.desktop",
        ]
    return paths


def agent_is_installed(nozich_home: Path) -> bool:
    """Return True when a usable Python agent install exists under NOZICH_HOME.
    ...
    if (agent_root / "nozich_cli").is_dir():
        return True
...
def gui_is_installed(nozich_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(nozich_home):
        if p.exists():
            return True
...
def gui_install_summary(nozich_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = nozich_home if nozich_home is not None else get_nozich_home()
...
def uninstall_gui(nozich_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``nozich-agent/nozich_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$NOZICH_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove nozich' or "
            "'sudo dnf remove nozich'). AppImage builds are a single file you "
