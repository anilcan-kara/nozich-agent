"""nozich hooks — inspect and manage shell-script hooks.

Usage::

    nozich hooks list
    nozich hooks test <event> [--for-tool X] [--payload-file F]
    nozich hooks revoke <command>
    nozich hooks doctor

Consent records live under ``~/.nozich/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.nozich/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``nozich hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: nozich hooks {list|test|revoke|doctor}")
        print("Run 'nozich hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.nozich/config.yaml.")
        print("See `nozich hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `nozich hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `nozich hooks test` and `nozich hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Nozich wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `nozich hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `nozich hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `nozich hooks doctor`.")
