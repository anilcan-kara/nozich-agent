"""``nozich logs`` — view and filter Nozich log files.
...
Usage examples::
    ...
    nozich logs                    # last 50 lines of agent.log
    nozich logs -f                 # follow agent.log in real time
    nozich logs errors             # last 50 lines of errors.log
    nozich logs gateway -n 100    # last 100 lines of gateway.log
    nozich logs gui -f            # follow gui.log (dashboard/pty/ws)
    nozich logs desktop -f        # follow desktop.log (Electron app boot/backend)
    nozich logs --level WARNING    # only WARNING+ lines
    nozich logs --session abc123   # filter by session ID substring
    nozich logs --component tools  # only tool-related lines
    nozich logs --since 1h         # lines from the last hour
    nozich logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_nozich_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Nozich runs — try 'nozich chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_nozich_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_nozich_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_nozich_home()}/logs/")
        return

    print(f"Log files in {display_nozich_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'nozich chat' to generate logs)")
