#!/usr/bin/env python3
"""
Nozich CLI - Main entry point.

Usage:
    nozich                     # Interactive chat (default)
    nozich chat                # Interactive chat
    nozich gateway             # Run gateway in foreground
    nozich gateway start       # Start gateway as service
    nozich gateway stop        # Stop gateway service
    nozich gateway status      # Show gateway status
    nozich gateway install     # Install gateway service
    nozich gateway uninstall   # Uninstall gateway service
    nozich setup               # Interactive setup wizard
    nozich logout              # Clear stored authentication
    nozich status              # Show status of all components
    nozich cron                # Manage cron jobs
    nozich cron list           # List cron jobs
    nozich cron status         # Check if cron scheduler is running
    nozich doctor              # Check configuration and dependencies
    nozich honcho setup                    # Configure Honcho AI memory integration
    nozich honcho status                   # Show Honcho config and connection status
    nozich honcho sessions                 # List directory → session name mappings
    nozich honcho map <name>               # Map current directory to a session name
    nozich honcho peer                     # Show peer names and dialectic settings
    nozich honcho peer --user NAME         # Set user peer name
    nozich honcho peer --ai NAME           # Set AI peer name
    nozich honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    nozich honcho mode                     # Show current memory mode
    nozich honcho mode [hybrid|honcho|local]  # Set memory mode
    nozich honcho tokens                   # Show token budget settings
    nozich honcho tokens --context N       # Set session.context() token cap
    nozich honcho tokens --dialectic N     # Set dialectic result char cap
    nozich honcho identity                 # Show AI peer identity representation
    nozich honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    nozich honcho migrate                  # Step-by-step migration guide: OpenClaw native → Nozich + Honcho
    nozich version             Show version
    nozich update              Update to latest version
    nozich uninstall           Uninstall Nozich Agent
    nozich acp                 Run as an ACP server for editor integration
    nozich sessions browse     Interactive session picker with search

    nozich claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: nozich_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``nozich_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``nozich update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``nozich_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, nozich crashes on import and the user can't run
# ``nozich update`` to recover.  Missing the bootstrap means UTF-8 stdio
...
def _set_process_title() -> None:
    """Set the process title to 'nozich' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``nozich tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``nozich.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("nozich")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"nozich", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"nozich")
        # Windows: the .exe name is already ``nozich.exe`` — nothing to do.
    except Exception:
        pass
...
# that run *before* nozich_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("NOZICH_HOME")
        if home:
            cfg_path = os.path.join(home, "config.yaml")
        else:
            cfg_path = os.path.join(os.path.expanduser("~"), ".nozich", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    ``--tui``/``NOZICH_TUI=1``, then ``display.interface`` in config.
    """
    if argv is None:
        argv = sys.argv[1:]
    if "--cli" in argv:
        return False
    if os.environ.get("NOZICH_TUI") == "1" or "--tui" in argv:
        return True
...
# `entry.tsx`; this is just the earlier cousin. ``NOZICH_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("NOZICH_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`nozich --tui … >log`, CI capture):
...
def _print_fast_version_info() -> None:
    from nozich_cli import __release_date__, __version__

    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir))
    print(f"Nozich Agent v{__version__} ({__release_date__})")
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``nozich --version`` before config/logging imports on Termux."""
    if os.environ.get("NOZICH_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
...
from nozich_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from nozich_cli.subcommands.cron import build_cron_parser
from nozich_cli.subcommands.gateway import build_gateway_parser
from nozich_cli.subcommands.profile import build_profile_parser
from nozich_cli.subcommands.model import build_model_parser
from nozich_cli.subcommands.setup import build_setup_parser
from nozich_cli.subcommands.postinstall import build_postinstall_parser
from nozich_cli.subcommands.whatsapp import build_whatsapp_parser
from nozich_cli.subcommands.slack import build_slack_parser
from nozich_cli.subcommands.login import build_login_parser
from nozich_cli.subcommands.logout import build_logout_parser
from nozich_cli.subcommands.auth import build_auth_parser
from nozich_cli.subcommands.status import build_status_parser
from nozich_cli.subcommands.webhook import build_webhook_parser
from nozich_cli.subcommands.hooks import build_hooks_parser
from nozich_cli.subcommands.doctor import build_doctor_parser
from nozich_cli.subcommands.security import build_security_parser
from nozich_cli.subcommands.dump import build_dump_parser
from nozich_cli.subcommands.debug import build_debug_parser
from nozich_cli.subcommands.backup import build_backup_parser
from nozich_cli.subcommands.import_cmd import build_import_cmd_parser
from nozich_cli.subcommands.config import build_config_parser
from nozich_cli.subcommands.version import build_version_parser
from nozich_cli.subcommands.update import build_update_parser
from nozich_cli.subcommands.uninstall import build_uninstall_parser
from nozich_cli.subcommands.dashboard import build_dashboard_parser
from nozich_cli.subcommands.gui import build_gui_parser
from nozich_cli.subcommands.logs import build_logs_parser
from nozich_cli.subcommands.prompt_size import build_prompt_size_parser
from nozich_cli.subcommands.memory import build_memory_parser
from nozich_cli.subcommands.acp import build_acp_parser
from nozich_cli.subcommands.tools import build_tools_parser
from nozich_cli.subcommands.insights import build_insights_parser
from nozich_cli.subcommands.skills import build_skills_parser
from nozich_cli.subcommands.pairing import build_pairing_parser
from nozich_cli.subcommands.plugins import build_plugins_parser
from nozich_cli.subcommands.mcp import build_mcp_parser
from nozich_cli.subcommands.claw import build_claw_parser


def _require_tty(command_name: str) -> None:
    """Exit with a clear error if stdin is not a terminal.

    Interactive TUI commands (nozich tools, nozich setup, nozich model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'nozich {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any nozich module import.
#
# Many modules cache NOZICH_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("NOZICH_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.nozich/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set NOZICH_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `nozich mcp add ... --args <command argv>`.
        ...
        ``--profile``), not to Nozich' own profile selector.
        """
        try:
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        """Resolve `sudo nozich -p <name>` against the invoking user's home.
        ...
        candidate = home / ".nozich" / "profiles" / name
        try:
            if candidate.is_dir():
    ...
    # after the subcommand (`nozich chat -p coder`), so keep scanning broadly.
    ...
    # Mirrors nozich_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If NOZICH_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.nozich/profiles/coder or
    # /opt/data/profiles/coder).  If NOZICH_HOME points to the nozich root
    # instead (e.g. systemd hardcodes NOZICH_HOME=/root/.nozich), we must
    # still read active_profile — the user may have switched profiles via
    # `nozich profile use` and the gateway should honour that choice.
    # See issue #22502.
    nozich_home_env = os.environ.get("NOZICH_HOME", "")
    if profile_name is None and nozich_home_env:
        if Path(nozich_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the nozich root.
    #
    # EXCEPTION: a supervised s6 gateway child (exported by the container
    # run-script as NOZICH_S6_SUPERVISED_CHILD=1) must NOT follow the sticky
    ...
    # reserved ``gateway-default`` slot runs bare ``nozich gateway run`` to mean
    # "the root NOZICH_HOME profile". If the reserved default child read
    ...
    if profile_name is None and not os.environ.get("NOZICH_S6_SUPERVISED_CHILD"):
        try:
            from nozich_constants import get_default_nozich_root

            active_path = get_default_nozich_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set NOZICH_HOME
    if profile_name is not None:
        try:
            from nozich_cli.profiles import resolve_profile_env

            nozich_home = resolve_profile_env(profile_name)
        except FileNotFoundError as exc:
            nozich_home = _resolve_sudo_user_profile_env(profile_name)
            if not nozich_home:
            ...
            # A bug in profiles.py must NEVER prevent nozich from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["NOZICH_HOME"] = nozich_home
...
# Load .env from ~/.nozich/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from nozich_cli.config import get_nozich_home
from nozich_cli.env_loader import load_nozich_dotenv

load_nozich_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → NOZICH_REDACT_SECRETS env
# var BEFORE nozich_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    _cfg_path = get_nozich_home() / "config.yaml"
    if _cfg_path.exists():
        with open(_cfg_path, encoding="utf-8") as _f:
            _early_cfg_raw = _yaml_early.safe_load(_f) or {}
        if "NOZICH_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["NOZICH_REDACT_SECRETS"] = str(_early_redact).lower()
...
# Initialize centralized file logging early — all `nozich` subcommands
...
try:
    from nozich_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from nozich_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if nozich_constants not importable yet
...
from nozich_cli import __version__, __release_date__

# Provider model-selection wizard flows extracted to nozich_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (nozich_cli.main._model_flow_*) keep resolving unchanged.
from nozich_cli.model_setup_flows import (
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_nozich_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("NOZICH_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("NOZICH_TERMUX_PREFETCH_UPDATES") == "1"
...
def _has_any_provider_configured() -> bool:
    ...
    from nozich_cli.config import get_env_path, get_nozich_home, load_config
    from nozich_cli.auth import get_auth_status

    # Determine whether Nozich itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from nozich_cli.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_nozich_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from nozich_cli.auth import PROVIDER_REGISTRY
    ...
    auth_file = get_nozich_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Nozich has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Nozich to use their tokens.
    if _has_nozich_config:
        try:
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, nozich_bin
        cli_args: the original CLI arguments (everything after 'nozich')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    nozich_bin = container_info["nozich_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo nozich {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo nozich {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  nozich --tui --resume {target}")
    if title:
        print(f'  nozich --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @nozich/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@nozich" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/nozich-ink/package.json",
    "packages/nozich-ink/index.js",
    "packages/nozich-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``NOZICH_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("NOZICH_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``NOZICH_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("NOZICH_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "NOZICH_HOME": nozich_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
    ...
    extras.extend([Path(nozich_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(nozich_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if nozich_cli_dir is None:
        nozich_cli_dir = Path(__file__).parent
    bundled = nozich_cli_dir / "tui_dist" / "entry.js"
    return bundled if bundled.is_file() else None


def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (NOZICH_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("NOZICH_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        path = shutil.which(bin)
        if not path and bin == "node":
            try:
                from nozich_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("NOZICH_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with NOZICH_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset NOZICH_TUI_DIR (e.g. `unset NOZICH_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if (
        ...
        npm = _node_bin("npm")
        if not os.environ.get("NOZICH_QUIET"):
    ...
    if tui_dev:
        # Keep the local @nozich/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @nozich/ink resolves through
        # packages/nozich-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "nozich-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from nozich_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    env = os.environ.copy()
    try:
        from nozich_cli.config import apply_terminal_config_to_env
        apply_terminal_config_to_env(env=env)
    except Exception:
        logger.debug("Failed to apply terminal config bridge for TUI launch", exc_info=True)
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="nozich-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["NOZICH_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["NOZICH_PYTHON_SRC_ROOT"] = os.environ.get(
        "NOZICH_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("NOZICH_PYTHON", sys.executable)
    env.setdefault("NOZICH_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["NOZICH_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["NOZICH_MODEL"] = model
        env["NOZICH_INFERENCE_MODEL"] = model
    if provider:
        env["NOZICH_TUI_PROVIDER"] = provider
        env["NOZICH_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["NOZICH_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["NOZICH_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["NOZICH_TUI_SKILLS"] = value
    if query:
        env["NOZICH_TUI_QUERY"] = query
    if image:
        env["NOZICH_TUI_IMAGE"] = image
    if checkpoints:
        env["NOZICH_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["NOZICH_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["NOZICH_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["NOZICH_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["NOZICH_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["NOZICH_ACCEPT_HOOKS"] = "1"
    ...
    # NOZICH_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `nozich --tui` try to
    ...
    # still set NOZICH_TUI_RESUME themselves.
    env.pop("NOZICH_TUI_RESUME", None)
    if resume_session_id:
        env["NOZICH_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `nozich update` so
    ...
    if code == 42:
        from nozich_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``NOZICH_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``nozich kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``nozich kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("NOZICH_KANBAN_BOARD"):
        return
    try:
        from nozich_cli.kanban_db import get_current_board

        os.environ["NOZICH_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.nozich/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Nozich — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Nozich still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    Precedence (highest first):
      ...
      2. ``--tui`` flag / ``NOZICH_TUI=1`` → always TUI
    ...
    if getattr(args, "cli", False):
        return False
    if getattr(args, "tui", False) or os.environ.get("NOZICH_TUI") == "1":
        return True
    try:
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'nozich sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        from nozich_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from nozich_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'nozich doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Nozich isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  nozich setup")
        print()

        from nozich_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'nozich setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from nozich_cli.banner import prefetch_update_check
    ...
    # setup (config, rules files, plugins, MCP servers) or from Nozich itself.
    # Implemented as a superset of --ignore-user-config + --ignore-rules plus
    # plugin/MCP discovery suppression (NOZICH_SAFE_MODE is checked by
    # nozich_cli/plugins.py and tools/mcp_tool.py).
    if getattr(args, "safe_mode", False):
        os.environ["NOZICH_SAFE_MODE"] = "1"
        os.environ["NOZICH_IGNORE_USER_CONFIG"] = "1"
        os.environ["NOZICH_IGNORE_RULES"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.nozich/config.yaml and return built-in defaults. Set BEFORE
    ...
    if getattr(args, "source", None):
        os.environ["NOZICH_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from nozich_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from nozich_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from nozich_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Nozich?")
        print()
    ...
    # has no creds.json.  Every subsequent `nozich gateway` then paid a 30s
    ...
    session_dir = get_nozich_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: nozich gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `nozich gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  nozich gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Nozich Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  nozich gateway")
        ...
        print()
        print("  Or install as a service: nozich gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'nozich whatsapp' to try again.")
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``nozich whatsapp`` (the Baileys bridge wizard) — the
    two adapters are complementary, not alternatives. See
    ``nozich_cli/setup_whatsapp_cloud.py``.
    """
    _require_tty("whatsapp-cloud")
    from nozich_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_setup(args):
    ...
    from nozich_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from nozich_cli.config import stamp_install_method
    from nozich_cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("⚕ Nozich post-install bootstrap")
    print()
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from nozich_cli.models import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``nozich model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from nozich_cli.auth import (
        resolve_provider,
        AuthError,
        format_auth_error,
    )
    from nozich_cli.config import (
        get_compatible_custom_providers,
        load_config,
        get_env_value,
    )
    from nozich_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("NOZICH_INFERENCE_PROVIDER") or "auto"
    )
    compatible_custom_providers = get_compatible_custom_providers(config)
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from nozich_cli.config import read_raw_config
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'nozich model' for "
                "available providers, or run 'nozich doctor' to diagnose config "
                "issues."
            )
    ...
    # PROVIDER_GROUPS in nozich_cli/models.py). A multi-member group shows one
    ...
    # a leftover OPENAI_BASE_URL in ~/.nozich/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.nozich/.env if the active provider is not 'custom'.
    ...
    """
    from nozich_cli.config import get_env_value, save_env_value, load_config
...
#
# Nozich uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `nozich model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `nozich model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`nozich_cli.plugins.PluginContext.register_auxiliary_task`) appear
...
def _save_aux_choice(
    ...
    *,
    ...
    """
    from nozich_cli.config import load_config, save_config
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Nozich only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``nozich model`` flow, then route aux tasks to them here.
    """
    from nozich_cli.config import load_config
    from nozich_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from nozich_cli.auth import _prompt_model_selection
    from nozich_cli.models import get_pricing_for_provider
...
def _aux_flow_custom_endpoint(task: str, task_cfg: dict) -> None:
    ...
    from nozich_cli.secret_prompt import masked_secret_prompt
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from nozich_cli.setup import _curses_prompt_choice
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from nozich_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Nozich URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _remove_custom_provider(config):
    ...
    try:
        from nozich_cli.curses_ui import curses_radiolist
...
# downstream call sites read `nozich_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `nozich` invocation — including
# fast paths like `nozich --version` and slash-command dispatch that never
...
def __getattr__(name):
    ...
    if name in _LAZY_MODEL_EXPORTS:
        from nozich_cli.models import _PROVIDER_MODELS
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``nozich setup`` / ``nozich model``.
    ...
    recover from a malformed paste without editing ``~/.nozich/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from nozich_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from nozich_cli.config import save_env_value
    from nozich_cli.secret_prompt import masked_secret_prompt
    ...
    from nozich_cli.env_loader import format_secret_source_suffix
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `nozich setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from nozich_constants import display_nozich_home as _dhh_fn

            print(
                f"    Nozich will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def cmd_login(args):
    """Authenticate Nozich CLI with a provider."""
    from nozich_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from nozich_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from nozich_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from nozich_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from nozich_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from nozich_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``nozich slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: nozich slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `nozich slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from nozich_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from nozich_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from nozich_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from nozich_cli.doctor import run_doctor

    run_doctor(args)


def cmd_security(args):
    """Dispatch `nozich security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from nozich_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from nozich_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from nozich_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from nozich_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Nozich home directory to a zip file."""
    if getattr(args, "quick", False):
        from nozich_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from nozich_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Nozich backup from a zip file."""
    from nozich_cli.backup import run_import

    run_import(args)


def _print_version_info(*, check_updates: bool = True) -> None:
    from nozich_cli.banner import format_banner_version_label
    ...
    try:
        from nozich_cli.banner import check_for_updates
        from nozich_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Nozich Agent (or just the Chat GUI with --gui)."""
    ...
    if getattr(args, "gui_summary", False):
        from nozich_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from nozich_cli.uninstall import run_gui_uninstall
    ...
    from nozich_cli.uninstall import run_uninstall
...
# Critical files that every ``nozich`` invocation imports at startup. If any
# of these fail to parse after a pull, the CLI is bricked — the user can't
# even run ``nozich update`` again to roll forward. The post-pull syntax
# guard validates these and auto-rolls-back on failure.
_UPDATE_CRITICAL_FILES = (
    "nozich_cli/main.py",
    "nozich_cli/config.py",
    "nozich_cli/__init__.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``nozich`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="nozich-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``nozich update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    still outputs to ``nozich_cli/web_dist/`` (per vite.config.ts
    outDir: "../nozich_cli/web_dist"), NOT to ``web/dist/``, so Python
    ...
    dist_dir = project_root / "nozich_cli" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``nozich`` launcher present but ``nozich_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the nozich venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — nozich venv python3, no nix-shell overhead
    ...
    # a nozich venv (pip / non-managed / bare-git installs).  The resolved
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``nozich update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``nozich web``).
    ...
    # build path usable on a stock `py -m nozich_cli.main web` invocation.
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (nozich web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#   - ``nozich update`` can unconditionally call ``nozich desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``nozich desktop`` (interactive launch) skips the build when the
#     stamp matches, making repeated launches fast
#
# Stamp file: $NOZICH_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $NOZICH_HOME."""
    from nozich_constants import get_nozich_home
    return get_nozich_home() / "desktop-build-stamp.json"
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``nozich update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Nozich.app/Contents/MacOS/Nozich"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Nozich.exe",
            release_dir / "win-ia32-unpacked" / "Nozich.exe",
            release_dir / "win-arm64-unpacked" / "Nozich.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "nozich",
            release_dir / "linux-unpacked" / "Nozich",
            release_dir / "linux-arm64-unpacked" / "nozich",
            release_dir / "linux-arm64-unpacked" / "Nozich",
        ]
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Nozich'`` desktop build failure: a corrupt zip in the
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``Nozich.exe`` keeps an exclusive lock on
    ``release/win-unpacked/Nozich.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\Nozich.exe: Access is
    ...
    an unrelated "Nozich" process is never touched. Best-effort: never raises.
...
def _desktop_macos_relaunchable_fixup(desktop_dir: Path) -> None:
    ...
    tampering and macOS reports "Nozich is damaged and can't be opened." The
    ...
    if exe is None:
        return
    # exe = .../Nozich.app/Contents/MacOS/Nozich  ->  app bundle = .../Nozich.app
    app = exe.parents[2]
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Nozich Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Nozich Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from nozich_logging import setup_logging as _setup_logging_gui
    ...
    if getattr(args, "fake_boot", False):
        env["NOZICH_DESKTOP_BOOT_FAKE"] = "1"
    if getattr(args, "ignore_existing", False):
        env["NOZICH_DESKTOP_IGNORE_EXISTING"] = "1"
    if getattr(args, "nozich_root", None):
        env["NOZICH_DESKTOP_NOZICH_ROOT"] = str(Path(args.nozich_root).expanduser().resolve())
    if getattr(args, "cwd", None):
        env["NOZICH_DESKTOP_CWD"] = str(Path(args.cwd).expanduser().resolve())
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  nozich gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            if not source_mode:
                ...
                # holds Nozich.exe locked on Windows, so the pack can't replace
            ...
            if build_result.returncode != 0 and not source_mode:
                ...
                # on the final `electron` -> `Nozich` rename: unpack-electron
                ...
                if purged or restored:
                    ...
                    for p in purged:
                        print(f"    - {p}")
                    # The purge can't remove a win-unpacked tree whose Nozich.exe
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == "win32":
                    print("  If this says \"Access is denied\" on Nozich.exe, close any")
                    print("  running Nozich desktop window and retry.")
                print("  If the log shows Electron download retries, rebuild via a mirror:")
                print("    ELECTRON_MIRROR=<mirror-base-url> nozich desktop --force-build")
                sys.exit(build_result.returncode or 1)
            packaged_executable = _desktop_packaged_executable(desktop_dir)
            if not source_mode:
                # Locally-built apps are ad-hoc signed; make them relaunchable after
                # an in-place self-update (otherwise macOS reports "Nozich is
    ...
    if source_mode:
        print("→ Launching Nozich Desktop from source build...")
    ...
    print(f"→ Launching packaged Nozich Desktop: {packaged_executable}")
...
def _find_stale_dashboard_pids(
    *,
    ...
    """Return PIDs of ``nozich dashboard`` processes other than ourselves.

    ``nozich dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``nozich update`` replaces files on disk, the running
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Nozich Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``nozich dashboard`` as
    ...
    environment variable ``NOZICH_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        "nozich dashboard",
        "nozich_cli.main dashboard",
        "nozich_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "nozich.*dashboard"`) keeps us consistent
            # with `nozich_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `nozich update`.
    ...
    print("  Preview now:  nozich curator run --dry-run")
    print("  Pause it:     nozich curator pause")
    print(
        "  Docs:         https://nozich-agent.anilcan-kara.com/docs/user-guide/features/curator"
    )
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``nozich update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``nozich update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: nozich curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``nozich dashboard`` processes.

    Called at the end of ``nozich update`` (default ``reason``) and also
    from ``nozich dashboard --stop`` (which overrides ``reason``).  The
    ...
    """
    # When the Nozich Desktop Electron app spawns this dashboard as a
    # backend child, it sets NOZICH_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("NOZICH_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    if killed:
        ...
        print("    nozich dashboard --port <port>")
...
def _update_via_zip(args):
    """Update Nozich Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `nozich update "
            f"--branch {branch}`, or update against main with `nozich update`."
        )
        sys.exit(1)
    zip_url = (
        f"https://github.com/anilcan-kara/nozich-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="nozich-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"nozich-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for nozich-agent itself
        ...
        # GitHub ZIPs extract to nozich-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"nozich-agent-{branch}")
    ...
    from nozich_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    try:
        from nozich_cli.model_catalog import seed_cache_from_checkout
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "nozich-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Nozich behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes nozich completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Nozich couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Nozich couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Nozich behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Nozich couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Nozich couldn't drop "
...
# =========================================================================
# Fork detection and upstream management for `nozich update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/anilcan-kara/nozich-agent.git",
    "git@github.com:anilcan-kara/nozich-agent.git",
    "https://github.com/anilcan-kara/nozich-agent",
    "git@github.com:anilcan-kara/nozich-agent",
}
OFFICIAL_REPO_URL = "https://github.com/anilcan-kara/nozich-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_nozich_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_nozich_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Nozich repository.")
        print("  This means you may miss updates from anilcan-kara/nozich-agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/anilcan-kara/nozich-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/anilcan-kara/nozich-agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``nozich update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from nozich_constants import get_default_nozich_root

    default_home = get_default_nozich_root()
...
# Install-scoped breadcrumb dropped right before ``nozich update`` mutates the
...
# survives and the next ``nozich`` launch finishes the install instead of
# limping along on a half-built venv (e.g. pip wiped, a core dep like Pillow
# never landed).  Lives next to the venv (not under $NOZICH_HOME) because the
...
def _recover_from_interrupted_install() -> None:
    """Finish a dependency install that a prior ``nozich update`` left half-done.
    ...
    protocol stream (``nozich acp`` speaks JSON-RPC on stdout) must never get
    ...
    try:
        ...
        print(
            "⚠ A previous `nozich update` was interrupted mid-install — "
            "finishing dependency installation now..."
        )

        try:
            from nozich_cli.managed_uv import ensure_uv
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``nozich update`` is still progressing even if pip/uv itself is silent.
    """
...
def _nozich_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [
        scripts_dir / "nozich.exe",
        scripts_dir / "nozich-gateway.exe",
    ]


def _detect_concurrent_nozich_instances(
    ...
    The Nozich Desktop Electron app spawns ``nozich.EXE`` as a backend child,
    so during ``nozich update`` the user-invoked process and the desktop's
    ...
    shims (``nozich.exe`` / ``nozich-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``nozich update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``nozich.exe``) is a separate process from the Python
    ...
    for shim in _nozich_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated nozich.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``nozich update``
    ...
    #      nozich.exe sitting *under* a non-Nozich parent (e.g. a Nozich
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "nozich.exe"
    lines = ["✗ Another nozich.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Nozich Desktop, exit any open `nozich` REPLs, and")
    lines.append("  stop the gateway (`nozich gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `nozich update --force` if you've already")
    lines.append("  confirmed those processes will not write to the venv.")
    return "\n".join(lines)


def _quarantine_running_nozich_exe(
    scripts_dir: Path, *, max_attempts: int = 4
) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``nozich.exe``.
    ...
    when ``nozich update`` runs, ``nozich.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``nozich.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next nozich invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Nozich Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Nozich Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _nozich_exe_shims(scripts_dir):
        ...
        print(
            "    Close Nozich Desktop, exit other `nozich` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `nozich update`."
        )
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_nozich_exe`` if uv didn't write replacements."""
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``nozich.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``nozich.exe`` is the running process —
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``nozich`` drops off PATH. This wraps
    ...
    if scripts_dir is not None:
        moved = _quarantine_running_nozich_exe(scripts_dir)
    try:
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``nozich.exe.old.*`` left by prior updates.

    Called early on every nozich invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``nozich update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `nozich update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``nozich.exe`` / ``nozich-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_nozich_exe`` for the rationale.
    """
    ...
    # hours later inside ``nozich update``'s desktop-rebuild or skill-sync
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``nozich update``, not the venv we just wrote
    ...
    #
    # Quarantine the running ``nozich.exe`` first: ``--reinstall -e .``
    # rewrites the entry-point shims, and on Windows pip can't overwrite the
    # live launcher, which would leave ``nozich`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `nozich update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `nozich update --force` after closing other nozich processes."
        )
        return
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from nozich_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$NOZICH_HOME/bin/uv``, but on Termux the official
    installer may not work (glibc vs bionic).  Fall back to ``pip install uv``
    which gets a Termux-compatible binary.
    """
    from nozich_cli.managed_uv import resolve_uv
...
def _update_node_dependencies() -> None:
    ...
    # need desktop during `nozich update`, so we install root-only first
...
class _UpdateOutputStream:
    """Stream wrapper used during ``nozich update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.nozich/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``nozich update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``nozich update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.nozich/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``nozich update --gateway``) the update is already
    ...
    try:
        ...
        # nozich_cli.config.get_nozich_home to simulate setup failure.
        from nozich_cli.config import get_nozich_home as _get_nozich_home

        logs_dir = _get_nozich_home() / "logs"
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``nozich update --check``: fetch and report without installing.
    ...
    dropping the flag.
    """
    from nozich_cli.config import detect_install_method
    method = detect_install_method(PROJECT_ROOT)
    if method == "docker":
        # Docker can't ``git fetch`` from within the container.  Surface the
        # same long-form ``docker pull`` guidance ``nozich update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from nozich_cli.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from nozich_cli.config import recommended_update_command
        from nozich_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``nozich update`` without requiring a reinstall.  The
    ...
    ``nozich`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/nozich.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v nozich'`` already resolves.  Idempotent.
    """
    if sys.platform != "linux":
        return
    try:
    ...
    # /usr/local/bin/nozich, code at /usr/local/lib/nozich-agent).
    fhs_link = Path("/usr/local/bin/nozich")
    ...
    path_comment = (
        "# Nozich Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of NOZICH_HOME before running the update.
    ...
    NOZICH_HOME directories.  The ``--backup`` flag on ``nozich update``
    ...
    try:
        from nozich_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_nozich_home so the user sees ~/.nozich/...
    try:
        from nozich_constants import get_nozich_home, display_nozich_home

        home = get_nozich_home()
        try:
            display_path = f"{display_nozich_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  nozich import {out_path}")
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    nozich.exe concurrent-instance guard does not see them. They still import
    ...
    try:
        ...
        from nozich_cli.gateway import (
    ...
    print("→ Stopping Windows gateway process(es) before updating Nozich...")
    try:
    ...
    if unmapped_pids:
        ...
        print("    Restart manually after update: nozich gateway run")
...
def _resume_windows_gateways_after_update(token: dict | None) -> None:
    ...
    try:
        from nozich_cli.gateway import launch_detached_profile_gateway_restart
    except Exception as exc:
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``nozich update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    """Update Nozich Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Nozich Agent")
        return
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `nozich update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_pip(args):
    """Update Nozich via pip (for PyPI installs)."""
    from nozich_cli import __version__
    from nozich_cli.config import is_uv_tool_install
    ...
    if is_uv_tool_install():
        if not uv:
            ...
            sys.exit(1)
        cmd = [uv, "tool", "upgrade", "nozich-agent"]
        ...
        cmd = [pipx, "upgrade", "nozich-agent"]
    elif uv:
        cmd = [uv, "pip", "install", "--upgrade", "nozich-agent"]
        if in_venv:
        ...
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "nozich-agent"]
    ...
    print("✓ Update complete! Restart nozich to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚕ Updating Nozich Agent...")
    print()

    # On Windows, abort early if another nozich.exe is holding the venv shim
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_nozich_instances(scripts_dir)
            if concurrent:
    ...
    try:
        ...
        # though `git pull` can't touch $NOZICH_HOME, this is cheap
        ...
        try:
            from nozich_cli.backup import create_quick_snapshot
        ...
        # orphan merge-conflict markers in nozich_cli/config.py bricked
        # every user who ran ``nozich update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``nozich
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``nozich update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_nozich_home added to nozich_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # the marker survives and the next ``nozich`` launch finishes the
        ...
        # build.  ``nozich desktop --build-only`` uses the content-hash stamp
        ...
        # never run ``nozich desktop`` shouldn't be forced into a full
        # Electron build by ``nozich update``.
        desktop_dir = PROJECT_ROOT / "apps" / "desktop"
        has_desktop_app = _desktop_packaged_executable(desktop_dir) is not None or _desktop_dist_exists(desktop_dir)
        if (desktop_dir / "package.json").exists() and shutil.which("npm") and has_desktop_app:
            print("→ Checking if desktop app needs rebuilding...")
            _desktop_build_cmd = [sys.executable, "-m", "nozich_cli.main", "desktop", "--build-only"]
            ...
            if build_result.returncode != 0:
                build_result = subprocess.run(_desktop_build_cmd, cwd=PROJECT_ROOT, check=False)
            if build_result.returncode != 0:
                print("  ⚠ Desktop build failed (non-fatal; run `nozich desktop` to retry)")
        ...
        # current — so copy it straight over ~/.nozich/cache/model_catalog.json
        ...
        # modules in this process.  Reload nozich_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_nozich_home() added since the last release.
        try:
            import importlib
            import nozich_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit NOZICH_HOME so
        # it is not affected by sync_skills()'s module-level NOZICH_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's NOZICH_HOME env var points at the default or a named profile.
        try:
            from nozich_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        try:
            from nozich_cli.profiles import backfill_profile_envs
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'nozich config migrate' to retry.")
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: nozich config migrate")
            else:
                print()
                print("Skipped. Run 'nozich config migrate' later to configure.")
        else:
        ...
        try:
            from nozich_cli.backup import restore_cron_jobs_if_emptied
        ...
        # check `nozich curator status`. Self-stamps after printing so it
        ...
        # Tying the refresh to ``nozich update`` gives users a predictable
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from nozich_cli.tools_config import install_cua_driver
        ...
        # When running as ``nozich update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_nozich_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... nozich-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... nozich-gateway*)
            ...
            # Discover all nozich-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. nozich-gateway.service or nozich-gateway-coder.service
                            ...
                            if _manage_cmd is None:
                                print(
                                    ...
                                    f"    To let `nozich update` restart it automatically, allow\n"
                            ...
                            # path in `nozich gateway restart`
                        ...
                        print(
                            ...
                            f"Check the gateway with: nozich gateway status"
                        )
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: nozich gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: nozich -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Nozich gateway unit files are still installed.
        # When both nozich.service (from a pre-rename install) and the
        # current nozich-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `nozich update` surfaces the issue until the user migrates.
        try:
            from nozich_cli.gateway import (
                has_legacy_nozich_units,
                _find_legacy_nozich_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_nozich_units():
                print()
                print("⚠ Legacy Nozich gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_nozich_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (nozich.service) fight the current")
                print("  nozich-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    nozich gateway migrate-legacy")
                print()
        ...
        print("  nozich model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``nozich -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from nozich_constants import display_nozich_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `nozich profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_nozich_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    ...
                    print(f"Alias:          {alias_display} → nozich -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.nozich)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  nozich profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     nozich -p {name} chat")
                else:
        ...
        from nozich_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from nozich_constants import get_nozich_home as _hh
        ...
        from nozich_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `nozich profile info {name}` for full manifest)")
        if alias_name:
            is_windows = sys.platform == "win32"
            wrapper = _get_wrapper_dir() / (f"{alias_name}.bat" if is_windows else alias_name)
            print(f"Alias:   {alias_name} → nozich -p {name}  ({wrapper})")
        print()
        ...
        from nozich_cli.profiles import profile_exists
        ...
        from nozich_cli.profiles import rename_profile
        ...
        from nozich_cli.profiles import export_profile
        ...
        from nozich_cli.profiles import import_profile
        ...
        import tempfile
        from nozich_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="nozich_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  nozich -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      nozich -p {plan.manifest.name} chat")
        ...
        from nozich_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `nozich profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"nozich -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from nozich_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("nozich_requires"):
            print(f"Requires:     Nozich {data['nozich_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from nozich_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.nozich_requires:
        print(f"  Requires: Nozich {mf.nozich_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``nozich dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``nozich dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No nozich dashboard processes running.")
        return 0

    print(f"{len(pids)} nozich dashboard process(es) running:")
    for pid in pids:
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No nozich dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `nozich update`.
    ...
    # launches the dashboard (`worker dashboard` → NOZICH_HOME points into
    ...
    if (
        ...
        and os.environ.get("NOZICH_DESKTOP") != "1"
    ):
        ...
        reexec_argv = [
            sys.executable, "-m", "nozich_cli.main",
            "-p", "default",
            "dashboard",
            "--port", str(args.port),
            "--host", args.host,
        ...
        # NOZICH_HOME.  We must resolve the root explicitly instead of just
        # dropping NOZICH_HOME: in the Docker layout the machine root is
        # /opt/data (set via `ENV NOZICH_HOME=/opt/data`), so an unset
        # NOZICH_HOME falls back to $HOME/.nozich = /opt/data/.nozich — an
        ...
        # update-button guard also misfires).  get_default_nozich_root()
        # returns the root for both layouts: ~/.nozich for a standard install
        ...
        try:
            from nozich_constants import get_default_nozich_root
            env["NOZICH_HOME"] = str(get_default_nozich_root())
        except Exception:
            # Best-effort: if root resolution fails, fall back to the prior
            # behaviour (drop NOZICH_HOME) rather than block the reroute.
            env.pop("NOZICH_HOME", None)
    ...
    # the same logs directory as every other Nozich surface.
    try:
        from nozich_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
    ...
    if "NOZICH_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["NOZICH_WEB_DIST"])
            if "NOZICH_WEB_DIST" in os.environ
            else PROJECT_ROOT / "nozich_cli" / "web_dist"
        )
    ...
    try:
        from nozich_cli.mcp_startup import start_background_mcp_discovery
    ...
    from nozich_cli.web_server import start_server
...
def cmd_dashboard_register(args):
    ...
    from nozich_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_completion(args, parser=None):
    ...
    from nozich_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from nozich_cli.prompt_size import cmd_prompt_size as _impl

    _impl(args)


def cmd_logs(args):
    """View and filter Nozich log files."""
    from nozich_cli.logs import tail_log, list_logs
...
# so that in ``nozich -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``nozich_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``nozich -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``nozich`` or only flags → defaults to ``chat``.
        return False
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("NOZICH_TUI") == "1")
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    from nozich_cli._parser import build_top_level_parser
    ...
    if getattr(args, "oneshot", None):
        ...
        from nozich_cli.oneshot import run_oneshot
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["NOZICH_DEFER_AGENT_STARTUP"] = "1"
            os.environ["NOZICH_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["NOZICH_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `nozich --tui` is the hot path on phones. The full parser setup imports
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        mem_dir = get_nozich_home() / "memories"
        ...
        if not existing:
            print(
                f"\n  Nothing to reset — no memory files found in {display_nozich_home()}/memories/\n"
            )
            return
        ...
        print(f"  Files were in: {display_nozich_home()}/memories/\n")
    else:
        from nozich_cli.memory_setup import memory_command

        memory_command(args)


def cmd_acp(args):
    """Launch Nozich Agent as an ACP server."""
    try:
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from nozich_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from nozich_cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from nozich_cli.tools_config import tools_command
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from nozich_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    else:
        from nozich_cli.skills_hub import skills_command

        skills_command(args)


def cmd_pairing(args):
    from nozich_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from nozich_cli.plugins_cmd import plugins_command

    plugins_command(args)


def cmd_mcp(args):
    from nozich_cli.mcp_config import mcp_command

    mcp_command(args)


def cmd_claw(args):
    from nozich_cli.claw import claw_command

    claw_command(args)


def main():
    """Main entry point for nozich CLI."""
    # Cosmetic: make the process show up as 'nozich' instead of 'python3.11'
    ...
    # Sweep stale ``nozich.exe.old.*`` quarantine files left by previous
    # ``nozich update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_nozich_exe``.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass

    # Self-heal a venv left half-built by an interrupted ``nozich update``
    ...
    # ``nozich skills install update``) merely defers recovery one launch;
    # under-matching (missing ``nozich -p work update``) would race a recovery
    ...
    # =========================================================================
    # model command  (parser built in nozich_cli/subcommands/model.py)
    # =========================================================================
    ...
    # =========================================================================
    from nozich_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://nozich-agent.anilcan-kara.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `nozich model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.nozich/.env.  Currently supports "
            "Bitwarden Secrets Manager.  See: "
            "https://nozich-agent.anilcan-kara.com/docs/user-guide/secrets/bitwarden"
        ),
    )
    ...
    from nozich_cli import secrets_cli as _secrets_cli
    ...
    # migrate command
    # =========================================================================
    from nozich_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in nozich_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in nozich_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)

    # =========================================================================
    # postinstall command  (parser built in nozich_cli/subcommands/postinstall.py)
    # =========================================================================
    build_postinstall_parser(subparsers, cmd_postinstall=cmd_postinstall)

    # =========================================================================
    # whatsapp command  (parser built in nozich_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    whatsapp_cloud_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "required). Distinct from `nozich whatsapp` which sets up "
    ...
    # =========================================================================
    # slack command  (parser built in nozich_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    from nozich_cli.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in nozich_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in nozich_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in nozich_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in nozich_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # cron command  (parser built in nozich_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)

    # =========================================================================
    # webhook command  (parser built in nozich_cli/subcommands/webhook.py)
    # =========================================================================
    ...
    # =========================================================================
    from nozich_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from nozich_cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    # hooks command  (parser built in nozich_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in nozich_cli/subcommands/doctor.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in nozich_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # dump command  (parser built in nozich_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in nozich_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in nozich_cli/subcommands/backup.py)
    # =========================================================================
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.nozich/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo nozich uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from nozich_cli.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in nozich_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # config command  (parser built in nozich_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # pairing command  (parser built in nozich_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in nozich_cli/subcommands/skills.py)
    # =========================================================================
    ...
    from nozich_cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in nozich_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    # subcommand — ``nozich --help``, ``nozich version``, ``nozich logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from nozich_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from nozich_cli.curator import register_cli as _register_curator_cli
    ...
    # =========================================================================
    # memory command  (parser built in nozich_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in nozich_cli/subcommands/tools.py)
    # =========================================================================
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use `nozich computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `nozich tools` runs when you first\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                ...
                print("  Refresh to latest: nozich computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: nozich computer-use install")
            return
    ...
    # =========================================================================
    # mcp command  (parser built in nozich_cli/subcommands/mcp.py)
    # =========================================================================
        ...
        if action == "list":
            ...
            if not args.yes:
                if not _confirm_prompt(
                    ...
                    print("Cancelled.")
                    return
            sessions_dir = get_nozich_home() / "sessions"
            ...
            # Launch nozich --resume <id> by replacing the current process
    ...
    # =========================================================================
    # insights command  (parser built in nozich_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)

    # =========================================================================
    # claw command  (parser built in nozich_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # =========================================================================
    # version command  (parser built in nozich_cli/subcommands/version.py)
    # =========================================================================
    build_version_parser(subparsers, cmd_version=cmd_version)

    # =========================================================================
    # update command  (parser built in nozich_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in nozich_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in nozich_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in nozich_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in nozich_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # for one release. The Nozich-Setup.exe success screen tells users to
    # run `nozich desktop` from a terminal, so the canonical name needs
    # to be the one that appears in --help (argparse promotes the primary
    # name; aliases stay hidden).
    # =========================================================================
    # gui command  (parser built in nozich_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in nozich_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in nozich_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    # e.g. ``nozich -c Pokemon Agent Dev`` → ``nozich -c 'Pokemon Agent Dev'``
    ...
    from nozich_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'nozich -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (nozich hooks list, cron
