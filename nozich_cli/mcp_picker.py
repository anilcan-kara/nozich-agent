"""MCP picker — interactive `nozich mcp picker` (also the default `nozich mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``nozich mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `nozich plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from nozich_cli.cli_output import prompt_yes_no
from nozich_cli.curses_ui import curses_single_select
from nozich_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Nozich session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from nozich_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `nozich mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: nozich mcp install <name>    Picker: nozich mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Nozich is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Nozich — run `nozich update` "
...
def show_catalog() -> None:
    """`nozich mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`nozich mcp picker` (and default `nozich mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`nozich mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `nozich mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
