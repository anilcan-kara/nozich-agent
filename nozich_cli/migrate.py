"""CLI handlers for ``nozich migrate ...``.

Currently exposes only ``nozich migrate xai`` — diagnoses and (with --apply)
...
def cmd_migrate(args: Any) -> int:
    """Dispatcher for ``nozich migrate <subtype>``."""
    sub = getattr(args, "migrate_type", None)
    if sub == "xai":
        return cmd_migrate_xai(args)

    print("usage: nozich migrate xai [--apply] [--no-backup]", file=sys.stderr)
    return 2
...
def cmd_migrate_xai(args: Any) -> int:
    ...
    if not apply:
        ...
        print(color(
            "Re-run with `nozich migrate xai --apply` to rewrite "
    ...
    print()
    print(color(
        "Run `nozich doctor` to confirm no retired xAI models remain.",
        Colors.DIM,
    ))
    return 0
