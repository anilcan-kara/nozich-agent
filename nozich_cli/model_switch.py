...
- ``nozich_cli.providers``        -- canonical provider identity + overlays
- ``nozich_cli.model_normalize``  -- per-provider name formatting
...
_NOZICH_MODEL_WARNING = (
    "Nous Research Nozich 3 & 4 models are NOT agentic and are not designed "
    "for use with Nozich Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research Nozich 3 / Nozich 4 chat families.
# The previous substring check (`"nozich" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``nozich-brain:qwen3-14b-ctx16k`` that just
# happen to carry "nozich" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   anilcan-kara/Nozich-3-Llama-3.1-70B, nozich-4-405b, openrouter/nozich3:70b
# Negative examples it must NOT match:
#   nozich-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_NOZICH_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])nozich[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_nous_nozich_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous Nozich 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_NOZICH_NON_AGENTIC_RE.search(model_name))


def _check_nozich_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Nozich 3/4 chat model."""
    if is_nous_nozich_non_agentic(model_name):
        return _NOZICH_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``nozich config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from nozich_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'nozich model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'nozich doctor' — config issues detected:"
        ...
        from nozich_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from nozich_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from nozich_cli.providers import resolve_user_provider as _ruser
    ...
    # Mirrors the same logic in nozich_cli.runtime_provider.resolve_runtime_provider;
    ...
    nozich_warn = _check_nozich_model_warning(new_model)
    if nozich_warn:
        warnings.append(nozich_warn)
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from nozich_cli.inventory import load_picker_context
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from nozich_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from nozich_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by nozich provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://nozich-agent.anilcan-kara.com/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Nozich release. Falls back to the in-repo
    # _PROVIDER_MODELS["nous"] snapshot when the manifest is unreachable.
    curated["nous"] = get_curated_nous_model_ids()
    # Ollama Cloud uses dynamic discovery (no static curated list)
    if "ollama-cloud" not in curated:
        from nozich_cli.models import fetch_ollama_cloud_models
    ...
    if "lmstudio" not in curated and (
        ...
        from nozich_cli.models import fetch_lmstudio_models
        from nozich_cli.auth import AuthError
    ...
    # --- 1. Check Nozich-mapped providers ---
    from nozich_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from nozich_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for nozich_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        _alias_target = _PROVIDER_ALIAS_TABLE.get(nozich_id)
        if (
            _alias_target
            and _alias_target != nozich_id
        ...
        pconfig = PROVIDER_REGISTRY.get(nozich_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (NOZICH_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from nozich_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and store.get("credential_pool", {}).get(nozich_id):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # /model picker sees the SAME list `nozich model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(nozich_id)
        if not model_ids:
            model_ids = curated.get(nozich_id, [])
            if nozich_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(nozich_id, model_ids)
        total = len(model_ids)
    ...
    # --- 2. Check Nozich-only providers (nous, openai-codex, copilot, opencode-go) ---
    from nozich_cli.providers import NOZICH_OVERLAYS
    from nozich_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Nozich provider ID.
    # NOZICH_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Nozich IDs ("copilot").
    _mdev_to_nozich = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in NOZICH_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Nozich slug — e.g. "github-copilot" → "copilot"
        nozich_slug = _mdev_to_nozich.get(pid, pid)
        if nozich_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(nozich_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, nozich_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or nozich_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(nozich_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", nozich_slug, exc)
        ...
        # configured.
        if not has_creds and nozich_slug == "anthropic":
            try:
                ...
                if (nozich_creds and nozich_creds.get("accessToken")) or \
        ...
        if nozich_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(nozich_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(nozich_slug)
                model_ids = _ids if _ids else (curated.get(nozich_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(nozich_slug, []) or curated.get(pid, [])
        elif nozich_slug == "nous":
            # Nous serves a large live /v1/models catalog (vendor-prefixed
            # models from many providers, returned alphabetically). The
            # `nozich model` picker deliberately shows ONLY the curated agentic
            ...
            try:
                ...
                from nozich_cli.auth import get_provider_auth_state as _nous_state
            ...
            if not model_ids:
                model_ids = curated.get(nozich_slug, []) or curated.get(pid, [])
                if nozich_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(nozich_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": nozich_slug,
            "name": get_label(nozich_slug),
            "is_current": nozich_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "nozich",
        ...
        seen_slugs.add(nozich_slug.lower())
        _record_builtin_endpoint(nozich_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or NOZICH_OVERLAYS (keeps /model in sync
    # with `nozich model`).
    try:
        from nozich_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Nozich's canonical write key (matches
            ...
            # Nozich writes ``models:`` as a dict keyed by model id
            # (see nozich_cli/main.py::_save_custom_provider); older
            ...
            if should_probe:
                try:
                    from nozich_cli.models import fetch_api_models
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Nozich's own writer uses; a hyphen variant is accepted
            ...
            # active model. Nozich's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, nozich_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`nozich_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from nozich_cli.models import fetch_openrouter_models
