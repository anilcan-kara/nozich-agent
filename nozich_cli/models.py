...
Add, remove, or reorder entries here — both `nozich setup` and
`nozich` provider-selection will pick up the change automatically.
"""
...
from nozich_cli import __version__ as _NOZICH_VERSION

# Identify ourselves so endpoints fronted by Cloudflare's Browser Integrity
# Check (error 1010) don't reject the default ``Python-urllib/*`` signature.
_NOZICH_USER_AGENT = f"nozich-cli/{_NOZICH_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `nozich model`
    flow without maintaining a separate static list.
    """
    from nozich_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $NOZICH_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $NOZICH_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. Nozich refreshes the cache from
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older Nozich that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older Nozich that doesn't ship
...
def check_nous_free_tier(*, force_fresh: bool = False) -> bool:
    ...
    try:
        from nozich_cli.nous_account import get_nous_portal_account_info
...
def _nous_recommended_disk_path() -> "Path":
    ...
    return get_nozich_home() / "cache" / "nous_recommended_cache.json"
...
def fetch_nous_recommended_models(
    ...
    *,
    ...
    (``$NOZICH_HOME/cache/nous_recommended_cache.json``) as last-known-good.
    ...
    """
    base = (portal_base_url or "https://portal.anilcan-kara.com").rstrip("/")
    now = time.monotonic()
...
def _resolve_nous_portal_url() -> str:
    ...
    try:
        ...
        return "https://portal.anilcan-kara.com"
...
# this list:  nozich model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `nozich model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `nozich model` TUI
...
#
# Some vendors expose several Nozich provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `nozich model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``nozich model``, the
...
# path. The *interactive* default (GUI onboarding / ``nozich model``) uses the
# richer free/paid-tier-aware resolver — see ``get_recommended_default_model``
# in nozich_cli/web_server.py and ``partition_nous_models_by_tier`` — which can
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``nozich auth add openai-codex`` without
    ``nozich model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``nozich model`` picker offers first. For metered aggregators
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    nozich-agent is tool-calling-first — every provider path assumes the model
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from nozich_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — nozich-agent
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from nozich_cli.model_catalog import get_curated_nous_models
...
_DEFAULT_NOUS_INFERENCE_BASE = "https://inference-api.anilcan-kara.com"
...
def _resolve_nous_pricing_credentials() -> tuple[str, str]:
    ...
    """
    try:
        from nozich_cli.auth import resolve_nous_runtime_credentials
...
def get_pricing_for_provider(provider: str, *, force_refresh: bool = False) -> dict[str, dict[str, str]]:
    ...
    if normalized == "nous":
        ...
        if base_url:
            # Nous base_url typically looks like https://inference-api.anilcan-kara.com/v1
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``nozich model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from nozich_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from nozich_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Nozich' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``nozich_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Nozich should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``nozich auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.nozich/.env``.
    ...
    """
    try:
        from nozich_cli.auth import resolve_api_key_provider_credentials
    ...
    try:
        ...
        from nozich_cli.copilot_auth import (
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a Nozich release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Nozich release). Falls back to the hardcoded catalog if no token
        # or the endpoint is unreachable.
        access_token = None
        try:
            from nozich_cli.auth import resolve_codex_runtime_credentials
    ...
    if normalized == "nous":
        ...
        try:
            from nozich_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
        ...
        # added Portal models still surface without a Nozich release.
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # agentic catalog so ``/model`` matches what ``nozich model`` shows.
...
#
# Cache strategy:
#   - One JSON file at $NOZICH_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    from nozich_constants import get_nozich_home
    return get_nozich_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    relevant tokens live in ``$NOZICH_HOME/auth.json`` and external
...
def copilot_default_headers() -> dict[str, str]:
    ...
    """
    try:
        from nozich_cli.copilot_auth import copilot_request_headers
        return copilot_request_headers(is_agent_turn=True)
    except ImportError:
        return {
            "Editor-Version": COPILOT_EDITOR_VERSION,
            "User-Agent": "NozichAgent/1.0",
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _NOZICH_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Nozich' default Claude IDs elsewhere use
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _NOZICH_USER_AGENT}
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from nozich_constants import get_nozich_home
    return get_nozich_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Nozich will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Nozich cannot verify the model name."
    ...
    if api_models is not None:
        ...
        if requested_for_lookup in set(api_models):
            ...
            # fetch) so we only accept models Nozich actually ships.  (#46850)
