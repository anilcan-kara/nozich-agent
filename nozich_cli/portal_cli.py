"""``nozich portal`` — the human-readable entry point for Nous Portal.

Running ``nozich portal`` with no subcommand performs the one-shot Portal
...
``nozich auth add nous --type oauth`` (which still works), is identical to
``nozich setup --portal``, and runs the same Nous flow as the first-time quick
setup.
...
already in ``nozich auth`` or ``nozich tools``. It's the onboarding + discovery
...
DEFAULT_PORTAL_URL = "https://portal.anilcan-kara.com"
SUBSCRIPTION_URL = "https://portal.anilcan-kara.com/manage-subscription"
DOCS_URL = "https://nozich-agent.anilcan-kara.com/docs/user-guide/features/tool-gateway"
...
def _cmd_status(args) -> int:
    ...
    if provider == "nous":
        ...
        print(f"  Model:   currently {provider} (switch with `nozich model`)")
...
def _cmd_tools(args) -> int:
    ...
    if not features.nous_auth_present:
        print(color("  Not logged into Nous Portal — sign in with `nozich portal`.", Colors.YELLOW))
        print()
...
def _cmd_login(args) -> int:
    ...
    This is the human-readable front door for `nozich auth add nous --type
    oauth`. It reuses the exact wiring behind `nozich setup --portal` (which in
    ...
    """
    from nozich_cli.setup import _run_portal_one_shot
...
def portal_command(args) -> int:
    """Top-level dispatch for `nozich portal <subcommand>`."""
    sub = getattr(args, "portal_command", None)
    if sub in {None, "", "login"}:
        # Default to the one-shot onboarding — `nozich portal` is the
        # human-readable alias for `nozich auth add nous --type oauth` /
        # `nozich setup --portal`.
    ...
    print("Run `nozich portal -h` for usage.", file=sys.stderr)
    return 1


def add_parser(subparsers) -> None:
    """Register `nozich portal` on the given argparse subparsers object."""
    portal_parser = subparsers.add_parser(
        "portal",
        help="Set up Nous Portal (login, model pick, Tool Gateway); see also `portal info`",
        description=(
            "Run `nozich portal` with no subcommand to log in to Nous Portal "
            "and set it up — pick a model, set Nous as your provider, and offer "
            "the Tool Gateway (the human-readable alias for `nozich auth add "
            "nous --type oauth`, identical to `nozich setup --portal`). "
