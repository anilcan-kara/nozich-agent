"""Profile distributions — shareable, packaged Nozich profiles via git.

A distribution is a Nozich profile published as a git repository (or
...
* ``nozich profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``nozich skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``nozich profile``, not a parallel tree):

    nozich profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "nozich-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            nozich_requires=str(data.get("nozich_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.nozich_requires:
            out["nozich_requires"] = self.nozich_requires
        if self.author:
...
def check_nozich_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Nozich {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``nozich profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Nozich profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Nozich distribution."
        )

    # Version check up-front so we fail fast
    check_nozich_requires(manifest.nozich_requires, nozich_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.nozich).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="nozich_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `nozich profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`nozich profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="nozich_dist_update_") as tmp:
        plan = plan_install(
