"""
Profile management for multiple isolated Nozich instances.

Each profile is a fully independent NOZICH_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.nozich/profiles/<name>/`` by default.

The "default" profile is ``~/.nozich`` itself — backward compatible,
zero migration needed.

Usage::

    nozich profile create coder          # fresh profile + bundled skills
    nozich profile create coder --clone  # also copy config, .env, SOUL.md, skills
    nozich profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    nozich -p coder chat                 # or via flag
    nozich profile use coder             # set as sticky default
    nozich profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See nozich_constants.get_subprocess_home().
    "home",
]
...
# default profile (``~/.nozich``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   nozich-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
#   backups             — `nozich backup` archives
...
# Marker file written by `nozich profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `nozich
...
# `nozich skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Nozich infrastructure directories that only the default profile
         (``~/.nozich``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_nozich_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.nozich) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "nozich-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "nozich", "default", "test", "tmp", "root", "sudo",
})

# Nozich subcommands that cannot be used as profile names/aliases
_NOZICH_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the nozich root, NOT to the current NOZICH_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where NOZICH_HOME points outside
    ``~/.nozich``, profiles live under ``NOZICH_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_nozich_home() / "profiles"


def _get_default_nozich_home() -> Path:
    """Return the default (pre-profile) NOZICH_HOME path.

    In standard deployments this is ``~/.nozich``.
    In Docker/custom deployments where NOZICH_HOME is outside ``~/.nozich``
    (e.g. ``/opt/data``), returns NOZICH_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_nozich_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Nozich installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its NOZICH_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_nozich_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, nozich subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _NOZICH_SUBCOMMANDS:
        return f"'{canon}' conflicts with a nozich subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            nozich_exe = shutil.which("nozich") or "nozich"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(nozich_exe)} -p {profile} "$@"\n')
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Nozich. If we
    ...
    try:
        ...
        # not be migrated. The next `nozich doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``nozich -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``nozich profile alias
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``nozich profile list`` for an unrelated profile.
    """
...
# ``config.yaml``. ``config.yaml`` is the user-facing Nozich config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_nozich_home()
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``nozich update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.nozich)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.nozich/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Keys pages, `nozich -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Nozich profile.\n"
    ...
    # Write the opt-out marker so seed_profile_skills() and `nozich update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`nozich profile create --no-skills`).\n"
    ...
    # Cloned configs can be older than the running Nozich (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `nozich profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `nozich -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches NOZICH_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``nozich profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "NOZICH_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_nozich_home() / ".env"
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.nozich).\n"
            "To remove everything, use: nozich uninstall"
        )
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``NOZICH_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from nozich_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set NOZICH_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("NOZICH_HOME")
    try:
        os.environ["NOZICH_HOME"] = str(profile_dir)
        from nozich_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["NOZICH_HOME"] = old_home
        elif "NOZICH_HOME" in os.environ:
            del os.environ["NOZICH_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.nozich/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: nozich profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from NOZICH_HOME.

    Returns ``"default"`` if NOZICH_HOME is not set or points to ``~/.nozich``.
    Returns the profile name if NOZICH_HOME points into ``~/.nozich/profiles/<name>``.
    Returns ``"custom"`` if NOZICH_HOME is set to an unrecognized path.
    """
    from nozich_constants import get_nozich_home
    nozich_home = get_nozich_home()
    resolved = nozich_home.resolve()

    default_resolved = _get_default_nozich_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.nozich itself — its parent is ~/ and its
        # directory name is ".nozich", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: nozich profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.nozich itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.nozich). "
            "Specify a different name: nozich profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="nozich_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"nozich_{old_name}"
    legacy_old_host = f"nozich.{old_name}"
    new_host = f"nozich_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_nozich_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("nozich_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a NOZICH_HOME path string.

    Called early in the CLI entry point, before any nozich modules
    are imported, to set the NOZICH_HOME environment variable.
    """
    canon = normalize_profile_name(profile_name)
    validate_profile_name(canon)
    profile_dir = get_profile_dir(canon)
