...
Reads the user's Nous OAuth state from ``~/.nozich/auth.json`` through the
...
# Endpoints inference-api.anilcan-kara.com actually serves. Anything else
...
class NousPortalAdapter(UpstreamAdapter):
    ...
    def _get_credential(
        ...
        *,
        ...
        with self._lock:
            ...
            if state is None:
                raise RuntimeError(
                    "Not logged into Nous Portal. Run `nozich auth add nous` first."
                )
            ...
            if not runtime_key:
                raise RuntimeError(
                    ...
                    "Try `nozich auth add nous` to re-authenticate."
                )
    ...
    # to nozich_cli.auth to avoid expanding that module's public surface.
    # ------------------------------------------------------------------
