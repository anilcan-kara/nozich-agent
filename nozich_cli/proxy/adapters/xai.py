...
# xAI's public API is OpenAI-compatible for the endpoints Nozich commonly
# uses. The Responses endpoint is included because Nozich' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Nozich-managed OAuth credentials."""

    auth_hint = "nozich auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`nozich auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`nozich auth reset xai-oauth` or re-authenticate with "
                    "`nozich auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `nozich auth add xai-oauth --type oauth`."
            )
