"""CLI subcommand: ``nozich send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(
                f"nozich send: {file_path} is not a text file. --file reads the "
                ...
                f'  nozich send --to telegram "MEDIA:{file_path}"\n'
                f'  nozich send --to telegram "optional caption MEDIA:{file_path}"\n'
                "Add [[as_document]] to deliver an image as an uncompressed file:\n"
                f'  nozich send --to telegram "[[as_document]] MEDIA:{file_path}"',
                file=sys.stderr,
            )
            sys.exit(_USAGE_EXIT)
        except OSError as exc:
            print(f"nozich send: cannot read {file_path}: {exc}", file=sys.stderr)
            sys.exit(_USAGE_EXIT)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"nozich send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"nozich send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"nozich send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"nozich send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `nozich gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.nozich/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_nozich_env() -> None:
    """Populate ``os.environ`` from ``~/.nozich/.env`` AND bridge top-level
    ...
    startup that ``nozich send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.nozich/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.nozich/config.yaml`` into
       ...
       via ``nozich config set``.

    See ``gateway/run.py`` for the canonical version of this bridge — we
    intentionally reimplement the minimum needed here so ``nozich send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.nozich/.env and ~/.nozich/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "nozich send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  nozich send --to telegram \"hello\"\n"
            "  nozich send --to discord:#ops --file report.md\n"
            "  nozich send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "nozich send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `nozich send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Nozich "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.nozich/.env + ~/.nozich/config.yaml) — no LLM, "
        ...
        epilog=(
            ...
            "  nozich send --to telegram \"MEDIA:/tmp/chart.png\"   # send a media attachment\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `nozich send --list telegram`.",
    )
