...
the existing module-level functions in nozich_cli.gateway and
nozich_cli.gateway_windows directly. This protocol is a thin facade
...
``nozich gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``nozich gateway start/stop/restart``).
    """
    ...
    # `nozich gateway start/stop/restart` fell through to host code that spawns
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged nozich
    ...
    detection always fails. Since every Nozich runtime call inside the
    container drops to nozich via ``s6-setuidgid``, that silent failure
...
# in ``nozich_cli.gateway`` (systemd/launchd) and ``nozich_cli.gateway_windows``
...
# active (set via the ``nozich -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in nozich_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from nozich_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from nozich_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from nozich_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from nozich_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in nozich_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from nozich_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from nozich_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from nozich_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from nozich_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``nozich_cli.gateway_windows`` (Scheduled Task /
    ...
    def is_running(self, name: str) -> bool:
        from nozich_cli import gateway_windows
        from nozich_cli.gateway import find_gateway_pids
...
#
# Per-profile gateways are registered dynamically when `nozich profile create`
# runs inside the container (Phase 4). Static services (main-nozich, dashboard)
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    ``gateway stop --all``. Do not write the caller's NOZICH_HOME blindly;
    derive the shared profile root from the current NOZICH_HOME and map the
    ...
    nozich_home = Path(os.environ.get("NOZICH_HOME", "/opt/data"))
    if nozich_home.parent.name == "profiles":
        root = nozich_home.parent.parent
    else:
        root = nozich_home
...
# UID/GID of the in-image ``nozich`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the nozich user.
    ...
    (the ``nozich`` user — UID 10000 — running every Nozich runtime
    ...
    is dispatched as the nozich user.
    ...
    with nozich ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           nozich:nozich, 0755 (parent must already exist)
    ``svc_dir/event/``                     nozich:nozich, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 nozich:nozich, 0755
    ``svc_dir/supervise/event/``           nozich:nozich, 03730
    ``svc_dir/supervise/control``          nozich:nozich, 0660    (FIFO)
    ...
    slot's supervise/ was nozich-owned.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        path.chmod(mode)
        try:
            os.chown(path, _NOZICH_UID, _NOZICH_GID)
        except PermissionError:
            # Running as the nozich user already — directory is nozich-
    ...
    if not control.exists():
        ...
        control.chmod(0o660)
        try:
            os.chown(control, _NOZICH_UID, _NOZICH_GID)
        except PermissionError:
            pass
    ...
    # when the parent slot's supervise/ is nozich-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                os.chown(log_control, _NOZICH_UID, _NOZICH_GID)
            except PermissionError:
                pass
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``nozich -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`nozich profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-nozich, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources NOZICH_HOME (and any extra env) via with-contenv —
             so e.g. ``-e NOZICH_HOME=/data/nozich`` is honored at run
          ...
          4. Drops to the nozich user and exec's
             ``nozich -p <profile> gateway run`` (or just ``nozich
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``nozich gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root NOZICH_HOME profile" (the implicit profile that exists at
        the top of $NOZICH_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``nozich_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$NOZICH_HOME/profiles/default/`` — a
        ...
        each supervised profile gateway loads its own ``NOZICH_HOME``,
        ...
        allocator in ``nozich_cli.profiles._allocate_gateway_port``).
        ...
        # guard.
        lines.append("export NOZICH_S6_SUPERVISED_CHILD=1")
        if profile == "default":
            gateway_cmd = "nozich gateway run"
        else:
            gateway_cmd = f"nozich -p {shlex.quote(profile)} gateway run"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID →
        # s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid nozich {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${NOZICH_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the NOZICH_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e NOZICH_HOME=/data/nozich``
        gets its logs under /data/nozich/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``nozich logs`` reads and what
        ...
        return (
            ...
            f': "${{NOZICH_HOME:=/opt/data}}"\n'
            f'log_dir="$NOZICH_HOME/logs/gateways/{prof}"\n'
            f'mkdir -p "$log_dir"\n'
            ...
            # later then runs its log service as nozich and crash-loops on
            ...
            f'chown nozich:nozich "$NOZICH_HOME/logs/gateways" 2>/dev/null || true\n'
            f'chown -R nozich:nozich "$log_dir" 2>/dev/null || true\n'
            f'rm -f "$log_dir/lock"\n'
            # Skip the drop when already non-root (CAP_SETGID).
            f'[ "$(id -u)" = 0 ] || exec s6-log 1 n10 s1000000 T "$log_dir"\n'
            f'exec s6-setuidgid nozich s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``nozich gateway
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        user explicitly runs ``nozich -p <profile> gateway start``.
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with nozich ownership
            ...
            # the nozich user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is nozich-owned (see ``_seed_supervise_skeleton``), and on
