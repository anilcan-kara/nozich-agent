"""
Interactive setup wizard for Nozich Agent.
...
_DOCS_BASE = "https://nozich-agent.anilcan-kara.com/docs"
...
# display_nozich_home imported lazily at call sites (stale-module safety during nozich update)
...
from nozich_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from nozich_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Nozich Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Nozich using environment variables or config commands:")
    print_info("  nozich config set model.provider custom")
    print_info("  nozich config set model.base_url http://localhost:8080/v1")
    print_info("  nozich config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'nozich setup' in an interactive terminal to use the full wizard.")
    print()
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from nozich_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'nozich setup')")


def _print_setup_summary(config: dict, nozich_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'nozich setup' to configure"))
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from nozich_cli.plugins import _ensure_plugins_discovered
    ...
    # Video generation — opt-in via `nozich tools` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from nozich_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'nozich setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'nozich setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'nozich setup terminal'"))
    ...
    # Spotify (OAuth via nozich auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'nozich setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {nozich_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('nozich setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('nozich setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('nozich setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('nozich setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('nozich setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('nozich config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('nozich config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('nozich config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('nozich', Colors.GREEN)}              Start chatting")
    print(f"   {color('nozich gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('nozich doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `nozich tools` and `nozich setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``nozich model``)
    ...
    provider added to ``nozich model`` is automatically available here.
    ...
    # Delegate to the shared nozich model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from nozich_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: nozich model")
    ...
    # on demand via `nozich auth add`, `nozich setup` vision, and
    # `nozich setup tts`. This keeps both quick and full setup thin.
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``nozich model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from nozich_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.nozich/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Nozich auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via nozich setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'nozich setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Nozich runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `nozich setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `nozich setup terminal`.
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into NOZICH_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("NOZICH_MAX_ITERATIONS")
    ...
    print_info("  Run `nozich setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # NOZICH_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram_auto_result():
    ...
    try:
        from nozich_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_nozich_home(Path(get_nozich_home()))
    except Exception:
        pass
...
def _profile_name_from_nozich_home(nozich_home) -> str | None:
    """Return the active profile name when NOZICH_HOME is a profile dir."""
    if nozich_home.parent.name == "profiles":
        return nozich_home.name
    return None
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Nozich delivers cron job results,")
...
def _setup_slack():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `nozich update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://nozich-agent.anilcan-kara.com/docs/user-guide/messaging/slack/")
    print()
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under NOZICH_HOME, and print
    ...
    """
    try:
        from nozich_cli.slack_cli import _build_full_manifest
        from nozich_constants import get_nozich_home

        manifest = _build_full_manifest(
            bot_name="Nozich",
            bot_description="Your Nozich agent on Slack",
        )
        target = Path(get_nozich_home()) / "slack-manifest.json"
        ...
        print_info(
            "   Re-run `nozich slack manifest --write` anytime to refresh after "
            "Nozich adds new commands."
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Nozich delivers cron job results and notifications.")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Nozich to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from nozich_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://nozich-agent.anilcan-kara.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://nozich-agent.anilcan-kara.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  nozich config edit")
    print_info("   Open config in your editor:  nozich config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from nozich_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Nozich from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'nozich setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     nozich config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_nozich_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd:
                    print_info("  Or as a boot-time service: sudo nozich gateway install --system")
                print_info("  Or run in foreground:  nozich gateway")
        else:
            from nozich_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   nozich gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   nozich gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `nozich setup tools` and `nozich tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``nozich_cli.auth`` — lists every supported
    ...
    """
    try:
        from nozich_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from nozich_cli.config
    ...
    if section_key == "model":
        ...
        from nozich_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_nozich.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_nozich migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_nozich", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Nozich.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Nozich to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Nozich at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Nozich at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Nozich at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Nozich at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Nozich equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Nozich config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Nozich.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Nozich's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(nozich_home: Path) -> bool:
    ...
    print_info("Nozich can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: nozich claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=nozich_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: nozich claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Nozich configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Nozich config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Nozich (use nozich claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``nozich setup --portal`` and ``nozich portal``. This is the
    ...
    Nozich session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``nozich setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``nozich model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``nozich portal`` always offers a model picker,
    ...
    print(color("│     ⚕ Nozich Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    print()
    print_info("  Sign up: https://portal.anilcan-kara.com/manage-subscription")
    print()
    ...
    # setup calls, so `nozich portal` == quick setup's Nous step.
    try:
        from nozich_cli.main import _model_flow_nous
        ...
        print_info("  You can retry later with `nozich portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `nozich portal`: %s", exc)
        print()
    ...
    print_info("  Run `nozich portal info` to inspect routing.")
    print_info("  Run `nozich` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      nozich setup           — full or quick (auto-detected)
      nozich setup model     — just model/provider
      nozich setup tts       — just text-to-speech
      nozich setup terminal  — just terminal backend
      nozich setup gateway   — just messaging platforms
      nozich setup tools     — just tool configuration
      nozich setup agent     — just agent settings
    """
    from nozich_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Nozich Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Nozich Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Nozich Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, nozich_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Nozich configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'nozich setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(nozich_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice(
            "How would you like to set up Nozich?",
            [
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, nozich_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {nozich_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'nozich config edit'")
    ...
    # Tune later with `nozich setup agent`.
    if not is_existing:
    ...
    _print_setup_summary(config, nozich_home)


def _run_first_time_quick_setup(config: dict, nozich_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``nozich setup <section>``
    or switch providers with ``nozich model``.
    """
    from nozich_cli.config import load_config
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'nozich setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    nozich setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  nozich setup gateway")
    print()

    _print_setup_summary(config, nozich_home)


def _run_quick_setup(config: dict, nozich_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'nozich setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Nozich to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'nozich setup gateway'.")
