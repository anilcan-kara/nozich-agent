...
on Japanese locales, etc.).  Nozich's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``nozich_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Nozich is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``NOZICH_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("NOZICH_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Nozich-managed Git install directories so
    ...
    # already-running shells don't see the change — which means nozich
    ...
    # install without Nozich-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Nozich's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Nozich's
    ``nozich config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Nozich (User env var in Windows Settings, or
    export in a PowerShell profile) and Nozich picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Nozich-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\nozich\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``nozich`` from right after install) retain their old PATH.

    Any subprocess Nozich spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Nozich-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "nozich", "git", "cmd"),
        os.path.join(local_appdata, "nozich", "git", "bin"),
        os.path.join(local_appdata, "nozich", "git", "usr", "bin"),
        # Nozich venv Scripts directory — host of the nozich.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes nozich, but harmless to include.
        os.path.join(local_appdata, "nozich", "nozich-agent", "venv", "Scripts"),
