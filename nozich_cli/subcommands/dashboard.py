"""``nozich dashboard`` subcommand parser.

Extracted verbatim from ``nozich_cli/main.py:main()`` (god-file Phase 2).
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Nozich Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    # for `nozich dashboard` cmdlines and SIGTERM them directly — the same
    # path `nozich update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running nozich dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running nozich dashboard processes and exit",
    )
    # Backward-compat shim: older Nozich desktop app shells (<= 0.15.x) spawn the
    # backend as `nozich dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Nozich couldn't start" with no actionable cause. Accept and silently
    ...
    # `nozich dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.nozich/.env.
    # Nested subparser so bare `nozich dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "NOZICH_DASHBOARD_OAUTH_CLIENT_ID into ~/.nozich/.env, and prints "
            "how to engage the login gate. Requires being logged in (nozich setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://nozich.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
        ...
        help=(
            ...
            "portal. Also settable via NOZICH_DASHBOARD_PORTAL_URL. Mainly for "
