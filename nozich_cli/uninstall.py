"""
Nozich Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.nozich/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Nozich PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing nozich-agent or nozich PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Nozich Agent" comment and following line
                if '# Nozich Agent' in line or '# nozich-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('nozich' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing nozich
                if 'nozich' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the nozich wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "nozich",
        Path("/usr/local/bin/nozich"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains nozich_cli reference)
                content = wrapper.read_text()
                if 'nozich_cli' in content or 'nozich-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(nozich_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``nozich`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Nozich home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (nozich_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``nozich gateway run`` processes
    ...
    try:
        from nozich_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from nozich_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``NOZICH_HOME`` and ``NOZICH_GIT_BASH_PATH``
...
def _nozich_path_markers(nozich_home: Path) -> list[str]:
    """Path-entry substrings that identify Nozich-owned User-PATH entries."""
    root = str(nozich_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare nozich-agent install dir.
    markers = [root + "\\nozich-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if NOZICH_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "nozich".  We don't want to catch
    # unrelated entries like "cnozich-foo" or "ephermeral", so we look for
    # backslash-nozich as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(nozich_home: Path) -> list[str]:
    """Strip Nozich-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _nozich_path_markers(nozich_home)
            kept: list[str] = []
...
def remove_nozich_env_vars_windows() -> list[str]:
    """Delete NOZICH_HOME and NOZICH_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("NOZICH_HOME", "NOZICH_GIT_BASH_PATH"):
                try:
...
def _is_default_nozich_home(nozich_home: Path) -> bool:
    """Return True when ``nozich_home`` points at the default (non-profile) root."""
    try:
        from nozich_constants import get_default_nozich_root
        return nozich_home.resolve() == get_default_nozich_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its NOZICH_HOME directory.

    We shell out to ``nozich -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current NOZICH_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m nozich_cli.main` so we don't depend on a `nozich`
    #    wrapper that may be half-removed mid-uninstall.
    nozich_invocation = [_sys.executable, "-m", "nozich_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                nozich_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's NOZICH_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``nozich uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$NOZICH_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from nozich_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(nozich_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ Nozich Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Nozich Chat GUI installation was found.")
        print(f"  Checked: {nozich_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Nozich agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(nozich_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Nozich agent at {nozich_home / 'nozich-agent'}")
        print(f"  • Your config, sessions, and secrets under {nozich_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Nozich agent is still installed. Run 'nozich' to use the CLI,")
    print("or 'nozich uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.nozich/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.nozich/ for future reinstall
    """
    project_root = get_project_root()
    nozich_home = get_nozich_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie NOZICH_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_nozich_home(nozich_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.nozich data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Nozich Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {nozich_home / 'config.yaml'}")
    print(f"  Secrets: {nozich_home / '.env'}")
    print(f"  Data:    {nozich_home / 'cron/'}, {nozich_home / 'sessions/'}, {nozich_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their NOZICH_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Nozich data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Nozich code but keep your configuration and data.")
...
def _perform_uninstall(
    *,
    ...
    ``nozich`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$NOZICH_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in nozich_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\nozich\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(nozich_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Nozich-owned PATH entries in User environment")

        log_info("Removing NOZICH_HOME / NOZICH_GIT_BASH_PATH User env vars...")
        removed_env = remove_nozich_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Nozich-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing nozich command...")
    ...
    #     (only when they still point into this Nozich home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Nozich-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(nozich_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Nozich-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(nozich_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE NOZICH_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from nozich_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(nozich_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.nozich/, just remove the nozich-agent subdir
            if nozich_home in project_root.parents or project_root.parent == nozich_home:
    ...
    #     under NOZICH_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(nozich_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(nozich_home)
    ...
    # 5. Optionally remove ~/.nozich/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile NOZICH_HOME dirs live under
        ...
        try:
            if nozich_home.exists():
                shutil.rmtree(nozich_home)
                log_success(f"Removed {nozich_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {nozich_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {nozich_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://nozich-agent.anilcan-kara.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://nozich-agent.anilcan-kara.com/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Nozich Agent! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m nozich_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import nozich_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``nozich_constants`` + ``nozich_cli.colors``
    (and lazily ``nozich_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m nozich_cli.uninstall")
    parser.add_argument(
        "--mode",
