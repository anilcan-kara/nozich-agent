"""nozich webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.nozich/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _nozich_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: nozich gateway run
"""
...
def webhook_command(args):
    """Entry point for 'nozich webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: nozich webhook {subscribe|list|remove|test}")
        print("Run 'nozich webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (nozich gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: nozich webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from nozich webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (nozich gateway run)")
