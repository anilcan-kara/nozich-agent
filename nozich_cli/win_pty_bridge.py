"""Windows ConPTY bridge for the `nozich dashboard` chat tab.

Drop-in counterpart to ``nozich_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``nozich_cli.web_server`` consumes: ``spawn``, ``read``,
