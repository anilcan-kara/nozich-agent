...
Pure logic: walks a Nozich config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `nozich doctor` and a future `nozich migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Nozich config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Nozich config and return retirement issues.
