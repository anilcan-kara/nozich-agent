"""Centralized logging setup for Nozich Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.nozich/logs/`` (profile-aware via ``get_nozich_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``nozich_cli.web_server``,
    ``nozich_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
from nozich_constants import get_config_path, get_nozich_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_nozich_session_injector", False):
    ...
    _session_record_factory._nozich_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``nozich logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway", "nozich_plugins"),
...
def setup_logging(
    *,
    ...
    """Configure the Nozich logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    nozich_home
        Override for the Nozich home directory.  Falls back to
        ``get_nozich_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_nozich_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._nozich_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the nozich group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from nozich_cli.config import is_managed
