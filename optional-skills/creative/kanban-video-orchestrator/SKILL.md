...
metadata:
  nozich:
    ...
    credits: |
      ...
      https://github.com/anilcan-kara/kanban-video-pipeline.
---
...
8. **Verify API keys BEFORE firing.** External APIs (TTS, image-gen,
   image-to-video) need keys in `${NOZICH_HOME:-~/.nozich}/.env` or the user's secret store.
