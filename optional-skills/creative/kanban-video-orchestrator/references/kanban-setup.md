...
the actual `setup.sh` that creates the project workspace, configures Nozich
...
> [anilcan-kara/kanban-video-pipeline](https://github.com/anilcan-kara/kanban-video-pipeline).
...
2. **Create profiles** — `nozich profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.nozich/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `nozich kanban create` assigned to the director
...
```bash
nozich profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.nozich/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.nozich/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Nozich
...
Each profile gets a `SOUL.md` at `~/.nozich/profiles/<name>/SOUL.md` that
...
```bash
nozich kanban create "Direct production of <video title>" \
    --assignee director \
...
the Nozich `.env` (`${NOZICH_HOME:-$HOME/.nozich}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _nozich_env="${NOZICH_HOME:-$HOME/.nozich}/.env"
    if grep -q "^${var}=" "$_nozich_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_nozich_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_nozich_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY nozich ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY nozich OPENROUTER_API_KEY || exit 1
# ...
```
