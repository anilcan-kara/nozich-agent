...
nozich kanban watch --tenant <project-slug>

# Snapshot of the board
nozich kanban list --tenant <project-slug>
nozich kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
nozich kanban stats --tenant <project-slug>

# Visual dashboard (browser)
nozich dashboard

# Inspect a specific task (includes comments + events)
nozich kanban show <task-id>

# Follow a single task's event stream
nozich kanban tail <task-id>
```

Verify available subcommands with `nozich kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `nozich kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `nozich kanban block <id>` then `nozich kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `nozich kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `nozich kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `nozich profile list` (profile exists?), `nozich status` (gateway/dispatcher up?), and verify tenant. |
...
nozich kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
nozich kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(nozich kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
nozich kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
nozich kanban block <task-id>
nozich kanban archive <task-id>

# Diagnose what happened
nozich kanban show <task-id>      # task body, comments, recent events
nozich kanban tail <task-id>      # follow the live event stream
nozich kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `nozich kanban create` for the director
...
while true; do
    clear
    nozich kanban list --tenant <slug>
    echo "---"
    nozich kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `nozich kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `nozich profile list` and start
