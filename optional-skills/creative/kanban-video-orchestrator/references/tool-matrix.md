...
Maps each role archetype to the Nozich skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public nozich-agent
...
## Nozich skills relevant to video production

### Visual / rendering skills (`nozich-agent/skills/creative/`)
...
### Design / pre-production skills (`nozich-agent/skills/creative/`)
...
### Audio / media skills (`nozich-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`nozich-agent/skills/devops/`)
...
These are **not** Nozich skills but external CLIs / APIs that profiles invoke.
...
## Built-in Nozich tools for media review

These are native Nozich tools — not invoked via terminal but through their own
...
key is present in `${NOZICH_HOME:-~/.nozich}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Nozich profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${NOZICH_HOME:-~/.nozich}/.env` → environment variable.
...
When a new Nozich-public video skill ships:
