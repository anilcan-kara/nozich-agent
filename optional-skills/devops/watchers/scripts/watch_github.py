...
Usage (via cron with --no-agent):
    ...
    nozich cron create nozich-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$NOZICH_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name nozich-issues --repo anilcan-kara/nozich-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Nozich .env file
(``${NOZICH_HOME:-~/.nozich}/.env``) to avoid the 60 req/hr
