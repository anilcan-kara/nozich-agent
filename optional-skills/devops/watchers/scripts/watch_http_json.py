...
Usage (via cron with --no-agent):
    ...
    nozich cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$NOZICH_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Nozich-Watcher/1.0"})
