#!/usr/bin/env python3
"""Canvas LMS API CLI for Nozich Agent.
...
def _check_config():
    ...
    if missing:
        nozich_env = os.path.join(
            os.environ.get("NOZICH_HOME", os.path.expanduser("~/.nozich")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {nozich_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Nozich Agent"
    )
