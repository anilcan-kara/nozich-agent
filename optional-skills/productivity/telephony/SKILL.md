---
name: telephony
description: Give Nozich phone capabilities without core tool changes. Provision and persist a Twilio number, send and receive SMS/MMS, make direct calls, and place AI-driven outbound calls through Bland.ai or Vapi.
version: 1.0.0
...
- save provider credentials into `${NOZICH_HOME:-~/.nozich}/.env`
...
3. Buy it and save it into `${NOZICH_HOME:-~/.nozich}/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
3. persist that number to `${NOZICH_HOME:-~/.nozich}/.env`
