...
## 5. GODMODE FAST — Nozich 4 405B (Zero Refusal)

**Model:** `anilcan-kara/nozich-4-405b`
**Strategy:** Nozich models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Nozich Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.nozich/config.yaml`:
