...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("NOZICH_HOME", os.path.expanduser("~/.nozich")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("NOZICH_HOME", Path.home() / ".nozich")) / "skills" / "red-teaming" / "godmode"
...
NOZICH_HOME = Path(os.getenv("NOZICH_HOME", Path.home() / ".nozich"))
CONFIG_PATH = NOZICH_HOME / "config.yaml"
PREFILL_PATH = NOZICH_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Nozich models — already uncensored, just needs clean prompt
    "nozich": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "nozich" in model_lower or "nous" in model_lower:
        return "nozich"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Nozich config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.nozich/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Nozich for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Nozich for changes to take effect.")
