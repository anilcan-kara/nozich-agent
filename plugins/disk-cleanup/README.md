...
Auto-tracks and cleans up ephemeral files created during Nozich Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$NOZICH_HOME` and `/tmp/nozich-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `NOZICH_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `NOZICH_HOME` or `/tmp/nozich-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$NOZICH_HOME/disk-cleanup/` is itself excluded
- `$NOZICH_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
