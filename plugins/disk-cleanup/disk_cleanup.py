"""disk_cleanup — ephemeral file cleanup for Nozich Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under NOZICH_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly NOZICH_HOME and /tmp/nozich-*
Never touches: ~/.nozich/logs/ or any system directory.
"""
...
try:
    ...
    def get_nozich_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("NOZICH_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".nozich").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$NOZICH_HOME/logs/``."""
    return get_nozich_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$NOZICH_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under NOZICH_HOME or ``/tmp/nozich-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    nozich_home = get_nozich_home()
    try:
        path.resolve().relative_to(nozich_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/nozich-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("nozich-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "nozich-agent", "backups", "profiles", ".worktrees",
})
...
# Paths under $NOZICH_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    """
    # Lazily build the set once per process so NOZICH_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        nozich_home = get_nozich_home()
        for parent in ("cron", "cronjobs"):
            base = nozich_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside NOZICH_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under NOZICH_HOME, but never recurse into known
    # durable state trees.  Some installs place the Nozich checkout, venv,
    # and desktop build under NOZICH_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    nozich_home = get_nozich_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in nozich_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    nozich_home = get_nozich_home()
    try:
        rel = path.resolve().relative_to(nozich_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "nozich-agent",
        }:
            return None
        ...
        # Path isn't under NOZICH_HOME (e.g. /tmp/nozich-*) — fall through.
        pass
