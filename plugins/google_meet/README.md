...
Let the nozich agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where nozich runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `nozich meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `nozich meet` CLI |
...
| `cli.py` | `nozich meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$NOZICH_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `nozich meet node {run,list,approve,remove,status,ping}` |
...
nozich meet install                                      # pip + Chromium
nozich meet setup                                        # preflight
nozich meet auth                                         # optional
nozich meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
nozich meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.nozich/.env
nozich meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
nozich meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
nozich meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.nozich/.env
nozich meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, nozich will **not** switch your system audio input automatically — the
...
nozich plugins enable google_meet
nozich meet node run --display-name my-mac --host 0.0.0.0 --port 18789
