...
nozich meet install                 # pip deps + Chromium (transcribe only)
nozich meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
nozich meet auth                    # optional; skips guest-lobby wait
nozich meet setup                   # preflight checks
```

`nozich meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
#   Then set OPENAI_API_KEY or NOZICH_MEET_REALTIME_KEY in ~/.nozich/.env
```
...
pip install playwright websockets && python -m playwright install chromium
nozich plugins enable google_meet
nozich meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
nozich meet node approve my-mac ws://<mac-ip>:18789 <token>
nozich meet node ping my-mac                   # confirm reachable
```

Run `nozich meet setup` to preflight local prereqs.

## Flow

1. **Join** — call `meet_join(url=..., mode=..., node=...)`. Returns immediately.
2. **Announce yourself** — no auto-consent. Say (in whatever channel the user is watching): "A Nozich agent bot is in this call taking notes."
...
- Guest mode sits in the lobby until a host admits. Warn the user; `nozich meet auth` avoids this.
- **Lobby timeout**: if the host doesn't admit the bot within 5 minutes (configurable via `NOZICH_MEET_LOBBY_TIMEOUT` env), the bot leaves and `meet_status` reports `leaveReason: "lobby_timeout"`.
...
| `realtimeDevice` | Audio device name the bot is feeding (e.g. `nozich_meet_src`). |
...
Local:
```
$NOZICH_HOME/workspace/meetings/<meeting-id>/transcript.txt
```
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$NOZICH_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `nozich meet node approve`.
