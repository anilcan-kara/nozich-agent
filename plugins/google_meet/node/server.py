...
Launched by ``nozich meet node run``.

Token handling
--------------
On first boot we mint 32 hex chars of entropy and persist them at
``$NOZICH_HOME/workspace/meetings/node_token.json``. Subsequent boots
...
via ``nozich meet node approve <name> <url> <token>``.
...
def _default_token_path() -> Path:
    return Path(get_nozich_home()) / "workspace" / "meetings" / "node_token.json"
...
class NodeServer:
    ...
    def __init__(
        ...
        display_name: str = "nozich-meet-node",
    ) -> None:
        self.host = host
        self.port = port
