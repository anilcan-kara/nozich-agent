...
session-scoped state file under ``$NOZICH_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $NOZICH_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_nozich_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this nozich install, leave it first —
    ...
    env = os.environ.copy()
    env["NOZICH_MEET_URL"] = url
    env["NOZICH_MEET_OUT_DIR"] = str(out)
    env["NOZICH_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["NOZICH_MEET_HEADED"] = "1"
    if auth_state:
        env["NOZICH_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["NOZICH_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if NOZICH_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["NOZICH_MEET_MODE"] = mode
    if realtime_model:
        env["NOZICH_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["NOZICH_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["NOZICH_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["NOZICH_MEET_REALTIME_KEY"] = realtime_api_key
