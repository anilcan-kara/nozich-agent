...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/nozich>
...
```bash
nozich memory setup honcho   # configure Honcho directly (works on a fresh install)
nozich memory setup          # generic picker, choose Honcho from the list
```

Or manually:
```bash
nozich config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.nozich/.env
```

> `nozich honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `nozich memory setup honcho`.
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$NOZICH_HOME/honcho.json` | Profile-local (isolated Nozich instances) |
| 2 | `~/.nozich/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Nozich profile: `nozich` (default) or `nozich_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `nozich honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Nozich session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`nozich-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`nozich`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-nozich-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening nozich in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Nozich session ID (timestamp + hex). Every `nozich` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Nozich profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`nozich`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Nozich profile: `nozich` (default) or `nozich_<profile>` (e.g. `nozich -p coder` -> host key `nozich_coder`). Older `nozich.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `NOZICH_HONCHO_HOST` | Host key override |
...
|---------|-------------|
| `nozich memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `nozich honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `nozich memory setup`) |
| `nozich honcho status` | Show resolved config for active profile |
| `nozich honcho enable` / `disable` | Toggle Honcho for active profile |
| `nozich honcho mode <mode>` | Change recall or observation mode |
| `nozich honcho peer --user <name>` | Update user peer name |
| `nozich honcho peer --ai <name>` | Update AI peer name |
| `nozich honcho tokens --context <N>` | Set context token budget |
| `nozich honcho tokens --dialectic <N>` | Set dialectic max chars |
| `nozich honcho map <name>` | Map current directory to a session name |
| `nozich honcho sync` | Create host blocks for all Nozich profiles |
