...
Handles: nozich honcho setup | status | sessions | map | peer
"""
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "nozich" else ""
...
def cmd_sync(args) -> None:
    ...
    Scans all Nozich profiles and creates host blocks for any that don't
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'nozich honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'nozich honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `nozich update` -- no output, no exceptions.
    """
    try:
        from nozich_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Nozich profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $NOZICH_HOME/honcho.json so each profile/instance gets
    ...
    """
    return get_nozich_home() / "honcho.json"
...
def _resolve_effective_identity_mapping(
    ...
    pin = False
    for val in (
        nozich_host.get("pinUserPeer"),
        nozich_host.get("pinPeerName"),
        cfg.get("pinUserPeer"),
        cfg.get("pinPeerName"),
    ):
        if val is not None:
    ...
    if "userPeerAliases" in nozich_host:
        aliases_src = nozich_host.get("userPeerAliases")
    ...
    if "runtimePeerPrefix" in nozich_host:
        prefix_src = nozich_host.get("runtimePeerPrefix")
...
def _scrub_identity_mapping(nozich_host: dict) -> None:
...
def _gateway_platforms() -> list[str] | None:
    ...
    guarded (matching the idiom nozich_cli already uses for gateway refs).
    """
    try:
...
def _apply_runtime_prefix(
    nozich_host: dict, current_prefix: str, prefix_from_root: bool, label: str
) -> None:
    ...
    if new_prefix and not (prefix_from_root and new_prefix == current_prefix):
        nozich_host["runtimePeerPrefix"] = new_prefix


def _echo_identity_mapping(nozich_host: dict) -> None:
    """Show the resulting keys so the operator can verify what was written."""
    aliases = nozich_host.get("userPeerAliases")
    prefix = nozich_host.get("runtimePeerPrefix")
    print("  resolved →")
    print(f"    pinUserPeer       = {bool(nozich_host.get('pinUserPeer'))}")
...
def _configure_raw_identity_mapping(
    ...
    _scrub_identity_mapping(nozich_host)
    nozich_host["pinUserPeer"] = pin
    if pin:
        return
    aliases = (
    ...
    while True:
        ...
        if "=" in entry:
            ...
            if rid and peer:
                aliases[rid] = peer
    if aliases:
        nozich_host["userPeerAliases"] = aliases
    _apply_runtime_prefix(
        nozich_host, current_prefix, prefix_from_root,
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Nozich persistent cross-session memory.")
    ...
    if is_local:
        ...
        current_host_key = nozich_host.get("apiKey", "")
        masked = (
        ...
        if new_local_key:
            nozich_host["apiKey"] = new_local_key
        ...
        if not cfg.get("apiKey"):
            ...
            print("  Run 'nozich honcho setup' again once you have a key.\n")
            return

    # --- 3. Identity ---
    current_peer = nozich_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        nozich_host["peerName"] = new_peer

    current_ai = nozich_host.get("aiPeer") or cfg.get("aiPeer", "nozich")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        nozich_host["aiPeer"] = new_ai

    current_workspace = nozich_host.get("workspace") or cfg.get("workspace", "nozich")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        nozich_host["workspace"] = new_workspace

    # --- 3b. Gateway identity mapping ---
    # These keys only affect the Nozich GATEWAY (Telegram/Discord/Slack/...),
    ...
    (
        ...
        aliases_from_root,
        prefix_from_root,
    ) = _resolve_effective_identity_mapping(cfg, nozich_host)
    ...
    if gw_platforms is None:
        ...
        run_mapping = _prompt(
            "Running the Nozich gateway (Telegram/Discord/etc.)? (y/N)",
            default="n",
    ...
    if run_mapping:
        peer_target = nozich_host.get("peerName") or current_peer or "user"
        ...
        if shape == "single":
            ...
            _scrub_identity_mapping(nozich_host)
            nozich_host["pinUserPeer"] = False
            if prior_aliases:
                nozich_host["userPeerAliases"] = prior_aliases
            _apply_runtime_prefix(
                nozich_host, current_prefix, prefix_from_root,
            ...
            if merged:
                nozich_host["userPeerAliases"] = merged
            _apply_runtime_prefix(
                nozich_host, current_prefix, prefix_from_root,
            ...
            _configure_raw_identity_mapping(
                nozich_host, current_pin, current_aliases, current_prefix,
    ...
    # --- 4. Observation mode ---
    current_obs = nozich_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in {"unified", "directional"}:
        nozich_host["observationMode"] = new_obs
    else:
        nozich_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(nozich_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        nozich_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        nozich_host["writeFrequency"] = new_wf if new_wf in {"async", "turn", "session"} else "async"

    # --- 6. Recall mode ---
    _raw_recall = nozich_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in {"hybrid", "context", "tools"}:
        nozich_host["recallMode"] = new_recall

    # --- 7. Context token budget ---
    current_ctx_tokens = nozich_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    if new_ctx_tokens.strip().lower() in {"none", "uncapped", "no limit"}:
        nozich_host.pop("contextTokens", None)
        ...
        try:
            ...
            if val >= 0:
                nozich_host["contextTokens"] = val
        except (ValueError, TypeError):
            pass  # keep current

    # --- 7b. Dialectic cadence ---
    current_dialectic = str(nozich_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2")
    print("\n  Dialectic cadence:")
    ...
    try:
        ...
        if val >= 1:
            nozich_host["dialecticCadence"] = val
    except (ValueError, TypeError):
        nozich_host["dialecticCadence"] = 2

    # --- 7c. Dialectic reasoning level ---
    current_reasoning = (
        nozich_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in {"minimal", "low", "medium", "high", "max"}:
        nozich_host["dialecticReasoningLevel"] = new_reasoning
    else:
        nozich_host["dialecticReasoningLevel"] = "low"

    # --- 8. Session strategy ---
    current_strat = nozich_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print("\n  Session strategy:")
    ...
    if new_strat in {"per-session", "per-repo", "per-directory", "global"}:
        nozich_host["sessionStrategy"] = new_strat

    nozich_host["enabled"] = True
    nozich_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        nozich_config = load_config()
        nozich_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(nozich_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}")
        print("  Run: nozich config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    nozich honcho status     -- show full config")
    print("    nozich honcho mode       -- change recall/observation mode")
    print("    nozich honcho tokens     -- tune context and dialectic budgets")
    print("    nozich honcho peer       -- update peer names")
    print("    nozich honcho map <name> -- map this directory to a session name\n")


def _active_profile_name() -> str:
    """Return the active Nozich profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: nozich honcho setup\n")
        return
    ...
    if not cfg:
        ...
        try:
            ...
            if _env_cfg.api_key or _env_cfg.base_url:
                ...
                print("  Run 'nozich honcho setup' to configure.\n")
                return
        except Exception:
            print(f"  No Honcho config found at {active_path}")
            print("  Run 'nozich honcho setup' to configure.\n")
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: nozich honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        nozich = hosts.get(_host_key(), {})
        user = nozich.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = nozich.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = nozich.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = nozich.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    Nozich' identity in Honcho. Seed with 'nozich honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print(f"\n  Set with: nozich honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print(f"\n  Set with: nozich honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = nozich.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = nozich.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = nozich.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("    AI-to-AI inference. Nozich asks Honcho's AI peer a question")
        ...
        print("\n  Set with: nozich honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'nozich honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'nozich honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    nozich honcho identity --show        — show both peer representations")
        print("    nozich honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Nozich + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Nozich\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Nozich persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  nozich honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'nozich honcho setup' now?", default="y")
        if answer.lower() in {"y", "yes"}:
            ...
            print()
            print("  Run 'nozich honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  nozich honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'nozich'")
        print("  with Honcho configured and no prior session history.")
        print("  (Nozich calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print("    nozich honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'nozich honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In Nozich, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'nozich honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    nozich honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  nozich honcho identity <path/to/SOUL.md>")
    ...
    print("    Nozich:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Nozich:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Nozich:   Honcho observes every message and updates representations")
    ...
    print("    Nozich:   per-session by default — each run gets its own session")
    print("              Map a custom name:  nozich honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. nozich honcho setup              — configure API key (required)")
        print("  2. nozich honcho migrate            — re-run this walkthrough")
    else:
        print("  1. nozich honcho status             — verify Honcho connection")
        print("  2. nozich                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. nozich honcho identity --show    — verify AI peer representation")
        print("  4. nozich honcho tokens             — tune context and dialectic budgets")
        print("  5. nozich honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'nozich memory setup'...\n")
        from nozich_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``nozich honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``nozich honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to nozich memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Nozich Honcho",
    )
