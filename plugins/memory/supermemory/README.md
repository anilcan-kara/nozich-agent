...
Config file: `$NOZICH_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `nozich` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `nozich-{identity}` → `nozich-coder`). |
...
All Supermemory API calls send `x-sm-source: nozich`, and document writes stamp
`metadata.sm_source: nozich`. This is a **functional routing key, not telemetry**:
it groups Nozich-written memories into a dedicated "Nozich" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Nozich profile:

```json
{
  "container_tag": "nozich-{identity}"
}
```

For a profile named `coder`, this resolves to `nozich-coder`. The default profile resolves to `nozich-default`. Without `{identity}`, all profiles share the same container.
