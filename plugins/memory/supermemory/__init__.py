...
def _load_supermemory_config(nozich_home: str) -> dict:
    config = _default_config()
    config_path = Path(nozich_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, nozich_home: str) -> None:
    config_path = Path(nozich_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str, search_mode: str = "hybrid"):
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "nozich"},
        )
    ...
    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        ...
        merged = {"sm_source": "nozich", **(metadata or {})}
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `nozich memory setup`.
        # All other options are documented for $NOZICH_HOME/supermemory.json
    ...
    def save_config(self, values, nozich_home):
        ...
        _save_supermemory_config(sanitized, nozich_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from nozich_constants import get_nozich_home
        self._nozich_home = kwargs.get("nozich_home") or str(get_nozich_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._nozich_home)
