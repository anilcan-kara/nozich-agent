# Nozich Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: nozich-achievements plugin
...
### `GET /api/plugins/nozich-achievements/achievements`
...
### `POST /api/plugins/nozich-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/nozich-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/nozich-achievements/overview`
...
- Persist plugin data under:
  - `~/.nozich/plugins/nozich-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.nozich/plugins/nozich-achievements/scan_checkpoint.json`
...
- `~/.nozich/plugins/nozich-achievements/`
