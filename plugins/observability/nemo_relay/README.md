...
Optional Nozich observability plugin that maps Nozich observer hooks to
...
not replace Nozich Agent's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets Nozich emit NeMo Relay lifecycle events for
the work Nozich already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, Nozich Agent can:

- Preserve Nozich execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
```bash
nozich plugins enable observability/nemo_relay
```

The `NOZICH_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `NOZICH_HOME` that the
agent run will use:

```bash
env NOZICH_HOME=/tmp/nozich-nemo-relay-test \
  nozich plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`NOZICH_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`NOZICH_HOME` is the Nozich profile/config home used by both
`nozich plugins enable ...` and the later `nozich chat ...` run. If unset,
Nozich uses the user's default home, usually `~/.nozich`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export NOZICH_HOME=/tmp/nozich-nemo-relay-test
nozich plugins enable observability/nemo_relay
nozich chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `nozich` command you run is built from the
...
uv run nozich plugins enable observability/nemo_relay
uv run nozich chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/nozich_agent-*.whl
...
The plugin can configure exporters directly from `NOZICH_NEMO_RELAY_*`
...
```bash
export NOZICH_NEMO_RELAY_ATOF_ENABLED=1
export NOZICH_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export NOZICH_NEMO_RELAY_ATIF_ENABLED=1
export NOZICH_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `NOZICH_NEMO_RELAY_ATOF_FILENAME`
- `NOZICH_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `NOZICH_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `NOZICH_NEMO_RELAY_ATIF_AGENT_NAME`
- `NOZICH_NEMO_RELAY_ATIF_AGENT_VERSION`
- `NOZICH_NEMO_RELAY_ATIF_MODEL_NAME`
- `NOZICH_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point Nozich at it:

```bash
export NOZICH_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
agent_name = "Nozich Agent"
agent_version = "local"
```

When `NOZICH_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`NOZICH_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`NOZICH_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Nozich does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Nozich keeps the direct env-var fallbacks active for that run.
...
exposes `llm.execute(...)` / `tools.execute(...)`, Nozich routes LLM and tool
...
For the full generic Nozich middleware contract, see
...
export NOZICH_HOME=/tmp/nozich-nemo-relay-docs/nozich-home
mkdir -p "$NOZICH_HOME"

cat > "$NOZICH_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent Nozich session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export NOZICH_NEMO_RELAY_ATOF_ENABLED=1
export NOZICH_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/nozich-nemo-relay-docs/subagent/atof
export NOZICH_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export NOZICH_NEMO_RELAY_ATOF_MODE=overwrite
export NOZICH_NEMO_RELAY_ATIF_ENABLED=1
export NOZICH_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/nozich-nemo-relay-docs/subagent/atif
export NOZICH_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export NOZICH_NEMO_RELAY_ATIF_AGENT_NAME='Nozich Agent E2E'
export NOZICH_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export NOZICH_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"nozich.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Nozich Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Nozich dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/nozich-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/nozich-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/nozich-nemo-relay-docs/workdir/beta.txt
cd /tmp/nozich-nemo-relay-docs/workdir

export NOZICH_NEMO_RELAY_ATOF_ENABLED=1
export NOZICH_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/nozich-nemo-relay-docs/parallel/atof
export NOZICH_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export NOZICH_NEMO_RELAY_ATOF_MODE=overwrite
export NOZICH_NEMO_RELAY_ATIF_ENABLED=1
export NOZICH_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/nozich-nemo-relay-docs/parallel/atif
export NOZICH_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export NOZICH_NEMO_RELAY_ATIF_AGENT_NAME='Nozich Agent E2E'
export NOZICH_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- Nozich sessions map to `agent` scopes.
- Nozich API request hooks map to `llm` scope start/end events.
- Nozich tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses Nozich execution middleware to hand
...
Enable it for Nozich:

```bash
export NOZICH_NEMO_RELAY_PLUGINS_TOML=/tmp/nozich-middleware-test/plugins.toml
```

When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, Nozich routes execution
through these boundaries:

```text
Nozich provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> Nozich provider adapter next_call(...)

Nozich tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> Nozich tool dispatcher next_call(...)
```
...
middleware for a local Nozich run. This path requires a NeMo Relay runtime that
...
```bash
export NOZICH_HOME=/tmp/nozich-middleware-test/nozich-home
mkdir -p "$NOZICH_HOME" /tmp/nozich-middleware-test/nemo-relay
...
cat > /tmp/nozich-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/nozich-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/nozich-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "Nozich Middleware E2E"
agent_version = "local"
...
export NOZICH_NEMO_RELAY_PLUGINS_TOML=/tmp/nozich-middleware-test/nemo-relay/plugins.toml
