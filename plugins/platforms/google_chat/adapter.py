...
# ``model_tools`` import time, so ``nozich status``, ``nozich chat``, etc.
...
# ``nozich_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Nozich is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a nozich MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${NOZICH_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _nozich_home = _get_nozich_home()
        except (ModuleNotFoundError, ImportError):
            _nozich_home = _Path.home() / ".nozich"
        self._thread_count_store = _ThreadCountStore(
            _nozich_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Nozich is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("NOZICH_HOME", str(_Path.home() / ".nozich"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${NOZICH_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a nozich MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Nozich is thinking…' marker message.
        ...
        "Nozich is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Nozich is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Nozich is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Nozich is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Nozich is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``nozich setup``.

    The setup wizard at ``nozich_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.nozich/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. nozich-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.nozich/.env")
    print_info("Restart the gateway: nozich gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``nozich cron`` running as a
    separate process from ``nozich gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Nozich plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'nozich-agent[google_chat]'",
        ...
        # the "Nozich is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Nozich is thinking…' marker message "
