"""
IRC Platform Adapter for Nozich Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Nozich agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        try:
            ...
            self.port = 6697
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "nozich-bot")
    ...
    async def connect(self) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Nozich Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Nozich Agent shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: nozich_, nozich_1, nozich_2...
...
def check_requirements() -> bool:
    ...
    # nozich setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `nozich gateway setup` flow for the IRC platform.

    Lazy-imports ``nozich_cli.setup`` helpers so the plugin stays importable
    ...
    print_info("Connect Nozich to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #nozich — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.nozich/.env")
    print_info("Restart the gateway for changes to take effect: nozich gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "nozich-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "nozich-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Nozich Agent (cron)")
