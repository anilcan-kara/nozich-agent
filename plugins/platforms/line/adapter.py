"""
LINE Messaging API platform adapter for Nozich Agent.
...
to Nozich Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``NOZICH_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            nozich_home = Path.home().joinpath(".nozich").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``nozich status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``nozich status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``nozich setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.nozich/.env`` via ``nozich_cli.config``.
    """
    print()
    ...
    try:
        from nozich_cli.config import get_env_var, set_env_var
    except ImportError:
        print("nozich_cli.config not available; set LINE_* vars manually in ~/.nozich/.env")
        return
