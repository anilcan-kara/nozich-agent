"""ntfy platform adapter (Nozich plugin).
...
``plugins/platforms/ntfy/``. The Nozich plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "nozich-in"              # subscribe topic (incoming)
          publish_topic: "nozich-out"     # optional — defaults to topic
...
_ECHO_TAG = "nozich-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``nozich cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Nozich dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `nozich gateway status` without
