...
This plugin connects Nozich Agent to iMessage (and other Spectrum
...
TypeScript-only, Nozich runs it inside a small supervised Node sidecar and
...
- **Outbound**: `send` / `send_typing` / reaction tapbacks are loopback POSTs
  ...
  `/unreact`), authenticated with a shared `X-Nozich-Sidecar-Token`.
...
There is no separate `login` command; like every other Nozich channel,
...
Run `nozich photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.nozich/.env` (the same place every other
...
Management metadata lives in `~/.nozich/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `nozich photon telemetry on\|off` (restart the gateway to apply) |
...
5. Verify end-to-end: `nozich photon status`, a DM and a group roundtrip,
