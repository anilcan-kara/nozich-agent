...
Credential storage mirrors every other Nozich channel:

    * runtime SDK creds  -> ``~/.nozich/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.nozich/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a nozich dependency
...
# registers Nozich as its own client_id.
...
# Default name of the project Nozich provisions for the operator.
DEFAULT_PROJECT_NAME = "Nozich Agent"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of nozich-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.nozich/auth.json`` honouring the active Nozich profile."""
    try:
        from nozich_constants import get_nozich_home
        return Path(get_nozich_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.nozich")) / "auth.json"
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.nozich/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.nozich/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.nozich/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: nozich_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `nozich photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `nozich photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `nozich photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `nozich photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `nozich photon setup`)"
