"""
``nozich photon ...`` CLI subcommands — registered by the plugin via
...
there is no standalone ``login`` verb (matching how every other Nozich
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire up `nozich photon ...` subcommands."""
    ...
    p_setup.add_argument("--project-name", default=None,
                         help="Project name (default: 'Nozich Agent')")
...
def _run_device_login(args: argparse.Namespace) -> int:
    ...
    no standalone ``nozich photon login`` command; Photon onboards
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 2. Find or create the "Nozich Agent" project.
    ...
    # 3. Rotate the project secret and persist creds (runtime -> ~/.nozich/.env,
    ...
    print("  Start the gateway:  nozich gateway start")
    return 0
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  sidecar deps        : {'✓ installed' if sidecar_installed else '✗ run `nozich photon install-sidecar`'}")
    print(f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`nozich photon telemetry on|off`)")
    return 0
...
def _telemetry_enabled() -> bool:
    """Read PHOTON_TELEMETRY from the env / ~/.nozich/.env.
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    if state is None:
        ...
        print("  Toggle with `nozich photon telemetry on` / `nozich photon telemetry off`.")
        return 0
    try:
        ...
        return 1
    print(f"✓ Spectrum telemetry turned {state} (PHOTON_TELEMETRY in ~/.nozich/.env)")
    print("  Restart the gateway for the sidecar to pick it up:  nozich gateway restart")
    return 0
...
#
# `nozich gateway setup` discovers platforms via the registry and calls each
...
# project + user + sidecar flow as ``nozich photon setup`` with interactive
# defaults (phone is prompted when stdin is a TTY).

def gateway_setup() -> None:
    """Run Photon first-time setup from the `nozich gateway setup` wizard."""
