...
Small Node helper that bridges Nozich Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Nozich is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Nozich-Sidecar-Token`)
...
The Nozich plugin's `nozich photon setup` command runs `npm install`
here automatically.
