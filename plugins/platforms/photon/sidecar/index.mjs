// Nozich Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Nozich): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Nozich -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Nozich-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `nozich photon telemetry`)
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-nozich-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Nozich supplied them so a known-good
