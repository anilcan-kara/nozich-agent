"""SimpleX Chat platform adapter (Nozich plugin).
...
This adapter ships as a Nozich platform plugin under
``plugins/platforms/simplex/``. The Nozich plugin loader scans the
...
Optional environment variables:
    ...
    NOZICH_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``nozich setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Nozich — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("NOZICH_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``nozich setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.nozich/.env`` via
    ``nozich_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "nozich_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.nozich/.env"
        )
        return
