    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            raise ValueError(
                ...
                f"  • Pick a different model via `nozich tools` → Video Generation."
            ) from exc
        raise
        ...
        if not _check_fal_video_available():
            return error_response(
                error=(
                    ...
                    "(run `nozich tools` → Video Generation → FAL to configure) "
                    "or sign in to Nous (`nozich setup`) for managed gateway access."
                ),
            ...
            if not endpoint:
                return error_response(
                    error=(
                        ...
                        f"via `nozich tools` → Video Generation."
                    ),
