...
``{title, url, description, position}`` rows every other Nozich web
provider produces.
...
prefers Nozich-managed xAI Grok OAuth (via ``nozich auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.nozich/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``nozich tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `nozich auth` to sign in with "
