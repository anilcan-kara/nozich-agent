# ============================================================================
# Nozich Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$NozichHome = $(if ($env:NOZICH_HOME) { $env:NOZICH_HOME } else { "$env:LOCALAPPDATA\nozich" }),
    [string]$InstallDir = $(if ($env:NOZICH_HOME) { "$env:NOZICH_HOME\nozich-agent" } else { "$env:LOCALAPPDATA\nozich\nozich-agent" }),
    ...
    # builds apps/desktop into a launchable Nozich.exe.
    #
    # Why opt-in:
    #   * Nozich-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Nozich.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Nozich.exe
    ...
    #     `nozich desktop` already builds on demand.
...
$RepoUrlSsh = "git@github.com:anilcan-kara/nozich-agent.git"
$RepoUrlHttps = "https://github.com/anilcan-kara/nozich-agent.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "|             * Nozich Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $NozichHome)) {
        New-Item -ItemType Directory -Force -Path $NozichHome | Out-Null
    }
    $envFile = Join-Path $NozichHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $NozichHome "node"
...
function Install-Uv {
    # Nozich owns its own uv at $NozichHome\bin\uv.exe.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (nozich_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `nozich update` stay in sync.
    $managedUv = Join-Path $NozichHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $NozichHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $NozichHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $NozichHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $NozichHome "bin"
...
function Resolve-UvCmd {
    ...
    # sibling process and NOZICH_HOME wasn't propagated).
...
function Install-Git {
    ...
    which Nozich uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\nozich\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Nozich needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Nozich copy of Git ourselves is predictable and
    ...
    ``NOZICH_GIT_BASH_PATH`` (User scope) so Nozich can find it in a fresh
    ...
    # Download PortableGit into $NozichHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $NozichHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Nozich features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Nozich needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``NOZICH_GIT_BASH_PATH`` (User env scope) so Nozich can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $NozichHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $NozichHome\git\usr\bin\bash.exe
    $candidates += "$NozichHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$NozichHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("NOZICH_GIT_BASH_PATH", $candidate, "User")
            $env:NOZICH_GIT_BASH_PATH = $candidate
            Write-Info "Set NOZICH_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Nozich may not find Git Bash."
    Write-Info "If needed, set NOZICH_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    $managedNode = "$NozichHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$NozichHome\node;$env:Path"
        Write-Success "Node.js $version found (Nozich-managed)"
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Nozich-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $NozichHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $NozichHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            if ($extractedDir) {
                if (Test-Path "$NozichHome\node") { Remove-Item -Recurse -Force "$NozichHome\node" }
                Move-Item $extractedDir.FullName "$NozichHome\node"
                ...
                $version = & "$NozichHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $NozichHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\nozich-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `nozich
                    # update` path (#4735).
                    ...
                    $stashName = "nozich-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if ($LASTEXITCODE -eq 0) {
                            ...
                            Write-Warn "Review git diff / git status if Nozich behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running nozich processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/anilcan-kara/nozich-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/anilcan-kara/nozich-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/anilcan-kara/nozich-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\nozich-agent-$zipLabel.zip"
    ...
    # next `nozich update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    if (Test-Path "venv") {
        ...
        # DLLs by any running nozich process. Windows denies deletion of loaded
        # DLLs, so kill any nozich.exe tree before removing the venv.
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running nozich processes before recreating venv..."
            & taskkill /F /T /IM nozich.exe /FI "PID ne $myPid" 2>$null | Out-Null
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `nozich.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install nozich-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and nozich.exe broken with
    ...
    # here, not 30 seconds later when the user runs `nozich`.
    if (-not $NoVenv) {
    ...
    # users hit and lazy-import errors from `nozich dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `nozich dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `nozich dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up nozich command..."
    
    if ($NoVenv) {
        $nozichBin = "$InstallDir"
    } else {
        $nozichBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so nozich is globally available
    # On Windows, the nozich.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$nozichBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$nozichBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $nozichBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set NOZICH_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\nozich instead
    # of the Unix default ~/.nozich
    $currentNozichHome = [Environment]::GetEnvironmentVariable("NOZICH_HOME", "User")
    if (-not $currentNozichHome -or $currentNozichHome -ne $NozichHome) {
        [Environment]::SetEnvironmentVariable("NOZICH_HOME", $NozichHome, "User")
        Write-Success "Set NOZICH_HOME=$NozichHome"
    }
    $env:NOZICH_HOME = $NozichHome
    
    # Update current session
    $env:Path = "$nozichBin;$env:Path"
    
    Write-Success "nozich command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.nozich-bootstrap-complete which tells the Nozich
    ...
    # Nozich-Setup.exe) or fall back to whatever git resolves in the
    # checkout. The desktop validates schemaVersion + pinnedCommit
    # length but doesn't enforce that HEAD matches the pin (users
    # update via `nozich update` which moves HEAD legitimately).
    ...
    $markerPath = Join-Path $InstallDir ".nozich-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the NOZICH_HOME directory structure ($NozichHome, default %LOCALAPPDATA%\nozich)
    New-Item -ItemType Directory -Force -Path "$NozichHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NozichHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NozichHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NozichHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NozichHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NozichHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NozichHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NozichHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NozichHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$NozichHome\.env"
    ...
    # Create config.yaml
    $configPath = "$NozichHome\config.yaml"
    ...
    # (the default PS5 behaviour), and Nozich's prompt-injection scanner
...
"@
    ...
    Write-Success "Configuration directory ready: $NozichHome"
    
    # Seed bundled skills into $NozichHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $NozichHome\skills ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to $NozichHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$NozichHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $NozichHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Nozich-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'nozich setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\nozich-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\nozich-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
...
# the final `electron` -> `Nozich` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Nozich.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_nozich_desktop command
    # resolves apps/desktop/release/win-unpacked/Nozich.exe directly,
    ...
    # enforces the build floor (^20.19 || >=22.12) and prepends the Nozich-managed
    ...
    # The --dir mode produces an unpacked Nozich.exe in
    ...
    # are NOT trying to work around). The Nozich icon + product name are
    # stamped onto Nozich.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\nozich-desktop-build-$(Get-Random).log"
    ...
    try {
        ...
        if ($code -ne 0) {
            ...
            # ENOENT on the final `electron` -> `Nozich` rename: app-builder's
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Nozich.exe",
        "$desktopDir\release\win-arm64-unpacked\Nozich.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Nozich.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Nozich icon + identity are stamped onto Nozich.exe by the
    ...
    #    Nozich.exe. We deliberately do NOT point them at `nozich desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Nozich.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Nozich.lnk')
        )
        ...
        # with the Nozich icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.nozich/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'nozich setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m nozich_cli.main setup
    } else {
        python -m nozich_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $nozichCmd = "$InstallDir\venv\Scripts\nozich.exe"
    if (-not (Test-Path $nozichCmd)) {
        $nozichCmd = "nozich"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$NozichHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'nozich whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: nozich gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$NozichHome\logs\gateway.log"
            Start-Process -FilePath $nozichCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$NozichHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: nozich gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: nozich gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$NozichHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$NozichHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$NozichHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$NozichHome\nozich-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   nozich              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   nozich setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   nozich config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   nozich config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   nozich gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   nozich update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Nozich repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Nozich-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Nozich to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://nozich-agent.anilcan-kara.com/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
