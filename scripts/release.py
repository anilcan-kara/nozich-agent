...
VERSION_FILE = REPO_ROOT / "nozich_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "jake@anilcan-kara.com": "simpolism",
    ...
    "nozich@marian.local": "Schrotti77",
    ...
    "teknium@anilcan-kara.com": "teknium1",
    ...
    "274096618+nozich-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "shannon@anilcan-kara.com": "shannonsands",
    ...
    "nozich@anilcan-kara.com": "anilcan-kara",
    ...
    "ben@anilcan-kara.com": "benbarclay",
    ...
    "emozilla@anilcan-kara.com": "emozilla",
    ...
    "jonny@anilcan-kara.com": "yoniebans",
    "jake@anilcan-kara.com": "simpolism",
    ...
    "nozich@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "github@nadyanozich.anonaddy.com": "ruangraung",  # PR #42308
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (NOZICH_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (NOZICH_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: nozich-eval community link)
    ...
    "nozichagent26@gmail.com": "nozichagent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+nozichagent26@users.noreply.github.com": "nozichagent26",
    ...
    "clement@anilcan-kara.com": "lemassykoi",  # PR #27042 (model-switch probe keyless providers)
    ...
    "vigo@nozich": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "hex.nozich@agentmail.to": "oseftg",
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local Nozich runtime files)
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level NOZICH_HOME state files)
...
def update_version_files(semver: str, calver_date: str):
    ...
    # Python package version. The desktop About panel reads the live Nozich
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"nozich-agent[acp]=={semver}"
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "nozich@anilcan-kara.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/anilcan-kara/nozich-agent",
    ...
    lines.append(f"# Nozich Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Nozich Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Nozich Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Nozich Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Nozich Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Nozich Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Nozich Agent v{new_version} ({calver_date})' "
