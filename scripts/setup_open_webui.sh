...
# Bootstrap Open WebUI against Nozich Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.nozich/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-nozich.sh
...
#   NOZICH_API_PORT=8642
#   NOZICH_API_HOST=127.0.0.1
#   NOZICH_API_MODEL_NAME='Nozich Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Nozich Agent WebUI}"
...
NOZICH_ENV_FILE="${NOZICH_ENV_FILE:-$HOME/.nozich/.env}"
NOZICH_API_PORT="${NOZICH_API_PORT:-8642}"
NOZICH_API_HOST="${NOZICH_API_HOST:-127.0.0.1}"
NOZICH_API_CONNECT_HOST="${NOZICH_API_CONNECT_HOST:-127.0.0.1}"
NOZICH_API_MODEL_NAME="${NOZICH_API_MODEL_NAME:-Nozich Agent}"
NOZICH_API_BASE_URL="http://${NOZICH_API_CONNECT_HOST}:${NOZICH_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-nozich.sh"
LOG_DIR="$HOME/.nozich/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$NOZICH_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$NOZICH_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.nozich.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.nozich</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.nozich"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.nozich"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-nozich.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Nozich Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-nozich.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.nozich/logs/openwebui.log
StandardError=append:%h/.nozich/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-nozich.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$NOZICH_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Nozich API server is configured...'
  upsert_env API_SERVER_ENABLED true "$NOZICH_ENV_FILE"
  upsert_env API_SERVER_HOST "$NOZICH_API_HOST" "$NOZICH_ENV_FILE"
  upsert_env API_SERVER_PORT "$NOZICH_API_PORT" "$NOZICH_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$NOZICH_API_MODEL_NAME" "$NOZICH_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$NOZICH_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Nozich gateway so API server settings take effect...'
  nozich gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${NOZICH_API_CONNECT_HOST}:${NOZICH_API_PORT}/health" >/dev/null; then
    log 'Nozich API server did not answer on the first check. Trying to start gateway in the background...'
    nohup nozich gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${NOZICH_API_CONNECT_HOST}:${NOZICH_API_PORT}/health" >/dev/null
  ...
  log "Nozich API endpoint: ${NOZICH_API_BASE_URL}"
