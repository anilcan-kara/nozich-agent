#!/usr/bin/env python3
"""Live test harness for Nozich Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any nozich imports.
ORIGINAL_HOME = os.environ.get("NOZICH_HOME")
ORIGINAL_AUTH = Path.home() / ".nozich" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.nozich/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.nozich/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="nozich_ts_live_"))
    nozich_home = home_dir / ".nozich"
    nozich_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, nozich_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".nozich" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, nozich_home / ".env")
        ...
        load_nozich_dotenv(nozich_home=str(Path.home() / ".nozich"))
    ...
    (nozich_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return nozich_home
...
def reset_module_state():
    """Drop cached modules so the new NOZICH_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "nozich_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["NOZICH_HOME"] = str(home)
...
def main():
    ...
    # Restore original NOZICH_HOME
    if ORIGINAL_HOME is not None:
        os.environ["NOZICH_HOME"] = ORIGINAL_HOME
    else:
