...
# 5. Symlinks the 'nozich' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Nozich Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/nozich-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink nozich into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up nozich command..."

NOZICH_BIN="$SCRIPT_DIR/venv/bin/nozich"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$NOZICH_BIN" "$COMMAND_LINK_DIR/nozich"
echo -e "${GREEN}✓${NC} Symlinked nozich → $COMMAND_LINK_DISPLAY_DIR/nozich"
...
# ============================================================================
# Seed bundled skills into ~/.nozich/skills/
# ============================================================================

NOZICH_SKILLS_DIR="${NOZICH_HOME:-$HOME/.nozich}/skills"
mkdir -p "$NOZICH_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.nozich/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$NOZICH_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  nozich status        # Check configuration"
if is_termux; then
    echo "  nozich gateway       # Run gateway in foreground"
else
    echo "  nozich gateway install # Install gateway service (messaging + cron)"
fi
echo "  nozich cron list     # View scheduled jobs"
echo "  nozich doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m nozich_cli.main setup
fi
