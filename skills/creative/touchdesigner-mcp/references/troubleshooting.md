...
## 5. Nozich Configuration

### Config location

`$NOZICH_HOME/config.yaml` (defaults to `~/.nozich/config.yaml` when `NOZICH_HOME` is unset)
...
Restart the Nozich session for changes to take effect. The MCP connection is
