...
NOZICH_HOME_DIR="${NOZICH_HOME:-$HOME/.nozich}"
NOZICH_CFG="${NOZICH_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Nozich config has twozero_td MCP entry ──
if [[ ! -f "$NOZICH_CFG" ]]; then
    echo -e " ${FAIL} Nozich config not found at ${NOZICH_CFG}"
    manual_steps+=("Create ${NOZICH_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$NOZICH_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Nozich config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Nozich config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${NOZICH_CFG} manually"); }
    manual_steps+=("Restart Nozich session to pick up config change")
fi
