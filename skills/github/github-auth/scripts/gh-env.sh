#!/usr/bin/env bash
# GitHub environment detection helper for Nozich Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _nozich_env="${NOZICH_HOME:-$HOME/.nozich}/.env"; [ -f "$_nozich_env" ] && grep -q "^GITHUB_TOKEN=" "$_nozich_env" 2>/dev/null; then
