...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _nozich_env="${NOZICH_HOME:-$HOME/.nozich}/.env"; [ -f "$_nozich_env" ] && grep -q "^GITHUB_TOKEN=" "$_nozich_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_nozich_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
