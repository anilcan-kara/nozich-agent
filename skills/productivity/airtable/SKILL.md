...
4. Store the token in `${NOZICH_HOME:-~/.nozich}/.env` (or via `nozich setup`):
   ```
...
- **`AIRTABLE_API_KEY` flows from `${NOZICH_HOME:-~/.nozich}/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
