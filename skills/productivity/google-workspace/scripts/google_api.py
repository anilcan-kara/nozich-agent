#!/usr/bin/env python3
"""Google Workspace API CLI for Nozich Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Nozich-facing JSON contract and falls back to the Python client
...
from _nozich_home import get_nozich_home
...
def _gws_binary() -> str | None:
    override = os.getenv("NOZICH_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Nozich Agent")
