#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Nozich Agent.
...
# Ensure sibling modules (_nozich_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _nozich_home import display_nozich_home, get_nozich_home

NOZICH_HOME = get_nozich_home()
TOKEN_PATH = NOZICH_HOME / "google_token.json"
CLIENT_SECRET_PATH = NOZICH_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = NOZICH_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Nozich profile to refresh consent."
    )
...
def install_deps():
    ...
    # Fallback: the interpreter has no pip (the Nozich Docker image's venv is
    ...
    print(
        "On environments without pip (e.g. Nix, or the Nozich Docker image's "
        "uv-managed venv), install the optional extra instead:"
    )
    print("  pip install 'nozich-agent[google]'")
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Nozich home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_nozich_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Nozich")
