...
- 96vc: ACP didn't set NOZICH_EXEC_ASK, so `check_all_command_guards`
...
1. Setting NOZICH_EXEC_ASK inside _run_agent (wraps the agent call).
...
class TestThreadLocalApprovalCallback:
    ...
    def test_sudo_password_cache_isolated_across_acp_sessions_on_same_pool_thread(self):
        ...
        binds NOZICH_SESSION_KEY per session, so the cache scope key differs
...
class TestAcpExecAskGate:
    """GHSA-96vc-wcxf-jjff: ACP's _run_agent must set NOZICH_INTERACTIVE so
    ...
    (NOZICH_EXEC_ASK takes the gateway-queue path which requires a
    ...
    def test_interactive_env_var_routes_to_callback(self, monkeypatch):
        """When NOZICH_INTERACTIVE is set and an approval callback is
        ...
        # Without NOZICH_INTERACTIVE: takes auto-approve path, callback NOT called
        result = check_all_command_guards(
            "rm -rf /tmp/test-exec-ask", "local", approval_callback=fake_cb,
        )
        assert result["approved"] is True
        assert called_with == [], (
            "without NOZICH_INTERACTIVE the non-interactive auto-approve "
            "path should fire without consulting the callback"
        )

        # With NOZICH_INTERACTIVE: callback IS called, approval flows through it
        monkeypatch.setenv("NOZICH_INTERACTIVE", "1")
        called_with.clear()
        result = check_all_command_guards(
            "rm -rf /tmp/test-exec-ask", "local", approval_callback=fake_cb,
        )
        assert called_with, (
            "with NOZICH_INTERACTIVE the approval path should consult the "
