"""Tests for ACP Registry metadata shipped with Nozich."""
...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["id"] == "nozich-agent"
    assert re.fullmatch(r"[a-z][a-z0-9-]*", data["id"])
    assert data["name"] == "Nozich Agent"
    assert data["description"]
    assert data["repository"] == "https://github.com/anilcan-kara/nozich-agent"
    assert data["website"].startswith("https://nozich-agent.anilcan-kara.com/")
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"nozich-agent[acp]=={data['version']}"
    assert uvx["args"] == ["nozich-acp"]
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"nozich-agent[acp]=={_pyproject_version()}"
    )
