...
def test_root_session_no_compression(db):
    ...
    assert prov["currentNozichSessionId"] == "root1"
    assert prov["rootNozichSessionId"] == "root1"
    assert prov["parentNozichSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_nozich_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentNozichSessionId"] == "old"
    assert prov["rootNozichSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousNozichSessionId"] == "old"
...
def test_multi_depth_chain(db):
    ...
    assert prov["rootNozichSessionId"] == "s0"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootNozichSessionId"] == "p"  # lineage root still walked
...
def test_no_false_rotation_when_head_unchanged(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "s", previous_nozich_session_id="s"
    )
    assert "reason" not in prov
    assert "creatorKind" not in prov
    assert prov["previousNozichSessionId"] == "s"
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"nozich"}
    assert "sessionProvenance" in meta["nozich"]
    assert meta["nozich"]["sessionProvenance"]["currentNozichSessionId"] == "root1"
