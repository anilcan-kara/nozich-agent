...
engine plugins (e.g. nozich-lcm) rely on:
...
def test_discover_context_engines_includes_plugin_registered_engines(monkeypatch):
    """Plugin-registered context engines appear in the ``nozich plugins`` picker."""
    from nozich_cli import plugins_cmd
    ...
    monkeypatch.setattr(
        "nozich_cli.plugins.discover_plugins",
        lambda *_a, **_kw: None,
    )
    monkeypatch.setattr(
        "nozich_cli.plugins.get_plugin_context_engine",
