...
def sample_repo(tmp_path: Path) -> Path:
    ...
    repo.mkdir()
    _git(repo, "init")
    _git(repo, "config", "user.name", "Nozich Tests")
...
@pytest.mark.asyncio
async def test_blocks_sensitive_home_and_nozich_paths(tmp_path: Path, monkeypatch):
    from agent.context_references import preprocess_context_references_async

    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("NOZICH_HOME", str(tmp_path / ".nozich"))

    nozich_env = tmp_path / ".nozich" / ".env"
    nozich_env.parent.mkdir(parents=True)
    nozich_env.write_text("API_KEY=super-secret\n", encoding="utf-8")
    ...
    result = await preprocess_context_references_async(
        "read @file:.nozich/.env and @file:.ssh/id_rsa",
        cwd=tmp_path,
