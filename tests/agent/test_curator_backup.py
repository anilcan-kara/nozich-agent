...
def backup_env(monkeypatch, tmp_path):
    """Isolate NOZICH_HOME + reload modules so every test starts clean."""
    ...
    # Reload so get_nozich_home picks up the env var fresh.
...
def test_real_run_takes_pre_snapshot(backup_env, monkeypatch):
    ...
    # Reload curator module against the freshly-env'd nozich_constants
...
def _write_cron_jobs(home: Path, jobs: list) -> Path:
    """Write a synthetic cron/jobs.json under NOZICH_HOME. Returns the path.
...
def _reload_cron_jobs(home: Path):
    """Reload cron.jobs so its module-level NOZICH_DIR picks up the tmp HOME."""
    import nozich_constants
    importlib.reload(nozich_constants)
...
def test_snapshot_without_cron_jobs_file_still_succeeds(backup_env):
    ...
    cb = backup_env["cb"]
    _write_skill(backup_env["skills"], "alpha")
    # Deliberately do not create ~/.nozich/cron/jobs.json
