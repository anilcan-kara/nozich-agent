...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_nozich_env(self):
        """Nozich' own .env inside NOZICH_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: nozich internal .env is in ~/.nozich/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.nozich/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.nozich/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    """Internal Nozich cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        nozich_home = tmp_path / ".nozich"
        cache = nozich_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._nozich_home_path", return_value=nozich_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal Nozich cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        nozich_home = tmp_path / ".nozich"
        hub = nozich_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_nozich_home(self, tmp_path):
        """The env basename guard does not depend on NOZICH_HOME resolution."""
        nozich_home = tmp_path / ".nozich"
        nozich_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        nozich_home = tmp_path / ".nozich"
        cache = nozich_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
