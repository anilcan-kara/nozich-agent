"""Tests for NOZICH_HOME credential-file read blocking in file_safety.

Regression for https://github.com/anilcan-kara/nozich-agent/issues/17656 —
``read_file`` was previously only sandboxed against ``NOZICH_HOME`` itself,
...
for the credential stores while leaving arbitrary ``NOZICH_HOME`` files
...
def fake_home(tmp_path, monkeypatch):
    """Point ``_nozich_home_path()`` at a tmp dir for isolated checks."""
    import agent.file_safety as fs

    home = tmp_path / "nozich_home"
    home.mkdir()
    monkeypatch.setattr(fs, "_nozich_home_path", lambda: home)
    return home
...
def test_arbitrary_nozich_home_file_not_blocked(fake_home):
    """Non-credential files inside NOZICH_HOME stay readable."""
    from agent.file_safety import get_read_block_error
...
def test_skills_hub_block_still_applies(fake_home):
    ...
    assert err is not None
    assert "internal Nozich cache file" in err


def test_path_traversal_resolves_to_blocked(fake_home, tmp_path):
    """A path that traverses through a sibling dir back into NOZICH_HOME's
    ...
    sibling.mkdir()
    traversal = sibling / ".." / "nozich_home" / "auth.json"
...
def test_symlink_to_auth_json_blocked(fake_home, tmp_path):
    """A symlink pointing at NOZICH_HOME/auth.json from outside the home
...
def test_read_file_tool_blocks_relative_path_under_terminal_cwd(
    ...
    ``read_file_tool`` against ``TERMINAL_CWD == NOZICH_HOME`` must still
    ...
    _create(fake_home, "auth.json")
    # Force the file_tools resolver to anchor relative paths at NOZICH_HOME
...
def test_dotenv_blocked(fake_home):
    """.env in NOZICH_HOME holds API keys — blocked."""
    from agent.file_safety import get_read_block_error
...
def test_identically_named_nozich_files_outside_home_not_blocked(
    fake_home, tmp_path
):
    """Nozich-specific filenames (``auth.json``, ``mcp-tokens/``, ``google_oauth.json``)
    outside NOZICH_HOME must remain readable — the gate is per-location for
    ...
    project.mkdir()
    # auth.json outside NOZICH_HOME — readable (per-location gate).
    p = project / "auth.json"
    p.write_text("not secret here", encoding="utf-8")
    assert get_read_block_error(str(p)) is None, (
        "auth.json outside NOZICH_HOME should NOT be blocked"
    )
...
def test_profile_mode_blocks_root_credentials(tmp_path, monkeypatch):
    """Under a profile, NOZICH_HOME = <root>/profiles/<name>, but
    ...
    monkeypatch.setattr(fs, "_nozich_home_path", lambda: profile)
