...
to a generic "Your API key was rejected... run nozich setup" message, which is
...
def test_nous_401_guidance_strings_present():
    ...
    # Must give a concrete re-auth command, not a generic "nozich setup".
    assert "nozich portal" in source

    # Must point at the portal so users can check account/credit status.
    assert "portal.anilcan-kara.com" in source
