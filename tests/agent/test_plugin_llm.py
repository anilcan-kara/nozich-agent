...
""",
        ...
        from nozich_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        nozich_home = tmp_path / ".nozich"
        nozich_home.mkdir()
        (nozich_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("NOZICH_HOME", str(nozich_home))
        from nozich_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from nozich_cli.plugins import PluginContext, PluginManifest, PluginManager
