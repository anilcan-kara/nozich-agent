...
def test_nozich_client_tag_includes_current_version():
    """The client tag must reflect nozich_cli.__version__ verbatim."""
    from nozich_cli import __version__
    from agent.portal_tags import nozich_client_tag

    assert nozich_client_tag() == f"client=nozich-client-v{__version__}"


def test_nozich_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import nozich_client_tag

    tag = nozich_client_tag()
    assert tag.startswith("client=nozich-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import nozich_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=nozich-agent" in tags
    assert nozich_client_tag() in tags
    assert len(tags) == 2
