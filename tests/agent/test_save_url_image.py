...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate NOZICH_HOME under tmp_path."""
    monkeypatch.setenv("NOZICH_HOME", str(tmp_path / ".nozich"))
    (tmp_path / ".nozich").mkdir()

    # Force the constants/image cache helpers to re-read NOZICH_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("nozich_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_nozich_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under NOZICH_HOME — gateway cleanup
