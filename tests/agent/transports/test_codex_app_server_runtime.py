...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Nozich passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("NOZICH_HOME", "/users/alice/.nozich/profiles/backend-worker")
        monkeypatch.setenv("NOZICH_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "NOZICH_KANBAN_DB",
            "/users/alice/.nozich/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.nozich/kanban/boards/smoke"]'
            in cmd
        )
