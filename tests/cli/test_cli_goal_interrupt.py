...
minimal ``NozichCLI`` stub (pattern used elsewhere in tests/cli).
"""
...
def nozich_home(tmp_path, monkeypatch):
    """Isolated NOZICH_HOME so SessionDB.state_meta writes stay hermetic."""
    home = tmp_path / ".nozich"
    home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setenv("NOZICH_HOME", str(home))

    # Bust the goal module's DB cache so it re-resolves NOZICH_HOME each test.
...
def _make_cli_with_goal(session_id: str, goal_text: str = "build a thing"):
    """Build a minimal NozichCLI stub with an active goal wired in."""
...
class TestInterruptAutoPause:
    def test_interrupted_turn_pauses_goal_and_skips_continuation(self, nozich_home):
        ...
        with patch("nozich_cli.goals.judge_goal") as judge_mock:
    ...
    def test_interrupted_turn_is_resumable(self, nozich_home):
        ...
        with patch("nozich_cli.goals.judge_goal"):
            cli._maybe_continue_goal_after_turn()
...
class TestEmptyResponseSkip:
    def test_empty_response_does_not_invoke_judge(self, nozich_home):
    ...
    def test_no_assistant_message_skipped(self, nozich_home):
...
class TestHealthyTurnStillRuns:
    ...
    def test_clean_response_marks_done_when_judge_says_done(self, nozich_home):
...
class TestInterruptFlagLifecycle:
    def test_chat_resets_flag_at_entry(self, nozich_home):
        ...
        src = inspect.getsource(NozichCLI.chat)
