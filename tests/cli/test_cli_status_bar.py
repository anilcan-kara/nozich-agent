...
class TestCLIStatusBar:
    ...
    def test_scrollback_box_width_returns_viewport_width(self):
        ...
        assert NozichCLI._scrollback_box_width(20) == 32
        assert NozichCLI._scrollback_box_width(32) == 32
        # Above the floor, return the actual viewport width — no cap.
        assert NozichCLI._scrollback_box_width(48) == 48
        assert NozichCLI._scrollback_box_width(80) == 80
        assert NozichCLI._scrollback_box_width(120) == 120
        assert NozichCLI._scrollback_box_width(200) == 200
...
class TestIdleSinceLastTurn:
    ...
    def test_hidden_before_first_turn(self):
        assert NozichCLI._format_idle_since(None, turn_live=False) == ""

    def test_hidden_while_turn_is_live(self):
        assert NozichCLI._format_idle_since(time.time() - 30, turn_live=True) == ""

    def test_shows_compact_idle_time_after_turn(self):
        label = NozichCLI._format_idle_since(time.time() - 42, turn_live=False)
        assert label.startswith("✓ ")
        assert label == "✓ 42s"

    def test_scales_to_minutes(self):
        label = NozichCLI._format_idle_since(time.time() - 3 * 60, turn_live=False)
        assert label == "✓ 3m"
