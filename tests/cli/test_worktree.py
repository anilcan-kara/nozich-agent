...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("nozich/nozich-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned nozich/* and pr-* branches."""

    def test_prunes_orphaned_nozich_branch(self, git_repo):
        """nozich/nozich-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "nozich/nozich-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "nozich/nozich-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "nozich/nozich-deadbeef" in result.stdout
        ...
        assert "nozich/nozich-deadbeef" in orphaned
        ...
        assert "nozich/nozich-deadbeef" not in result.stdout
