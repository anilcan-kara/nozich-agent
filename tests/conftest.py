"""Shared fixtures for the nozich-agent test suite.
...
2. **Isolated NOZICH_HOME.** NOZICH_HOME points to a per-test tempdir so
   code reading ``~/.nozich/*`` via ``get_nozich_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".nozich"`` instead of the canonical
   ``get_nozich_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No NOZICH_SESSION_* inheritance** — the agent's current gateway
...
# NOZICH_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_NOZICH_BEHAVIORAL_VARS = frozenset({
    "NOZICH_YOLO_MODE",
    "NOZICH_INTERACTIVE",
    "NOZICH_QUIET",
    "NOZICH_TOOL_PROGRESS",
    "NOZICH_TOOL_PROGRESS_MODE",
    "NOZICH_MAX_ITERATIONS",
    "NOZICH_SESSION_PLATFORM",
    "NOZICH_SESSION_CHAT_ID",
    "NOZICH_SESSION_CHAT_NAME",
    "NOZICH_SESSION_THREAD_ID",
    "NOZICH_SESSION_SOURCE",
    "NOZICH_SESSION_KEY",
    "NOZICH_GATEWAY_SESSION",
    "NOZICH_CRON_SESSION",
    "_NOZICH_GATEWAY",
    "NOZICH_PLATFORM",
    "NOZICH_MODEL",
    "NOZICH_INFERENCE_MODEL",
    "NOZICH_INFERENCE_PROVIDER",
    "NOZICH_TUI_PROVIDER",
    "NOZICH_MANAGED",
    "NOZICH_DEV",
    "NOZICH_CONTAINER",
    ...
    "NOZICH_TIMEZONE",
    "NOZICH_REDACT_SECRETS",
    "NOZICH_BACKGROUND_NOTIFICATIONS",
    "NOZICH_EXEC_ASK",
    "NOZICH_HOME_MODE",
    "NOZICH_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.nozich/kanban.db instead of the per-test NOZICH_HOME.
    "NOZICH_KANBAN_DB",
    "NOZICH_KANBAN_BOARD",
    "NOZICH_KANBAN_HOME",
    "NOZICH_KANBAN_WORKSPACES_ROOT",
    "NOZICH_KANBAN_LOGS_ROOT",
    "NOZICH_KANBAN_TASK",
    "NOZICH_KANBAN_WORKSPACE",
    "NOZICH_KANBAN_RUN_ID",
    "NOZICH_KANBAN_CLAIM_LOCK",
    "NOZICH_KANBAN_DISPATCH_IN_GATEWAY",
    "NOZICH_TENANT",
    ...
    "NOZICH_DASHBOARD_OAUTH_CLIENT_ID",
    "NOZICH_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and NOZICH_HOME to per-test tempdirs so code that
    reads ``~/.nozich/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral NOZICH_* vars that could change test semantics.
    for name in _NOZICH_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect NOZICH_HOME to a per-test tempdir. Code that reads
    #    ``~/.nozich/*`` via ``get_nozich_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.nozich/*`` via
    #    ``Path.home() / ".nozich"`` instead of ``get_nozich_home()``
    #    is a bug to fix at the callsite.
    fake_nozich_home = tmp_path / "nozich_test"
    fake_nozich_home.mkdir()
    (fake_nozich_home / "sessions").mkdir()
    (fake_nozich_home / "cron").mkdir()
    (fake_nozich_home / "memories").mkdir()
    (fake_nozich_home / "skills").mkdir()
    monkeypatch.setenv("NOZICH_HOME", str(fake_nozich_home))
    ...
    #    ~/.nozich/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import nozich_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_nozich_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal nozich config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``nozich-gateway`` process
...
# correlated with a ``tests/nozich_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> nozich-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart nozich-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting nozich/python patterns are also blocked.
    """
    ...
    def _matches_nozich_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _NOZICH_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_nozich_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch nozich-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m nozich_cli.main".
                if (
                    "nozich" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live nozich-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting nozich/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `nozich update` (or the
        # equivalent `python -m nozich_cli.main update`).  These commands
        ...
        if "update" in low and (
            # nozich update / nozich update --gateway / setsid bash -c ... nozich update
            ("nozich" in low and "update" in low.split())
            or
            # python -m nozich_cli.main update --gateway
            ("nozich_cli" in low and "update" in low.split())
            or
            # venv/bin/nozich update  (absolute path variant used in tests)
            (".venv/bin/nozich" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`nozich update` against the real checkout, fetching "
