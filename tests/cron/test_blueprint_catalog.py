...
the shared /blueprint command handler (nozich_cli/blueprint_cmd.py), and
the docs generator. Uses an isolated NOZICH_HOME for anything that touches the
cron job store.
"""
...
class TestRenderers:
    ...
    def test_deeplink_shape(self):
        ...
        assert url.startswith("nozich://blueprint/morning-brief?")
    ...
    def test_catalog_entry_has_all_surfaces(self):
        ...
        assert entry["appUrl"].startswith("nozich://")
...
class TestCommandHandler:
    ...
    def test_name_match_is_forgiving(self, isolated_home):
        from nozich_cli.blueprint_cmd import handle_blueprint_command, match_blueprint
