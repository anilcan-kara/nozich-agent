...
Override the image with ``NOZICH_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("NOZICH_TEST_IMAGE", "nozich-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``NOZICH_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("NOZICH_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged nozich user
# ---------------------------------------------------------------------------
#
# Background: every Nozich runtime path inside the container drops to UID
# 10000 (the ``nozich`` user) via ``s6-setuidgid nozich``. ``docker exec``
...
# fails (PermissionError swallowed) for nozich, so a test that ran as root
...
# helpers below run every probe under ``-u nozich`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: nozich).
