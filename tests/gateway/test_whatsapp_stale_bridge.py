...
``nozich update``, serving pre-update bridge.js behavior forever (e.g.
...
class TestDepRefreshStamp:
    ...
    async def test_reinstalls_when_package_json_changed(self, tmp_path):
        bridge_dir = _setup_bridge_dir(tmp_path)
        _fresh_node_modules(bridge_dir)
        # Simulate `nozich update` bumping the Baileys pin
        ...
        from gateway.platforms.whatsapp import _file_content_hash
        stamp = (bridge_dir / "node_modules" / ".nozich-pkg-hash").read_text().strip()
...
class TestCacheDirEnvPassthrough:
    ...
    async def test_bridge_spawn_env_has_cache_dirs(self, tmp_path):
        ...
        assert env["NOZICH_IMAGE_CACHE_DIR"] == str(get_image_cache_dir())
        assert env["NOZICH_AUDIO_CACHE_DIR"] == str(get_audio_cache_dir())
        assert env["NOZICH_DOCUMENT_CACHE_DIR"] == str(get_document_cache_dir())
