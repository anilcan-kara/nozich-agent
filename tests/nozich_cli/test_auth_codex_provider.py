"""Tests for Codex auth — tokens stored in Nozich auth store (~/.nozich/auth.json)."""
...
def _setup_nozich_auth(nozich_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Nozich auth store."""
...
def test_read_codex_tokens_success(tmp_path, monkeypatch):
    ...
    _setup_nozich_auth(nozich_home)
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_nozich_auth(nozich_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("NOZICH_HOME", str(nozich_home))
    ...
    monkeypatch.setattr("nozich_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    nozich_home = tmp_path / "nozich"
    _setup_nozich_auth(nozich_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("NOZICH_HOME", str(nozich_home))
...
def test_save_codex_tokens_syncs_credential_pool(tmp_path, monkeypatch):
    ...
    auth = json.loads((nozich_home / "auth.json").read_text())
...
def test_save_codex_tokens_syncs_manual_device_code_entries(tmp_path, monkeypatch):
    ...
    fix landed would have run ``nozich auth add openai-codex`` as a
    ...
    clobbered independent accounts added via ``nozich auth add openai-codex``.
    ...
    (nozich_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # Independent account from `nozich auth add openai-codex` —
...
def test_save_codex_tokens_does_not_overwrite_independent_manual_entries(tmp_path, monkeypatch):
    ...
    Regression for #39236: ``nozich auth add openai-codex`` for accounts B and C
    ...
    (nozich_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # ``nozich auth add openai-codex`` (account B and account C).
...
def test_save_codex_tokens_still_refreshes_legacy_manual_alias(tmp_path, monkeypatch):
    ...
    ``nozich auth add openai-codex`` as a workaround when there was no
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Nozich auth store, not ~/.codex/."""
    nozich_home = tmp_path / "nozich"
    codex_home = tmp_path / "codex-cli"
    nozich_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
    ...
    _save_codex_tokens({"access_token": "nozich-at", "refresh_token": "nozich-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Nozich store
    assert not (codex_home / "auth.json").exists()

    # Nozich auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "nozich-at"


def test_resolve_returns_nozich_auth_store_source(tmp_path, monkeypatch):
    nozich_home = tmp_path / "nozich"
    _setup_nozich_auth(nozich_home)
    monkeypatch.setenv("NOZICH_HOME", str(nozich_home))
...
def test_refresh_429_classified_as_quota_not_auth_failure(monkeypatch):
    ...
    than a misleading "run nozich auth".
    """
    from nozich_cli.auth import (
    ...
    assert "nozich auth" not in rendered
...
def test_refresh_429_without_retry_after_header(monkeypatch):
    ...
    from nozich_cli.auth import CODEX_RATE_LIMITED_CODE
...
def test_is_rate_limited_auth_error_distinguishes_credential_errors():
    ...
    from nozich_cli.auth import CODEX_RATE_LIMITED_CODE, is_rate_limited_auth_error
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "nozich_cli.auth.resolve_codex_runtime_credentials",
    ...
    monkeypatch.setattr("nozich_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("nozich_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
