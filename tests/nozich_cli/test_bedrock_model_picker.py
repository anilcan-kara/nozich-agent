...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (NOZICH_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via NOZICH_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in NOZICH_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from nozich_cli.providers import NOZICH_OVERLAYS
        assert "bedrock" in NOZICH_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from nozich_cli.providers import NOZICH_OVERLAYS
        assert NOZICH_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from nozich_cli.providers import NOZICH_OVERLAYS
        assert NOZICH_OVERLAYS["bedrock"].auth_type == "aws_sdk"
