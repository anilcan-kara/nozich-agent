"""Tests for nozich_cli configuration management."""
...
class TestGetNozichHome:
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            ...
            assert home == Path.home() / ".nozich"

    def test_env_override(self):
        with patch.dict(os.environ, {"NOZICH_HOME": "/custom/path"}):
            home = get_nozich_home()
            assert home == Path("/custom/path")


class TestEnsureNozichHome:
    def test_creates_subdirs(self, tmp_path):
        with patch.dict(os.environ, {"NOZICH_HOME": str(tmp_path)}):
            ensure_nozich_home()
...
class TestLoadConfigParseFailure:
    ...
    their config.yaml was being ignored. The helper must:
      * log at WARNING (so ``nozich logs`` surfaces it)
    ...
    def test_logs_and_warns_on_parse_failure(self, tmp_path, caplog, capsys):
        ...
        from nozich_cli import config as cfg_mod
        ...
        with patch.dict(os.environ, {"NOZICH_HOME": str(tmp_path)}):
            ...
            import logging
            with caplog.at_level(logging.WARNING, logger="nozich_cli.config"):
                config = load_config()
            ...
            # stands out at nozich startup before logging is configured)
            captured = capsys.readouterr()
            assert "nozich config:" in captured.err
    ...
    def test_rewarns_after_file_edit(self, tmp_path, capsys):
        ...
        with patch.dict(os.environ, {"NOZICH_HOME": str(tmp_path)}):
            ...
            assert "nozich config:" in after_edit, "edited file should re-warn"
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_updates_process_environment(self, tmp_path):
        with patch.dict(os.environ, {"NOZICH_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"NOZICH_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ(self, tmp_path):
        ...
        with patch.dict(os.environ, {"NOZICH_HOME": str(tmp_path), "MY_KEY": "my_value"}):
    ...
    def test_handles_missing_env_file(self, tmp_path):
        with patch.dict(os.environ, {"NOZICH_HOME": str(tmp_path), "GHOST_KEY": "ghost"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"NOZICH_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
    ...
    def test_remove_env_value_preserves_existing_file_mode_on_posix(self, tmp_path):
        ...
        with patch.dict(os.environ, {"NOZICH_HOME": str(tmp_path), "DROP": "gone"}):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from nozich_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
    ...
    def test_max_iterations_not_offered_as_env_var(self):
        """NOZICH_MAX_ITERATIONS must NOT be in OPTIONAL_ENV_VARS (issue #17534).

        Offering it as an editable env var (dashboard, `nozich setup`) lets a
        ...
        via config.yaml; NOZICH_MAX_ITERATIONS remains a read-only backward-compat
        fallback in the gateway/CLI, never a promoted write target.
        """
        from nozich_cli.config import OPTIONAL_ENV_VARS
        assert "NOZICH_MAX_ITERATIONS" not in OPTIONAL_ENV_VARS
...
class TestEnvWriteDenylist:
    ...
    ``PATH``, ``EDITOR``, etc. — or any ``NOZICH_*`` runtime flag.
    ...
    ``LD_PRELOAD=/tmp/evil.so`` in ``.env`` and own the next Nozich
    process on next startup via the dotenv → ``os.environ`` chain in
    ``nozich_cli/env_loader.py``.
    ...
    def _nozich_home(self, tmp_path, monkeypatch):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "NOZICH_SPOTIFY_CLIENT_ID",
            "NOZICH_QWEN_BASE_URL",
            "NOZICH_MAX_ITERATIONS",
        ],
    )
    def test_nozich_integration_keys_still_writable(self, allowed_key):
        """``NOZICH_*`` overall is NOT blocked — only the four runtime
        location names (HOME/PROFILE/CONFIG/ENV) are. Integration
        credentials following the ``NOZICH_*`` convention must keep
    ...
    def test_arbitrary_user_key_still_works(self):
        ...
        denylist and outside ``NOZICH_*``) keep working. The denylist
