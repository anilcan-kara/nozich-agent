...
def container_env(tmp_path, monkeypatch):
    """Set up a fake NOZICH_HOME with .container-mode file."""
    nozich_home = tmp_path / ".nozich"
    nozich_home.mkdir()
    monkeypatch.setenv("NOZICH_HOME", str(nozich_home))
    monkeypatch.delenv("NOZICH_DEV", raising=False)

    container_mode = nozich_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=nozich-agent\n"
        "exec_user=nozich\n"
        "nozich_bin=/data/current-package/bin/nozich\n"
    )
    return nozich_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("nozich_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "nozich-agent"
    assert info["exec_user"] == "nozich"
    assert info["nozich_bin"] == "/data/current-package/bin/nozich"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("nozich_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_nozich_dev(container_env, monkeypatch):
    """Returns None when NOZICH_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("NOZICH_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_nozich_dev_zero(container_env, monkeypatch):
    """NOZICH_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("NOZICH_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        nozich_home = Path(tmpdir) / ".nozich"
        nozich_home.mkdir()
        (nozich_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("nozich_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_nozich_home": lambda: nozich_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("NOZICH_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=nozich-custom\n"
        "exec_user=myuser\n"
        "nozich_bin=/opt/nozich/bin/nozich\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "nozich-custom"
    assert info["exec_user"] == "myuser"
    assert info["nozich_bin"] == "/opt/nozich/bin/nozich"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from nozich_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "nozich"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "nozich-agent" in cmd
    assert "/data/current-package/bin/nozich" in cmd
    assert "chat" in cmd
