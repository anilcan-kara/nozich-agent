...
class TestUnifiedDashboardRouting:
    def test_profile_launch_attaches_to_running_dashboard(self, main_mod, monkeypatch):
        monkeypatch.setattr(
            "nozich_cli.profiles.get_active_profile_name", lambda: "worker_x"
        )
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        # NOZICH_HOME.  For a standard install (NOZICH_HOME unset) that root is
        # the platform-native default (~/.nozich), NOT dropped — see the Docker
        # test below for why we resolve explicitly instead of popping.
        from nozich_constants import get_default_nozich_root
        assert env.get("NOZICH_HOME") == str(get_default_nozich_root())

    def test_reexec_pins_docker_machine_root(self, main_mod, monkeypatch):
        """In the Docker layout (NOZICH_HOME=/opt/data, profiles under
        /opt/data/profiles/<name>) the reroute must pin the child to the
        machine root /opt/data — NOT drop NOZICH_HOME.

        Dropping it makes the child fall back to $HOME/.nozich
        (= /opt/data/.nozich), an empty auto-seeded home, so the dashboard
        ...
        """
        monkeypatch.setenv("NOZICH_HOME", "/opt/data/profiles/oracle")
        monkeypatch.setattr(
            "nozich_cli.profiles.get_active_profile_name", lambda: "oracle"
        )
        monkeypatch.setattr(main_mod, "_dashboard_listening", lambda host, port: False)
        execs = []
    ...
    def test_default_profile_launch_skips_routing(self, main_mod, monkeypatch):
        monkeypatch.setattr(
            "nozich_cli.profiles.get_active_profile_name", lambda: "default"
        )
        listening_calls = []
        monkeypatch.setattr(
            main_mod, "_dashboard_listening",
    ...
    def test_dashboard_starts_mcp_discovery_for_ws_backend(self, main_mod, monkeypatch):
        ...
        monkeypatch.delenv("NOZICH_WEB_DIST", raising=False)
        ...
        calls = []
        monkeypatch.setattr(
            "nozich_cli.mcp_startup.start_background_mcp_discovery",
            lambda **kwargs: calls.append(kwargs),
        )
        monkeypatch.setitem(
            sys.modules,
            "nozich_cli.web_server",
