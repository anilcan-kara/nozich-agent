"""Regression tests for #34107 — Docker UID/GID handling in ensure_nozich_home.

When Nozich runs in Docker with ``NOZICH_UID=1000`` / ``NOZICH_GID=911``,
the entrypoint chowns the top-level ``NOZICH_HOME`` once at startup. But
...
The fix is a ``_chown_to_nozich_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_nozich_uid_gid
# ---------------------------------------------------------------------------


class TestResolveNozichUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("NOZICH_UID", "1000")
        monkeypatch.setenv("NOZICH_GID", "911")
        from nozich_cli.config import _resolve_nozich_uid_gid
        uid, gid = _resolve_nozich_uid_gid()
        assert uid == 1000
        assert gid == 911

    def test_returns_none_when_unset(self, monkeypatch):
        monkeypatch.delenv("NOZICH_UID", raising=False)
        monkeypatch.delenv("NOZICH_GID", raising=False)
        from nozich_cli.config import _resolve_nozich_uid_gid
        uid, gid = _resolve_nozich_uid_gid()
        assert uid is None
        assert gid is None
    ...
    def test_invalid_uid_returns_none_for_that_field(self, monkeypatch):
        monkeypatch.setenv("NOZICH_UID", "not-a-number")
        monkeypatch.setenv("NOZICH_GID", "911")
        from nozich_cli.config import _resolve_nozich_uid_gid
        uid, gid = _resolve_nozich_uid_gid()
        assert uid is None
    ...
    def test_empty_string_treated_as_unset(self, monkeypatch):
        monkeypatch.setenv("NOZICH_UID", "")
        monkeypatch.setenv("NOZICH_GID", "")
        from nozich_cli.config import _resolve_nozich_uid_gid
        uid, gid = _resolve_nozich_uid_gid()
        assert uid is None
        assert gid is None
...
# ---------------------------------------------------------------------------
# _chown_to_nozich_uid
# ---------------------------------------------------------------------------


class TestChownToNozichUid:
    def test_calls_os_chown_when_both_set(self, tmp_path, monkeypatch):
        monkeypatch.setenv("NOZICH_UID", "1000")
        monkeypatch.setenv("NOZICH_GID", "911")
        from nozich_cli import config as cfg

        d = tmp_path / "subdir"
        d.mkdir()

        with patch.object(cfg.os, "chown") as mock_chown:
            cfg._chown_to_nozich_uid(d)
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_nozich_uid(d)  # must not raise
