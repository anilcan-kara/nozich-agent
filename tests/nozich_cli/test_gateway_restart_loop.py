...
Covers:
- Defense 1: gateway stop/restart refuse when _NOZICH_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "nozich  gateway  restart",         # double spaces
        "Hermez Gateway Restart".lower().replace("z", "s"),  # case handled
        "NOZICH GATEWAY RESTART",           # uppercase
    ])
    def test_nozich_gateway_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "launchctl kickstart gui/501/ai.nozich.gateway",
        "launchctl unload ~/Library/LaunchAgents/ai.nozich.gateway.plist",
        "launchctl stop ai.nozich.gateway",
        "systemctl restart nozich-gateway",
        "systemctl stop nozich-gateway.service",
        "systemctl start nozich-gateway",
    ])
    def test_service_manager_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "kill nozich gateway process",
        "pkill -f nozich.*gateway",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "nozich cron list",
        "nozich update",
        "nozich config set model claude",
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_nozich_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade nozich then run nozich gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_launchctl_kickstart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="0 9 * * *",
            prompt="Run launchctl kickstart -k gui/501/ai.nozich.gateway",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
...
class TestGatewaySelfTargetingGuard:
    """Verify nozich gateway stop/restart refuse when _NOZICH_GATEWAY=1."""
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_NOZICH_GATEWAY", raising=False)
