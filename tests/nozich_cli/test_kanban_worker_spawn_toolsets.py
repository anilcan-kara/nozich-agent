...
def test_default_spawn_pins_assignee_profile_cli_toolsets(monkeypatch, tmp_path):
    ...
    NOZICH_KANBAN_TASK: the worker must not collapse to only kanban lifecycle
...
""".lstrip(),
    ...
    monkeypatch.setattr(kb, "_resolve_nozich_argv", lambda: ["nozich"])
    ...
    assert pid == 4242
    assert captured["env"]["NOZICH_HOME"] == str(profile)
    assert captured["env"]["NOZICH_KANBAN_TASK"] == "t_spawn_tools"
