"""Tests for the top-level `./nozich` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./nozich` should use `nozich_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "nozich"
    called = []

    fake_main_module = types.ModuleType("nozich_cli.main")

    def fake_main():
        called.append("nozich_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "nozich_cli.main", fake_main_module)
