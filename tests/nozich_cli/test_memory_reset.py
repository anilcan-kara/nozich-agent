"""Tests for the `nozich memory reset` CLI command.
...
- Profile-scoped reset (uses NOZICH_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake NOZICH_HOME with memory files."""
    nozich_home = tmp_path / ".nozich"
    memories = nozich_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("NOZICH_HOME", str(nozich_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nNozich repo is at ~/.nozich/nozich-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `nozich memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        nozich_home = tmp_path / ".nozich"
        (nozich_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("NOZICH_HOME", str(nozich_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's NOZICH_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_nozich_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
