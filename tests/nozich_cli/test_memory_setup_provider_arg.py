"""Tests for `nozich memory setup [provider]` routing.
...
``nozich memory setup honcho``) without the interactive picker — which matters
because the per-provider ``nozich <provider>`` subcommand is only registered
...
from nozich_cli import memory_setup
...
class TestMemorySetupProviderRouting:
    ...
    def test_unknown_provider_reports_and_returns_early(self, capsys):
        ...
        assert "nozich memory setup" in out
