...
appear in ``/model`` without a Nozich release.
"""
...
class TestProviderModelIdsPreferred:
    ...
    def test_opencode_go_offline_falls_back_to_curated(self):
        ...
        with patch("agent.models_dev.list_agentic_models", return_value=[]):
            out = provider_model_ids("opencode-go")
        # Curated floor (see nozich_cli/models.py _PROVIDER_MODELS["opencode-go"])
        assert "mimo-v2-pro" in out
        assert "kimi-k2.6" in out
    ...
    def test_kimi_setup_flow_uses_same_coding_plan_catalog(self):
        ...
        from nozich_cli.model_setup_flows import _model_flow_kimi
        ...
        with (
            patch("nozich_cli.main._prompt_api_key", return_value=("sk-kimi-test", False)),
            patch("nozich_cli.auth._prompt_model_selection", side_effect=fake_select),
            patch("nozich_cli.config.get_env_value", return_value=""),
...
class TestOpenRouterAndNousUnchanged:
    ...
    def test_openrouter_does_not_call_merge(self):
        ...
        with patch(
            "nozich_cli.models._merge_with_models_dev",
