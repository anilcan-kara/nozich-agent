...
def test_all_aux_tasks_includes_plugin_registered(patched_manager):
    from nozich_cli.main import _AUX_TASKS, _all_aux_tasks
...
def test_all_aux_tasks_swallows_plugin_discovery_failure(monkeypatch):
    ...
    monkeypatch.setattr(
        "nozich_cli.plugins.get_plugin_auxiliary_tasks", _broken
    )
...
def test_reset_aux_to_auto_resets_plugin_tasks(tmp_path, monkeypatch, patched_manager):
    ...
    from nozich_cli.main import _reset_aux_to_auto

    monkeypatch.setenv("NOZICH_HOME", str(tmp_path / ".nozich"))
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    (tmp_path / ".nozich").mkdir(exist_ok=True)
