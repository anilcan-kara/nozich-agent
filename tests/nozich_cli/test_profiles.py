"""Comprehensive tests for nozich_cli.profiles module.
...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and NOZICH_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.nozich/profiles)
    * NOZICH_HOME  -> tmp_path/.nozich  (so get_nozich_home() agrees)
    * Creates the bare-minimum ~/.nozich directory.
    """
...
class TestValidateProfileName:
    ...
    @pytest.mark.parametrize("name", ["nozich", "test", "tmp", "root", "sudo"])
    def test_reserved_names_rejected(self, name):
        """Reserved names collide with the Nozich install itself or with
...
class TestCreateProfile:
    ...
    def test_clone_all_excludes_sibling_profiles_tree(self, profile_env):
        """--clone-all from default ~/.nozich must not copy profiles/* (nested explosion)."""
        tmp_path = profile_env
        default_home = tmp_path / ".nozich"
    ...
    def test_clone_all_excludes_default_infrastructure(self, profile_env):
        """--clone-all from default profile excludes nozich-agent, .worktrees,
        ...
        (default_home / "nozich-agent" / ".git").mkdir(parents=True)
        (default_home / "nozich-agent" / "venv" / "bin").mkdir(parents=True)
        (default_home / "nozich-agent" / "README.md").write_text("repo")
        ...
        assert not (profile_dir / "nozich-agent").exists()
...
class TestNoSkillsOptOut:
    """Tests for `nozich profile create --no-skills` and the opt-out marker."""
    ...
    def test_seed_profile_skills_respects_marker(self, profile_env):
        ...
        called directly (e.g. by `nozich update`'s all-profile sync loop)."""
        profile_dir = create_profile("orchestrator", no_alias=True, no_skills=True)
...
class TestBackfillProfileEnvs:
    """Tests for backfill_profile_envs() — the `nozich update` pass that
    ...
    def test_copies_default_env_into_envless_profiles(self, profile_env):
        import stat
        tmp_path = profile_env
        (tmp_path / ".nozich" / ".env").write_text("OPENROUTER_API_KEY=root-key\n")
    ...
    def test_never_overwrites_existing_profile_env(self, profile_env):
        tmp_path = profile_env
        (tmp_path / ".nozich" / ".env").write_text("KEY=root\n")
...
class TestDeleteProfile:
    ...
    def test_removes_directory(self, profile_env):
        ...
        with patch("nozich_cli.profiles._cleanup_gateway_service"):
    ...
    def test_rmtree_failure_raises(self, profile_env):
        ...
        with patch("nozich_cli.profiles._cleanup_gateway_service"), \
             patch("nozich_cli.profiles.shutil.rmtree", side_effect=PermissionError("locked")):
...
class TestActiveProfile:
    ...
    def test_empty_file_returns_default(self, profile_env):
        tmp_path = profile_env
        active_path = tmp_path / ".nozich" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_default_nozich_home_returns_default(self, profile_env):
        # NOZICH_HOME points to tmp_path/.nozich which is the default
        assert get_active_profile_name() == "default"

    def test_profile_path_returns_profile_name(self, profile_env, monkeypatch):
        tmp_path = profile_env
        create_profile("coder", no_alias=True)
        profile_dir = tmp_path / ".nozich" / "profiles" / "coder"
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom NOZICH_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("NOZICH_HOME", str(custom))
        # With Docker-aware roots, a custom NOZICH_HOME is the default —
...
class TestAliasCollision:
    ...
    def test_reserved_name_returns_message(self, profile_env):
        result = check_alias_collision("nozich")
        assert result is not None
...
class TestWrapperScript:
    ...
    def test_creates_sh_on_posix(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        monkeypatch.setattr("nozich_cli.profiles.shutil.which", lambda name: "/opt/nozich/bin/nozich")
        from nozich_cli.profiles import create_wrapper_script
        wrapper = create_wrapper_script("mybot")
        assert wrapper is not None
        assert wrapper.name == "mybot"
        content = wrapper.read_text()
        assert content.startswith("#!/bin/sh")
        assert "exec /opt/nozich/bin/nozich -p mybot" in content
    ...
    def test_creates_bat_on_windows(self, profile_env, monkeypatch):
        ...
        assert "nozich -p mybot" in content
        assert "%*" in content

    def test_remove_finds_bat_on_windows(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "win32")
        from nozich_cli.profiles import create_wrapper_script, remove_wrapper_script
        wrapper = create_wrapper_script("mybot")
        assert wrapper is not None
    ...
    def test_remove_returns_false_when_absent(self, profile_env):
        from nozich_cli.profiles import remove_wrapper_script
    ...
    def test_custom_alias_target_on_posix(self, profile_env, monkeypatch):
        ...
        content = wrapper.read_text()
        assert content.startswith("#!/bin/sh")
        assert "nozich -p redqueen" in content
...
class TestFindAliasForProfile:
    ...
    def test_profile_named_alias(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        from nozich_cli.profiles import create_wrapper_script, find_alias_for_profile
    ...
    def test_no_alias_returns_none(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        from nozich_cli.profiles import find_alias_for_profile
    ...
    def test_ignores_unrelated_files(self, profile_env, monkeypatch):
        ...
        monkeypatch.setattr("sys.platform", "darwin")
        from nozich_cli.profiles import _get_wrapper_dir, find_alias_for_profile
...
class TestRenameProfile:
    ...
    def test_renames_root_honcho_host_without_changing_ai_peer(self, profile_env):
        ...
        assert "nozich.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["nozich_heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["nozich_heimdall"]["peerName"] == "user-peer"
    ...
    def test_pins_ai_peer_when_absent_on_honcho_host_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "nozich.ssi_health": {"workspace": "nozich", "enabled": True}
            }
        }))
        ...
        assert "nozich.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["nozich_heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["nozich_heimdall"]["workspace"] == "nozich"
    ...
    def test_does_not_overwrite_existing_honcho_host_on_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "nozich.ssi_health": {"aiPeer": "ssi_health"},
                "nozich_heimdall": {"aiPeer": "heimdall"},
            }
        }))
        ...
        cfg = json.loads(honcho_path.read_text())
        assert cfg["hosts"]["nozich.ssi_health"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["nozich_heimdall"]["aiPeer"] == "heimdall"
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for d in ("nozich-agent", ".worktrees", "profiles", "bin",
        ...
        for f in ("state.db", "gateway.pid", "gateway_state.json",
                  "processes.json", "errors.log", ".nozich_history",
        ...
        excluded_prefixes = [
            "default/nozich-agent", "default/.worktrees", "default/profiles",
        ...
        excluded_files = [
            ...
            "default/errors.log", "default/.nozich_history",
...
# ===================================================================
# TestGetProfilesRoot / TestGetDefaultNozichHome (internal helpers)
# ===================================================================

class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_nozich_home()."""

    def test_profiles_root_under_home(self, profile_env):
        tmp_path = profile_env
        root = _get_profiles_root()
        assert root == tmp_path / ".nozich" / "profiles"

    def test_default_nozich_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_nozich_home()
        assert home == tmp_path / ".nozich"

    def test_profiles_root_docker_deployment(self, tmp_path, monkeypatch):
        """In Docker (NOZICH_HOME outside ~/.nozich), profiles go under NOZICH_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NOZICH_HOME", str(docker_home))
        root = _get_profiles_root()
        assert root == docker_home / "profiles"

    def test_default_nozich_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_nozich_home() returns NOZICH_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NOZICH_HOME", str(docker_home))
    ...
    def test_profiles_root_profile_mode(self, tmp_path, monkeypatch):
        """In profile mode (NOZICH_HOME under ~/.nozich), profiles root is still ~/.nozich/profiles."""
        native = tmp_path / ".nozich"
    ...
    def test_active_profile_path_docker(self, tmp_path, monkeypatch):
        """In Docker, active_profile file lives under NOZICH_HOME."""
        from nozich_cli.profiles import _get_active_profile_path
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NOZICH_HOME", str(docker_home))
    ...
    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under NOZICH_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NOZICH_HOME", str(docker_home))
    ...
    def test_active_profile_name_docker_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NOZICH_HOME", str(profile))
...
class TestEdgeCases:
    ...
    def test_create_profile_returns_correct_path(self, profile_env):
        ...
        expected = tmp_path / ".nozich" / "profiles" / "mybot"
        assert result == expected
    ...
    def test_gateway_running_check_with_pid_file(self, profile_env):
        ...
        from nozich_cli.profiles import _check_gateway_running
        tmp_path = profile_env
        default_home = tmp_path / ".nozich"
