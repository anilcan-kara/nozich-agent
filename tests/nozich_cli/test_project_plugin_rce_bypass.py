...
Execution via the ``NOZICH_ENABLE_PROJECT_PLUGINS`` bypass in the web
server's dashboard plugin loader.

Two primitives combined into the original advisory chain:

1. ``nozich_cli.web_server._discover_dashboard_plugins`` opted into
   the untrusted ``./.nozich/plugins/`` source via
   ``os.environ.get("NOZICH_ENABLE_PROJECT_PLUGINS")`` — truthy for
...
2. ``nozich_cli.web_server._mount_plugin_api_routes`` then imported
...
# ---------------------------------------------------------------------------
# Layer 1 — NOZICH_ENABLE_PROJECT_PLUGINS env gate uses truthy semantics.
# ---------------------------------------------------------------------------
...
class TestProjectPluginsEnvGate:
    ...
    def project_plugin(self, tmp_path, monkeypatch):
        """Plant a project-source plugin under CWD's ``.nozich/plugins``
        and isolate the user-plugins dir to an empty tmp tree."""
        monkeypatch.setenv("NOZICH_HOME", str(tmp_path / "home"))
        (tmp_path / "home").mkdir()
    ...
    def test_falsy_values_keep_project_plugins_disabled(
        ...
        if value == "":
            monkeypatch.delenv("NOZICH_ENABLE_PROJECT_PLUGINS", raising=False)
        else:
            monkeypatch.setenv("NOZICH_ENABLE_PROJECT_PLUGINS", value)

        plugins = web_server._get_dashboard_plugins(force_rescan=True)
        names = {p["name"] for p in plugins}
        assert "evil" not in names, (
            f"NOZICH_ENABLE_PROJECT_PLUGINS={value!r} must NOT enable the "
...
class TestEndToEndPocBlocked:
    ...
    ``NOZICH_ENABLE_PROJECT_PLUGINS=0`` (so the operator believed the
    ...
    def test_full_chain_blocked(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("NOZICH_ENABLE_PROJECT_PLUGINS", "0")
        ...
        for call in spec.call_args_list:
            ...
            assert module_name != "nozich_dashboard_plugin_evil"
            assert target != payload_py
            assert "evil-repo" not in target.parts
        assert "nozich_dashboard_plugin_evil" not in sys.modules
