"""Tests for `nozich chat --safe-mode` — pristine troubleshooting runs.
...
_VARS = ("NOZICH_SAFE_MODE", "NOZICH_IGNORE_USER_CONFIG", "NOZICH_IGNORE_RULES")
...
class TestSafeModeEnvWiring:
    ...
    def test_safe_mode_sets_all_gates(self):
        # Mirrors the cmd_chat logic in nozich_cli/main.py.
        class Args:
            safe_mode = True
        ...
        assert os.environ.get("NOZICH_SAFE_MODE") == "1"
...
class TestSafeModePluginDiscovery:
    """Plugin discovery must be a no-op under NOZICH_SAFE_MODE=1."""

    def test_discovery_skipped(self, monkeypatch):
        monkeypatch.setenv("NOZICH_SAFE_MODE", "1")
    ...
    def test_discovery_runs_without_safe_mode(self, monkeypatch):
        monkeypatch.delenv("NOZICH_SAFE_MODE", raising=False)
        from nozich_cli.plugins import PluginManager
...
class TestSafeModeMCP:
    """_load_mcp_config must return no servers under NOZICH_SAFE_MODE=1."""
...
class TestSafeModeParser:
    """--safe-mode must parse on both the root parser and `nozich chat`."""
