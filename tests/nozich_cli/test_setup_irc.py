"""Tests for IRC gateway configuration via `nozich setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Nozich install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Nozich install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#nozich")
            monkeypatch.setenv("IRC_NICKNAME", "nozich-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `nozich setup gateway` experience with IRC present."""
