...
import pytest
from nozich_cli.config import DEFAULT_CONFIG, load_config, save_config
...
class TestNonInteractiveSetup:
    ...
    def test_cmd_setup_allows_noninteractive_flag_without_tty(self):
        ...
        from nozich_cli.main import cmd_setup

        args = _make_setup_args(non_interactive=True)

        with (
            patch("nozich_cli.setup.run_setup_wizard") as mock_run_setup,
    ...
    def test_cmd_setup_defers_no_tty_handling_to_setup_wizard(self):
        """Bare `nozich setup` should reach setup.py, which prints headless guidance."""
        from nozich_cli.main import cmd_setup
    ...
    def test_non_interactive_flag_skips_wizard(self, capsys):
        ...
        with (
            patch("nozich_cli.setup.ensure_nozich_home"),
            patch("nozich_cli.setup.load_config", return_value={}),
            patch("nozich_cli.setup.get_nozich_home", return_value="/tmp/.nozich"),
            patch("nozich_cli.auth.get_active_provider", side_effect=AssertionError("wizard continued")),
            patch("builtins.input", side_effect=AssertionError("input should not be called")),
        ):
            run_setup_wizard(args)

        out = capsys.readouterr().out
        assert "nozich config set model.provider custom" in out
    ...
    def test_chat_first_run_headless_skips_setup_prompt(self, capsys):
        """Bare `nozich` should not prompt for input when no provider exists and stdin is headless."""
        from nozich_cli.main import cmd_chat

        args = _make_chat_args()

        with (
            patch("nozich_cli.main._has_any_provider_configured", return_value=False),
            patch("nozich_cli.main.cmd_setup") as mock_setup,
            patch("sys.stdin") as mock_stdin,
            patch("builtins.input", side_effect=AssertionError("input should not be called")),
        ):
            mock_stdin.isatty.return_value = False
    ...
    def test_main_accepts_tts_setup_section(self, monkeypatch):
        """`nozich setup tts` should parse and dispatch like other setup sections."""
        ...
        monkeypatch.setattr("sys.argv", ["nozich", "setup", "tts"])
