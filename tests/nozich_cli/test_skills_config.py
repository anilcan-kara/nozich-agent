"""Tests for nozich_cli/skills_config.py and skills_tool disabled filtering."""
...
class TestSaveDisabledSkills:
    @patch("nozich_cli.skills_config.save_config")
    def test_saves_global_sorted(self, mock_save):
        from nozich_cli.skills_config import save_disabled_skills
        config = {}
...
class TestIsSkillDisabled:
    @patch("nozich_cli.config.load_config")
    ...
    @patch("nozich_cli.config.load_config")
    @patch.dict("os.environ", {"NOZICH_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_explicit_platform_param(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("NOZICH_PLATFORM", raising=False)
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """NOZICH_SESSION_PLATFORM should be used when NOZICH_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_nozich_platform_takes_precedence(self, tmp_path, monkeypatch):
        """NOZICH_PLATFORM should win over NOZICH_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
        ...
        monkeypatch.setenv("NOZICH_HOME", str(tmp_path))
        monkeypatch.setenv("NOZICH_PLATFORM", "telegram")
        monkeypatch.setenv("NOZICH_SESSION_PLATFORM", "discord")
    ...
    def test_explicit_param_overrides_env_vars(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("NOZICH_HOME", str(tmp_path))
        monkeypatch.setenv("NOZICH_PLATFORM", "telegram")
        monkeypatch.setenv("NOZICH_SESSION_PLATFORM", "telegram")
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from nozich_cli.skills_config import _get_categories
        skills = [
