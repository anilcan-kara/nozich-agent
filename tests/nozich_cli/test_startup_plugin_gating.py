...
``nozich_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``nozich --help``, ``nozich version``,
``nozich logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``nozich --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from nozich_cli import main as _main

    argv_backup = sys.argv[:]
    sys.argv = ["nozich", "--help"]
    buf = io.StringIO()
    try:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["nozich", "-w", "--tui"], None),
        (["nozich", "version"], "version"),
        (["nozich", "--tui", "chat"], "chat"),
        (["nozich", "-w", "logs"], "logs"),
        (["nozich", "chat", "hello world"], "chat"),
        (["nozich", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["nozich", "-m", "gpt5", "chat"], "chat"),
        (["nozich", "--model", "gpt5", "chat", "hi"], "chat"),
        (["nozich", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["nozich", "-z", "hello world"], None),
        (["nozich", "-z", "hello", "chat"], "chat"),
        (["nozich", "--model=gpt5", "chat"], "chat"),     # inline form
        (["nozich", "--", "chat"], "chat"),               # -- terminator
        (["nozich", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["nozich", "some-plugin-cmd"], "some-plugin-cmd"),
        (["nozich", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["nozich"],                          # bare → chat
        ["nozich", "--help"],                # top-level help
        ["nozich", "-h"],
        ["nozich", "version"],               # known built-in
        ["nozich", "logs"],
        ["nozich", "gateway", "run"],
        ["nozich", "--tui"],
        ["nozich", "-w", "--tui"],
        ["nozich", "chat", "hi"],
        ["nozich", "help"],                  # accepted built-in-ish
        ["nozich", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["nozich", "meet", "join"],          # potential google_meet plugin
        ["nozich", "honcho", "status"],      # potential memory plugin
        ["nozich", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``nozich help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"nozich_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
