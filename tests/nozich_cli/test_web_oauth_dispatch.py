"""Regression tests for the OAuth dispatcher in nozich_cli.web_server.
...
from nozich_cli.web_server import _SESSION_TOKEN, app

client = TestClient(app)
HEADERS = {"X-Nozich-Session-Token": _SESSION_TOKEN}
...
def _fake_nous_device_data():
    return {
        ...
        "user_code": "NOUS-1234",
        "verification_uri": "https://portal.anilcan-kara.com/device",
        "verification_uri_complete": (
            "https://portal.anilcan-kara.com/device?user_code=NOUS-1234"
        ),
        "expires_in": 600,
        "interval": 5,
    }
...
def _invoke_scope_refusal():
    request = httpx.Request("POST", "https://portal.anilcan-kara.com/oauth/device/code")
...
def test_minimax_login_does_not_launch_anthropic_flow():
    ...
    with patch(
        "nozich_cli.auth._minimax_request_user_code",
        return_value=fake_user_code_resp,
    ), patch(
        "nozich_cli.auth._minimax_pkce_pair",
        return_value=("verifier-stub", "challenge-stub", "stub-state"),
    ), patch(
        "nozich_cli.web_server._minimax_poller",
        return_value=None,
    ):
        resp = client.post(
...
def test_nous_dashboard_device_flow_ignores_legacy_scope_override(monkeypatch):
    ...
    from nozich_cli import web_server as ws
...
def test_oauth_provider_status_uses_profile_query(tmp_path, monkeypatch):
    ...
    def fake_status():
        observed_homes.append(get_nozich_home())
    ...
    fake_catalog = ({
        ...
        "name": "Fake OAuth",
        "flow": "pkce",
        "cli_command": "nozich auth add fake-oauth",
...
def test_nous_dashboard_device_flow_does_not_retry_legacy_scope_on_invoke_refusal(monkeypatch):
    ...
    monkeypatch.delenv("NOZICH_AGENT_USE_LEGACY_SESSION_KEYS", raising=False)
    monkeypatch.setattr(auth_mod, "_request_device_code", fake_request_device_code)
    monkeypatch.setattr(ws, "_nous_poller", lambda sid: None)
...
def test_codex_dashboard_worker_persists_inside_session_profile(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        auth_mod,
        "_save_codex_tokens",
        lambda tokens: saved_homes.append(get_nozich_home()),
    )
...
def test_minimax_dashboard_poller_accepts_absolute_ms_expired_in():
    ...
    try:
        with patch(
            "nozich_cli.auth._minimax_poll_token",
            return_value={
                "status": "success",
                "access_token": "access",
            ...
            "nozich_cli.auth._minimax_save_auth_state",
...
def test_anthropic_pkce_branch_still_works():
    ...
    with patch(
        "nozich_cli.web_server._start_anthropic_pkce",
...
def test_oauth_catalog_marks_external_providers_not_disconnectable():
    """External CLI credentials are visible in Accounts but cannot be removed by Nozich."""
    resp = client.get("/api/providers/oauth", headers=HEADERS)
    assert resp.status_code == 200, resp.text
    providers = {p["id"]: p for p in resp.json()["providers"]}
...
def test_unknown_pkce_provider_rejected_cleanly():
    ...
    fake_entry = {
        ...
        "flow": "pkce",
        "cli_command": "nozich auth add hypothetical-pkce-provider",
        "docs_url": "https://example.com",
        "status_fn": None,
    }
