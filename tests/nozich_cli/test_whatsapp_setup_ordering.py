...
Before the fix, ``nozich whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``nozich gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    nozich = home / ".nozich"
    nozich.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("NOZICH_HOME", str(nozich))
...
def _env_value(nozich_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    monkeypatch.setattr("nozich_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``nozich whatsapp`` with an existing paired session and
