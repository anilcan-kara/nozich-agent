...
Run from the PR worktree:
    ...
    cd ~/.nozich/nozich-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.nozich/nozich-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.nozich/nozich-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a nozich-agent checkout"
)
assert (PR_DIR / "tools" / "browser_tool.py").exists(), (
    f"PR_DIR={PR_DIR} doesn't look like a nozich-agent checkout"
)
...
# Isolated NOZICH_HOME for the config write.
