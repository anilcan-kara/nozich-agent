...
Verifies that:
 ...
 2. User plugins at $NOZICH_HOME/plugins/model-providers/<name>/ override bundled
...
def _clear_provider_caches():
    ...
    for mod in list(sys.modules.keys()):
        if (
            ...
            or mod.startswith("_nozich_user_provider")
        ):
            del sys.modules[mod]
...
def test_general_plugin_manager_skips_model_provider_kind(tmp_path, monkeypatch):
    ...
    from nozich_cli import plugins as plugin_mod
    ...
    user_plugin = nozich_home / "plugins" / "test-model-provider"
