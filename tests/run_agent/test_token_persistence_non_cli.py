...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    nozich_state = ModuleType("nozich_state")
    nozich_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "nozich_state", nozich_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Nozich"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Nozich"
