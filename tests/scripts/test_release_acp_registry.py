        ...
        json.dumps(
            {
                ...
                "distribution": {
                    "uvx": {
                        "package": f"nozich-agent[acp]=={version}",
                        "args": ["nozich-acp"],
                    }
                },
            },
...
def test_update_acp_registry_versions_bumps_manifest_and_pin(monkeypatch, tmp_path):
    ...
    assert manifest["distribution"]["uvx"]["package"] == "nozich-agent[acp]==0.14.0"
    # args stay untouched so we don't accidentally rewrite them.
    assert manifest["distribution"]["uvx"]["args"] == ["nozich-acp"]
...
def test_update_version_files_bumps_manifest_alongside_pyproject(
    ...
    version_dir = tmp_path / "nozich_cli"
    version_dir.mkdir()
