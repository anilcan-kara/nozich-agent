...
class TestNozichConstantsFallback:
    """Tests for _nozich_home.py fallback when nozich_constants is unavailable."""

    HELPER_PATH = (
        Path(__file__).resolve().parents[2]
        / "skills/productivity/google-workspace/scripts/_nozich_home.py"
    )

    def _load_helper(self, monkeypatch):
        """Load _nozich_home.py with nozich_constants blocked."""
        monkeypatch.setitem(sys.modules, "nozich_constants", None)
        spec = importlib.util.spec_from_file_location("_nozich_home_test", self.HELPER_PATH)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        return module

    def test_fallback_uses_nozich_home_env_var(self, monkeypatch, tmp_path):
        """When nozich_constants is missing, NOZICH_HOME comes from env var."""
        monkeypatch.setenv("NOZICH_HOME", str(tmp_path / "custom-nozich"))
        module = self._load_helper(monkeypatch)
        assert module.get_nozich_home() == tmp_path / "custom-nozich"

    def test_fallback_defaults_to_dot_nozich(self, monkeypatch):
        """When nozich_constants is missing and NOZICH_HOME unset, default to ~/.nozich."""
        monkeypatch.delenv("NOZICH_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.get_nozich_home() == Path.home() / ".nozich"

    def test_fallback_ignores_empty_nozich_home(self, monkeypatch):
        """Empty/whitespace NOZICH_HOME is treated as unset."""
        monkeypatch.setenv("NOZICH_HOME", "  ")
        module = self._load_helper(monkeypatch)
        assert module.get_nozich_home() == Path.home() / ".nozich"

    def test_fallback_display_nozich_home_shortens_path(self, monkeypatch):
        """Fallback display_nozich_home() uses ~/ shorthand like the real one."""
        monkeypatch.delenv("NOZICH_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_nozich_home() == "~/.nozich"

    def test_fallback_display_nozich_home_profile_path(self, monkeypatch):
        """Fallback display_nozich_home() handles profile paths under ~/."""
        monkeypatch.setenv("NOZICH_HOME", str(Path.home() / ".nozich/profiles/coder"))
        module = self._load_helper(monkeypatch)
        assert module.display_nozich_home() == "~/.nozich/profiles/coder"

    def test_fallback_display_nozich_home_custom_path(self, monkeypatch):
        """Fallback display_nozich_home() returns full path for non-home locations."""
        monkeypatch.setenv("NOZICH_HOME", "/opt/nozich-custom")
        module = self._load_helper(monkeypatch)
        assert module.display_nozich_home() == "/opt/nozich-custom"

    def test_delegates_to_nozich_constants_when_available(self):
        """When nozich_constants IS importable, _nozich_home delegates to it."""
        spec = importlib.util.spec_from_file_location(
            "_nozich_home_happy", self.HELPER_PATH
        )
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        import nozich_constants
        assert module.get_nozich_home is nozich_constants.get_nozich_home
        assert module.display_nozich_home is nozich_constants.display_nozich_home
...
class TestInstallDeps:
    ...
    Regression coverage for the Nozich Docker image, whose venv is built with
    ...
    def test_returns_false_when_no_pip_and_no_uv(self, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "nozich-agent[google]" in out
