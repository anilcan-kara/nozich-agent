...
binary download are exercised manually by `nozich secrets bitwarden
...
def nozich_home(tmp_path, monkeypatch):
    """Point Nozich at an isolated home directory."""
    home = tmp_path / ".nozich"
    home.mkdir()
    monkeypatch.setenv("NOZICH_HOME", str(home))
    # Some modules cache get_nozich_home; clear if needed.
    import nozich_constants
    if hasattr(nozich_constants, "_NOZICH_HOME_CACHE"):
        nozich_constants._NOZICH_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_rejects_malicious_member(nozich_home, monkeypatch):
...
def test_install_bws_happy_path(nozich_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(nozich_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(nozich_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from nozich_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under nozich_home/cache/."""
    home = tmp_path / ".nozich"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
