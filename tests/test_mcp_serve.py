"""
Tests for mcp_serve — Nozich MCP server.
...
def _isolate_nozich_home(tmp_path, monkeypatch):
    """Redirect NOZICH_HOME to a temp directory."""
    monkeypatch.setenv("NOZICH_HOME", str(tmp_path))
    try:
        import nozich_constants
        monkeypatch.setattr(nozich_constants, "get_nozich_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking nozich_state schema."""
