"""Tests for nozich_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Nozich entry point
(nozich, nozich-agent, nozich-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Nozich entry point imports nozich_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported nozich_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("nozich_bootstrap", None)
    import nozich_bootstrap  # noqa: WPS433
    return nozich_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Nozich CLI
...
class TestEntryPointsImportBootstrap:
    """Every Nozich entry point must import nozich_bootstrap as its
    ...
    # Entry points that invoke Nozich as a process.  Each one must
    # import nozich_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "nozich_cli/main.py",   # nozich CLI (console_script)
        "run_agent.py",          # nozich-agent (console_script)
        "acp_adapter/entry.py",  # nozich-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import nozich_bootstrap' and that
        ...
        half-finished ``nozich update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``nozich_bootstrap`` as a top-level module) leaves nozich
        ...
        """
        # Resolve relative to the nozich-agent repo root.  Tests live
        # at tests/test_nozich_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # nozich start even when nozich_bootstrap hasn't been
        ...
        assert first_import_name == "nozich_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'nozich_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import nozich_bootstrap' line to be the first import."
        )
