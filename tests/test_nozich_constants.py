"""Tests for nozich_constants module."""
...
class TestGetDefaultNozichRoot:
    """Tests for get_default_nozich_root() — Docker/custom deployment awareness."""

    def test_no_nozich_home_returns_native(self, tmp_path, monkeypatch):
        """When NOZICH_HOME is not set, returns ~/.nozich."""
        monkeypatch.delenv("NOZICH_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_nozich_root() == tmp_path / ".nozich"

    def test_nozich_home_is_native(self, tmp_path, monkeypatch):
        """When NOZICH_HOME = ~/.nozich, returns ~/.nozich."""
        native = tmp_path / ".nozich"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NOZICH_HOME", str(native))
        assert get_default_nozich_root() == native

    def test_nozich_home_is_profile(self, tmp_path, monkeypatch):
        """When NOZICH_HOME is a profile under ~/.nozich, returns ~/.nozich."""
        native = tmp_path / ".nozich"
    ...
    def test_nozich_home_is_docker(self, tmp_path, monkeypatch):
        """When NOZICH_HOME points outside ~/.nozich (Docker), returns NOZICH_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NOZICH_HOME", str(docker_home))
        assert get_default_nozich_root() == docker_home

    def test_nozich_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any NOZICH_HOME outside ~/.nozich is treated as the root."""
        custom = tmp_path / "my-nozich-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NOZICH_HOME", str(custom))
        assert get_default_nozich_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (NOZICH_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NOZICH_HOME", str(profile))
        assert get_default_nozich_root() == docker_root

    def test_no_nozich_home_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
        """Native Windows falls back to %LOCALAPPDATA%\\nozich, not ~/.nozich."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("NOZICH_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(nozich_constants.sys, "platform", "win32")

        assert get_default_nozich_root() == local_appdata / "nozich"

    def test_no_nozich_home_uses_windows_path_when_localappdata_missing(self, tmp_path, monkeypatch):
        """Windows fallback still uses AppData/Local/nozich without LOCALAPPDATA."""
        home = tmp_path / "Home"
        monkeypatch.delenv("NOZICH_HOME", raising=False)
        ...
        assert get_default_nozich_root() == home / "AppData" / "Local" / "nozich"


class TestGetNozichHome:
    """Tests for get_nozich_home() platform-aware fallback."""

    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When NOZICH_HOME is unset on Windows, use %LOCALAPPDATA%\\nozich."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("NOZICH_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(nozich_constants.sys, "platform", "win32")
        monkeypatch.setattr(nozich_constants, "_profile_fallback_warned", False)

        assert get_nozich_home() == local_appdata / "nozich"
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(nozich_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(nozich_constants, "_container_detected", True)
        assert is_container() is True
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".nozich"
