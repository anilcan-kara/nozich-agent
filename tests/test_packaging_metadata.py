...
def test_every_on_disk_subpackage_is_covered_by_packages_find():
    ...
    top-level package is listed twice — bare (``nozich_cli``) for the package
    itself and ``nozich_cli.*`` for its subpackages — EXCEPT when someone
    forgets the wildcard. v0.15.x listed ``nozich_cli`` without ``nozich_cli.*``,
    so the wheel shipped ``nozich_cli/*.py`` but dropped the ``dashboard_auth``
    and ``proxy`` subpackages. The dashboard then died on every install with
    ``ModuleNotFoundError: No module named 'nozich_cli.dashboard_auth'``.
...
def test_packaging_declared_as_core_dependency():
    ...
    nozich_cli/main.py) yet was undeclared, so it only reached users
...
def test_bundled_plugin_manifests_ship_in_both_wheel_and_sdist():
    ...
    Plugin discovery (nozich_cli/plugins.py) registers each bundled plugin by
...
def test_optional_mcps_manifests_ship_in_both_wheel_and_sdist():
    ...
    nozich_cli/mcp_catalog.py resolves the catalog via get_optional_mcps_dir()
    ...
    `nozich mcp catalog` and the dashboard catalog screen come up empty on
