...
class TestToolsetConsistency:
    ...
    def test_nozich_platforms_share_core_tools(self):
        """All nozich-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on nozich-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["nozich-cli", "nozich-telegram", "nozich-discord", "nozich-whatsapp", "nozich-slack", "nozich-signal", "nozich-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_nozich_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("nozich-whatsapp")

    def test_nozich_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("nozich-api-server")
