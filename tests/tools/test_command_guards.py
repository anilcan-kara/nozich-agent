...
def _clean_state():
    ...
    saved = {}
    for k in ("NOZICH_INTERACTIVE", "NOZICH_GATEWAY_SESSION", "NOZICH_EXEC_ASK", "NOZICH_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["NOZICH_INTERACTIVE"] = "1"
        session_key = os.getenv("NOZICH_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No NOZICH_INTERACTIVE or NOZICH_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
...
class TestGatewayApprovalAllowPermanent:
    ...
    def _capture_gateway_payload(self, command, session_key):
        ...
        os.environ["NOZICH_SESSION_KEY"] = session_key
        try:
            check_all_command_guards(command, "local")
        finally:
            os.environ.pop("NOZICH_GATEWAY_SESSION", None)
            os.environ.pop("NOZICH_EXEC_ASK", None)
            os.environ.pop("NOZICH_SESSION_KEY", None)
