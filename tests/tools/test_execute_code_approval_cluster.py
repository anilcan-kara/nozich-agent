...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad NOZICH_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: NOZICH_GATEWAY_SESSION set, a bound session
    ...
    monkeypatch.delenv("NOZICH_CRON_SESSION", raising=False)
...
def test_guard_cron_deny_blocks(monkeypatch):
    monkeypatch.setenv("NOZICH_CRON_SESSION", "1")
    monkeypatch.delenv("NOZICH_GATEWAY_SESSION", raising=False)
    monkeypatch.setattr(A, "_get_approval_mode", lambda: "manual")
...
def test_env_scrub_nozich_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "NOZICH_HOME": "/h", "NOZICH_PROFILE": "p",
        "NOZICH_CONFIG": "/c.yaml", "NOZICH_ENV": "/e",
        # other NOZICH_* → dropped (broad prefix removed)
        "NOZICH_BASE_URL": "https://x", "NOZICH_INTERACTIVE": "1",
        "NOZICH_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in ("NOZICH_HOME", "NOZICH_PROFILE", "NOZICH_CONFIG", "NOZICH_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "NOZICH_BASE_URL", "NOZICH_INTERACTIVE", "NOZICH_KANBAN_DB",
...
def test_env_scrub_logs_dropped_nozich_vars(caplog):
    """Dropping a non-allowlisted, non-secret NOZICH_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset NOZICH_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "NOZICH_HOME": "/h",          # allowlisted → kept, not logged
        "NOZICH_BASE_URL": "https://x",   # dropped → logged
        "NOZICH_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "NOZICH_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "NOZICH_HOME" in out and "PATH" in out
    assert "NOZICH_BASE_URL" not in out and "NOZICH_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "NOZICH_BASE_URL" in msgs and "NOZICH_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "NOZICH_API_KEY" not in msgs


def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped NOZICH_* vars."""
    import logging
