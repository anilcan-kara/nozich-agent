...
class TestFileDedup(unittest.TestCase):
    ...
    def setUp(self):
        _read_tracker.clear()
        self._tmpdir = _make_safe_tempdir("nozich-dedup-")
    ...
    def test_write_allows_large_file_that_quotes_status_text(self, mock_ops):
        ...
        Nozich' own docs / SKILL.md files may legitimately mention the dedup
...
class TestConfigOverride(unittest.TestCase):
    ...
    @patch("tools.file_tools._get_file_ops")
    @patch("nozich_cli.config.load_config", return_value={"file_read_max_chars": 50})
    ...
    @patch("tools.file_tools._get_file_ops")
    @patch("nozich_cli.config.load_config", return_value={"file_read_max_chars": 500_000})
...
class TestWriteInvalidatesDedup(unittest.TestCase):
    ...
    Regression test for https://github.com/anilcan-kara/nozich-agent/issues/13144
    """

    def setUp(self):
        _read_tracker.clear()
        self._tmpdir = _make_safe_tempdir("nozich-write-dedup-")
