...
def test_nozich_provider_subclass_exists():
    """NozichMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _NOZICH_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _NOZICH_PROVIDER_CLS is not None
    assert issubclass(_NOZICH_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_nozich_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns NozichMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _NOZICH_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("NOZICH_HOME", str(tmp_path))
    _set_interactive_stdin(monkeypatch)
    ...
    assert _NOZICH_PROVIDER_CLS is not None
    assert isinstance(provider, _NOZICH_PROVIDER_CLS)
    assert provider._nozich_server_name == "srv"
