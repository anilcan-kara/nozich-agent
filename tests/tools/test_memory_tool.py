...
class TestScanMemoryContent:
    ...
    def test_nozich_config_mod_blocked(self):
        result = _scan_memory_content("edit .nozich/config.yaml to change settings")
        assert "Blocked" in result
        assert "nozich_config_mod" in result
        result = _scan_memory_content("update .nozich/SOUL.md with new personality")
        assert "Blocked" in result
        assert "nozich_config_mod" in result
    ...
    def test_nozich_config_mod_no_false_positives(self):
        """Merely mentioning nozich config files should not trigger; only modify intent should."""
        assert _scan_memory_content("Check .nozich/config.yaml for settings") is None
        assert _scan_memory_content("Read .nozich/SOUL.md for agent personality") is None
        assert _scan_memory_content("The .nozich/config.yaml file contains runtime options") is None
