...
When NOZICH_UID is remapped at container boot, ``usermod -u`` only rewrites
files under the nozich user's home directory ($NOZICH_HOME == /opt/data).
Runtime-writable trees under ``/opt/nozich`` must be explicitly chowned to the
new UID before services drop privileges. ``/opt/nozich/gateway`` is one such
