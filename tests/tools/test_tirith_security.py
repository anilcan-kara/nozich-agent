...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._nozich_bin_dir", return_value="/nonexistent"), \
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(nozich_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(nozich_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_nozich_bin(self):
        """After _INSTALL_FAILED, manual install in NOZICH_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        nozich_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(nozich_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(nozich_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._nozich_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == nozich_bin
            assert _tirith_mod._resolved_path == nozich_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# NOZICH_HOME isolation
# ---------------------------------------------------------------------------

class TestNozichHomeIsolation:
    def test_nozich_bin_dir_respects_nozich_home(self):
        """_nozich_bin_dir must use NOZICH_HOME, not hardcoded ~/.nozich."""
        from tools.tirith_security import _nozich_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"NOZICH_HOME": tmpdir}):
            result = _nozich_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_nozich_home(self):
        """_failure_marker_path must use NOZICH_HOME, not hardcoded ~/.nozich."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"NOZICH_HOME": "/custom/nozich"}):
            result = _failure_marker_path()
        assert result == "/custom/nozich/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets NOZICH_HOME; verify it's active."""
        nozich_home = os.getenv("NOZICH_HOME")
        assert nozich_home is not None, "NOZICH_HOME should be set by conftest"
        assert "nozich_test" in nozich_home, "Should point to test temp dir"

    def test_get_nozich_home_fallback(self):
        """Without NOZICH_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_nozich_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove NOZICH_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".nozich")
            result = _get_nozich_home()
