...
class TestWriteDenyExactPaths:
    ...
    def test_nozich_env(self):
        # ``.env`` under the active NOZICH_HOME (profile-aware, not just
        # ``~/.nozich``) must be write-denied. The hermetic test conftest
        # points NOZICH_HOME at a tempdir — resolve via get_nozich_home()
        # to match the denylist.
        from nozich_constants import get_nozich_home
        path = str(get_nozich_home() / ".env")
        assert _is_write_denied(path) is True

    def test_nozich_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.nozich/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "nozich_root"
        ...
        # Sanity check: NOZICH_HOME does point to the profile dir, not the root.
        from nozich_constants import get_nozich_home, get_default_nozich_root
        assert get_nozich_home() == profile_home
        assert get_default_nozich_root() == root
...
class TestWriteAllowed:
    ...
    def test_nozich_control_files_requested_writable(self):
        from nozich_constants import get_nozich_home
