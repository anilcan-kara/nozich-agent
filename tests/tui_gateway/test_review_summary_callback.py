...
def server():
    with patch.dict(
        "sys.modules",
        {
            "nozich_constants": MagicMock(
                get_nozich_home=MagicMock(return_value="/tmp/nozich_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'nozich-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'nozich-release' patched"
    }
