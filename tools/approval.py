...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("NOZICH_YOLO_MODE", ""))
...
def _fire_approval_hook(hook_name: str, **kwargs) -> None:
    ...
    """
    try:
        from nozich_cli.plugins import invoke_hook
    except Exception:
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("NOZICH_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("NOZICH_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("NOZICH_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set NOZICH_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind NOZICH_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds NOZICH_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("NOZICH_CRON_SESSION"):
        return False
    if env_var_enabled("NOZICH_GATEWAY_SESSION"):
        return True
    return bool(_get_session_platform())

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $NOZICH_HOME, or by the resolved absolute
# active profile home path such as /home/nozich/.nozich/config.yaml. The
# resolved-absolute form is folded into the ~/.nozich/ patterns at detection
...
# go stale when NOZICH_HOME is set after this module is imported, e.g. under the
# hermetic test conftest or any deferred-profile-resolution path).
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_NOZICH_ENV_PATH = (
    r'(?:~\/\.nozich/|'
    r'(?:\$home|\$\{home\})/\.nozich/|'
    r'(?:\$nozich_home|\$\{nozich_home\})/)'
    r'\.env\b'
)
# ~/.nozich/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _NOZICH_ENV_PATH; matches the NOZICH_HOME override form as
# well as ~/.nozich/.
_NOZICH_CONFIG_PATH = (
    r'(?:~\/\.nozich/|'
    r'(?:\$home|\$\{home\})/\.nozich/|'
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bnozich\s+gateway\s+(stop|restart)\b', "stop/restart nozich gateway (kills running agents)"),
    (r'\bnozich\s+update\b', "nozich update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `nozich gateway restart`
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart nozich-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart nozich-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(nozich|gateway|cli\.py)\b', "kill nozich/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill nozich` but not
    # `kill -9 $(pgrep -f nozich)` because the substitution is opaque
    ...
    # cp/mv/install OVERWRITING a sensitive credential/SSH/shell-rc/Nozich file.
    # The tee/redirection patterns above already gate _SENSITIVE_WRITE_TARGET
    # (~/.ssh/*, ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files,
    # ~/.nozich/config.yaml/.env), but cp/mv/install was only paired for /etc and
    ...
    # In-place edit of a Nozich-managed security file (~/.nozich/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_NOZICH_CONFIG_PATH}|{_NOZICH_ENV_PATH})', "in-place edit of Nozich config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_NOZICH_CONFIG_PATH}|{_NOZICH_ENV_PATH})', "in-place edit of Nozich config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_NOZICH_CONFIG_PATH}|{_NOZICH_ENV_PATH})', "in-place edit of Nozich config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/.nozich/ form so the Nozich config/env patterns catch it. In Docker and
    # gateway deployments the agent often references the resolved absolute path
    # directly (e.g. `sed -i ... /home/nozich/.nozich/config.yaml`) rather than
    # ~, $HOME, or $NOZICH_HOME. Done at detection time (not via an import-time
    # pattern snapshot) so it tracks the live NOZICH_HOME even when that is set
    # after this module is imported — as the hermetic test conftest does.
    command = _rewrite_resolved_nozich_home(command)
    return command
...
def _rewrite_resolved_nozich_home(command: str) -> str:
    """Rewrite the resolved absolute Nozich home prefix to ``~/.nozich/``.

    Resolves the active ``NOZICH_HOME`` at call time (and its symlink-resolved
    form) and replaces an occurrence of ``<home>/`` in *command* with
    ``~/.nozich/`` so the static ``_NOZICH_CONFIG_PATH`` / ``_NOZICH_ENV_PATH``
    patterns match. No-op when the path can't be resolved or doesn't appear.
    """
    try:
        from nozich_constants import get_nozich_home
        home = get_nozich_home().expanduser()
        candidates = [
            str(home).rstrip("/"),
    ...
    for path in candidates:
        if not path or path in seen:
            continue
        seen.add(path)
        # Guard against a degenerate NOZICH_HOME (e.g. "/" or "") rewriting
        ...
        # /home/nozich/.nozich or a per-test tempdir, never a bare root.
        normalized = path.rstrip("/")
        if not normalized.startswith("/") or normalized.count("/") < 2:
            continue
        command = command.replace(normalized + "/", "~/.nozich/")
    return command
...
def prompt_dangerous_approval(command: str, description: str,
    ...
    os.environ["NOZICH_SPINNER_PAUSE"] = "1"
    try:
        ...
        if "NOZICH_SPINNER_PAUSE" in os.environ:
            del os.environ["NOZICH_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def check_dangerous_command(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("NOZICH_INTERACTIVE")
    ...
    if not is_cli and not is_gateway:
        ...
        logger.warning(
            ...
            "(pattern: %s): %s — set NOZICH_INTERACTIVE or NOZICH_GATEWAY_SESSION to require approval.",
            description, command[:200],
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("NOZICH_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("NOZICH_INTERACTIVE")
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("NOZICH_EXEC_ASK")
