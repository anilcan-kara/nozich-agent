...
`nozich skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.nozich.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.nozich.blueprint`` is a mapping containing
    ...
    nozich = meta.get("nozich") if isinstance(meta, dict) else None
    blueprint = nozich.get("blueprint") if isinstance(nozich, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.nozich.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.nozich.blueprint`` block) they can hand
    to ``nozich skills publish`` to share. ``body`` is the plain-language
