...
One ``CDPSupervisor`` runs per Nozich ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "nozich-dialog-bridge.invalid"
...
(() => {
  if (window.__nozichDialogBridgeInstalled) return;
  window.__nozichDialogBridgeInstalled = true;
  const ENDPOINT = "http://nozich-dialog-bridge.invalid/";
