...
from nozich_cli.config import DEFAULT_CONFIG, cfg_get
...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    nozich_node_bin = str(nozich_home / "node" / "bin")
    nozich_node_root = str(nozich_home / "node")
    nozich_nm_bin = str(nozich_home / "node_modules" / ".bin")
    return [nozich_node_bin, nozich_node_root, nozich_nm_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
...
# plugins drop in under ``~/.nozich/plugins/browser/<vendor>/``.
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    ...
    browser plugins (``~/.nozich/plugins/browser/<vendor>/``) participate
...
def _lightpanda_fallback_reason(engine: str, command: str, result: Dict[str, Any]) -> Optional[str]:
    ...
    the fallback result so CLI/TUI/gateway users can see when Nozich silently
...
def _run_chrome_fallback_command(
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            #   in the Nozich CLI where sys.stdout and sys.stderr both report
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-nozich_…`` the resulting socket path exceeds the
...
def _emergency_cleanup_all_sessions():
    ...
    crashed nozich processes — this way every clean nozich exit sweeps
    ...
    # Sweep orphans from other crashed nozich processes.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live nozich processes.
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record the current nozich PID as the owner of a browser socket dir.

    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the
    orphan reaper can distinguish daemons owned by a live nozich process
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning nozich process is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         referenced nozich PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent nozich instances won't reap each
         other's daemons.
    ...
    socket_dirs += glob.glob(os.path.join(tmpdir, "agent-browser-nozich_*"))
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            # Owner is alive — this session belongs to a live nozich process.
            continue
...
def _find_agent_browser() -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, Nozich-managed
    ...
    # Build an extended search PATH including Nozich-managed Node, macOS
    ...
    try:
        ...
        if ensure_dependency("browser"):
            ...
            if not recheck:
                nozich_nm = str(get_nozich_home() / "node_modules" / ".bin")
                recheck = shutil.which("agent-browser", path=nozich_nm)
            if not recheck:
                nozich_node_bin = str(get_nozich_home() / "node" / "bin")
                recheck = shutil.which("agent-browser", path=nozich_node_bin)
            if not recheck:
                nozich_node_root = str(get_nozich_home() / "node")
                recheck = shutil.which("agent-browser", path=nozich_node_root)
            if recheck:
...
def _run_browser_command(
    ...
    try:
        ...
        os.makedirs(task_socket_dir, mode=0o700, exist_ok=True)
        # Record this nozich PID as the session owner (cross-process safe
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = nozich_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    conversation so the model can inspect it on the next turn; otherwise Nozich
    ...
    from nozich_constants import get_nozich_dir
    screenshots_dir = get_nozich_dir("cache/screenshots", "browser_screenshots")
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    ...
                    print("       docker pull ghcr.io/anilcan-kara/nozich-agent:latest")
                else:
