...
Lets the LLM write a Python script that calls Nozich tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `nozich_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `nozich_tools.py` with file-based RPC stubs
...
# available on every platform Nozich itself runs on.
...
# match a safe prefix, the operational NOZICH_ allowlist, or (on Windows) an
# OS-essential name.
#
# NB: the broad "NOZICH_" prefix was deliberately removed (#27303) — it leaked
# NOZICH_*-named config that lacks a secret substring (e.g. NOZICH_BASE_URL,
# NOZICH_KANBAN_DB, NOZICH_*_WEBHOOK).  The child only needs the few
# location/profile vars in _NOZICH_CHILD_ALLOWED below; NOZICH_RPC_SOCKET /
# NOZICH_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational NOZICH_* vars the child legitimately needs by exact name — these
# are non-secret runtime-location flags (the same set nozich_cli treats as the
# runtime location) that repo-root modules a sandbox script imports may read at
# import time.  None match _SECRET_SUBSTRINGS.
_NOZICH_CHILD_ALLOWED = frozenset({
    "NOZICH_HOME",
    "NOZICH_PROFILE",
    "NOZICH_CONFIG",
    "NOZICH_ENV",
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational NOZICH_* vars (_NOZICH_CHILD_ALLOWED) pass by exact name.
    ...
    scrubbed = {}
    # Non-secret NOZICH_* vars dropped by the tightened allowlist (#27303). The
    # broad "NOZICH_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like NOZICH_KANBAN_DB / NOZICH_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
            scrubbed[k] = v
            continue
        if k in _NOZICH_CHILD_ALLOWED:
            scrubbed[k] = v
            continue
        if is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS:
            scrubbed[k] = v
            continue
        if k.startswith("NOZICH_"):
            # Non-secret (secrets were already dropped above) and not in any
            # allowlist — a deliberately-dropped NOZICH_* var.
            _dropped_nozich.append(k)
    if _dropped_nozich:
        logger.debug(
            "execute_code: dropped %d non-allowlisted NOZICH_* var(s) from the "
...
# ---------------------------------------------------------------------------
# nozich_tools.py code generator
# ---------------------------------------------------------------------------
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-Nozich-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-Nozich-profile soft guard."""',
...
def generate_nozich_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the nozich_tools.py stub module.
...
"""Auto-generated Nozich tools RPC stubs."""
...
def _connect():
    ...
    NOZICH_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["NOZICH_RPC_SOCKET"]
...
"""Auto-generated Nozich tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("NOZICH_RPC_DIR") or os.path.join(tempfile.gettempdir(), "nozich_rpc")
_seq = 0
...
def _execute_remote(
    ...
    The script and the generated nozich_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/nozich_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_nozich_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/nozich_tools.py", tools_src)
        ...
        env_prefix = (
            f"NOZICH_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("NOZICH_TIMEZONE", "").strip()
        if tz:
            env_prefix += f" TZ={tz}"
...
def execute_code(
    ...
    # --- Set up temp directory with nozich_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="nozich_sandbox_")
    ...
    # selector from NOZICH_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"nozich_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated nozich_tools module.
        ...
        tools_src = generate_nozich_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "nozich_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  NOZICH_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["NOZICH_RPC_SOCKET"] = rpc_endpoint
        ...
        # Ensure the nozich-agent root is importable in the sandbox so
        # repo-root modules are available to child scripts.  We also prepend
        # the staging tmpdir so ``from nozich_tools import ...`` resolves even
        # when the subprocess CWD is not tmpdir (project mode).
        _nozich_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir, _nozich_root]
        if _existing_pp:
            _pp_parts.append(_existing_pp)
        child_env["PYTHONPATH"] = os.pathsep.join(_pp_parts)
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.  Only TZ is set —
        # NOZICH_TIMEZONE is an internal Nozich setting and must not leak
        # into child processes.
        _tz_name = os.getenv("NOZICH_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("NOZICH_TIMEZONE", None)

        from nozich_constants import apply_subprocess_home_env
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.nozich/.env')).
...
def _load_config() -> dict:
    ...
    ``nozich --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (nozich-agent's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``nozich tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and nozich-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.nozich/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call Nozich tools programmatically. "
        ...
        f"Available via `from nozich_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed — built into nozich_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from nozich_tools import {import_str}` "
