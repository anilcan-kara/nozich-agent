...
_FORCE_SYNC_ENV = "NOZICH_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.nozich") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.nozich to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``NOZICH_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, nozich_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.nozich/`` directory as a tar archive,
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="nozich-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.nozich/skills/a.md`` →
        ``~/.nozich/skills/a.md``, a new remote file at
        ``/root/.nozich/skills/b.md`` maps to ``~/.nozich/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
        for host, remote in mapping:
