...
class SSHEnvironment(BaseEnvironment):
    ...
    def __init__(self, host: str, user: str, cwd: str = "~",
        ...
        self.control_dir = Path(tempfile.gettempdir()) / "nozich-ssh"
        ...
        self._sync_manager = FileSyncManager(
            get_files_fn=lambda: iter_sync_files(f"{self._remote_home}/.nozich"),
    ...
    def _ensure_remote_dirs(self) -> None:
        """Create base ~/.nozich directory tree on remote in one SSH call."""
        base = f"{self._remote_home}/.nozich"
    ...
    def _ssh_bulk_upload(self, files: list[tuple[str, str]]) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="nozich-ssh-bulk-") as staging:
    ...
    def _ssh_bulk_download(self, dest: Path) -> None:
        """Download remote .nozich/ as a tar archive."""
        # Tar from / with the full path so archive entries preserve absolute
        # paths (e.g. home/user/.nozich/skills/f.py), matching _pushed_hashes keys.
        rel_base = f"{self._remote_home}/.nozich".lstrip("/")
        ssh_cmd = self._build_ssh_command()
