#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Nozich Skills Hub.
...
Used by nozich_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            nozich_meta = metadata.get("nozich", {})
            if isinstance(nozich_meta, dict):
                tags = nozich_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    ~/.nozich/skills/ during setup.  They are discoverable via the Skills Hub
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                nozich_meta = meta_block.get("nozich", {})
                if isinstance(nozich_meta, dict):
                    tags = nozich_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# Nozich centralized index source
# ---------------------------------------------------------------------------

NOZICH_INDEX_URL = "https://nozich-agent.anilcan-kara.com/docs/api/skills-index.json"
NOZICH_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "nozich-index.json"
NOZICH_INDEX_TTL = 6 * 3600  # 6 hours


def _load_nozich_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for NOZICH_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if NOZICH_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - NOZICH_INDEX_CACHE_FILE.stat().st_mtime
            if age < NOZICH_INDEX_TTL:
                return json.loads(NOZICH_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(NOZICH_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("Nozich index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Nozich index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        NOZICH_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        NOZICH_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class NozichIndexSource(SkillSource):
    """Skill source backed by the centralized Nozich Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_nozich_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "nozich-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "nozich-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        NozichIndexSource(auth=auth), # Centralized index (search + resolved install paths)
