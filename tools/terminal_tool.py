...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or Nozich exit
...
# display_nozich_home imported lazily at call site (stale-module safety during nozich update)
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of nozich directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "nozich-*")):
...
def _get_sudo_password_cache_scope() -> str:
    ...
    try:
        ...
        session_key = os.getenv("NOZICH_SESSION_KEY", "")
    if session_key:
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = env_var_enabled("NOZICH_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (NOZICH_INTERACTIVE=1).
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    (NOZICH_INTERACTIVE=1 or a registered sudo password callback):
    ...
    # interactive Nozich password prompt or the sudo -S password-pipe path.
...
For servers/watchers, do NOT use shell-level background wrappers (nohup/disown/setsid/trailing '&') in foreground mode. Use background=true so Nozich can track lifecycle and output.
...
def _maybe_reap_docker_orphans(container_config: Dict[str, Any]) -> None:
    ...
    Sweeps long-Exited containers labeled ``nozich-agent=1`` for the current
    profile that match the issue #20561 leak class — containers left behind
    by Nozich processes that exited without firing ``atexit`` (SIGKILL,
    ...
    * ``terminal.docker_orphan_reaper: false`` disables it entirely (the
      ...
      Nozich processes in the same profile and don't trust the
    ...
    * ``_docker_orphan_reaper_ran`` flag — sweep runs once per Python
    ...
    # 2 × lifetime_seconds gives sibling Nozich processes a generous grace
...
def _resolve_container_task_id(task_id: Optional[str]) -> str:
    ...
    Exception: RL / benchmark environments (TerminalBench2, NozichSweEnv, ...)
...
def _parse_env_var(name: str, default: str, converter: Any = int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.nozich/.env or environment variables."
        )
...
def _get_env_config() -> Dict[str, Any]:
    ...
    return {
        ...
        # Startup orphan reaper for nozich-tagged containers left behind by
...
def _create_environment(env_type: str, image: str, cwd: str, timeout: int,
    ...
    if env_type == "local":
        ...
        # prior Nozich processes that hit SIGKILL / OOM / a closed terminal
...
def _foreground_background_guidance(command: str) -> str | None:
    ...
    if _SHELL_LEVEL_BACKGROUND_RE.search(unquoted):
        return (
            ...
            "Use terminal(background=true) so Nozich can track the process, then run "
    ...
    try:
        ...
        if background:
            ...
            try:
                ...
                # nozich-agent dev work. May 2026 PRs that surfaced this
                ...
                if background and command:
                    ...
                    if _bad_shape:
                        ...
                        canonical_hint = (
                            ...
                            "repeatedly in nozich-agent dev work (PRs #31329, "
                ...
                if background and (notify_on_complete or watch_patterns):
                    ...
                    _gw_platform = _gse("NOZICH_SESSION_PLATFORM", "")
                    if _gw_platform:
                        _gw_chat_id = _gse("NOZICH_SESSION_CHAT_ID", "")
                        _gw_thread_id = _gse("NOZICH_SESSION_THREAD_ID", "")
                        _gw_user_id = _gse("NOZICH_SESSION_USER_ID", "")
                        _gw_user_name = _gse("NOZICH_SESSION_USER_NAME", "")
                        _gw_message_id = _gse("NOZICH_SESSION_MESSAGE_ID", "")
