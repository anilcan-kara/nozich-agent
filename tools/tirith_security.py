...
it is automatically downloaded from GitHub releases to $NOZICH_HOME/bin/tirith.
...
def _get_nozich_home() -> str:
    """Return the Nozich home directory, respecting NOZICH_HOME env var."""
    return str(get_nozich_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_nozich_home(), ".tirith-install-failed")
...
def _nozich_bin_dir() -> str:
    """Return $NOZICH_HOME/bin, creating it if needed."""
    d = os.path.join(_get_nozich_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $NOZICH_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_nozich_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $NOZICH_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $NOZICH_HOME/bin/tirith
    ...
    nozich_bin = os.path.join(_nozich_bin_dir(), "tirith")
    if os.path.isfile(nozich_bin) and os.access(nozich_bin, os.X_OK):
        _resolved_path = nozich_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return nozich_bin
