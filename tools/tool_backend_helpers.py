...
def nous_tool_gateway_unavailable_message(
    ...
    *,
    ...
    try:
        ...
        pass
    return (
        f"{capability} is unavailable. Run `nozich model` to refresh your "
...
def fal_key_is_configured() -> bool:
    ...
    Consults both ``os.environ`` and ``~/.nozich/.env`` (via
    ``nozich_cli.config.get_env_value`` when available) so tool-side
