...
Backend is selected during ``nozich tools`` setup (web.backend in config.yaml).
When available, Nozich can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via Nozich config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through Nozich' config/.env layer (``nozich config set``, ``nozich tools``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.nozich/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``nozich tools``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `nozich tools` repaint.
        try:
...
def _is_nous_auxiliary_client(client: Any) -> bool:
    ...
    return host == "anilcan-kara.com" or host.endswith(".anilcan-kara.com")
        ...
        if provider is None:
            response_data = {
                ...
                "error": (
                    ...
                    "Run `nozich tools` to set one up."
                ),
            }
        else:
            logger.info(
