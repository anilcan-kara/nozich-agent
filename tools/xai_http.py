...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``nozich tools`` repaint, tool-registration scans,
    ...
    2. ``~/.nozich/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_nozich_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.nozich/.env`` first, then ``os.environ``.

    Wraps :func:`nozich_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from nozich_cli.config import get_env_value as _nozich_get_env_value

        value = _nozich_get_env_value(name)
...
def nozich_xai_user_agent() -> str:
    """Return a stable Nozich-specific User-Agent for xAI HTTP calls."""
    try:
        from nozich_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Nozich-Agent/{__version__}"


def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    """Resolve bearer credentials for direct xAI HTTP endpoints.

    Prefers Nozich-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``nozich_cli.config.get_env_value`` so keys
    stored in ``~/.nozich/.env`` (the standard Nozich location) are honored —
