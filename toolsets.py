...
_NOZICH_CORE_TOOLS = [
    ...
    "terminal", "process",
    # Read the desktop GUI's embedded terminal pane (gated on NOZICH_DESKTOP
    ...
    # spawned as a kanban worker (NOZICH_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `nozich tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (NOZICH_KANBAN_TASK env "
    ...
    # Coding posture (base Nozich — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in nozich_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full Nozich toolsets (CLI + messaging platforms)
    #
    ...
    "nozich-cli": {
        ...
        "tools": _NOZICH_CORE_TOOLS,
        "includes": []
    },

    "nozich-cron": {
        # Mirrors nozich-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `nozich tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as nozich-cli; gated by `nozich tools`",
        "tools": _NOZICH_CORE_TOOLS,
        "includes": []
    },
    ...
    "nozich-email": {
        "description": "Email bot toolset - interact with Nozich via email (IMAP/SMTP)",
        "tools": _NOZICH_CORE_TOOLS,
        "includes": []
    },
    ...
    "nozich-sms": {
        "description": "SMS bot toolset - interact with Nozich via SMS (Twilio)",
        "tools": _NOZICH_CORE_TOOLS,
        "includes": []
    },
    ...
    "nozich-webhook": {
        ...
        "tools": _NOZICH_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "nozich-gateway": {
        ...
        "tools": [],
        "includes": ["nozich-telegram", "nozich-discord", "nozich-whatsapp", "nozich-slack", "nozich-signal", "nozich-bluebubbles", "nozich-homeassistant", "nozich-email", "nozich-sms", "nozich-mattermost", "nozich-matrix", "nozich-dingtalk", "nozich-feishu", "nozich-wecom", "nozich-wecom-callback", "nozich-weixin", "nozich-qqbot", "nozich-webhook", "nozich-yuanbao"]
    }
}
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (nozich-<name>).
        # Gives them _NOZICH_CORE_TOOLS plus any tools the plugin registered
        # into a toolset matching the platform name.
        if name.startswith("nozich-"):
            platform_name = name[len("nozich-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_NOZICH_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
