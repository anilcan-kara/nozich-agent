...
React + Ink terminal UI for Nozich. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `NOZICH_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.nozich/.nozich_history` or under `NOZICH_HOME`.
...
```text
ui-tui/
  packages/nozich-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      nozich-ink.d.ts    type declarations for @nozich/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent NozichCLI subprocess for slash commands
```
