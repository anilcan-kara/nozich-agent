...
await build({
  ...
  // Skip the prebuilt @nozich/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@nozich/ink': resolve(root, 'packages/nozich-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The nozich_cli launcher
