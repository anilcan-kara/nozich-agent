import type { RunExternalProcess } from '@nozich/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`nozich ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching nozich: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`nozich ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
