...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.NOZICH_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Nozich on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via NOZICH_TUI_TRUECOLOR=1.
