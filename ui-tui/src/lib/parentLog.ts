...
const logDir = join(process.env.NOZICH_HOME?.trim() || join(homedir(), '.nozich'), 'logs')
...
// a real ~/.nozich (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('nozich-tui: parent lifecycle log unavailable\n')
    }
  }
}
