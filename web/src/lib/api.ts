...
// (e.g. https://mission-control.tilos.com/nozich/). The Python backend
// injects ``window.__NOZICH_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__NOZICH_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const NOZICH_BASE_PATH = readBasePath();
const BASE = NOZICH_BASE_PATH;
...
declare global {
  interface Window {
    __NOZICH_SESSION_TOKEN__?: string;
    __NOZICH_BASE_PATH__?: string;
    ...
    __NOZICH_AUTH_REQUIRED__?: boolean;
  }
}
let _sessionToken: string | null = null;
const SESSION_HEADER = "X-Nozich-Session-Token";
...
export async function fetchJSON<T>(
  ...
  const token = window.__NOZICH_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``nozich update``, ``nozich gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__NOZICH_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__NOZICH_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
      let alreadyReloaded = false;
      try {
        alreadyReloaded =
          sessionStorage.getItem("nozich.tokenReloadAttempted") === "1";
      } catch {
        /* SSR / privacy mode — fall through to throw */
      }
      if (!alreadyReloaded) {
        try {
          sessionStorage.setItem("nozich.tokenReloadAttempted", "1");
        } catch {
  ...
  if (res.ok) {
    ...
    // current ``window.__NOZICH_SESSION_TOKEN__`` is valid, so the next
    // 401 — if any — should be allowed to trigger its own reload cycle.
    try {
      sessionStorage.removeItem("nozich.tokenReloadAttempted");
    } catch {
      /* SSR / privacy mode — ignore */
    }
  }
...
async function getSessionToken(): Promise<string> {
  ...
  const injected = window.__NOZICH_SESSION_TOKEN__;
  if (injected) {
    _sessionToken = injected;
    return _sessionToken;
  }
  throw new Error("Session token not available — page must be served by the Nozich dashboard server");
}

/**
 * Fetch a single-use ticket for a WebSocket upgrade in gated mode.
 *
 * The dashboard's gated-mode WS auth (``nozich_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__NOZICH_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Nozich-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``nozich_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__NOZICH_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export const api = {
  ...
  updateNozich: () =>
    fetchJSON<ActionResponse>("/api/nozich/update", { method: "POST" }),
  checkNozichUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/nozich/update/check${force ? "?force=true" : ""}`,
    ),
...
export interface SkillHubSource {
  ...
  /** nozich-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
   ...
   * updates are handled by the outer launcher instead of ``nozich update``. */
  can_update_nozich?: boolean;
  config_path: string;
