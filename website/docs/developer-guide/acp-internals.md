...
The ACP adapter wraps Nozich' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
nozich acp / nozich-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.nozich/.env
  -> configure stderr logging
  -> construct NozichACPAgent
  -> acp.run_agent(agent, use_unstable_protocol=True)
```

The Zed ACP Registry path launches the same adapter through `uvx --from 'nozich-agent[acp]==<version>' nozich-acp`, pointed at the `nozich-agent` PyPI release.
...
- `allow_once` -> Nozich `once`
- `allow_always` -> Nozich `always`
- reject options -> Nozich `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Nozich tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["nozich-acp"])
...
Instead it reuses Nozich' runtime resolver:

- `acp_adapter/auth.py`
- `nozich_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Nozich provider/credentials. It also always advertises a terminal setup auth method (`nozich-setup`, args `--setup`) so first-run registry clients can open Nozich' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.nozich/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `nozich-acp` toolset definition
- `nozich_cli/main.py` — `nozich acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `nozich-acp` script
