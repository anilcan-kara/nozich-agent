...
sidebar_position: 1
title: "Architecture"
description: "Nozich Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Nozich Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ nozich_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # NozichCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── nozich_state.py           # SQLite session/state database with FTS5
├── nozich_constants.py       # NOZICH_HOME, profile-aware paths
...
│
├── nozich_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `nozich` subcommands (large file)
...
│   ├── skills_config.py      # nozich skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # nozich tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # nozich gateway start/stop
│
...
```text
User input → NozichCLI.process_input()
...
Three discovery sources: `~/.nozich/plugins/` (user), `.nozich/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `nozich plugins` or `config.yaml`.

→ [Plugin Guide](/guides/build-a-nozich-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes Nozich as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`nozich -p <name>`) gets its own NOZICH_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
