...
description: "How Nozich stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `nozich_cli/cron.py` | CLI `nozich cron` subcommands |
...
Jobs are stored in `~/.nozich/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `nozich cron` commands are run or during active CLI sessions.
...
```python
# ~/.nozich/scripts/check_competitors.py
...
2. **Environment variable** — `NOZICH_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.nozich/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `nozich cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `nozich cron` CLI provides direct job management:

```bash
nozich cron list                    # Show all jobs
nozich cron create                  # Interactive job creation (alias: add)
nozich cron edit <job_id>           # Edit job configuration
nozich cron pause <job_id>          # Pause a running job
nozich cron resume <job_id>         # Resume a paused job
nozich cron run <job_id>            # Trigger immediate execution
nozich cron remove <job_id>         # Delete a job
```
