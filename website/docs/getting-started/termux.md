...
description: "Run Nozich Agent directly on an Android phone with Termux"
---

# Nozich on Android with Termux

This is the tested path for running Nozich Agent directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Nozich from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Nozich now ships a Termux-aware installer path:
...
- links `nozich` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `nozich` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/nozich" "$PREFIX/bin/nozich"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `nozich` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.nozich/.env`.
...
### `nozich doctor` says ripgrep or Node is missing
