...
description: "Diagnose and fix common Nozich cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`nozich gateway` for foreground, or `nozich gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `nozich cron tick`.
...
```bash
date
nozich cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.nozich/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.nozich/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.nozich/.env` |
...
| `local` | Write access to `~/.nozich/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `nozich cron list` for updated `last_error` field (if available).
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `nozich skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-daily-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `nozich skills list`.
...
2. `~/.nozich/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `nozich cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Nozich config directory). Verify:
```bash
ls ~/.nozich/scripts/your-script.py   # Must exist
nozich cron edit <job_id> --script ~/.nozich/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `nozich skills install <skill-name>`.
...
Jobs are stored in `~/.nozich/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.nozich/cron/jobs.json
chmod 600 ~/.nozich/cron/jobs.json   # Your user should own it
```
...
```bash
nozich cron list                    # Show all jobs, states, next_run times
nozich cron run <job_id>            # Schedule for next tick (for testing)
nozich cron edit <job_id>           # Fix configuration issues
nozich logs                         # View recent Nozich logs
nozich skills list                  # Verify installed skills
```
...
1. Run the job with `nozich cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.nozich/logs/agent.log` for scheduler messages and `~/.nozich/logs/errors.log` for warnings
3. Open an issue at [github.com/anilcan-kara/nozich-agent](https://github.com/anilcan-kara/nozich-agent) with:
