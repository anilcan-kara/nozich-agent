...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  Nozich Agent  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Nozich instantly when PRs are opened or updated.
:::
...
- **Nozich Agent installed** — see the [Installation guide](/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  nozich gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.nozich/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure Nozich can access GitHub. Start a chat:
...
```
Run: gh pr list --repo anilcan-kara/nozich-agent --state open --limit 3
```
...
Still in the chat, ask Nozich to review a real PR:

```
Review this pull request. Read the diff, check for bugs, security issues,
and code quality. Be specific about line numbers and quote problematic code.

Run: gh pr diff 3888 --repo anilcan-kara/nozich-agent
```
...
A skill gives Nozich consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.nozich/skills/code-review
```

Create `~/.nozich/skills/code-review/SKILL.md`:
...
Verify it loaded — start `nozich` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach Nozich your team's standards:
...
2. Teach Nozich your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
nozich gateway status    # Is the gateway running?
nozich cron list         # Is the job enabled?
```
...
- **[Build a Plugin](/guides/build-a-nozich-plugin)** — wrap the review logic into a shareable plugin
