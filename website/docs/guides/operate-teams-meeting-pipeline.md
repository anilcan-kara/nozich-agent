...
```bash
nozich teams-pipeline validate
```
...
```bash
nozich teams-pipeline token-health
nozich teams-pipeline token-health --force-refresh
```
...
```bash
nozich teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
nozich teams-pipeline maintain-subscriptions
nozich teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Nozich cron (recommended if you already run the Nozich gateway)

Nozich ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.nozich/scripts/`. First create the script:

```bash
mkdir -p ~/.nozich/scripts
cat > ~/.nozich/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec nozich teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.nozich/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
nozich cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/nozich-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Nozich Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=nozich
EnvironmentFile=/etc/nozich/env
ExecStart=/usr/local/bin/nozich teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/nozich-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Nozich Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now nozich-teams-pipeline-maintain.timer
systemctl list-timers nozich-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/nozich teams-pipeline maintain-subscriptions >> /var/log/nozich/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.nozich/.env` at the top of a wrapper script that crontab calls.
...
```bash
nozich teams-pipeline subscriptions   # should show expirationDateTime advanced
nozich teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
nozich teams-pipeline list
nozich teams-pipeline list --status failed
nozich teams-pipeline show <job-id>
```

### Replay a stored job

```bash
nozich teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
nozich teams-pipeline fetch --meeting-id <meeting-id>
nozich teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `nozich teams-pipeline maintain-subscriptions --dry-run`
- inspect `nozich teams-pipeline list --status failed`
...
- run `nozich teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `nozich teams-pipeline run`
...
- [ ] `nozich teams-pipeline validate` returns an OK snapshot
- [ ] `nozich teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Nozich cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
