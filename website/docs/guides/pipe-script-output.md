...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `nozich send`."
---

# Pipe Script Output to Messaging Platforms

`nozich send` is a small, scriptable CLI that pushes a message to any
messaging platform Nozich is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | nozich send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `nozich
...
nozich send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
nozich send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
nozich send --list

# Filter by platform
nozich send --list telegram
```
...
Any platform Nozich ships adapters for works as a target:
...
`nozich send` resolves the message body in this order:

1. **Positional argument** — `nozich send --to telegram "hi"`
2. **`--file PATH`** — `nozich send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | nozich send --to telegram`

When stdin is a TTY (no pipe), Nozich does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  nozich send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `nozich send` reuses your Nozich config, the same script works on
any host where Nozich is installed — no need to export bot tokens into
...
`nozich send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  nozich send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | nozich send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.nozich/bin/nozich send \
...
```bash
./train.py --epochs 200 && \
  nozich send --to telegram "training done" || \
  nozich send --to telegram "training failed (exit $?)"
```
...
nozich send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(nozich send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `nozich send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `nozich send` calls
the platform's REST endpoint directly using credentials from
`~/.nozich/.env` and `~/.nozich/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `nozich gateway start` and retry.
...
# Machine-readable
nozich send --list --json
```

The listing is built from `~/.nozich/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`nozich gateway
...
| Approach | Multi-platform | Reuses Nozich creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `nozich send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`nozich send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `nozich send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `nozich send` shares with cron delivery.
