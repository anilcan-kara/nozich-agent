...
This tutorial walks you through setting up a Telegram bot powered by Nozich Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Nozich Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.nozich/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Nozich itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Nozich Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_nozich_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Nozich Agent. DM me for help with code, research, debugging, and more.
   ```
...
```
[Gateway] Starting Nozich Gateway...
...
# View live logs
journalctl --user -u nozich-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `nozich gateway install` to update the plist.
:::
...
```bash
nozich gateway stop && nozich gateway start
```
...
nozich pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.nozich/.env`:
...
Control how much detail the bot shows when using tools. In `~/.nozich/config.yaml`:
...
Customize how the bot communicates by editing `~/.nozich/SOUL.md`:

For a full guide, see [Use SOUL.md with Nozich](/guides/use-soul-with-nozich).
...
```bash
# From the CLI
nozich cron list          # View all scheduled jobs
nozich cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u nozich-gateway` (Linux) or `~/.nozich/logs/gateway.log` (macOS) |
| Cron job output | `~/.nozich/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.nozich/cron/jobs.json` |
| Pairing data | `~/.nozich/pairing/` |
| Session history | `~/.nozich/sessions/` |
