---
sidebar_position: 6
title: "Use MCP with Nozich"
description: "A practical guide to connecting MCP servers to Nozich Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Nozich

This guide shows how to actually use MCP with Nozich Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Nozich tool
- you want Nozich to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Nozich to internal APIs, databases, or company systems without modifying Nozich core

Do not use MCP when:
- a built-in Nozich tool already solves the job well
...
- Nozich remains the agent
- MCP servers contribute tools
- Nozich discovers those tools at startup or reload time
...
If you installed Nozich with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Nozich banner/status should show MCP integration when configured
- ask Nozich what tools it has available
...
## WSL2: bridge Nozich in WSL to Windows Chrome

This is the practical setup when:

- Nozich runs inside WSL2
...
In this setup, Nozich does **not** connect to Chrome directly. Instead:

- Nozich runs in WSL
- Nozich starts a local stdio MCP server
...
```text
Nozich (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Nozich stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Nozich core browser transport
...
```bash
nozich mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
nozich mcp test chrome-devtools-win
```

Then start a fresh Nozich session or run:
...
Once loaded, Nozich can use the MCP-prefixed browser tools directly. For example:
...
If Nozich runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Nozich from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Nozich from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Nozich:
...
2. Nozich-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Nozich will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Nozich to reason over a bounded workspace.
...
Now Nozich can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Nozich core.
...
Because Nozich now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
