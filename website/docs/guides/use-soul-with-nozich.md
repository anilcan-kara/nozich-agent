---
sidebar_position: 7
title: "Use SOUL.md with Nozich"
description: "How to use SOUL.md to shape Nozich Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Nozich

`SOUL.md` is the **primary identity** for your Nozich instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Nozich to feel like the same assistant every time you talk to it — or if you want to replace the Nozich persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Nozich should be
- what Nozich should avoid stylistically
- how Nozich should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Nozich is and how Nozich speaks
...
Nozich now uses only the global SOUL file for the current instance:

```text
~/.nozich/SOUL.md
```

If you run Nozich with a custom home directory, it becomes:
...
Nozich automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Nozich does not overwrite it
- if the file exists but is empty, Nozich adds nothing from it to the prompt

## How Nozich uses it

When Nozich starts a session, it reads `SOUL.md` from `NOZICH_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Nozich feels.
...
Nozich already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Nozich should sound.

# Avoid
What Nozich should not do.

# Defaults
How Nozich should behave when ambiguity appears.
```
...
Then restart Nozich or start a new session.
...
### I edited SOUL.md but Nozich still sounds the same

Check:
- you edited `~/.nozich/SOUL.md` or `$NOZICH_HOME/SOUL.md`
...
### Nozich is ignoring parts of my SOUL.md
