---
sidebar_position: 8
title: "Use Voice Mode with Nozich"
description: "A practical guide to setting up and using Nozich voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Nozich
...
- you want Nozich sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Nozich.
...
## Step 1: make sure normal Nozich works first
...
Nozich supports both local and cloud speech stacks.
...
### If you use `nozich setup`

If you choose NeuTTS in the setup wizard, Nozich checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Nozich transcribes and responds
...
- asking Nozich to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Nozich loop.

## Tuning CLI behavior

### Silence threshold

If Nozich starts/stops too aggressively, tune:
...
Nozich stays a normal chat bot, but can speak replies.
...
- you want Nozich to function like a portable research or ops assistant
...
Nozich joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Nozich detects speech boundaries
- transcripts are posted in the associated text channel
- Nozich responds in text and audio
...
1. get text Nozich working
2. install `nozich-agent[voice]`
