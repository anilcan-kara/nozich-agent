...
description: "Official optional skills shipped with nozich-agent — install via nozich skills install official/<category>/<skill>"
---

# Optional Skills Catalog

Optional skills ship with nozich-agent under `optional-skills/` but are **not active by default**. Install them explicitly:

```bash
nozich skills install official/<category>/<skill>
```

For example:

```bash
nozich skills install official/blockchain/solana
nozich skills install official/mlops/flash-attention
```
...
```bash
nozich skills uninstall <skill-name>
```
...
| [**honcho**](/docs/user-guide/skills/optional/autonomous-ai-agents/autonomous-ai-agents-honcho) | Configure and use Honcho memory with Nozich -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshoo... |
...
| [**blender-mcp**](/docs/user-guide/skills/optional/creative/creative-blender-mcp) | Control Blender directly from Nozich via socket connection to the blender-mcp addon. Create 3D objects, materials, animations, and run arbitrary Blender Python (bpy) code. Use when user wants to create or modify anything in Blender. |
...
| [**kanban-video-orchestrator**](/docs/user-guide/skills/optional/creative/creative-kanban-video-orchestrator) | Plan, set up, and monitor a multi-agent video production pipeline backed by Nozich Kanban. Use when the user wants to make ANY video — narrative film, product/marketing, music video, explainer, ASCII/terminal art, abstract/generative loo... |
...
| [**nozich-s6-container-supervision**](/docs/user-guide/skills/optional/devops/devops-nozich-s6-container-supervision) | Modify, debug, or extend the s6-overlay supervision tree inside the Nozich Agent Docker image — adding new services, debugging profile gateways, understanding the Architecture B main-program pattern. |
...
| Skill | Description |
|-------|-------------|
| [**agentmail**](/docs/user-guide/skills/optional/email/email-agentmail) | Give the agent its own dedicated email inbox via AgentMail. Send, receive, and manage email autonomously using agent-owned email addresses (e.g. nozich-agent@agentmail.to). |
...
| [**telephony**](/docs/user-guide/skills/optional/productivity/productivity-telephony) | Give Nozich phone capabilities without core tool changes. Provision and persist a Twilio number, send and receive SMS/MMS, make direct calls, and place AI-driven outbound calls through Bland.ai or Vapi. |
