...
This page covers all commands related to [Nozich profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `nozich profile` without a subcommand shows help.
...
```bash
nozich profile use <name>
```

Sets `<name>` as the active profile. All subsequent `nozich` commands (without `-p`) will use this profile.
...
nozich profile use default
```

## `nozich profile create`

```bash
nozich profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `nozich profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `nozich update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.nozich`), use `nozich skills opt-out` / `nozich skills opt-in`. |
...
nozich profile create work2 --clone-from work

# Clone everything from a specific profile
nozich profile create work2-backup --clone-from work --clone-all
```

## `nozich profile describe`

```bash
nozich profile describe [<name>] [options]
```
...
nozich profile describe researcher

# Set it explicitly
nozich profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
nozich profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
nozich profile describe --all --auto
```

## `nozich profile delete`

```bash
nozich profile delete <name> [options]
```
...
```bash
nozich profile delete mybot
nozich profile delete mybot --yes
```
...
```bash
nozich profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Nozich home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ nozich profile show work
Profile: work
Path:    ~/.nozich/profiles/work
...
## `nozich profile alias`

```bash
nozich profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Nozich installation.
...
```bash
nozich profile alias work
# Creates/updates ~/.local/bin/work

nozich profile alias work --name mywork
# Creates ~/.local/bin/mywork

nozich profile alias work --remove
# Removes the wrapper script
```

## `nozich profile rename`

```bash
nozich profile rename <old-name> <new-name>
```
...
```bash
nozich profile rename mybot assistant
# ~/.nozich/profiles/mybot → ~/.nozich/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `nozich profile export`

```bash
nozich profile export <name> [options]
```
...
```bash
nozich profile export work
# Creates work.tar.gz in the current directory

nozich profile export work -o ./work-2026-03-29.tar.gz
```

## `nozich profile import`

```bash
nozich profile import <archive> [options]
```
...
```bash
nozich profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

nozich profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`nozich profile export` / `import` are still the right commands for
...
### `nozich profile install`

```bash
nozich profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `nozich -p telemetry`). |
...
nozich profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
nozich profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
nozich profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
nozich profile install ./telemetry/
```

### `nozich profile update`

```bash
nozich profile update <name> [--force-config] [--yes]
```
...
```bash
nozich profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Nozich version, author, env var requirements, the source URL/path, and
...
`nozich profile list` also shows the distribution name and version in a
`Distribution` column, and `nozich profile show <name>` / `delete <name>`
...
nozich profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
nozich profile install https://github.com/your-org/internal-assistant.git
```
...
`nozich_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Nozich version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Nozich can clone from.
3. Tell recipients to run `nozich profile install <your-repo-url>`.
...
## `nozich -p` / `nozich --profile`

```bash
nozich -p <name> <command> [options]
nozich --profile <name> <command> [options]
```

Global flag to run any Nozich command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
nozich -p work chat -q "Check the server status"
nozich --profile dev gateway start
nozich -p personal skills list
nozich -p work config edit
```

## `nozich completion`

```bash
nozich completion <shell>
```
...
- `nozich profile <TAB>` — subcommands (list, use, create, etc.)
- `nozich profile use <TAB>` — profile names
- `nozich -p <TAB>` — profile names
