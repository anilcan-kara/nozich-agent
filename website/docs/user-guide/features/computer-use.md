...
Nozich Agent can drive your Mac's desktop — clicking, typing, scrolling,
...
Use `nozich computer-use status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `nozich tools`, pick `🖱️ Computer Use (macOS)` → `cua-driver (background)`.
...
4. Start a session with the toolset enabled:
   ...
   ```
   or add `computer_use` to your enabled toolsets in `~/.nozich/config.yaml`.

## Keeping cua-driver up to date

The cua-driver project ships fixes regularly (e.g. v0.1.6 fixed a Safari
window-focus bug for UTM workflows). Nozich refreshes the binary in two
places so you don't get stuck on a stale release:

- **`nozich update`** — when you update Nozich itself, if `cua-driver` is
...
- **`nozich computer-use install --upgrade`** — manual force-refresh.
...
`nozich computer-use status` shows the installed version next to the
binary path.
...
Nozich applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.nozich/config.yaml` if you want every action confirmed.

## Token efficiency

Screenshots are expensive. Nozich applies four layers of optimisation:
...
- **Private SPI risk.** Apple can change SkyLight's symbol surface in any
  OS update. Pin the driver version with the `NOZICH_CUA_DRIVER_VERSION`
...
`nozich computer-use install` to fetch the cua-driver binary, or run
`nozich tools` and enable the Computer Use toolset.
...
- [Universal skill: `macos-computer-use`](https://github.com/anilcan-kara/nozich-agent/blob/main/skills/apple/macos-computer-use/SKILL.md)
