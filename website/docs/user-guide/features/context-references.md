...
Type `@` followed by a reference to inject content directly into your message. Nozich expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Nozich env: `$NOZICH_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$NOZICH_HOME/skills/.hub/`
