...
When Nozich Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.nozich/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.nozich/config.yaml` (switchable per session
via `/personality`).
...
If you use Nozich' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.nozich/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Nozich Agent's deliverable
