...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Nozich's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Nozich instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
nozich memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `nozich` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Nozich syncs them back at session init.
...
The `nozich honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `nozich memory setup honcho` (or run `nozich memory setup` and pick it from the list); the `nozich honcho` subcommand then appears on the next invocation.

```bash
nozich memory setup honcho    # Configure Honcho directly (works before activation)
nozich honcho status          # Connection status, config, and key settings
nozich honcho setup           # Redirects to `nozich memory setup` (post-activation alias)
nozich honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
nozich honcho peer            # Show or update peer names + dialectic reasoning level
nozich honcho mode            # Show or set recall mode (hybrid/context/tools)
nozich honcho tokens          # Show or set token budget for context and dialectic
nozich honcho identity        # Seed or show the AI peer's Honcho identity
nozich honcho sync            # Sync Honcho config to all existing profiles
nozich honcho peers           # Show peer identities across all profiles
nozich honcho sessions        # List known Honcho session mappings
nozich honcho map             # Map current directory to a Honcho session name
nozich honcho enable          # Enable Honcho for the active profile
nozich honcho disable         # Disable Honcho for the active profile
nozich honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `nozich honcho`

If you previously used the standalone `nozich honcho setup`:
...
No re-login or re-setup needed. Run `nozich memory setup` and select "honcho" — the wizard detects your existing config.
