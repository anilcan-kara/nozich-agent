...
Nozich runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Nozich
...
1. Nozich captures a baseline of current diagnostics for the file.
...
…). Nozich auto-detects the binary on PATH or in
`<NOZICH_HOME>/lsp/bin/`.
...
`node_modules` tree — Nozich installs both packages together when you
run `nozich lsp install typescript` or auto-install fires on first
use.

## CLI

```
nozich lsp status          # service state + per-server install status
nozich lsp list            # registry, optionally --installed-only
nozich lsp install <id>    # eagerly install one server
nozich lsp install-all     # try every server with a known recipe
nozich lsp restart         # tear down running clients
nozich lsp which <id>      # print resolved binary path
```

`nozich lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <NOZICH_HOME>/lsp/bin
...
When `install_strategy: auto`, Nozich installs binaries into
`<NOZICH_HOME>/lsp/bin/`. NPM packages land in
`<NOZICH_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Nozich-owned and is
...
Servers are kept alive for the life of the Nozich process. There's
...
**`nozich lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<NOZICH_HOME>/lsp/bin/`. Run
`nozich lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `nozich lsp status`**
...
When `nozich lsp status` shows a `Backend warnings` section, install
...
`~/.nozich/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.nozich/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `nozich lsp restart` to clear the set;
