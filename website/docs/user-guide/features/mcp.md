...
description: "Connect Nozich Agent to external tool servers via MCP — and control exactly which MCP tools Nozich loads"
---

# MCP (Model Context Protocol)

MCP lets Nozich Agent connect to external tool servers so the agent can use tools that live outside Nozich itself — GitHub, databases, file systems, browser stacks, internal APIs, and more.

If you have ever wanted Nozich to use a tool that already exists somewhere else, MCP is usually the cleanest way to do it.

## What MCP gives you

- Access to external tool ecosystems without writing a native Nozich tool first
- Local stdio servers and remote HTTP MCP servers in the same config
- Automatic tool discovery and registration at startup
- Utility wrappers for MCP resources and prompts when supported by the server
- Per-server filtering so you can expose only the MCP tools you actually want Nozich to see
...
```bash
cd ~/.nozich/nozich-agent
uv pip install -e ".[mcp]"
```

2. Add an MCP server to `~/.nozich/config.yaml`:
...
4. Ask Nozich to use the MCP-backed capability.
...
Nozich will discover the MCP server's tools and use them like any other tool.

## Catalog: one-click install for Nous-approved MCPs

Nozich ships a curated catalog of MCP servers that Nous staff has reviewed
and merged. They're disabled by default — install only what you actually
want.

```bash
nozich mcp                # interactive picker (default)
nozich mcp catalog        # plain-text list, scriptable
nozich mcp install n8n    # install a catalog entry by name
```

The picker shows each entry with its current status:

```
n8n          available              Manage and inspect n8n workflows from Nozich
...
`optional-mcps/` in the nozich-agent repo — presence in that directory means
Nous approval. There is no community submission tier; entries are added by
merging a PR.

Catalog entries can require:

- **API key** — Nozich prompts at install time and writes the value to
  `~/.nozich/.env`. Non-secret values (base URLs) go to the same file.
- **OAuth** (remote MCP) — written as `auth: oauth` in your config; the MCP
  client opens a browser on first connection.
- **OAuth** (third-party provider like Google/GitHub) — Nozich points you at
  `nozich auth <provider>` if you haven't authenticated already.

### Tool selection at install time

After credentials are configured, Nozich probes the MCP server to list every
...
written (if not). Re-run `nozich mcp configure <name>` once the server is
reachable to refine.
...
the nozich-agent repo, so Nous has reviewed each entry before it shipped —
...
Manifests live at
[`optional-mcps/<name>/manifest.yaml`](https://github.com/anilcan-kara/nozich-agent/tree/main/optional-mcps)
...
PR adds an entry with a newer `manifest_version` than your installed Nozich
understands, the picker will surface a warning (`⚠ '<name>' requires a newer
Nozich`) for that entry instead of silently hiding it. Run `nozich update`
to install the latest Nozich when you see that.
...
from environment variables (which include everything in `~/.nozich/.env`).
...
MCPs are never auto-updated. Re-run `nozich mcp install <name>` to refresh
after a Nozich update if a manifest version changed.

To add an MCP to the catalog, open a PR against
[`optional-mcps/`](https://github.com/anilcan-kara/nozich-agent/tree/main/optional-mcps).
...
HTTP MCP servers are remote endpoints Nozich connects to directly.
...
- you do not want Nozich spawning a local subprocess for that integration

### OAuth-authenticated HTTP servers

Most hosted MCP servers (Linear, Sentry, Atlassian, Asana, Figma, Stripe, …) require OAuth 2.1 instead of a static bearer token. Set `auth: oauth` and Nozich handles discovery, dynamic client registration, PKCE, token exchange, refresh, and step-up auth via the MCP Python SDK.
...
On first connect, Nozich prints an authorize URL, opens your browser when possible, and waits for the OAuth callback on a local loopback port. Tokens are cached at `~/.nozich/mcp-tokens/<server>.json` with 0o600 perms; subsequent runs reuse them silently until refresh fails.

**Remote / headless hosts.** When Nozich runs on a different machine than your browser, the loopback callback can't reach your laptop. Two ways to complete the flow:

- **Paste-back (no setup):** on an interactive terminal Nozich prints "Or paste the redirect URL here…" alongside the authorize URL. Open the URL in your browser, approve, copy the full URL the browser ends up on (the redirect will show a connection error — that's expected), paste it at the prompt. Bare `?code=…&state=…` query strings work too.
- **SSH port forward:** `ssh -N -L <port>:127.0.0.1:<port> user@host` in a separate terminal, then let the redirect flow normally.

See [OAuth over SSH / Remote Hosts](../../guides/oauth-over-ssh.md#mcp-servers) for the full walkthrough, including DCR-less servers (e.g. Slack), pre-registered `client_id`/`client_secret`, scope customization, and re-auth via `nozich mcp login <server>`.

**Pitfall — providers that don't support automatic registration (Google Drive, Atlassian).** Some servers reject the dynamic client registration step (RFC 7591) that bare `auth: oauth` relies on — Google's official Drive server (`https://drivemcp.googleapis.com/mcp/v1`) returns a `400 Bad Request`, so no OAuth client is created and no token is acquired. The symptom is subtle: these servers also serve `tools/list` *without* auth, so `nozich mcp login` can list the tools and look like it worked, but every real tool call later times out. `nozich mcp login` now detects this (it checks that a token actually landed on disk) and tells you to supply your own OAuth client. Create one in the provider's console and add it to config:
...
Then run `nozich mcp login googledrive` — with the pre-registered client, Nozich skips registration and runs the normal browser authorization flow.

**Pitfall — config auto-reload race.** When you edit `~/.nozich/config.yaml` from inside a running Nozich session, the CLI auto-reloads MCP connections with a 30s timeout. That's not enough for an interactive OAuth flow. Add the entry, then run `nozich mcp login <server>` from a fresh terminal — it waits the full 5 minutes for you to complete auth.

## mTLS / client certificates

Remote HTTP MCP servers that require mutual TLS (client-certificate authentication) are supported via `client_cert` / `client_key`. Nozich passes the resolved certificate to the underlying HTTP client for the TLS handshake.
...
Nozich reads MCP config from `~/.nozich/config.yaml` under `mcp_servers`.
...
| `enabled` | bool | If `false`, Nozich skips the server entirely |
...
For well-known MCP servers, `nozich mcp add` accepts a `--preset` flag that fills in the transport details so you don't have to look up the command and args. The preset only supplies defaults — anything else (env vars, headers, filtering) you pass on the same command line still wins.
...
nozich mcp add codex --preset codex
```
...
You can pick any local name (`nozich mcp add my-codex --preset codex` is fine); the preset only provides the `command`/`args` defaults.

## How Nozich registers MCP tools

Nozich prefixes MCP tools so they do not collide with built-in names:
...
In practice, you usually do not need to call the prefixed name manually — Nozich sees the tool and chooses it during normal reasoning.

## MCP utility tools

When supported, Nozich also registers utility tools around MCP resources and prompts:
...
- Nozich only registers resource utilities if the MCP session actually supports resource operations
- Nozich only registers prompt utilities if the MCP session actually supports prompt operations

So a server that exposes callable tools but no resources/prompts will not get those extra wrappers.

## Per-server filtering

You can control which tools each MCP server contributes to Nozich, allowing fine-grained management of your tool namespace.
...
If `enabled: false`, Nozich skips the server completely and does not even attempt a connection.
...
You can also separately disable Nozich-added utility wrappers:
...
If your config filters out all callable tools and disables or omits all supported utilities, Nozich does not create an empty runtime MCP toolset for that server.
...
Nozich discovers MCP servers at startup and registers their tools into the normal tool registry.

### Dynamic Tool Discovery

MCP servers can notify Nozich when their available tools change at runtime by sending a `notifications/tools/list_changed` notification. When Nozich receives this notification, it automatically re-fetches the server's tool list and updates the registry — no manual `/reload-mcp` required.
...
For stdio servers, Nozich does not blindly pass your full shell environment.
...
Then verify your config and restart Nozich.
...
Because Nozich now only registers those wrappers when both are true:
...
When `supports_parallel_tool_calls` is `true`, Nozich may execute multiple tools from that server at the same time within a single tool-call batch, just like it does for built-in read-only tools (web_search, read_file, etc.).
...
MCP servers can request LLM inference from Nozich via the `sampling/createMessage` protocol. This allows an MCP server to ask Nozich to generate text on its behalf — useful for servers that need LLM capabilities but don't have their own model access.
...
## Running Nozich as an MCP server

In addition to connecting **to** MCP servers, Nozich can also **be** an MCP server. This lets other MCP-capable agents (Claude Code, Cursor, Codex, or any MCP client) use Nozich's messaging capabilities — list conversations, read message history, and send messages across all your connected platforms.

### When to use this

- You want Claude Code, Cursor, or another coding agent to send and read Telegram/Discord/Slack messages through Nozich
- You want a single MCP server that bridges to all of Nozich's connected messaging platforms at once
- You already have a running Nozich gateway with connected platforms
...
Add Nozich to your MCP client config. For example, in Claude Code's `~/.claude/claude_desktop_config.json`:
...
Or if you installed Nozich in a specific location:

```json
{
  "mcpServers": {
    "nozich": {
      "command": "/home/user/.nozich/nozich-agent/venv/bin/nozich",
      "args": ["mcp", "serve"]
    }
  }
}
...
The MCP server exposes 10 tools, matching OpenClaw's channel bridge surface plus a Nozich-specific channel browser:
...
The MCP server includes a live event bridge that polls Nozich's session database for new messages. This gives MCP clients near-real-time awareness of incoming conversations:
...
```bash
nozich mcp serve              # Normal mode
nozich mcp serve --verbose    # Debug logging on stderr
```

### How it works

The MCP server reads conversation data directly from Nozich's session store (`~/.nozich/sessions/sessions.json` and the SQLite database). A background thread polls the database for new messages and maintains an in-memory event queue. For sending messages, it uses the same `send_message` infrastructure as the Nozich agent itself.
...
- The embedded `nozich mcp serve` exposes a **stdio-only** MCP server today. If you need an HTTP MCP server, run a separate adapter — or, much more commonly, use the MCP **client** side of Nozich, which already speaks both stdio and HTTP (`url` + `headers` in `mcp_servers.yaml` / `config.yaml`; see [HTTP servers](#http-servers) above).
