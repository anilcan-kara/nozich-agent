...
description: "Customize Nozich Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Nozich Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `NOZICH_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Nozich is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Nozich now seeds a default `SOUL.md` automatically in:

```text
~/.nozich/SOUL.md
```

More precisely, it uses the current instance's `NOZICH_HOME`, so if you run Nozich with a custom home directory, it will use:
...
- Nozich creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Nozich loads `SOUL.md` only from `NOZICH_HOME`
- Nozich does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Nozich falls back to a built-in default identity
...
If Nozich loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `NOZICH_HOME`, the personality belongs to the Nozich instance itself.

That also makes it easier to teach users:
- "Edit `~/.nozich/SOUL.md` to change Nozich' default personality."
...
- how Nozich should handle uncertainty, disagreement, or ambiguity
...
## What Nozich injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Nozich falls back to a built-in default identity ("You are Nozich Agent, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Nozich ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Nozich, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Nozich its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.nozich/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.nozich/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-nozich)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Nozich speaks
- `display.skin` and `/skin` affect how Nozich looks in the terminal
