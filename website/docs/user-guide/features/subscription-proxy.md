...
chat completions — use your Nozich-managed provider subscription as their
...
| Use case | "Use Nozich as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Nozich stores
the refresh token in `~/.nozich/auth.json` — the same place all Nozich
...
Model:      Nozich-4-70B    # or Nozich-4.3-36B, Nozich-4-405B
```
...
interface in `nozich_cli/proxy/adapters/`.
...
If you see `not logged in`, run `nozich portal`. If you see
...
supports; check `portal.anilcan-kara.com/models`.
...
[portal.anilcan-kara.com](https://portal.anilcan-kara.com).
...
`nozich_cli/proxy/adapters/<provider>.py` and registering it in
