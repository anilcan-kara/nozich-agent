...
The web dashboard is a browser-based UI for managing your Nozich Agent installation. Instead of editing YAML files or running CLI commands, you can configure settings, manage API keys, and monitor sessions from a clean web interface.

:::tip
Hosted-mode auth uses Nous Portal OAuth; if you also want the dashboard to talk to a real backend, `nozich setup --portal` wires up the model and tool gateway too. See [Nous Portal](/integrations/nous-portal).
:::
...
```bash
# Custom port
nozich dashboard --port 8080

# Bind to all interfaces (use with caution on shared networks)
nozich dashboard --host 0.0.0.0

# Start without opening browser
nozich dashboard --no-open
```
...
child with the selected profile's `NOZICH_HOME`, so the conversation runs
...
processes (manage them via `nozich -p <name> gateway …`), each profile's
session database, and cron schedulers (the Cron page already aggregates
across profiles with its own filter).

## Prerequisites

The default `nozich-agent` install does not ship the HTTP stack or PTY helper — those are optional extras. The **web dashboard** needs FastAPI and Uvicorn (`web` extra). The **Chat** tab also needs `ptyprocess` to spawn the embedded TUI behind a pseudo-terminal (`pty` extra on POSIX). Install both with:

```bash
pip install 'nozich-agent[web,pty]'
```

The `web` extra pulls in FastAPI/Uvicorn; `pty` pulls in `ptyprocess` (POSIX) or `pywinpty` (native Windows — note that the embedded TUI itself still requires WSL). `pip install nozich-agent[all]` includes both extras and is the easiest path if you also want messaging/voice/etc.

When you run `nozich dashboard` without the dependencies, it will tell you what to install. If the frontend hasn't been built yet and `npm` is available, it builds automatically on first launch.

The Chat tab is part of every `nozich dashboard` launch — the embedded browser chat pane (running the TUI over PTY/WebSocket) is always available, with no extra flag required.
...
- Node.js (same requirement as `nozich --tui`; the TUI bundle is built on first launch)
- `ptyprocess` — installed by the `pty` extra (`pip install 'nozich-agent[web,pty]'`, or `[all]` covers both)
...
```ini
[Service]
EnvironmentFile=%h/.nozich/.env
ExecStart=/path/to/venv/bin/python -m nozich_cli.main dashboard \
    --host 0.0.0.0 --port 9119 --no-open
```

with `~/.nozich/.env` containing:

```bash
NOZICH_DASHBOARD_BASIC_AUTH_USERNAME=admin
NOZICH_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
NOZICH_DASHBOARD_BASIC_AUTH_SECRET=<32+ random bytes; openssl rand -base64 32>
```
...
:::tip
Config changes take effect on the next agent session or gateway restart. The web dashboard edits the same `config.yaml` file that `nozich config set` and the gateway read from.
:::
...
Create and manage [profiles](../profiles.md) — isolated Nozich instances with their own config, skills, and sessions.
...
- **Set as active** — flips the sticky default that **future CLI/gateway runs** pick up (same as `nozich profile use`). This does *not* change what the dashboard manages — that's the profile switcher's job
- **Edit model / description / SOUL** — inline editors writing into that profile
- **Rename / Delete** — named profiles only

### Skills

Browse, search, and toggle installed skills and toolsets, and install new ones from the hub. Skills are loaded from `~/.nozich/skills/` and grouped by category.
...
- **Browse hub** — a third view searches the skill hub across all sources (the same as `nozich skills search`), installs any result by identifier with a live install log, and offers an "Update all" button to refresh installed skills.
...
block in `config.yaml` that `nozich mcp` reads from.
...
`nozich mcp catalog` / `nozich mcp install` use.
...
Connect Nozich to any messaging platform from the browser — full parity with
`nozich setup gateway`. The page lists every supported channel (Telegram,
...
- **Restart gateway** — credentials are written to `~/.nozich/.env` and the enabled flag to `config.yaml`; the gateway connects each enabled channel on its next restart, which you can trigger right from the page.
...
- **Host** — live system stats: OS / kernel, architecture, hostname, Python and Nozich versions, CPU core count + utilization, memory, disk usage of the Nozich home, uptime, and load average. (CPU/memory/disk come from `psutil` when installed; identity fields are always shown.) The Nozich version shows an **update-status badge** (up to date / N commits behind) and a **Check for updates** button. When an update is available on a git or pip install, an **Update now** button opens a confirmation dialog — showing how many commits you'll pull — before running `nozich update` in the background. On Docker/Nix/Homebrew installs the dashboard can't apply the update in place, so it shows the correct out-of-band command instead.
- **Nous Portal** — login status, the active inference provider, and the Tool Gateway routing table (which tools run via the Portal vs. locally), with a link to manage your subscription. Read-only mirror of `nozich portal`.
- **Skill curator** — the background skill-maintenance status (active / paused, interval, last run) with pause/resume and a run-now button. Mirrors `nozich curator`.
...
This re-reads `~/.nozich/.env` into the running process's environment. Useful when you've added a new provider key via the dashboard and want to use it immediately.
...
`NOZICH_HOME`. Omitted = the dashboard's own profile. Unknown profile names
...
| `GET /api/nozich/update/check` | Report update availability (commits behind, install method) without applying. For git/pip installs that are behind, also returns a `commits` list (`sha`, `summary`, `author`, `at`) of what's changed. `?force=1` busts the 6h cache |
...
When the dashboard is bound to a public or non-loopback address — anything other than `127.0.0.1` / `localhost` — Nozich Agent engages an auth gate. Every request must carry a verified session cookie or it's bounced to the login page. Three providers ship in the box:
...
|-------|-----------|----------|
| `nozich dashboard` (default — binds to `127.0.0.1`) | OFF | Local development |
| `nozich dashboard --host 0.0.0.0` | **ON** | Remote / production — protect with the username/password provider or OAuth |
...
If the gate would engage but **no** `DashboardAuthProvider` is registered (no Nous plugin, no custom plugin), `nozich dashboard` refuses to bind with an explicit error message. There is no "default-deny but accept everything" fallback — a misconfigured gated dashboard never starts.
...
- **CLI — `nozich dashboard register`.** Run it on the host where the dashboard lives. It resolves your existing Nous login (run `nozich setup` first if you're not logged in), registers a self-hosted OAuth client with the Portal, and writes `NOZICH_DASHBOARD_OAUTH_CLIENT_ID` into `~/.nozich/.env` for you. Optional flags: `--name` (a human-readable label, otherwise auto-generated) and `--redirect-uri` (a public HTTPS callback URL for an internet-facing host).

  ```bash
  nozich dashboard register
  # ✓ Registered dashboard "swift_falcon"
  # …writes NOZICH_DASHBOARD_OAUTH_CLIENT_ID to ~/.nozich/.env
  ```

- **GUI — the Local Dashboards page.** Open [`/local-dashboards`](https://portal.anilcan-kara.com/local-dashboards) in the Nous Portal to register, name, manage, and revoke self-hosted dashboards from the browser. Copy the resulting `agent:{id}` client ID into `NOZICH_DASHBOARD_OAUTH_CLIENT_ID` (env) or `dashboard.oauth.client_id` (config.yaml). This is also where you revoke a dashboard registered via the CLI.
...
|---------|-----------|--------|----------------|
| `NOZICH_DASHBOARD_OAUTH_CLIENT_ID` | `dashboard.oauth.client_id` | `agent:{instance_id}` | `nozich dashboard register` |

Per the Nozich Agent convention (`~/.nozich/.env` is for API keys / secrets only), **`config.yaml` is the recommended place to set these values** for local dev, on-prem, and any deployment you control directly. The environment-variable path exists so a hosting platform's secret injection can push per-deploy `client_id`s without anyone having to edit `config.yaml` inside the image — that's its primary purpose.
...
From a logged-in Nozich install to a Nous-gated dashboard in three steps.

**1. Log in and register the dashboard.** `nozich dashboard register` uses your existing Nous login to provision an OAuth client and writes `NOZICH_DASHBOARD_OAUTH_CLIENT_ID` into `~/.nozich/.env` for you:

```bash
nozich setup            # if you're not already logged into Nous Portal
nozich dashboard register
# ✓ Registered dashboard "swift_falcon"
# …writes NOZICH_DASHBOARD_OAUTH_CLIENT_ID to ~/.nozich/.env
```
...
```bash
nozich dashboard --host 0.0.0.0 --port 9119 --no-open
```
...
`GET /api/auth/me` then returns the verified session (`provider: nous`). For an internet-facing host, register with `--redirect-uri https://nozich.example.com/auth/callback` and set `NOZICH_DASHBOARD_PUBLIC_URL` so the OAuth callback resolves to your public URL (see [Public URL override](#public-url-override)).
...
|---------|-----------|-------|
| `NOZICH_DASHBOARD_BASIC_AUTH_USERNAME` | `dashboard.basic_auth.username` | required to activate |
| `NOZICH_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` | `dashboard.basic_auth.password_hash` | preferred (no plaintext at rest) |
| `NOZICH_DASHBOARD_BASIC_AUTH_PASSWORD` | `dashboard.basic_auth.password` | plaintext; **wins over a config `password_hash`** so you can rotate via env |
| `NOZICH_DASHBOARD_BASIC_AUTH_SECRET` | `dashboard.basic_auth.secret` | token-signing key |
| `NOZICH_DASHBOARD_BASIC_AUTH_TTL_SECONDS` | `dashboard.basic_auth.session_ttl_seconds` | access-token lifetime |
...
**1. Set credentials in `~/.nozich/.env`.** Hash the password so no plaintext sits at rest, and set a stable signing secret so sessions survive restarts:
...
cat >> ~/.nozich/.env <<EOF
NOZICH_DASHBOARD_BASIC_AUTH_USERNAME=admin
NOZICH_DASHBOARD_BASIC_AUTH_PASSWORD_HASH=$HASH
NOZICH_DASHBOARD_BASIC_AUTH_SECRET=$(openssl rand -base64 32)
EOF
chmod 600 ~/.nozich/.env
```
...
dashboard:
  oauth:
    ...
    self_hosted:
      issuer: https://auth.example.com/application/o/nozich/   # required
      client_id: nozich-dashboard                              # required
...
| Env var | Overrides | Notes |
|---------|-----------|-------|
| `NOZICH_DASHBOARD_OIDC_ISSUER` | `dashboard.oauth.self_hosted.issuer` | OIDC issuer URL — required |
| `NOZICH_DASHBOARD_OIDC_CLIENT_ID` | `dashboard.oauth.self_hosted.client_id` | Public client id — required |
| `NOZICH_DASHBOARD_OIDC_SCOPES` | `dashboard.oauth.self_hosted.scopes` | Defaults to `openid profile email` |
...
**1. Run Keycloak with a pre-configured realm.** Save this realm export as `realm-nozich.json` — it defines a `nozich` realm, a **public PKCE client** (`nozich-dashboard`), and a test user, all imported on boot so there's nothing to click in the admin UI:
...
```json
{
  ...
  "clients": [
    {
      "clientId": "nozich-dashboard",
      "name": "Nozich Agent Dashboard",
      "enabled": true,
      "publicClient": true,
...
docker run --rm -p 8080:8080 \
  ...
  -v "$PWD/realm-nozich.json:/opt/keycloak/data/import/realm-nozich.json:ro" \
  quay.io/keycloak/keycloak:26.0 \
  start-dev --import-realm
```

Once it's up, the realm advertises standard OIDC discovery at
`http://localhost:8080/realms/nozich/.well-known/openid-configuration` (issuer
`http://localhost:8080/realms/nozich`). The admin console is at
...
```bash
export NOZICH_DASHBOARD_OIDC_ISSUER="http://localhost:8080/realms/nozich"
export NOZICH_DASHBOARD_OIDC_CLIENT_ID="nozich-dashboard"
export NOZICH_DASHBOARD_PUBLIC_URL="http://localhost:9119"
nozich dashboard --host 0.0.0.0 --port 9119 --no-open
```

`NOZICH_DASHBOARD_PUBLIC_URL` tells the dashboard its OAuth callback is
...
> admin console (Clients → nozich-dashboard → Settings). The same pattern works
...
For deploys behind reverse proxies that don't reliably forward those headers (manual nginx setups, on-prem ingresses, custom-domain deploys with partial proxy chains), set `dashboard.public_url` (or `NOZICH_DASHBOARD_PUBLIC_URL`) to the **complete public URL** the dashboard is reached at:

```yaml
dashboard:
  public_url: "https://dashboard.example.com/nozich"
```
...
|---------|---------------|-------------|
| `dashboard.public_url` in `config.yaml` | `NOZICH_DASHBOARD_PUBLIC_URL` | Local dev / on-prem (canonical) |
| `NOZICH_DASHBOARD_PUBLIC_URL` env var | — | Hosting-platform secrets / CI |
...
The provider implements the [Nous Portal OAuth contract v1](https://github.com/anilcan-kara/nous-account-service/blob/main/docs/agent-dashboard-oauth-contract.md) — authorization-code grant with PKCE (S256):
...
3. Server stashes PKCE state in a short-lived cookie, redirects user to `https://portal.anilcan-kara.com/oauth/authorize?…`.
4. User authenticates with Portal, lands at `/auth/callback?code=…&state=…`.
5. Server exchanges the code for an access token at `POST /api/oauth/token`, verifies the JWT signature against the Portal's JWKS (`/.well-known/jwks.json`), and sets the `nozich_session_at` cookie.
...
| Name | Lifetime | Notes |
|------|----------|-------|
| `nozich_session_at` | Token TTL (15 min) | HttpOnly, SameSite=Lax, Secure-when-HTTPS |
| `nozich_session_pkce` | 10 min | HttpOnly; holds the PKCE verifier + provider hint during the round trip |
| `nozich_session_rt` | unused in v1 | Reserved for forward-compat; not written when `refresh_token` is empty |
...
Every login start, success, failure, and session-verify failure is written as a JSON line to `$NOZICH_HOME/logs/dashboard-auth.log`. Sensitive fields (`access_token`, `refresh_token`, `code`, `code_verifier`, `state`, `Authorization` header) are redacted before logging.
...
```python
# ~/.nozich/plugins/dashboard-auth-myidp/__init__.py
from nozich_cli.dashboard_auth import DashboardAuthProvider, Session, LoginStart
...
```bash
# Quick env-var path.
NOZICH_DASHBOARD_OAUTH_CLIENT_ID=agent:test \
  nozich dashboard --host 0.0.0.0
...
Nozich Desktop can drive a Nozich backend running on another machine (a VPS, a home server, a Mini behind Tailscale). In the app this lives under **Settings → Gateway → Remote gateway**, which asks for a **Remote URL** and a way to **Sign in**. (For the desktop app itself — install, settings, chat — see the [Nozich Desktop](/user-guide/desktop) page.)

You protect the remote dashboard with one of the bundled auth providers, and the desktop app signs in against whichever one the backend advertises. For a backend reachable beyond your own machine — a VPS, a public host, anything internet-facing — the recommended provider is **OAuth (Nous Portal)** (register it with [`nozich dashboard register`](#registering-a-dashboard) and sign in with *Sign in with Nous Research*). The bundled [username/password provider](#usernamepassword-provider-no-oauth-idp) is the quickest option when the backend is on a trusted LAN or reachable only over a VPN, but is **not suitable for direct public-internet exposure**. Binding the dashboard to a non-loopback address engages its auth gate; once signed in, Desktop reuses the session for the chat WebSocket automatically — there is no token to copy or paste.
...
```bash
# 1. Set the dashboard login credentials in ~/.nozich/.env (secrets file, 0600).
cat >> ~/.nozich/.env <<'EOF'
NOZICH_DASHBOARD_BASIC_AUTH_USERNAME=admin
NOZICH_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
...
nozich dashboard --no-open --host 0.0.0.0 --port 9119
```

Prefer no plaintext at rest? Use `NOZICH_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` with a scrypt hash instead — see [Username/password provider](#usernamepassword-provider-no-oauth-idp) for the full surface.

If you run the dashboard as a systemd service, `~/.nozich/.env` is picked up automatically when the unit has `EnvironmentFile=%h/.nozich/.env`, so the credentials are in the environment at boot.
...
- **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/nozich` are supported if you front it with a reverse proxy)
...
The session refreshes automatically and survives restarts when `NOZICH_DASHBOARD_BASIC_AUTH_SECRET` is set on the backend.

### Environment-variable override

Instead of the in-app setting, you can point the desktop at a backend with an env var before launching it. When `NOZICH_DESKTOP_REMOTE_URL` is set, it overrides the saved in-app URL (the Gateway settings panel shows an "env override" badge and disables editing); you still **Sign in** with your username and password from the panel.

| Env var | Value |
|---------|-------|
| `NOZICH_DESKTOP_REMOTE_URL` | `http://<backend-host>:9119` |

### Troubleshooting

- **"Remote gateway incomplete"** — you haven't entered a remote URL.
- **Sign-in fails with 401 / "Invalid credentials"** — the username or password doesn't match the backend's `NOZICH_DASHBOARD_BASIC_AUTH_USERNAME` / `NOZICH_DASHBOARD_BASIC_AUTH_PASSWORD`. The backend returns the same generic error for unknown user and wrong password, so check both. Confirm the gate with `curl -s http://<host>:9119/api/status | jq '.auth_required, .auth_providers'` — it should report `true` and include `"basic"`.
...
- **Signed out on every restart** — set `NOZICH_DASHBOARD_BASIC_AUTH_SECRET` to a stable value; otherwise the signing key is regenerated per boot.
...
The frontend is built with React 19, TypeScript, Tailwind CSS v4, and shadcn/ui-style components. Production builds output to `nozich_cli/web_dist/` which the FastAPI server serves as a static SPA.

## Automatic Build on Update

When you run `nozich update`, the web frontend is automatically rebuilt if `npm` is available. This keeps the dashboard in sync with code updates. If `npm` isn't installed, the update skips the frontend build and `nozich dashboard` will build it on first launch.
...
| Theme | Character |
|-------|-----------|
| **Nozich Teal** (`default`) | Dark teal + cream, system fonts, comfortable spacing |
| **Nozich Teal (Large)** (`default-large`) | Same as default with 18px text and roomier spacing |
