...
title: "Git Worktrees"
description: "Run multiple Nozich agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Nozich Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Nozich so each session has a clean, isolated working directory.

## Why Use Worktrees with Nozich?

Nozich treats the **current working directory** as the project root:

- CLI: the directory where you run `nozich` or `nozich chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.nozich/config.yaml`
...
git worktree add ../repo-feature feature/nozich-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/nozich-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Nozich there:

```bash
cd ../repo-feature

# Start Nozich in the worktree
nozich
```
...
git worktree add ../repo-experiment-a feature/nozich-a
git worktree add ../repo-experiment-b feature/nozich-b
```
...
- Works on its own branch (`feature/nozich-a` vs `feature/nozich-b`).
...
- Nozich checkpoint data under `~/.nozich/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Nozich experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/nozich-checkpoints-docs`, `feature/nozich-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Nozich from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `nozich -w` (Automatic Worktree Mode)

Nozich has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `nozich/nozich-<hash>`).
...
```bash
nozich -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `nozich -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Nozich session its own clean checkout.
