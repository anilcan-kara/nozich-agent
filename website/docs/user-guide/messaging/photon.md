...
Connect Nozich to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Nozich — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Nozich runs it in a
...
nozich photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Nozich Agent` project on your account.
...
Runtime credentials are written to `~/.nozich/.env`
...
(device token, dashboard project id) lives in `~/.nozich/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Nozich
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Nozich replies with a pairing code. Approve it with:

```bash
nozich pairing approve photon <CODE>
```

Use `nozich pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.nozich/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.nozich/.env`):
...
By default Nozich responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Nozich` and
`@Nozich agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Nozich will reply.

## Status & troubleshooting

```bash
nozich photon status
```

Prints saved credentials, sidecar health, your registered number, and the
assigned iMessage line Nozich uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run nozich photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `nozich photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `nozich photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `nozich photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Nozich sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | Nozich wake words  | JSON list / comma / newline regex patterns for group mentions |
