...
Connect Nozich to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Nozich supports two WeCom integration modes:
...
> Run `nozich gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Nozich decrypts the message, queues it for the agent
...
(Use `nozich gateway start` only after `nozich gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Nozich and the token the
...
in `~/.nozich/.env` values around `=` will also break signature checks. After
fixing, restart `nozich gateway run`.
...
Check `nozich gateway run` logs for the bound host/port. If the adapter bound to
