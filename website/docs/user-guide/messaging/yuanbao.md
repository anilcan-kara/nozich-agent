...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Nozich Agent to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect Nozich to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.nozich/.env`:
...
All standard Nozich commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.nozich/logs/gateway.log`
...
4. Consider enabling verbose logging: `NOZICH_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. Nozich automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
NOZICH_LOG_LEVEL=debug nozich gateway
```
...
```bash
nozich chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
