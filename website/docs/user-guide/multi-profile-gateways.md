...
If you only run one Nozich agent, you don't need this page — see
[Profiles](./profiles.md) for the basics.

## When to use this

You want this setup when you have two or more Nozich agents that should all
...
(`ai.nozich.gateway-<name>.plist`) or systemd user service
(`nozich-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
nozich profile create coder
nozich profile create personal-bot
nozich profile create research
...
`~/.local/bin/nozich-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: nozich-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    nozich gateway "$action"
  else
    nozich -p "$profile" gateway "$action"
  fi
}
...
```bash
nozich-gateways start      # start every configured profile
nozich-gateways stop       # stop every configured profile
nozich-gateways restart    # restart all
nozich-gateways status     # status across all
nozich-gateways list       # delegates to `nozich gateway list`
```

:::tip
The `default` profile is targeted with `nozich gateway <action>` (no `-p`),
not `nozich -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `nozich -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.nozich.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/nozich-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.nozich.gateway.plist` /
`nozich-gateway.service`.
...
```bash
# Default profile
tail -f ~/.nozich/logs/gateway.log
tail -f ~/.nozich/logs/gateway.error.log

# Named profile
tail -f ~/.nozich/profiles/<name>/logs/gateway.log
tail -f ~/.nozich/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.nozich/logs/gateway.log ~/.nozich/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
nozich logs -f                  # follow default profile
nozich -p coder logs -f         # follow one profile
nozich logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
nozich profile list             # profiles + model + gateway state
nozich-gateways status          # full status across every profile
launchctl list | grep nozich    # macOS — PIDs and labels
systemctl --user list-units 'nozich-gateway-*'   # Linux — units
```
...
```
~/.nozich/profiles/<name>/
├── .env              # API keys, bot tokens (chmod 600)
├── config.yaml       # model, provider, toolsets, gateway settings
└── SOUL.md           # personality / system prompt
```

The default profile uses `~/.nozich/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
nozich config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.nozich/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=nozich --why="gateways running" \
  sleep infinity &
...
`nozich-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.nozich/.env ~/.nozich/profiles/*/.env
```

## Updating the code

`nozich update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `nozich gateway start` after a previous `nozich gateway stop`. The
...
```bash
ps -ef | grep "nozich_cli.*-p <profile>"
cat ~/.nozich/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.nozich.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.nozich.gateway-<profile>.plist

# Linux
systemctl --user restart nozich-gateway-<profile>.service
```

### Health check

```bash
nozich doctor                  # default profile
nozich -p <profile> doctor     # one profile
```
