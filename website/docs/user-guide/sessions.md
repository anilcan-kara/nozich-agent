...
Nozich Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.nozich/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
Nozich stores session history so it can resume conversations, but it does not
...
Nozich explicitly injects for that turn.
...
For example, if a user sends an image and asks Nozich to make a meme from it,
Nozich may inspect that image once with vision and run an image-processing
...
`nozich sessions prune` only when you want to delete old ended sessions from
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`nozich` or `nozich chat`) |
...
nozich -c "my project"   # → resumes "my project #3"
```
...
nozich --resume 20250305_091523_a1b2c3d4
nozich -r 20250305_091523_a1b2c3d4

# Resume by title
nozich --resume "refactoring auth"

# Or with the chat subcommand
nozich chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `nozich sessions list`.

### Conversation Recap on Resume

When you resume a session, Nozich displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Nozich session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.nozich/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `nozich -r "<title>"` from the shell) and pick up where the platform left off.
...
Nozich automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `nozich sessions list` or `nozich sessions browse`.
...
```bash
nozich sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Nozich creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`nozich -c "my project"`), it automatically picks the most recent session in the lineage.
...
Nozich provides a full set of session management commands via `nozich sessions`:

### List Sessions

```bash
# List recent sessions (default: last 20)
nozich sessions list

# Filter by platform
nozich sessions list --source telegram

# Show more sessions
nozich sessions list --limit 50
```
...
nozich sessions export backup.jsonl

# Export sessions from a specific platform
nozich sessions export telegram-history.jsonl --source telegram

# Export a single session
nozich sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
nozich sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
nozich sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
nozich sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
nozich sessions prune

# Custom age threshold
nozich sessions prune --older-than 30

# Only prune sessions from a specific platform
nozich sessions prune --source telegram --older-than 60

# Skip confirmation
nozich sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`nozich insights`](/reference/cli-commands#nozich-insights).
...
When Nozich cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Nozich uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Nozich in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.nozich/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.nozich/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `~/.nozich/sessions/sessions.json` | Maps session keys to active session IDs (origin metadata, expiry flags) |
...
`*.jsonl` files in `~/.nozich/sessions/`. They are no longer written or
read by Nozich. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Nozich process in the same `NOZICH_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.nozich/config.yaml`:
...
nozich sessions delete <session_id>
