...
- Give it a name like "nozich-agent"
- Select scopes:
...
- Give it a title like "nozich-agent-&lt;machine-name>"
...
elif [ -f ~/.nozich/.env ] && grep -q "^GITHUB_TOKEN=" ~/.nozich/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.nozich/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
