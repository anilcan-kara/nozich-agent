...
description: "Configure and use Honcho memory with Nozich -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Nozich -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `nozich skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Nozich

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Nozich profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/nozich#running-honcho-locally-with-nozich

### Verify

```bash
nozich honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Nozich profile's AI peer
...
Honcho models conversations as interactions between **peers**. Nozich creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Nozich instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Nozich run |
| `global` | Single session across all directories |

Manual override: `nozich honcho map my-project-name`
...
Each Nozich profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block nozich.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `nozich.coder` host block in `honcho.json`
...
```bash
nozich honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Nozich when Honcho memory is active.
...
Config file: `$NOZICH_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `nozich honcho setup`. Ensure `memory.provider: honcho` is in `~/.nozich/config.yaml`.

### Memory not persisting across sessions
Check `nozich honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `nozich profile create <name> --clone`. For existing profiles: `nozich honcho sync`.
...
| Command | Description |
|---------|-------------|
| `nozich honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `nozich honcho status` | Show resolved config, connection test, peer info for active profile |
| `nozich honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `nozich honcho disable` | Disable Honcho for the active profile |
| `nozich honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `nozich honcho peers` | Show peer identities across all profiles |
| `nozich honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `nozich honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `nozich honcho sessions` | List known directory-to-session-name mappings |
| `nozich honcho map <name>` | Map current working directory to a Honcho session name |
| `nozich honcho identity` | Seed AI peer identity or show both peer representations |
| `nozich honcho sync` | Create host blocks for all Nozich profiles that don't have one yet |
| `nozich honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Nozich + Honcho |
| `nozich memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `nozich memory status` | Show active memory provider and config |
| `nozich memory off` | Disable external memory provider |
