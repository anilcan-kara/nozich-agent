...
| | |
|---|---|
| Source | Optional — install with `nozich skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Nozich or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Nozich. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.nozich/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.nozich/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Nozich integration, either:

- configure the server in `~/.nozich/config.yaml` using the `native-mcp` skill, or
...
### Nozich cannot see the deployed server

The server-building part may be correct while the Nozich config is not. Load the `native-mcp` skill and configure the server in `~/.nozich/config.yaml`, then restart Nozich.
