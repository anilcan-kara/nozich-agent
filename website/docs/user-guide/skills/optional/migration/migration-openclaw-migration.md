---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into Nozich Agent"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into Nozich Agent"
---
...
Migrate a user's OpenClaw customization footprint into Nozich Agent. Imports Nozich-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `nozich skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration/openclaw-migration` |
| Version | `1.0.0` |
| Author | Nozich Agent (Nous Research) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `Nozich`, `Memory`, `Persona`, `Import` |
| Related skills | [`nozich-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-nozich-agent) |
...
# OpenClaw -> Nozich Migration

Use this skill when a user wants to move their OpenClaw setup into Nozich Agent with minimal manual cleanup.
...
```bash
nozich claw migrate              # Full interactive migration
...
nozich claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `nozich setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_nozich.py` to:

- import `SOUL.md` into the Nozich home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Nozich memory entries
- merge OpenClaw command approval patterns into Nozich `command_allowlist`
- migrate Nozich-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Nozich working-directory configuration
- copy OpenClaw skills into `~/.nozich/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Nozich workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.nozich/tts/`
- archive non-secret docs that do not have a direct Nozich destination
...
- `scripts/openclaw_to_nozich.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.nozich/skills/migration/openclaw-migration/scripts/openclaw_to_nozich.py`

Do not guess a shorter path like `~/.nozich/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.nozich/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Nozich-compatible secrets, currently:
...
Nozich CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Nozich. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Nozich workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Nozich skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.nozich/skills/migration/openclaw-migration/scripts/openclaw_to_nozich.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.nozich/skills/migration/openclaw-migration/scripts/openclaw_to_nozich.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.nozich/skills/migration/openclaw-migration/scripts/openclaw_to_nozich.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.nozich/skills/migration/openclaw-migration/scripts/openclaw_to_nozich.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.nozich/skills/migration/openclaw-migration/scripts/openclaw_to_nozich.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.nozich/skills/migration/openclaw-migration/scripts/openclaw_to_nozich.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Nozich unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Nozich targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Nozich destination. Unsupported auth blobs must still be reported as skipped.
...
- Nozich persona state imported
- Nozich memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.nozich/skills/openclaw-imports/`
