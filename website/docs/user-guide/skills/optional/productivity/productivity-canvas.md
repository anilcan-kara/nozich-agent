...
| | |
|---|---|
| Source | Optional — install with `nozich skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Nozich Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `~/.nozich/.env`:
...
```bash
CANVAS="python $NOZICH_HOME/skills/productivity/canvas/scripts/canvas_api.py"
