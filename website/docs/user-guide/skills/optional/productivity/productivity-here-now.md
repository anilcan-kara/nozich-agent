...
| | |
|---|---|
| Source | Optional — install with `nozich skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${NOZICH_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${NOZICH_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${NOZICH_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client nozich
```
...
```bash
PUBLISH="${NOZICH_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client nozich
```
...
```bash
DRIVE="${NOZICH_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `nozich`)    |
