...
| | |
|---|---|
| Source | Optional — install with `nozich skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Nozich Agent |
| License | MIT |
...
import upstream classes into Nozich itself.
...
```bash
mkdir -p ~/.nozich/cache/darwinian-evolver && cd ~/.nozich/cache/darwinian-evolver
...
```bash
cd ~/.nozich/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.nozich/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.nozich/skills/research/darwinian-evolver
DE_DIR=~/.nozich/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Nozich core.
   Custom driver scripts under `~/.nozich/skills/...` are user-side and fine.
