...
| | |
|---|---|
| Source | Optional — install with `nozich skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Nozich Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Nozich terminal calls (desktop app flow only)
...
### Service Account (recommended for Nozich)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.nozich/.env` (the skill will prompt for this on first load).
...
## Nozich Execution Pattern (desktop app flow)

Nozich terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/nozich-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/nozich-op.sock"
