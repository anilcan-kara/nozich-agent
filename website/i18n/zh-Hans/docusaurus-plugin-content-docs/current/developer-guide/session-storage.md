...
Nozich Agent 使用 SQLite 数据库（`~/.nozich/state.db`）跨 CLI 和 gateway 会话持久化会话元数据、完整消息历史及模型配置。这替代了早期的逐会话 JSONL 文件方案。
...
多个 nozich 进程（gateway + CLI 会话 + worktree agent）共享同一个 `state.db`。`SessionDB` 类通过以下方式处理写入竞争：
...
db = SessionDB()                           # 默认：~/.nozich/state.db
...
默认路径：`~/.nozich/state.db`

该路径由 `nozich_constants.get_nozich_home()` 推导，默认解析为 `~/.nozich/`，或 `NOZICH_HOME` 环境变量的值。
