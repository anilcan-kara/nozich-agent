...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/anilcan-kara/nozich-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    nozich_meta = (fm.get("metadata") or {}).get("nozich") or {}
    tags = nozich_meta.get("tags") or []
    related = nozich_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `nozich skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Nozich Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Nozich ships with a large built-in skill library copied into `~/.nozich/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Nozich also syncs bundled skills on `nozich update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.nozich/skills/` tree, it is still shipped with Nozich; restore it with `nozich skills reset <name> --restore`.",
        "",
